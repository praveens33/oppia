...
app_feedback_report_models, exp_models, suggestion_models, user_models = (
    models.Registry.import_models([models.Names.APP_FEEDBACK_REPORT, models
    .Names.EXPLORATION, models.Names.SUGGESTION, models.Names.USER]))
...
class CronJobTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.testapp_swap = self.swap(self, 'testapp', webtest.TestApp(main
            .app_without_context))
        ...
        def _mock_taskqueue_service_defer(unused_function_id: str,
            unused_queue_name: str) ->None:
            """Mocks taskqueue_services.defer() so that it can be checked
            if the method is being invoked or not.
            """
            self.task_status = 'Started'
        self.taskqueue_service_defer_swap = self.swap(taskqueue_services,
            'defer', _mock_taskqueue_service_defer)
    ...
    def test_run_cron_to_hard_delete_models_marked_as_deleted(self) ->None:
        ...
        completed_activities_model = user_models.CompletedActivitiesModel(id
            =admin_user_id, exploration_ids=[], collection_ids=[],
            story_ids=[], learnt_topic_ids=[], last_updated=datetime.
            datetime.utcnow() - self.NINE_WEEKS, deleted=True)
        completed_activities_model.update_timestamps(update_last_updated_time
            =False)
    ...
    def test_run_cron_to_hard_delete_versioned_models_marked_as_deleted(self
        ...
        with self.mock_datetime_utcnow(datetime.datetime.utcnow() - self.
            NINE_WEEKS):
    ...
    def test_run_cron_to_mark_old_models_as_deleted(self) ->None:
        ...
        user_query_model = user_models.UserQueryModel(id='query_id',
            user_ids=[], submitter_id=admin_user_id, query_status=feconf.
            USER_QUERY_STATUS_PROCESSING, last_updated=datetime.datetime.
            utcnow() - self.FIVE_WEEKS)
    ...
    def test_run_cron_to_scrub_app_feedback_reports_scrubs_reports(self
        ...
        self.login(self.CURRICULUM_ADMIN_EMAIL, is_super_admin=True)
        report_timestamp = get_current_datetime_utc() - self.FOURTEEN_WEEKS
        report_submitted_timestamp = report_timestamp
        ticket_creation_timestamp = from_milliseconds_utc(1616173836 * 1000.0)
        android_report_info = {'user_feedback_selected_items': [],
        ...
        report_id = (app_feedback_report_models.AppFeedbackReportModel.
            generate_id('android', report_submitted_timestamp))
        report_model = app_feedback_report_models.AppFeedbackReportModel(id
            =report_id, platform='android', ticket_id='%s.%s.%s' % (
            'random_hash', ticket_creation_timestamp.second,
            '16CharString1234'), submitted_on=report_submitted_timestamp,
            local_timezone_offset_hrs=0, report_type='suggestion', category
            ='other_suggestion', platform_version='0.1-alpha-abcdef1234',
            android_device_country_locale_code='in', android_device_model=
            'Pixel 4a', android_sdk_version=23, entry_point=
            'navigation_drawer', text_language_code='en',
            audio_language_code='en', android_report_info=
            android_report_info, android_report_info_schema_version=1)
        ...
        self.assertEqual(scrubbed_model.scrubbed_by, feconf.
            APP_FEEDBACK_REPORT_SCRUBBER_BOT_ID)
        self.assertEqual(scrubbed_report_info[
...
class CronMailReviewersContributorDashboardSuggestionsHandlerTests(test_utils
    .GenericTestBase):
    target_id = 'exp1'
    language_code = 'en'
    ...
    def _create_translation_suggestion(self
        ...
        add_translation_change_dict = {'cmd': exp_domain.
            ...
            'language_code': self.language_code, 'content_html': feconf.
            DEFAULT_STATE_CONTENT_STR, 'translation_html': self.
            default_translation_html, 'data_format': 'html'}
    ...
    def _assert_reviewable_suggestion_email_infos_are_equal(self,
        reviewable_suggestion_email_info: suggestion_registry.
        ReviewableSuggestionEmailInfo,
        expected_reviewable_suggestion_email_info: suggestion_registry.
        ...
        self.assertEqual(reviewable_suggestion_email_info.
            suggestion_content, expected_reviewable_suggestion_email_info.
            suggestion_content)
        self.assertEqual(reviewable_suggestion_email_info.
            submission_datetime, expected_reviewable_suggestion_email_info.
            submission_datetime)

    def _mock_send_contributor_dashboard_reviewers_emails(self,
        reviewer_ids: List[str], reviewers_suggestion_email_infos: List[
        List[suggestion_registry.ReviewableSuggestionEmailInfo]]) ->None:
        """Mocks
    ...
    def setUp(self) ->None:
        ...
        user_services.allow_user_to_review_translation_in_language(self.
            reviewer_id, self.language_code)
        translation_suggestion = self._create_translation_suggestion()
        self.expected_reviewable_suggestion_email_info = (suggestion_services
    ...
    def test_email_not_sent_if_sending_reviewer_emails_is_not_enabled(self
        ...
        self.login(self.CURRICULUM_ADMIN_EMAIL, is_super_admin=True)
        with self.testapp_swap:
            with self.swap(email_manager,
                'send_mail_to_notify_contributor_dashboard_reviewers', self
                ._mock_send_contributor_dashboard_reviewers_emails):
                self.get_json(
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False), (platform_parameter_list.ParamName
    ...
    def test_email_sent_to_reviewer_if_sending_reviewer_emails_is_enabled(self
        ...
        self._assert_reviewable_suggestion_email_infos_are_equal(self.
            reviewers_suggestion_email_infos[0][0], self.
...
class CronMailReviewerNewSuggestionsHandlerTests(test_utils.GenericTestBase):
    ...
    def _create_translation_suggestion_for_en_language(self
        ) ->suggestion_registry.BaseSuggestion:
        """Creates a translation suggestion."""
        add_translation_change_dict = {'cmd': exp_domain.
            CMD_ADD_WRITTEN_TRANSLATION, 'state_name': feconf.
    ...
    def _create_translation_suggestion_for_hi_language(self
        ) ->suggestion_registry.BaseSuggestion:
        """Creates a translation suggestion."""
        add_translation_change_dict = {'cmd': exp_domain.
            CMD_ADD_WRITTEN_TRANSLATION, 'state_name': feconf.
            DEFAULT_INIT_STATE_NAME, 'content_id': 'content_0',
            'language_code': 'hi', 'content_html': feconf.
            DEFAULT_STATE_CONTENT_STR, 'translation_html': self.
            default_translation_html, 'data_format': 'html'}
        return suggestion_services.create_suggestion(feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.
    ...
    def test_email_not_sent_if_sending_reviewer_emails_is_not_enabled(self
        ...
        with mock.patch.object(email_manager, 'send_reviewer_notifications',
    ...
    def setUp(self) ->None:
        ...
        translation_suggestion = (self.
            _create_translation_suggestion_for_en_language())
        self.expected_reviewable_suggestion_email_info = (suggestion_services
            .create_reviewable_suggestion_email_info_from_suggestion(
            translation_suggestion))
        self.testapp_swap = self.swap(self, 'testapp', webtest.TestApp(main
    ...
    def test_email_sent_to_reviewer_if_sending_reviewer_emails_is_enabled(self
        ...
        with mock.patch.object(email_manager, 'send_reviewer_notifications',
            ...
            mock_send.assert_called_once_with({'en': [self.reviewer_id]}, {
                'en': [mock.ANY]})
            self.logout()
...
class CronMailAdminContributorDashboardBottlenecksHandlerTests(test_utils.
    GenericTestBase):
    target_id = 'exp1'
    ...
    def _create_question_suggestion(self) ->suggestion_registry.BaseSuggestion:
        ...
        add_question_change_dict: Dict[str, Union[str, question_domain.
            QuestionDict, float]] = {'cmd': question_domain.
            ...
            next_content_id_index, 'version': 44, 'id': ''}, 'skill_id':
    ...
    def mock_send_mail_to_notify_admins_that_reviewers_are_needed(self,
        admin_ids: List[str], translation_admin_ids: List[str],
        question_admin_ids: List[str], suggestion_types_needing_reviewers:
    ...
    def _mock_send_mail_to_notify_admins_suggestions_waiting(self,
        admin_ids: List[str], translation_admin_ids: List[str],
        question_admin_ids: List[str], reviewable_suggestion_email_infos:
        List[suggestion_registry.ReviewableSuggestionEmailInfo]) ->None:
        """Mocks
    ...
    def setUp(self) ->None:
        ...
        self.expected_reviewable_suggestion_email_infos = [suggestion_services
            .create_reviewable_suggestion_email_info_from_suggestion(
            suggestion) for suggestion in [suggestion_1, suggestion_2,
            suggestion_3]]
        self.expected_suggestion_types_needing_reviewers = {feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT: {'en', 'fr'}, feconf.
    ...
    def test_email_sent_to_admin_if_sending_admin_need_reviewers_emails_enabled(
        ...
        self.assertDictEqual(self.suggestion_types_needing_reviewers, self.
            expected_suggestion_types_needing_reviewers)

    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'admin'), (platform_parameter_list.ParamName.
        EMAIL_FOOTER, 'dummy_footer'), (platform_parameter_list.ParamName.
        ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True), (
        platform_parameter_list.ParamName.
        ENABLE_ADMIN_NOTIFICATIONS_FOR_SUGGESTIONS_NEEDING_REVIEW, True)])
    def test_email_sent_to_admin_if_notifying_admins_about_suggestions_enabled(
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            ...
            self.admin_id, 'Updating value.', [platform_parameter_domain.
            PlatformParameterRule.from_dict({'filters': [{'type':
            'platform_type', 'conditions': [['=', 'Web']]}],
            'value_when_matched': True})], False)
        with self.testapp_swap:
            with self.swap(suggestion_models,
                'SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS', 0):
                with self.swap(email_manager,
        ...
        self._assert_reviewable_suggestion_email_infos_are_equal(self.
            reviewable_suggestion_email_infos[0], self.
            expected_reviewable_suggestion_email_infos[0])
        self._assert_reviewable_suggestion_email_infos_are_equal(self.
            reviewable_suggestion_email_infos[1], self.
            expected_reviewable_suggestion_email_infos[1])
        self._assert_reviewable_suggestion_email_infos_are_equal(self.
            reviewable_suggestion_email_infos[2], self.
            expected_reviewable_suggestion_email_infos[2])

    def test_cron_exploration_recommendations_handler(self) ->None:
        self.login(self.CURRICULUM_ADMIN_EMAIL, is_super_admin=True)
        swap_with_checks = self.swap_with_checks(beam_job_services,
            'run_beam_job', lambda **_: None, expected_kwargs=[{'job_class':
...
class CronMailChapterPublicationsNotificationsHandlerTests(test_utils.
    GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.set_curriculum_admins([self.CURRICULUM_ADMIN_USERNAME])
        self.story_publication_timeliness_1 = (story_domain.
            StoryPublicationTimeliness('story_1', 'Story', 'Topic', [
            'Chapter 1'], ['Chapter 2']))
        self.story_publication_timeliness_2 = (story_domain.
            StoryPublicationTimeliness('story_2', 'Story 2', 'Topic', [
            'Chapter 3'], ['Chapter 4']))
        self.testapp_swap = self.swap(self, 'testapp', webtest.TestApp(main
            .app_without_context))
        self.curriculum_admin_ids: List[str] = []
        self.chapter_notifications_list: List[story_domain.
            StoryPublicationTimeliness] = []

    def _mock_send_reminder_mail_to_notify_curriculum_admins(self,
        curriculum_admin_ids: List[str], chapter_notifications_list: List[
    ...
    def _mock_get_chapter_notifications_stories_list(self) ->List[story_domain
        .StoryPublicationTimeliness]:
        """Mocks
        story_services.get_chapter_notifications_stories_list.
        """
        chapter_notifications_stories_list: List[story_domain.
            StoryPublicationTimeliness] = [self.
            story_publication_timeliness_1, self.story_publication_timeliness_2
            ]
    ...
    def test_email_not_sent_if_sending_emails_is_not_enabled(self) ->None:
        ...
        with self.testapp_swap:
            with self.swap(email_manager,
                'send_reminder_mail_to_notify_curriculum_admins', self.
                _mock_send_reminder_mail_to_notify_curriculum_admins):
                self.get_json(
                    '/cron/mail/curriculum_admins/chapter_publication_notfications'
                    )
    ...
    def test_email_sent_if_sending_emails_is_enabled(self) ->None:
        ...
        with self.testapp_swap:
            with self.swap(email_manager,
                'send_reminder_mail_to_notify_curriculum_admins', self.
                _mock_send_reminder_mail_to_notify_curriculum_admins):
                with self.swap(story_services,
                    'get_chapter_notifications_stories_list', self.
                    _mock_get_chapter_notifications_stories_list):
                    self.get_json(
                        '/cron/mail/curriculum_admins/chapter_publication_notfications'
                        )
        ...
        self.assertEqual(self.chapter_notifications_list[0], self.
            story_publication_timeliness_1)
        self.assertEqual(self.chapter_notifications_list[1], self.
            story_publication_timeliness_2)
        self.logout()
