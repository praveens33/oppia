...
exp_models, stats_models, translation_models, user_models = (models.
    Registry.import_models([models.Names.EXPLORATION, models.Names.
    STATISTICS, models.Names.TRANSLATION, models.Names.USER]))
...
class BaseEditorControllerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.voiceover_admin = user_services.get_user_actions_info(self.
            voiceover_admin_id)
    ...
    def assert_can_edit(self, exp_id: str) ->None:
        ...
        editable.
        """
        response = self.get_json('%s/%s' % (feconf.
            USER_PERMISSIONS_URL_PREFIX, exp_id))
...
class EditorTests(BaseEditorControllerTests):
    ...
    def test_editor_page(self) ->None:
        ...
        self.assertIn(b'<exploration-editor-page></exploration-editor-page>',
            response.body)
    ...
    def test_that_default_exploration_cannot_be_published(self) ->None:
        ...
        exp_id = self.post_json(feconf.NEW_EXPLORATION_URL, {}, csrf_token=
            csrf_token)[creator_dashboard.EXPLORATION_ID_KEY]
        csrf_token = self.get_new_csrf_token()
        publish_url = '%s/%s' % (feconf.EXPLORATION_STATUS_PREFIX, exp_id)
        self.put_json(publish_url, {'make_public': True}, csrf_token=
            csrf_token, expected_status_int=400)
        self.logout()
    ...
    def test_add_new_state_error_cases(self) ->None:
        ...
        def _get_payload(new_state_name: str, version: Optional[int]=None
            ...
            result: Dict[str, Union[str, List[Dict[str, Union[str, int]]], int]
                ] = {'change_list': [{'cmd': 'add_state', 'state_name':
                new_state_name, 'content_id_for_state_content':
                content_id_generator.generate(translation_domain.
                ContentType.CONTENT), 'content_id_for_default_outcome':
                content_id_generator.generate(translation_domain.
                ContentType.DEFAULT_OUTCOME)}, {'cmd':
                'edit_exploration_property', 'property_name':
                'next_content_id_index', 'new_value': content_id_generator.
                next_content_id_index}], 'commit_message': 'Add new state'}
            if version is not None:
                result['version'] = version
            return result

        def _put_and_expect_400_error(payload: Dict[str, Union[str, List[
            Dict[str, Union[str, int]]], int]]) ->Dict[str, str]:
            """Puts a request with no version number and hence, expects 400
            error.
            """
            return self.put_json('/createhandler/data/0', payload,
                csrf_token=csrf_token, expected_status_int=400)
        response_dict = _put_and_expect_400_error(_get_payload('New state'))
        self.assertIn('Missing key in handler args: version.',
            response_dict['error'])
        response_dict = _put_and_expect_400_error(_get_payload('New state',
            version=123))
        self.assertIn('which is not possible', response_dict['error'])
        response_dict = _put_and_expect_400_error(_get_payload('', version=
            current_version))
        self.assertIn('should be between 1 and 50', response_dict['error'])
        response_dict = _put_and_expect_400_error(_get_payload('a' * 100,
            version=current_version))
        self.assertIn('should be between 1 and 50', response_dict['error'])
        response_dict = _put_and_expect_400_error(_get_payload(
            '[Bad State Name]', version=current_version))
        self.assertIn('Invalid character [', response_dict['error'])
        response_dict = _put_and_expect_400_error(_get_payload('  aa',
            version=current_version))
        ...
        response_dict = _put_and_expect_400_error(_get_payload('\n',
            version=current_version))
        self.assertIn('end with whitespace', response_dict['error'])
        response_dict = _put_and_expect_400_error(_get_payload('The   B',
            version=current_version))
    ...
    def test_publish_exploration(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        publish_url = '%s/%s' % (feconf.EXPLORATION_STATUS_PREFIX, exp_id)
        exploration_rights = self.put_json(publish_url, {}, csrf_token=
            csrf_token)['rights']
        self.assertEqual(exploration_rights['status'], 'private')
        exploration_rights = self.put_json(publish_url, {'make_public': 
            True}, csrf_token=csrf_token)['rights']
    ...
    def test_lock_exploration(self) ->None:
        ...
        self.put_json(edits_allowed_url, {'edits_are_allowed': False},
    ...
    def test_cannot_update_exploration_when_locked(self) ->None:
        ...
        content_id_generator = translation_domain.ContentIdGenerator(
            exploration.next_content_id_index)
        self.assertEqual(exploration.edits_allowed, False)
        response_dict = self.put_json('%s/%s' % (feconf.
            EXPLORATION_DATA_PREFIX, exp_id), {'version': exploration.
            version, 'commit_message': 'dummy update', 'change_list': [{
            ...
            (translation_domain.ContentType.DEFAULT_OUTCOME)}, {'cmd':
            'edit_state_property', 'state_name': 'State 4', 'property_name':
            'widget_id', 'new_value': 'TextInput'}, {'cmd':
            'edit_exploration_property', 'property_name':
            'next_content_id_index', 'new_value': content_id_generator.
            next_content_id_index}]}, csrf_token=csrf_token,
...
"""
    ...
    def test_exploration_download_handler_for_default_exploration(self) ->None:
        ...
        exp_id = 'eid'
        self.save_new_valid_exploration(exp_id, owner_id, title=
            'The title for ZIP download handler test!', category=
            'This is just a test category', objective='')
        exploration = exp_fetchers.get_exploration_by_id(exp_id)
        content_id_generator = translation_domain.ContentIdGenerator(
            exploration.next_content_id_index)
        ...
        exp_services.update_exploration(owner_id, exp_id, [exp_domain.
            ...
            'auto_tts_enabled', 'new_value': True}), exp_domain.
            ExplorationChange({'cmd': exp_domain.CMD_ADD_STATE,
            'state_name': 'State A', 'content_id_for_state_content':
            ...
            exp_domain.CMD_ADD_STATE, 'state_name': 'State 2',
            'content_id_for_state_content': content_id_generator.generate(
            translation_domain.ContentType.CONTENT),
            'content_id_for_default_outcome': content_id_generator.generate
            (translation_domain.ContentType.DEFAULT_OUTCOME)}), exp_domain.
            ExplorationChange({'cmd': exp_domain.CMD_ADD_STATE,
            'state_name': 'State 3', 'content_id_for_state_content':
            content_id_generator.generate(translation_domain.ContentType.
            CONTENT), 'content_id_for_default_outcome':
            content_id_generator.generate(translation_domain.ContentType.
            DEFAULT_OUTCOME)}), exp_domain.ExplorationChange({'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_INTERACTION_ID, 'state_name': 'State A',
            'new_value': 'TextInput'}), exp_domain.ExplorationChange({'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_INTERACTION_CUST_ARGS, 'state_name': 'State A',
            ...
            STATE_PROPERTY_INTERACTION_ID, 'state_name': 'State 2',
            'new_value': 'TextInput'}), exp_domain.ExplorationChange({'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_INTERACTION_CUST_ARGS, 'state_name': 'State 2',
            'new_value': {'placeholder': {'value': {'content_id':
            content_id_generator.generate(translation_domain.ContentType.
            CUSTOMIZATION_ARG, extra_prefix='placeholder'), 'unicode_str':
            ''}}, 'rows': {'value': 1}, 'catchMisspellings': {'value': 
            False}}}), exp_domain.ExplorationChange({'cmd': exp_domain.
            CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain.
            STATE_PROPERTY_INTERACTION_ID, 'state_name': 'State 3',
            'new_value': 'TextInput'}), exp_domain.ExplorationChange({'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_INTERACTION_CUST_ARGS, 'state_name': 'State 3',
            'new_value': {'placeholder': {'value': {'content_id':
            content_id_generator.generate(translation_domain.ContentType.
            CUSTOMIZATION_ARG, extra_prefix='placeholder'), 'unicode_str':
            ''}}, 'rows': {'value': 1}, 'catchMisspellings': {'value': 
            False}}}), exp_domain.ExplorationChange({'cmd': exp_domain.
            CMD_RENAME_STATE, 'old_state_name': 'State 2', 'new_state_name':
            'State B'}), exp_domain.ExplorationChange({'cmd': exp_domain.
            CMD_DELETE_STATE, 'state_name': 'State 3'}), exp_domain.
            ExplorationChange({'cmd': 'edit_exploration_property',
            'property_name': 'next_content_id_index', 'new_value':
            content_id_generator.next_content_id_index})], 'changes')
        ...
        self.assertEqual(zf_saved.open(
            'The title for ZIP download handler test.yaml').read(), zf_gold
            .open('The title for ZIP download handler test!.yaml').read())
        exp_services.update_exploration(owner_id, exp_id, [exp_domain.
            ExplorationChange({'cmd': exp_domain.
            CMD_EDIT_EXPLORATION_PROPERTY, 'property_name': 'objective',
            'new_value': 'Test JSON download'})],
            'Updates exploration objective')
        self.maxDiff = None
        download_url = '/createhandler/download/%s?output_format=%s' % (exp_id,
    ...
    def test_exploration_download_handler_with_unicode_title(self) ->None:
        ...
        owner_id = self.get_user_id_from_email(self.EDITOR_EMAIL)
        exp_id = 'eid'
        self.save_new_valid_exploration(exp_id, owner_id, title='¡Hola!',
            category='This is just a test category', objective='')
        download_url = '/createhandler/download/%s' % exp_id
        response = self.get_custom_response(download_url, 'text/plain')
    ...
    def test_state_yaml_handler(self) ->None:
        ...
        exp_id = 'eid'
        self.save_new_valid_exploration(exp_id, owner_id, title=
            'The title for states download handler test!', category=
            'This is just a test category')
        exploration = exp_fetchers.get_exploration_by_id(exp_id)
        exploration.add_states(['State A', 'State 2', 'State 3'])
        content_id_generator = translation_domain.ContentIdGenerator(
            exploration.next_content_id_index)
        self.set_interaction_for_state(exploration.states['State A'],
            'TextInput', content_id_generator)
        csrf_token = self.get_new_csrf_token()
        response = self.post_json('/createhandler/state_yaml/%s' % exp_id,
            {'state_dict': exploration.states['State A'].to_dict(), 'width':
    ...
    def test_exploration_download_handler_with_invalid_exploration_id(self
        ...
        self.login(self.OWNER_EMAIL)
        self.get_json('/createhandler/download/invalid_id',
    ...
    def test_exploration_download_handler_with_invalid_output_format(self
        ...
        error_msg = """At 'http://localhost/createhandler/download/exp_id1?output_format=invalid_output_format' these errors are happening:
Schema validation for 'output_format' failed: Received invalid_output_format which is not in the allowed range of choices: ['zip', 'json']"""
...
class ExplorationSnapshotsHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_with_invalid_exploration_id_raises_error(self) ->None:
        self.login(self.OWNER_EMAIL)
        self.get_json('/createhandler/snapshots/invalid_id',
            expected_status_int=404)
        self.logout()
    ...
    def test_get_exploration_snapshot_history(self) ->None:
        ...
        exp_services.update_exploration(owner_id, exp_id, [exp_domain.
            ...
            'next_content_id_index', 'new_value': content_id_generator.
            next_content_id_index, 'old_value': 0})], 'Addes state')
        snapshots = exp_services.get_exploration_snapshots_metadata(exp_id)
        for snapshot in snapshots:
            snapshot.update({'committer_id': 'owner'})
        response = self.get_json('/createhandler/snapshots/%s' % exp_id)
...
class ExplorationStatisticsHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_with_invalid_exploration_id_raises_error(self) ->None:
        self.login(self.OWNER_EMAIL)
        self.get_json('/createhandler/statistics/invalid_id',
            expected_status_int=404)
        self.logout()

    def test_guest_cannot_access_exploration_statistics_handler(self) ->None:
        self.save_new_valid_exploration('exp_id', 'owner_id')
        self.get_json('/createhandler/statistics/exp_id',
            expected_status_int=404)
    ...
    def test_get_exploration_statistics(self) ->None:
        ...
        content_id_generator = translation_domain.ContentIdGenerator(
            exploration.next_content_id_index)
        exp_stats = stats_services.get_exploration_stats(exp_id,
            exploration.version)
...
class StartedTutorialEventHandlerTests(test_utils.GenericTestBase):
    ...
    def test_record_user_saw_tutorial(self) ->None:
        ...
        self.post_json('/createhandler/started_tutorial_event/%s' % exp_id,
            {}, csrf_token=csrf_token)
        user_settings = user_services.get_user_settings(owner_id)
...
class TopUnresolvedAnswersHandlerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.owner_id = self.get_user_id_from_email(self.OWNER_EMAIL)
        self.exp_id = 'exp_id'
        self.exploration = self.save_new_valid_exploration(self.exp_id,
            self.owner_id)
...
class StateInteractionStatsHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_with_invalid_exploration_id_raises_error(self) ->None:
        self.login(self.OWNER_EMAIL)
        self.get_json('/createhandler/state_interaction_stats/%s/%s' % (
            'invalid_id', 'state_name'), expected_status_int=404)
        self.logout()

    def test_cannot_get_learner_answer_statistics_with_invalid_state_name(self
        ) ->None:
        observed_log_messages = []

        def _mock_logging_function(msg: str, *args: str, **unused_kwargs: str
            ) ->None:
            """Mocks logging.error()."""
        ...
        with logging_swap:
            self.get_json('/createhandler/state_interaction_stats/%s/%s' %
                (exp_id, 'invalid_state_name'), expected_status_int=404)
    ...
    def test_get_learner_answer_statistics_for_state(self) ->None:
        ...
        exploration = self.save_new_valid_exploration(exp_id, owner_id)
        response = self.get_json(
            '/createhandler/state_interaction_stats/%s/%s' % (exp_id,
...
class ExplorationDeletionRightsTests(BaseEditorControllerTests):
    ...
    def test_deletion_rights_for_unpublished_exploration(self) ->None:
        ...
        rights_manager.assign_role_for_exploration(self.owner,
            unpublished_exp_id, self.editor_id, rights_domain.ROLE_EDITOR)
        self.login(self.EDITOR_EMAIL)
        self.delete_json('/createhandler/data/%s' % unpublished_exp_id,
    ...
    def test_deletion_rights_for_published_exploration(self) ->None:
        ...
        rights_manager.assign_role_for_exploration(self.owner,
            published_exp_id, self.editor_id, rights_domain.ROLE_EDITOR)
        rights_manager.assign_role_for_exploration(self.voiceover_admin,
            published_exp_id, self.voice_artist_id, rights_domain.
            ROLE_VOICE_ARTIST)
        self.login(self.EDITOR_EMAIL)
        self.delete_json('/createhandler/data/%s' % published_exp_id,
            expected_status_int=401)
        self.logout()
        self.login(self.VIEWER_EMAIL)
        self.delete_json('/createhandler/data/%s' % published_exp_id,
    ...
    def test_logging_info_after_deletion(self) ->None:
        ...
        with self.swap(logging, 'info', mock_logging_function), self.swap(
            ...
            self.assertEqual(observed_log_messages, [
                '(%s) %s tried to delete exploration %s' % ([feconf.
                ROLE_ID_FULL_USER], self.owner_id, exp_id), 
                '(%s) %s deleted exploration %s' % ([feconf.
                ROLE_ID_FULL_USER], self.owner_id, exp_id)])
            self.logout()
            observed_log_messages = []
            exp_id = 'unpub_eid3'
            ...
            self.assertEqual(observed_log_messages, [
                '(%s) %s tried to delete exploration %s' % ([feconf.
                ROLE_ID_FULL_USER, feconf.ROLE_ID_MODERATOR], self.
                moderator_id, exp_id), '(%s) %s deleted exploration %s' % (
                [feconf.ROLE_ID_FULL_USER, feconf.ROLE_ID_MODERATOR], self.
                moderator_id, exp_id)])
            self.logout()
...
class VersioningIntegrationTest(BaseEditorControllerTests):
    ...
    def setUp(self) ->None:
        ...
        rights_manager.release_ownership_of_exploration(self.system_user,
            self.EXP_ID)
        self.login(self.EDITOR_EMAIL)
        self.editor_id = self.get_user_id_from_email(self.EDITOR_EMAIL)
        ...
        exp_services.update_exploration(self.editor_id, self.EXP_ID, [
            ...
            'new_value': 'the objective'}), exp_domain.ExplorationChange({
            'cmd': 'edit_state_property', 'property_name': 'content',
            'state_name': exploration.init_state_name, 'new_value': {
            'content_id': init_state.content.content_id, 'html':
            '<p>ABC</p>'}})], 'Change objective and init state content')

    def test_get_with_disabled_exploration_id_raises_error(self) ->None:
        self.get_html_response('%s/%s' % (feconf.EDITOR_URL_PREFIX, feconf.
            DISABLED_EXPLORATION_IDS[0]), expected_status_int=404)
    ...
    def test_reverting_to_old_exploration(self) ->None:
        ...
        for rev_version in (2, 3, 4, '10'):
            response_dict = self.post_json('/createhandler/revert/%s' %
                self.EXP_ID, {'current_version': 2, 'revert_to_version':
                rev_version}, csrf_token=csrf_token, expected_status_int=400)
            self.assertIn('Cannot revert to version', response_dict['error'])
            reader_dict = self.get_json('%s/%s' % (feconf.
                EXPLORATION_INIT_URL_PREFIX, self.EXP_ID))
        ...
        for rev_version in ('abc', ()):
            response_dict = self.post_json('/createhandler/revert/%s' %
                self.EXP_ID, {'current_version': 2, 'revert_to_version':
                rev_version}, csrf_token=csrf_token, expected_status_int=400)
            self.assertIn("Schema validation for 'revert_to_version' failed:",
                response_dict['error'])
        rev_version = 1
        response_dict = self.post_json('/createhandler/revert/%s' % self.
            EXP_ID, {'current_version': 2, 'revert_to_version': rev_version
            }, csrf_token=csrf_token)
        reader_dict = self.get_json('%s/%s' % (feconf.
            EXPLORATION_INIT_URL_PREFIX, self.EXP_ID))
        init_state_name = reader_dict['exploration']['init_state_name']
    ...
    def test_versioning_for_default_exploration(self) ->None:
        ...
        self.assertIn('ABC', init_content)
        self.assertNotIn('Hi, welcome to Oppia!', init_content)
        reader_dict = self.get_json('%s/%s?v=1' % (feconf.
            EXPLORATION_INIT_URL_PREFIX, self.EXP_ID))
        init_state_name = reader_dict['exploration']['init_state_name']
        init_state_data = reader_dict['exploration']['states'][init_state_name]
        init_content = init_state_data['content']['html']
        self.assertIn('Hi, welcome to Oppia!', init_content)
        self.assertNotIn('ABC', init_content)
        reader_dict = self.get_json('%s/%s?v=2' % (feconf.
            EXPLORATION_INIT_URL_PREFIX, self.EXP_ID))
        init_state_name = reader_dict['exploration']['init_state_name']
        init_state_data = reader_dict['exploration']['states'][init_state_name]
        init_content = init_state_data['content']['html']
        self.assertIn('ABC', init_content)
        self.assertNotIn('Hi, welcome to Oppia!', init_content)
        self.get_json('%s/%s?v=3' % (feconf.EXPLORATION_INIT_URL_PREFIX,
            self.EXP_ID), expected_status_int=404)

    def test_revert_with_invalid_current_version_raises_error(self) ->None:
        csrf_token = self.get_new_csrf_token()
        response = self.post_json('/createhandler/revert/%s' % self.EXP_ID,
            {'current_version': 'invalid_version', 'revert_to_version': 1},
            csrf_token=csrf_token, expected_status_int=400)
        error_msg = """At 'http://localhost/createhandler/revert/0' these errors are happening:
Schema validation for 'current_version' failed: Could not convert str to int: invalid_version"""
        self.assertEqual(response['error'], error_msg)
...
class ExplorationRightsIntegrationTest(BaseEditorControllerTests):
    ...
    def test_for_deassign_editor_role(self) ->None:
        ...
        self.save_new_valid_exploration(exp_id, self.owner_id, title=
            'Title for rights handler test!', category='My category')
        exploration = exp_fetchers.get_exploration_by_id(exp_id)
        exploration.add_states(['State A', 'State 2', 'State 3'])
        content_id_generator = translation_domain.ContentIdGenerator(
            exploration.next_content_id_index)
        self.set_interaction_for_state(exploration.states['State A'],
            'TextInput', content_id_generator)
        self.set_interaction_for_state(exploration.states['State 2'],
            'TextInput', content_id_generator)
        self.set_interaction_for_state(exploration.states['State 3'],
            'TextInput', content_id_generator)
        self.logout()
        self.login(self.COLLABORATOR_EMAIL)
        self.get_json('%s/%s' % (feconf.USER_PERMISSIONS_URL_PREFIX, exp_id
            ), expected_status_int=404)
        self.logout()
        self.login(self.OWNER_EMAIL)
        csrf_token = self.get_new_csrf_token()
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        self.put_json(rights_url, {'version': exploration.version,
            'new_member_username': self.COLLABORATOR_USERNAME,
            'new_member_role': rights_domain.ROLE_EDITOR,
            'viewable_if_private': None}, csrf_token=csrf_token)
        self.logout()
        self.login(self.COLLABORATOR_EMAIL)
        self.assert_can_edit(exp_id)
        ...
        self.logout()
        self.login(self.OWNER_EMAIL)
        self.delete_json(rights_url, params={'username': self.
    ...
    def test_for_deassign_sole_owner_from_exploration(self) ->None:
        ...
        self.set_interaction_for_state(exploration.states['State 3'],
            'TextInput', content_id_generator)
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        response = self.delete_json(rights_url, params={'username': self.
            OWNER_USERNAME}, expected_status_int=400)
    ...
    def test_users_cannot_assign_other_role_to_itself(self) ->None:
        ...
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        csrf_token = self.get_new_csrf_token()
        response = self.put_json(rights_url, {'version': exploration.
            version, 'new_member_username': self.OWNER_USERNAME,
            'new_member_role': rights_domain.ROLE_EDITOR,
            'make_community_owned': False, 'viewable_if_private': None},
            csrf_token=csrf_token, expected_status_int=400)
        self.assertEqual(response['error'],
    ...
    def test_for_deassign_viewer_role_from_exploration(self) ->None:
        ...
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        self.put_json(rights_url, {'version': exploration.version,
            'new_member_username': self.VIEWER_USERNAME, 'new_member_role':
            rights_domain.ROLE_VIEWER, 'viewable_if_private': None},
            csrf_token=csrf_token)
        self.logout()
        self.login(self.VIEWER_EMAIL)
        self.assert_cannot_edit(exp_id)
    ...
    def test_role_must_be_provided_for_a_new_member(self) ->None:
        ...
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        response = self.put_json(rights_url, {'version': exploration.
            version, 'new_member_username': self.COLLABORATOR_USERNAME,
            'viewable_if_private': None}, csrf_token=csrf_token,
            expected_status_int=400)
        self.assertEqual(response['error'],
    ...
    def test_that_an_editor_can_edit_the_exploration(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        reader_dict = self.get_json('%s/%s' % (feconf.
            EXPLORATION_DATA_PREFIX, exp_id))
        self.assertNotIn('State B', reader_dict['states'])
        response = self.put_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX,
            exp_id), {'version': exploration.version, 'commit_message':
            'Added State B', 'change_list': [{'cmd': 'add_state',
            'state_name': 'State B', 'content_id_for_state_content':
            content_id_generator.generate(translation_domain.ContentType.
            CONTENT), 'content_id_for_default_outcome':
            content_id_generator.generate(translation_domain.ContentType.
            DEFAULT_OUTCOME)}, {'cmd': 'edit_exploration_property',
            'property_name': 'next_content_id_index', 'new_value':
            content_id_generator.next_content_id_index}]}, csrf_token=
            csrf_token, expected_status_int=200)
    ...
    def test_that_an_editor_cannot_assign_role_to_others(self) ->None:
        ...
        reader_dict = self.get_json('%s/%s' % (feconf.
            EXPLORATION_DATA_PREFIX, exp_id))
        self.assertNotIn(self.COLLABORATOR2_USERNAME, reader_dict['rights']
            ['editor_names'])
        response = self.put_json(rights_url, {'version': exploration.
            version, 'new_member_username': self.COLLABORATOR2_USERNAME,
            'new_member_role': rights_domain.ROLE_EDITOR}, csrf_token=
            csrf_token, expected_status_int=401)
        error_msg = (
    ...
    def test_that_a_voice_artist_cannot_edit_the_exploration(self) ->None:
        ...
        rights_manager.assign_role_for_exploration(self.voiceover_admin,
            exp_id, self.voice_artist_id, rights_domain.ROLE_VOICE_ARTIST)
        voiceover_artist_email = user_services.get_email_from_user_id(self.
            voice_artist_id)
        ...
        reader_dict = self.get_json('%s/%s' % (feconf.
            EXPLORATION_DATA_PREFIX, exp_id))
        self.assertNotIn('State B', reader_dict['states'])
        self.put_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX, exp_id), {
            'version': exploration.version, 'commit_message':
            'Added State B', 'change_list': [{'cmd': 'add_state',
            'state_name': 'State B', 'content_id_for_state_content':
            content_id_generator.generate(translation_domain.ContentType.
            CONTENT), 'content_id_for_default_outcome':
    ...
    def test_for_checking_username_is_valid(self) ->None:
        ...
        response = self.delete_json('%s/%s' % (feconf.
            EXPLORATION_RIGHTS_PREFIX, exp_id), params={'username':
            'any_username'}, expected_status_int=400)
        self.assertEqual(response['error'],
    ...
    def test_transfering_ownership_to_the_community(self) ->None:
        ...
        exp_id = 'exp_id'
        self.save_new_valid_exploration(exp_id, self.owner_id, title=
            'My Exploration', end_state_name='END')
        csrf_token = self.get_new_csrf_token()
        rights_manager.publish_exploration(self.owner, exp_id)
        exploration = exp_fetchers.get_exploration_by_id(exp_id)
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        self.put_json(rights_url, {'version': exploration.version,
            'make_community_owned': True, 'new_member_username': None,
            'new_member_role': None, 'viewable_if_private': None},
            csrf_token=csrf_token)
        self.logout()
    ...
    def test_cannot_transfer_ownership_of_invalid_exp_to_the_community(self
        ...
        content_id_generator = translation_domain.ContentIdGenerator()
        model = exp_models.ExplorationModel(id=exp_id, category='category',
            title='title', language_code='invalid_language_code',
            init_state_name=feconf.DEFAULT_INIT_STATE_NAME, states={feconf.
            DEFAULT_INIT_STATE_NAME: state_domain.State.
            create_default_state('End', content_id_generator.generate(
            translation_domain.ContentType.CONTENT), content_id_generator.
            generate(translation_domain.ContentType.DEFAULT_OUTCOME),
            is_initial_state=True).to_dict()}, next_content_id_index=
            content_id_generator.next_content_id_index,
            states_schema_version=feconf.CURRENT_STATE_SCHEMA_VERSION)
        commit_cmd = exp_domain.ExplorationChange({'cmd': exp_domain.
            CMD_CREATE_NEW, 'title': 'title', 'category': 'category'})
        commit_cmds_dict = [commit_cmd.to_dict()]
        model.commit(self.owner_id, 'commit_message', commit_cmds_dict)
        csrf_token = self.get_new_csrf_token()
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        response = self.put_json(rights_url, {'version': 1,
            'make_community_owned': True, 'new_member_username': None,
            'new_member_role': None, 'viewable_if_private': None},
            csrf_token=csrf_token, expected_status_int=400)
        self.assertEqual(response['error'],
    ...
    def test_get_with_invalid_version_raises_error(self) ->None:
        ...
        exp_id = 'exp_id'
        self.save_new_valid_exploration(exp_id, self.owner_id)
        self.get_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX, exp_id),
    ...
    def test_put_with_long_commit_message_raises_error(self) ->None:
        ...
        response_dict = self.put_json('%s/%s' % (feconf.
            EXPLORATION_DATA_PREFIX, exp_id), {'version': exploration.
            version, 'commit_message': long_commit_message, 'change_list':
            [{'cmd': 'add_state', 'state_name': 'State 4',
            'content_id_for_state_content': content_id_generator.generate(
            translation_domain.ContentType.CONTENT),
            'content_id_for_default_outcome': content_id_generator.generate
...
Schema validation for 'commit_message' failed: Validation failed: has_length_at_most ({'max_value': 375}) for object %s"""
    ...
    def test_put_with_invalid_new_member_raises_error(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        exploration = exp_fetchers.get_exploration_by_id(exp_id)
        response = self.put_json('%s/%s' % (feconf.
            EXPLORATION_RIGHTS_PREFIX, exp_id), {'version': exploration.
            version, 'new_member_username': 'invalid_new_member_username',
            'make_community_owned': False, 'new_member_role': None,
            'viewable_if_private': None}, csrf_token=csrf_token,
            expected_status_int=400)
        self.assertEqual(response['error'],
            'Sorry, we could not find the specified user.')
    ...
    def test_put_with_deleted_user_raises_error(self) ->None:
        ...
        response = self.put_json('%s/%s' % (feconf.
            EXPLORATION_RIGHTS_PREFIX, exp_id), {'version': exploration.
            version, 'new_member_username': self.VIEWER_USERNAME,
            'make_community_owned': False, 'new_member_role': None,
            'viewable_if_private': None}, csrf_token=csrf_token,
            expected_status_int=400)
        self.assertEqual(response['error'],
    ...
    def test_make_private_exploration_viewable(self) ->None:
        ...
        self.put_json('%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id),
            {'version': exploration.version, 'viewable_if_private': True,
            'new_member_username': None, 'new_member_role': None,
            'make_community_owned': False}, csrf_token=csrf_token)
        exploration = exp_fetchers.get_exploration_by_id(exp_id)
        exploration_rights = rights_manager.get_exploration_rights(exp_id)
    ...
    def test_put_with_no_specified_changes_raise_error(self) ->None:
        ...
        response = self.put_json('%s/%s' % (feconf.
            EXPLORATION_RIGHTS_PREFIX, exp_id), {'version': exploration.
            version, 'new_member_username': None, 'make_community_owned': 
            False, 'new_member_role': None, 'viewable_if_private': None},
            csrf_token=csrf_token, expected_status_int=400)
        self.assertEqual(response['error'],
    ...
    def test_can_not_assign_roles_with_invalid_payload_version(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        rights_url = '%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, exp_id)
        response_dict = self.put_json(rights_url, {'version': None,
            'new_member_username': self.COLLABORATOR_USERNAME,
            'new_member_role': rights_domain.ROLE_EDITOR}, csrf_token=
            csrf_token, expected_status_int=400)
        self.assertEqual(response_dict['error'],
            """At 'http://localhost/createhandler/rights/eid' these errors are happening:
Missing key in handler args: version."""
            )
        response_dict = self.put_json(rights_url, {'version': 2,
            'new_member_username': self.COLLABORATOR_USERNAME,
            'new_member_role': rights_domain.ROLE_EDITOR}, csrf_token=
            csrf_token, expected_status_int=400)
        self.assertEqual(response_dict['error'],
            'Trying to update version 1 of exploration from version 2, which is too old. Please reload the page and try again.'
            )
        self.logout()
...
class UserExplorationEmailsIntegrationTest(BaseEditorControllerTests):
    ...
    def test_user_exploration_emails_handler(self) ->None:
        ...
        exp_id = 'eid'
        self.save_new_valid_exploration(exp_id, self.owner_id, title=
            'Title for emails handler test!', category='Category')
        csrf_token = self.get_new_csrf_token()
        exp_email_preferences = (user_services.
            get_email_preferences_for_exploration(self.owner_id, exp_id))
        ...
        self.put_json(emails_url, {'mute': True, 'message_type': 'feedback'
            }, csrf_token=csrf_token)
        exp_email_preferences = (user_services.
            get_email_preferences_for_exploration(self.owner_id, exp_id))
        self.assertTrue(exp_email_preferences.mute_feedback_notifications)
        self.assertFalse(exp_email_preferences.mute_suggestion_notifications)
        self.put_json(emails_url, {'mute': True, 'message_type':
            'suggestion'}, csrf_token=csrf_token)
        self.put_json(emails_url, {'mute': False, 'message_type':
            'feedback'}, csrf_token=csrf_token)
        exp_email_preferences = (user_services.
            get_email_preferences_for_exploration(self.owner_id, exp_id))
        self.assertFalse(exp_email_preferences.mute_feedback_notifications)
    ...
    def test_put_with_invalid_message_type_raises_error(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        response = self.put_json('%s/%s' % (feconf.
            USER_EXPLORATION_EMAILS_PREFIX, exp_id), {'message_type':
            'invalid_message_type'}, csrf_token=csrf_token,
            expected_status_int=400)
        error_msg = """At 'http://localhost/createhandler/notificationpreferences/eid' these errors are happening:
Schema validation for 'message_type' failed: Received invalid_message_type which is not in the allowed range of choices: ['feedback', 'suggestion']"""
        self.assertEqual(response['error'], error_msg)
        self.logout()
...
class ModeratorEmailsTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.signup(self.MODERATOR_EMAIL, self.MODERATOR_USERNAME)
        self.set_moderators([self.MODERATOR_USERNAME])
        self.save_new_valid_exploration(self.EXP_ID, self.editor_id, title=
            'My Exploration', end_state_name='END')
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            ...
            'Updating email body.', [platform_parameter_domain.
    ...
    def test_error_cases_when_can_send_emails_param_is_false(self) ->None:
        self.login(self.MODERATOR_EMAIL)
        csrf_token = self.get_new_csrf_token()
        response_dict = self.put_json('/createhandler/moderatorrights/%s' %
            self.EXP_ID, {'email_body': None, 'version': 1}, csrf_token=
            csrf_token, expected_status_int=400)
        self.assertIn('Missing key in handler args: email_body.',
            response_dict['error'])
        response_dict = self.put_json('/createhandler/moderatorrights/%s' %
            self.EXP_ID, {'email_body': '', 'version': 1}, csrf_token=
            csrf_token, expected_status_int=400)
        error_msg = (
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_FOOTER, 'footer'), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin')])
    def test_error_cases_when_can_send_emails_param_is_true(self) ->None:
        self.login(self.MODERATOR_EMAIL)
        csrf_token = self.get_new_csrf_token()
        valid_payload = {'email_body': 'Your exploration is featured!',
            'version': 1}
        self.put_json('/createhandler/moderatorrights/%s' % self.EXP_ID,
    ...
    def test_email_is_sent_correctly_when_unpublishing(self) ->None:
        ...
        self.assertEqual(messages[0].sender, 'Site Admin <%s>' % feconf.
...
You can change your email preferences via the Preferences page."""
             % (self.EDITOR_USERNAME, new_email_body, self.MODERATOR_USERNAME))
        self.assertEqual(messages[0].html, 
            'Hi %s,<br><br>%s<br><br>Thanks!<br>%s (Oppia moderator)<br><br>You can change your email preferences via the <a href="http://localhost:8181/preferences">Preferences</a> page.'
             % (self.EDITOR_USERNAME, new_email_body, self.MODERATOR_USERNAME))
        self.logout()
    ...
    def test_email_functionality_cannot_be_used_by_non_moderators(self) ->None:
        ...
        valid_payload = {'email_body': new_email_body, 'version': 1}
        self.put_json('/createhandler/moderatorrights/%s' % self.EXP_ID,
            valid_payload, csrf_token=csrf_token, expected_status_int=401)
        self.logout()
...
class FetchIssuesPlaythroughHandlerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.save_new_valid_exploration(self.EXP_ID, self.editor_id, title=
            'My Exploration', end_state_name='END')
        rights_manager.publish_exploration(self.editor, self.EXP_ID)
        self.playthrough_id1 = stats_models.PlaythroughModel.create(self.
            EXP_ID, 1, 'EarlyQuit', {'state_name': {'value': 'state_name1'},
            'time_spent_in_exp_in_msecs': {'value': 200}}, [{'action_type':
            'ExplorationStart', 'action_customization_args': {'state_name':
            {'value': 'state_name1'}}, 'schema_version': 1}])
        self.playthrough_id2 = stats_models.PlaythroughModel.create(self.
            EXP_ID, 1, 'MultipleIncorrectSubmissions', {'state_name': {
            'value': 'state_name1'}, 'num_times_answered_incorrectly': {
            'value': 7}}, [{'action_type': 'ExplorationStart',
            'action_customization_args': {'state_name': {'value':
            'state_name1'}}, 'schema_version': 1}])
        stats_models.ExplorationIssuesModel.create(self.EXP_ID, 1, [{
            'issue_type': 'EarlyQuit', 'issue_customization_args': {
            'state_name': {'value': 'state_name1'},
            'time_spent_in_exp_in_msecs': {'value': 200}},
            'playthrough_ids': [self.playthrough_id1], 'schema_version': 1,
            'is_valid': True}, {'issue_type':
            'MultipleIncorrectSubmissions', 'issue_customization_args': {
            'state_name': {'value': 'state_name1'},
            'num_times_answered_incorrectly': {'value': 7}},
            'playthrough_ids': [self.playthrough_id2], 'schema_version': 1,
            'is_valid': True}])

    def test_cannot_fetch_issues_with_invalid_version(self) ->None:
        self.get_json('/issuesdatahandler/%s' % self.EXP_ID, params={
            'exp_version': 2}, expected_status_int=404)

    def test_cannot_fetch_playthrough_with_invalid_playthrough_id(self) ->None:
        self.get_json('/playthroughdatahandler/%s/%s' % (self.EXP_ID,
            'invalid_playthrough_id'), expected_status_int=404)

    def test_fetch_issues_handler_with_disabled_exp_id(self) ->None:
        self.get_json('/issuesdatahandler/5', params={'exp_version': 2},
            expected_status_int=404)
    ...
    def test_fetch_issues_handler(self) ->None:
        ...
        self.assertEqual(response['unresolved_issues'][0]['issue_type'],
            'EarlyQuit')
        self.assertEqual(response['unresolved_issues'][1]['issue_type'],
            'MultipleIncorrectSubmissions')

    def test_invalid_issues_are_not_retrieved(self) ->None:
        """Test that invalid issues are not retrieved."""
        exp_issues_model = stats_models.ExplorationIssuesModel.get_model(self
            .EXP_ID, 1)
    ...
    def test_fetch_playthrough_handler(self) ->None:
        ...
        response = self.get_json('/playthroughdatahandler/%s/%s' % (self.
            EXP_ID, self.playthrough_id1))
        ...
        self.assertEqual(response['issue_customization_args'], {
            'state_name': {'value': 'state_name1'},
            'time_spent_in_exp_in_msecs': {'value': 200}})
        self.assertEqual(response['actions'], [{'action_type':
            'ExplorationStart', 'action_customization_args': {'state_name':
            {'value': 'state_name1'}}, 'schema_version': 1}])
...
class ResolveIssueHandlerTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.playthrough_id2 = stats_models.PlaythroughModel.create(self.
            ...
            'time_spent_in_exp_in_msecs': {'value': 200}}, [{'action_type':
            'ExplorationStart', 'action_customization_args': {'state_name':
            {'value': 'state_name2'}}, 'schema_version': 1}])
        stats_models.ExplorationIssuesModel.create(self.EXP_ID, 1, [{
            'issue_type': 'EarlyQuit', 'issue_customization_args': {
            'state_name': {'value': 'state_name1'},
            'time_spent_in_exp_in_msecs': {'value': 200}},
        ...
        self.exp_issue_dict = {'issue_type': 'EarlyQuit',
            ...
            'state_name1'}, 'time_spent_in_exp_in_msecs': {'value': 200}},
            'playthrough_ids': [self.playthrough_id1, self.playthrough_id2],
            'schema_version': 1, 'is_valid': True}
    ...
    def test_resolve_issue_handler(self) ->None:
        ...
        with self.login_context(self.MODERATOR_EMAIL):
            csrf_token = self.get_new_csrf_token()
            self.post_json('/resolveissuehandler/%s' % self.EXP_ID, {
    ...
    def test_error_on_passing_non_matching_exp_issue_dict(self) ->None:
        ...
        """
        exp_issue_dict = {'issue_type': 'EarlyQuit',
            'issue_customization_args': {'state_name': {'value':
            'state_name2'}, 'time_spent_in_exp_in_msecs': {'value': 200}},
            'playthrough_ids': [self.playthrough_id1, self.playthrough_id2],
            'schema_version': 1, 'is_valid': True}
        with self.login_context(self.MODERATOR_EMAIL):
            csrf_token = self.get_new_csrf_token()
...
class EditorAutosaveTest(BaseEditorControllerTests):
    ...
    EXP_ID2: Final = '2'
    EXP_ID3: Final = '3'
    NEWER_DATETIME: Final = get_current_datetime_utc() + datetime.timedelta(30)
    OLDER_DATETIME: Final = datetime.datetime.strptime('2015-03-16', '%Y-%m-%d'
        )
    DRAFT_CHANGELIST: Final = [{'cmd': 'edit_exploration_property',
        'property_name': 'title', 'old_value': None, 'new_value':
        'Updated title'}]
    NEW_CHANGELIST: Final = [{'cmd': 'edit_exploration_property',
        'property_name': 'title', 'old_value': None, 'new_value': 'New title'}]
    INVALID_CHANGELIST: Final = [{'cmd': 'edit_exploration_property',
        'property_name': 'title', 'new_value': 1}]
    ...
    def _create_exp_user_data_model_objects_for_tests(self) ->None:
        ...
        user_models.ExplorationUserDataModel(id='%s.%s' % (self.owner_id,
            self.EXP_ID1), user_id=self.owner_id, exploration_id=self.
            EXP_ID1, draft_change_list=self.DRAFT_CHANGELIST,
            draft_change_list_last_updated=self.NEWER_DATETIME,
            draft_change_list_exp_version=1, draft_change_list_id=1).put()
        user_models.ExplorationUserDataModel(id='%s.%s' % (self.owner_id,
            self.EXP_ID2), user_id=self.owner_id, exploration_id=self.
            EXP_ID2, draft_change_list=self.DRAFT_CHANGELIST,
            draft_change_list_last_updated=self.OLDER_DATETIME,
            draft_change_list_exp_version=1, draft_change_list_id=1).put()
        user_models.ExplorationUserDataModel(id='%s.%s' % (self.owner_id,
            self.EXP_ID3), user_id=self.owner_id, exploration_id=self.EXP_ID3
            ).put()
    ...
    def test_exploration_loaded_with_draft_applied(self) ->None:
        response = self.get_json('/createhandler/data/%s' % self.EXP_ID2,
    ...
    def test_exploration_loaded_without_draft_as_draft_does_not_exist(self
        ...
        response = self.get_json('/createhandler/data/%s' % self.EXP_ID3,
            params={'apply_draft': True})
        self.assertEqual(response['title'], 'A title')
    ...
    def test_exploration_not_updated_because_cmd_is_invalid(self) ->None:
        ...
        response = self.put_json('/createhandler/data/%s' % self.EXP_ID3,
            payload, csrf_token=self.csrf_token, expected_status_int=400)
        error_msg = """At 'http://localhost/createhandler/data/3' these errors are happening:
Schema validation for 'change_list' failed: Command edit_exploration_propert is not allowed"""
        self.assertEqual(response['error'], error_msg)

    def test_draft_not_updated_because_newer_draft_exists(self) ->None:
        payload = {'change_list': self.NEW_CHANGELIST, 'version': 1}
        response = self.put_json('/createhandler/autosave_draft/%s' % self.
            EXP_ID1, payload, csrf_token=self.csrf_token)
        exp_user_data = user_models.ExplorationUserDataModel.get_by_id(
    ...
    def test_draft_not_updated_because_cmd_is_invalid(self) ->None:
        ...
        response = self.put_json('/createhandler/autosave_draft/%s' % self.
            EXP_ID1, payload, csrf_token=self.csrf_token,
            expected_status_int=400)
        error_msg = """At 'http://localhost/createhandler/autosave_draft/1' these errors are happening:
Schema validation for 'change_list' failed: Command edit_exploration_propert is not allowed"""
        self.assertEqual(response['error'], error_msg)

    def test_draft_not_updated_validation_error(self) ->None:
        self.put_json('/createhandler/autosave_draft/%s' % self.EXP_ID2, {
            'change_list': self.DRAFT_CHANGELIST, 'version': 1}, csrf_token
            =self.csrf_token)
        response = self.put_json('/createhandler/autosave_draft/%s' % self.
            EXP_ID2, {'change_list': self.INVALID_CHANGELIST, 'version': 2},
    ...
    def test_draft_updated_version_valid(self) ->None:
        payload = {'change_list': self.NEW_CHANGELIST, 'version': 1}
        response = self.put_json('/createhandler/autosave_draft/%s' % self.
            EXP_ID2, payload, csrf_token=self.csrf_token)
        exp_user_data = user_models.ExplorationUserDataModel.get_by_id(
            '%s.%s' % (self.owner_id, self.EXP_ID2))
    ...
    def test_draft_not_updated_without_editing_rights(self) ->None:
        ...
        exp_user_data = user_models.ExplorationUserDataModel.get_by_id(
            '%s.%s' % (self.owner_id, self.EXP_ID2))
        self.assertNotEqual(exp_user_data.draft_change_list, self.
            NEW_CHANGELIST)
        ...
        get_voiceover_swap = self.swap_to_always_return(rights_manager,
            'check_can_edit_activity', value=False)
        with get_voiceover_swap:
            response = self.put_json('/createhandler/autosave_draft/%s' %
                self.EXP_ID2, payload, csrf_token=self.csrf_token,
                expected_status_int=400)
            exp_user_data = user_models.ExplorationUserDataModel.get_by_id(
                '%s.%s' % (self.owner_id, self.EXP_ID2))
            self.assertNotEqual(exp_user_data.draft_change_list, self.
    ...
    def test_discard_draft(self) ->None:
        self.post_json('/createhandler/autosave_draft/%s' % self.EXP_ID2, {
...
class HasSeenTutorialTests(BaseEditorControllerTests):
    ...
    def test_get_user_has_seen_editor_tutorial(self) ->None:
        ...
        exp_id = exp_fetchers.get_new_exploration_id()
        self.save_new_valid_exploration(exp_id, self.owner_id)
        response = self.get_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX,
            exp_id))
    ...
    def test_get_user_has_seen_translation_tutorial(self) ->None:
        ...
        user_services.record_user_started_state_translation_tutorial(self.
            owner_id)
        response = self.get_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX,
            exp_id))
...
class StateAnswerStatisticsHandlerTests(BaseEditorControllerTests):
    ...
    def test_get_invalid_exploration_id(self) ->None:
        with self.login_context(self.OWNER_EMAIL):
            illegal_id = '@#$%^&*'
            self.get_json('%s/%s' % (feconf.
                EXPLORATION_STATE_ANSWER_STATS_PREFIX, illegal_id),
                expected_status_int=400)
    ...
    def test_get_missing_exploration_id(self) ->None:
        with self.login_context(self.OWNER_EMAIL):
            missing_id = '0'
            self.get_json('%s/%s' % (feconf.
                EXPLORATION_STATE_ANSWER_STATS_PREFIX, missing_id),
                expected_status_int=404)
    ...
    def test_get_returns_empty_values_from_unvisited_exploration(self) ->None:
        with self.login_context(self.OWNER_EMAIL) as owner_id:
            ...
            exp_id = exp_fetchers.get_new_exploration_id()
            self.save_new_valid_exploration(exp_id, owner_id)
            state_stats = self.get_json('%s/%s' % (feconf.
                EXPLORATION_STATE_ANSWER_STATS_PREFIX, exp_id))
    ...
    def test_get_returns_assigned_interaction_ids_of_exploration_states(self
        ...
        with self.login_context(self.OWNER_EMAIL) as owner_id:
            assert owner_id is not None
            exp_id = exp_fetchers.get_new_exploration_id()
            self.save_new_linear_exp_with_state_names_and_interactions(exp_id,
                owner_id, ['A', 'B', 'End'], ['FractionInput', 'TextInput'])
            state_stats = self.get_json('%s/%s' % (feconf.
                EXPLORATION_STATE_ANSWER_STATS_PREFIX, exp_id))
...
class LearnerAnswerInfoHandlerTests(BaseEditorControllerTests):
    ...
    def setUp(self) ->None:
        ...
        interaction_id = self.exploration.states[self.state_name
            ].interaction.id
        assert interaction_id is not None
        self.interaction_id = interaction_id
        self.customization_args = self.exploration.states[self.state_name
            ].interaction.to_dict()['customization_args']
        self.answer = 'This is an answer'
        self.answer_details = 'These are the answer details'
        self.state_reference = (stats_services.
            get_state_reference_for_exploration(self.exp_id, self.state_name))
        stats_services.record_learner_answer_info(self.entity_type, self.
            state_reference, self.interaction_id, self.answer, self.
            answer_details)

    def test_get_learner_answer_details_of_exploration_states(self) ->None:
        self.login(self.OWNER_EMAIL)
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', 
            False):
            response = self.get_json('%s/%s/%s' % (feconf.
                LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_EXPLORATION, self.exp_id), expected_status_int=404)
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True
            ...
            learner_answer_info_dicts = [learner_answer_info.to_dict() for
                learner_answer_info in learner_answer_details.
                learner_answer_info_list]
            learner_answer_info_data = {'learner_answer_info_data': [{
                'state_name': self.state_name, 'interaction_id': self.
                interaction_id, 'customization_args': self.
                customization_args, 'learner_answer_info_dicts':
                learner_answer_info_dicts}]}
            response = self.get_json('%s/%s/%s' % (feconf.
                LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_EXPLORATION, self.exp_id))
    ...
    def test_get_learner_answer_details_of_question_states(self) ->None:
        ...
        content_id_generator = translation_domain.ContentIdGenerator()
        question = self.save_new_question(question_id, self.owner_id, self.
            _create_valid_question_data('ABC', content_id_generator), [
            'skill_1'], content_id_generator.next_content_id_index)
        ...
        stats_services.record_learner_answer_info(feconf.
            ENTITY_TYPE_QUESTION, state_reference, self.interaction_id,
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True
            ...
            learner_answer_info_dicts = [learner_answer_info.to_dict() for
                ...
                learner_answer_info_list]
            learner_answer_info_data = {'learner_answer_info_data': {
                'interaction_id': interaction_id, 'customization_args':
                customization_args, 'learner_answer_info_dicts':
                learner_answer_info_dicts}}
            response = self.get_json('%s/%s/%s' % (feconf.
                LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_QUESTION, question_id))
            self.assertEqual(response, learner_answer_info_data)
        self.logout()
    ...
    def test_delete_learner_answer_info_of_exploration_states(self) ->None:
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', 
            ...
            self.delete_json(
                ...
                feconf.LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_EXPLORATION, self.exp_id, self.state_name,
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True
            ...
            learner_answer_details = stats_services.get_learner_answer_details(
                self.entity_type, self.state_reference)
            assert learner_answer_details is not None
            self.assertEqual(len(learner_answer_details.
                learner_answer_info_list), 1)
            learner_answer_info_id = (learner_answer_details.
                learner_answer_info_list[0].id)
            ...
            updated_learner_answer_details = (stats_services.
                get_learner_answer_details(self.entity_type, self.
                state_reference))
            assert updated_learner_answer_details is not None
            self.assertEqual(len(updated_learner_answer_details.
                learner_answer_info_list), 0)
            self.delete_json('%s/%s/%s?learner_answer_info_id=%s' % (feconf
                .LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_EXPLORATION, self.exp_id,
                learner_answer_info_id), expected_status_int=400)
            self.delete_json('%s/%s/%s?state_name=%s' % (feconf.
                LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_EXPLORATION, self.exp_id, self.state_name),
                expected_status_int=400)
        self.logout()
    ...
    def test_delete_learner_answer_info_of_question_states(self) ->None:
        ...
        with self.swap(constants, 'ENABLE_SOLICIT_ANSWER_DETAILS_FEATURE', True
            ...
            self.delete_json('%s/%s/%s?learner_answer_info_id=%s' % (feconf
                .LEARNER_ANSWER_INFO_HANDLER_URL, feconf.
                ENTITY_TYPE_QUESTION, question_id, learner_answer_info_id))
            updated_learner_answer_details = (stats_services.
                get_learner_answer_details(feconf.ENTITY_TYPE_QUESTION,
                state_reference))
            assert updated_learner_answer_details is not None
            self.assertEqual(len(updated_learner_answer_details.
                learner_answer_info_list), 0)
...
class ImageUploadHandlerTests(BaseEditorControllerTests):
    ...
    def test_return_error_when_image_not_uploaded(self) ->None:
        ...
        publish_url = '%s/%s/%s' % (feconf.EXPLORATION_IMAGE_UPLOAD_PREFIX,
        ...
        response = self.post_json(publish_url, {'image': 'sample_image',
            'filename': filename, 'filename_prefix': filename_prefix},
            csrf_token=csrf_token, expected_status_int=400, upload_files=[(
            'image', 'unused_filename', b'')])
    ...
    def test_return_error_when_uploaded_image_already_exists(self) ->None:
        ...
        with utils.open_file(os.path.join(feconf.TESTS_DATA_DIR, 'img.png'),
            ...
            raw_image = f.read()
        get_image_exists_swap = self.swap_to_always_return(fs_services.
            GcsFileSystem, 'isfile', value=True)
        with get_image_exists_swap:
            response = self.post_json(publish_url, {'image': 'img',
                'filename': filename, 'filename_prefix': filename_prefix},
                csrf_token=csrf_token, expected_status_int=400,
    ...
    def test_upload_successful_when_image_uploaded(self) ->None:
        ...
        with utils.open_file(os.path.join(feconf.TESTS_DATA_DIR, 'img.png'),
            'rb', encoding=None) as f:
            raw_image = f.read()
        response = self.post_json(publish_url, {'image': 'img', 'filename':
            filename, 'filename_prefix': filename_prefix}, csrf_token=
            csrf_token, expected_status_int=200, upload_files=[('image',
            'unused_filename', raw_image)])
...
class EntityTranslationsBulkHandlerTest(test_utils.GenericTestBase):
    ...
    def test_fetching_entity_translations_in_bulk(self) ->None:
        ...
        translations_mapping: Dict[str, feconf.TranslatedContentDict] = {
            'content_0': {'content_value': 'Translated content',
            'content_format': 'html', 'needs_update': False}}
        language_codes = ['hi', 'bn']
        for language_code in language_codes:
            translation_models.EntityTranslationsModel.create_new('exploration'
                , 'exp1', 5, language_code, translations_mapping).put()
        self.signup(self.VIEWER_EMAIL, self.VIEWER_USERNAME)
        self.login(self.VIEWER_EMAIL)
        url = '/entity_translations_bulk_handler/exploration/exp1/5'
        entity_translations_bulk_dict = self.get_json(url)
        for language in language_codes:
            self.assertEqual(entity_translations_bulk_dict[language][
