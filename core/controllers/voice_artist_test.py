...
class VoiceArtistTest(BaseVoiceArtistControllerTests):
    ...
    EXP_ID: Final = 'exp1'
    RECORDED_VOICEOVERS: Final = {'voiceovers_mapping': {'ca_placeholder_2':
        {}, 'content_0': {'en': {'filename': 'testFile.mp3',
        'file_size_bytes': 12200, 'needs_update': False, 'duration_secs': 
        4.5}}, 'default_outcome_1': {}}}
    ...
    def setUp(self) ->None:
        ...
        self.save_new_valid_exploration(self.EXP_ID, self.owner_id,
            end_state_name='End card')
        self.publish_exploration(self.owner_id, self.EXP_ID)
        rights_manager.assign_role_for_exploration(self.voiceover_admin,
            self.EXP_ID, self.voice_artist_id, rights_domain.ROLE_VOICE_ARTIST)
        self.logout()
    ...
    def test_put_with_no_payload_version_raises_error(self) ->None:
        with self.assertRaisesRegex(Exception,
            ...
            self.put_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX, self.
                EXP_ID), {'change_list': [{'cmd': 'edit_state_property',
                'state_name': feconf.DEFAULT_INIT_STATE_NAME,
                'property_name': 'recorded_voiceovers', 'new_value': self.
                RECORDED_VOICEOVERS}], 'commit_message':
                'Translated first state content'}, csrf_token=self.csrf_token)
    ...
    def test_put_with_payload_version_different_from_exp_version_raises_error(
        ...
        with self.assertRaisesRegex(Exception,
            ...
            self.put_json('%s/%s' % (feconf.EXPLORATION_DATA_PREFIX, self.
                ...
                'property_name': 'recorded_voiceovers', 'new_value': self.
                RECORDED_VOICEOVERS}], 'commit_message':
                'Translated first state content', 'version': 3}, csrf_token
                =self.csrf_token)

    def test_voice_artist_can_save_valid_change_list(self) ->None:
        response = self.put_json('/createhandler/data/%s' % self.EXP_ID, {
            'change_list': [{'cmd': 'edit_state_property', 'state_name':
            feconf.DEFAULT_INIT_STATE_NAME, 'property_name':
            'recorded_voiceovers', 'new_value': self.RECORDED_VOICEOVERS}],
            'commit_message': 'Translated first state content', 'version': 
            1}, csrf_token=self.csrf_token)
        self.assertEqual(response['states'][feconf.DEFAULT_INIT_STATE_NAME]
            ['recorded_voiceovers'], self.RECORDED_VOICEOVERS)

    def test_voice_artist_cannot_save_invalid_change_list(self) ->None:
        response = self.put_json('/createhandler/data/%s' % self.EXP_ID, {
            'change_list': [{'cmd': 'edit_exploration_property',
            'property_name': 'objective', 'new_value': 'the objective'}],
            'commit_message': 'Changed exp objective', 'version': 1},
...
class VoiceArtistAutosaveTest(BaseVoiceArtistControllerTests):
    ...
    RECORDED_VOICEOVERS: Final = {'voiceovers_mapping': {'ca_placeholder_2':
        ...
        'file_size_bytes': 12200, 'needs_update': False, 'duration_secs': 
        4.5}}, 'default_outcome_1': {}}}
    VALID_DRAFT_CHANGELIST: Final = [{'cmd': 'edit_state_property',
        'state_name': feconf.DEFAULT_INIT_STATE_NAME, 'property_name':
        'recorded_voiceovers', 'old_value': None, 'new_value':
        RECORDED_VOICEOVERS}]
    INVALID_DRAFT_CHANGELIST: Final = [{'cmd': 'edit_exploration_property',
    ...
    def setUp(self) ->None:
        ...
        self.logout()
        self.login(self.VOICE_ARTIST_EMAIL)
        user_models.ExplorationUserDataModel(id='%s.%s' % (self.
            voice_artist_id, self.EXP_ID), user_id=self.voice_artist_id,
            exploration_id=self.EXP_ID, draft_change_list=self.
            VALID_DRAFT_CHANGELIST, draft_change_list_last_updated=self.
            OLDER_DATETIME, draft_change_list_exp_version=1,
    ...
    def test_draft_updated_version_valid(self) ->None:
        ...
        response = self.put_json('/createhandler/autosave_draft/%s' % self.
            EXP_ID, payload, csrf_token=self.csrf_token)
    ...
    def test_draft_not_updated_validation_error(self) ->None:
        response = self.put_json('/createhandler/autosave_draft/%s' % self.
            EXP_ID, {'change_list': self.INVALID_DRAFT_CHANGELIST,
            'version': 1}, csrf_token=self.csrf_token, expected_status_int=400)
        exp_user_data = user_models.ExplorationUserDataModel.get_by_id(
            '%s.%s' % (self.voice_artist_id, self.EXP_ID))
        self.assertEqual(exp_user_data.draft_change_list, self.
            VALID_DRAFT_CHANGELIST)
    ...
    def test_discard_draft(self) ->None:
        self.post_json('/createhandler/autosave_draft/%s' % self.EXP_ID, {},
...
class TranslationFirstTimeTutorialTest(BaseVoiceArtistControllerTests):
    ...
    def test_firsttime_translation_tutorial(self) ->None:
        ...
        self.post_json(
            '/createhandler/started_translation_tutorial_event/%s' % self.
            EXP_ID, {}, csrf_token=self.csrf_token, expected_status_int=200)

    def test_cannot_start_translation_tutorial_if_user_is_not_logged_in(self
        ) ->None:
        self.logout()
        response = self.post_json(
            '/createhandler/started_translation_tutorial_event/%s' % self.
            EXP_ID, {}, csrf_token=self.get_new_csrf_token(),
...
class VoiceArtistManagementTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.voice_artist_id = self.get_user_id_from_email(self.
            VOICE_ARTIST_EMAIL)
        self.voiceover_admin_id = self.get_user_id_from_email(self.
            VOICEOVER_ADMIN_EMAIL)
        self.owner = user_services.get_user_actions_info(self.owner_id)
        ...
        user_services.add_user_role(self.voiceover_admin_id, feconf.
    ...
    def test_owner_cannot_assign_voice_artist(self) ->None:
        ...
        self.post_json('/voice_artist_management_handler/exploration/%s' %
    ...
    def test_voiceover_admin_can_deassign_voice_artist(self) ->None:
        ...
        self.post_json('/voice_artist_management_handler/exploration/%s' %
            self.published_exp_id_1, params, csrf_token=csrf_token)
        self.delete_json('/voice_artist_management_handler/exploration/%s' %
            self.published_exp_id_1, params={'voice_artist': self.
    ...
    def test_cannot_assign_voice_artist_to_random_user(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        response = self.post_json(
            '/voice_artist_management_handler/exploration/%s' % self.
            published_exp_id_1, params, csrf_token=csrf_token,
    ...
    def test_cannot_deassign_random_user_from_voice_artist(self) ->None:
        ...
        response = self.delete_json(
            '/voice_artist_management_handler/exploration/%s' % self.
            published_exp_id_1, params={'voice_artist': 'random_user'},
            expected_status_int=400)
        self.assertIn(
