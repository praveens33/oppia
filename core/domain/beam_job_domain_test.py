...
class BeamJobTests(test_utils.TestBase):
    ...
    def test_usage(self) ->None:
        job = beam_job_domain.BeamJob(model_validation_jobs.
            AuditAllStorageModelsJob)
...
class BeamJobRunTests(test_utils.TestBase):
    ...
    def test_usage(self) ->None:
        run = beam_job_domain.BeamJobRun('123', 'FooJob', 'RUNNING', self.
            NOW, self.NOW, True)
    ...
    def test_to_dict(self) ->None:
        run = beam_job_domain.BeamJobRun('123', 'FooJob', 'RUNNING', self.
            NOW, self.NOW, True)
        self.assertEqual(run.to_dict(), {'job_id': '123', 'job_name':
            'FooJob', 'job_state': 'RUNNING', 'job_started_on_msecs': utils
            .get_time_in_millisecs(self.NOW), 'job_updated_on_msecs': utils
            .get_time_in_millisecs(self.NOW), 'job_is_synchronous': True})
