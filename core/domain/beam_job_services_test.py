...
class BeamJobServicesTests(test_utils.TestBase):
    ...
    def test_gets_jobs_from_registry(self) ->None:
        ...
        self.assertItemsEqual([j.name for j in beam_jobs], jobs_registry.
            get_all_job_names())
...
class BeamJobRunServicesTests(test_utils.GenericTestBase):
    ...
    def create_beam_job_run_model(self, dataflow_job_id: str='abc', job_id:
        Optional[str]=None, job_name: str='FooJob', job_state: str=
        ...
        return beam_job_models.BeamJobRunModel(id=job_id, dataflow_job_id=
            dataflow_job_id, job_name=job_name, latest_job_state=job_state)

    def assert_domains_equal_models(self, beam_job_runs: List[
        beam_job_domain.BeamJobRun], beam_job_run_models: List[
        beam_job_models.BeamJobRunModel]) ->None:
        ...
        for i, (run, model) in enumerate(zip(runs, run_models)):
            with self.subTest('i=%d' % i):
                ...
                self.assertEqual(run.job_is_synchronous, model.
                    dataflow_job_id is None)
    ...
    def test_run_beam_job_using_job_name(self) ->None:
        ...
        self.assertEqual(beam_job_services.get_beam_job_run_from_model(
            model).to_dict(), run.to_dict())
    ...
    def test_cancel_beam_job(self) ->None:
        ...
        self.assertEqual(run.to_dict(), beam_job_services.
            get_beam_job_run_from_model(model).to_dict())
    ...
    def test_cancel_beam_job_which_has_no_dataflow_job_id_raises_an_error(self
        ) ->None:
        model = beam_job_services.create_beam_job_run_model('NoOpJob',
    ...
    def test_get_beam_job_runs(self) ->None:
        beam_job_run_models = [self.create_beam_job_run_model(job_state=
            beam_job_models.BeamJobState.DONE.value), self.
            create_beam_job_run_model(job_state=beam_job_models.
            BeamJobState.RUNNING.value), self.create_beam_job_run_model(
        ...
        self.assert_domains_equal_models(beam_job_services.
            get_beam_job_runs(refresh=False), beam_job_run_models)
    ...
    def test_create_beam_job_run_result_model(self) ->None:
        model = beam_job_services.create_beam_job_run_result_model('123',
            'abc', '123')
        model.put()
    ...
    def test_is_state_terminal(self) ->None:
        ...
        cancelled_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.CANCELLED.value, now, now, True)
        drained_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.DRAINED.value, now, now, True)
        updated_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.UPDATED.value, now, now, True)
        done_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.DONE.value, now, now, True)
        failed_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.FAILED.value, now, now, True)
        cancelling_beam_job_run = beam_job_domain.BeamJobRun('123',
            'FooJob', beam_job_models.BeamJobState.CANCELLING.value, now,
            now, True)
        draining_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.DRAINING.value, now, now, True)
        pending_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.PENDING.value, now, now, True)
        running_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.RUNNING.value, now, now, True)
        stopped_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.STOPPED.value, now, now, True)
        unknown_beam_job_run = beam_job_domain.BeamJobRun('123', 'FooJob',
            beam_job_models.BeamJobState.UNKNOWN.value, now, now, True)
