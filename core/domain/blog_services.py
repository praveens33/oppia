...
def get_blog_post_from_model(blog_post_model: blog_models.BlogPostModel
    ...
    return blog_domain.BlogPost(blog_post_model.id, blog_post_model.
        ...
        blog_post_model.url_fragment, blog_post_model.tags, blog_post_model
        .thumbnail_filename, blog_post_model.last_updated, blog_post_model.
        published_on)
...
@overload
def get_blog_post_by_id(blog_post_id: str, *, strict: Literal[True]
    ) ->blog_domain.BlogPost:
    ...


@overload
def get_blog_post_by_id(blog_post_id: str, *, strict: Literal[False]
    ) ->Optional[blog_domain.BlogPost]:
    ...


def get_blog_post_by_id(blog_post_id: str, strict: bool=True) ->Optional[
    blog_domain.BlogPost]:
...
def get_blog_post_by_url_fragment(url_fragment: str) ->Optional[blog_domain
    .BlogPost]:
    """Returns a domain object representing a blog post.
    ...
    Returns:
        ...
        given ID, or None if it does not exist.
    """
    blog_post_model = blog_models.BlogPostModel.get_by_url_fragment(
        url_fragment)
...
def get_blog_post_summary_from_model(blog_post_summary_model: blog_models.
    ...
    """
    return blog_domain.BlogPostSummary(blog_post_summary_model.id,
        blog_post_summary_model.author_id, blog_post_summary_model.title,
        blog_post_summary_model.summary, blog_post_summary_model.
        url_fragment, blog_post_summary_model.tags, blog_post_summary_model
        .thumbnail_filename, blog_post_summary_model.last_updated,
        blog_post_summary_model.published_on, blog_post_summary_model.deleted)
...
@overload
def get_blog_post_summary_by_id(blog_post_id: str, *, strict: Literal[True]
    ) ->blog_domain.BlogPostSummary:
    ...


@overload
def get_blog_post_summary_by_id(blog_post_id: str, *, strict: Literal[False]
    ) ->Optional[blog_domain.BlogPostSummary]:
    ...


def get_blog_post_summary_by_id(blog_post_id: str, strict: bool=True
    ) ->Optional[blog_domain.BlogPostSummary]:
    ...
    """
    blog_post_summary_model = blog_models.BlogPostSummaryModel.get(blog_post_id
        , strict=strict)
...
def get_blog_post_summary_models_by_ids(blog_post_ids: List[str]) ->List[
    blog_domain.BlogPostSummary]:
    ...
    blog_post_summary_models = blog_models.BlogPostSummaryModel.get_multi(
        blog_post_ids)
    return [get_blog_post_summary_from_model(model) for model in
        blog_post_summary_models if model is not None]


def get_blog_post_summary_models_list_by_user_id(user_id: str,
    blog_post_is_published: bool) ->List[blog_domain.BlogPostSummary]:
    ...
    blog_post_summary_models = blog_models.BlogPostSummaryModel.get_multi(
        blog_post_ids)
    blog_post_summaries = []
    blog_post_summaries = [get_blog_post_summary_from_model(model) for
        model in blog_post_summary_models if model is not None]
    ...
    return sorted(blog_post_summaries, key=sort_blog_post_summaries,
        reverse=True) if len(blog_post_summaries) != 0 else []
...
def filter_blog_post_ids(user_id: str, blog_post_is_published: bool) ->List[str
    ...
    if blog_post_is_published:
        blog_post_rights_models = (blog_models.BlogPostRightsModel.
            get_published_models_by_user(user_id))
    else:
        blog_post_rights_models = (blog_models.BlogPostRightsModel.
            get_draft_models_by_user(user_id))
    model_ids = []
...
def get_blog_post_summary_by_title(title: str) ->Optional[blog_domain.
    BlogPostSummary]:
    ...
    blog_post_summary_model: Sequence[blog_models.BlogPostSummaryModel
        ] = blog_models.BlogPostSummaryModel.query(blog_models.
        BlogPostSummaryModel.title == title).fetch()
...
def get_blog_post_rights_from_model(blog_post_rights_model: blog_models.
    BlogPostRightsModel) ->blog_domain.BlogPostRights:
    ...
    """
    return blog_domain.BlogPostRights(blog_post_rights_model.id,
        blog_post_rights_model.editor_ids, blog_post_rights_model.
...
@overload
def get_blog_post_rights(blog_post_id: str, *, strict: Literal[True]
    ) ->blog_domain.BlogPostRights:
    ...


@overload
def get_blog_post_rights(blog_post_id: str, *, strict: Literal[False]
    ) ->Optional[blog_domain.BlogPostRights]:
    ...


def get_blog_post_rights(blog_post_id: str, strict: bool=True) ->Optional[
    blog_domain.BlogPostRights]:
...
def get_published_blog_post_summaries_by_user_id(user_id: str, max_limit:
    int, offset: int=0) ->List[blog_domain.BlogPostSummary]:
    ...
    blog_post_summary_models: Sequence[blog_models.BlogPostSummaryModel
        ...
        BlogPostSummaryModel.author_id == user_id).filter(blog_models.
        BlogPostSummaryModel.published_on != None).order(-blog_models.
        BlogPostSummaryModel.published_on).fetch(max_limit, offset=offset)
...
def does_blog_post_with_url_fragment_exist(url_fragment: str) ->bool:
    ...
    if not isinstance(url_fragment, str):
        raise utils.ValidationError(
            'Blog Post URL fragment should be a string. Recieved:%s' %
            url_fragment)
...
def _save_blog_post_summary(blog_post_summary: blog_domain.BlogPostSummary
    ) ->None:
    ...
    Args:
        blog_post_summary: BlogPostSummary. The summary object for the given
            blog post summary.
    """
    model = blog_models.BlogPostSummaryModel.get(blog_post_summary.id,
        strict=False)
    if model:
        ...
        model = blog_models.BlogPostSummaryModel(id=blog_post_summary.id,
            author_id=blog_post_summary.author_id, title=blog_post_summary.
            title, summary=blog_post_summary.summary, tags=
            blog_post_summary.tags, published_on=blog_post_summary.
            published_on, thumbnail_filename=blog_post_summary.
            thumbnail_filename, url_fragment=blog_post_summary.url_fragment)
    model.update_timestamps()
    model.put()
...
def save_blog_post_rights(blog_post_rights: blog_domain.BlogPostRights) ->None:
    ...
    Args:
        blog_post_rights: BlogPostRights. The rights object for the given
            blog post.
    """
    model = blog_models.BlogPostRightsModel.get(blog_post_rights.id, strict
        =True)
...
def check_can_edit_blog_post(user: user_domain.UserActionsInfo,
    blog_post_rights: Optional[blog_domain.BlogPostRights]) ->bool:
...
def generate_summary_of_blog_post(content: str) ->str:
    ...
    """
    raw_html = re.sub('<strong>?(.*?)</strong>', '', re.sub(
        '<h1>?(.*?)</h1>', '', content, flags=re.DOTALL), flags=re.DOTALL)
...
def compute_summary_of_blog_post(blog_post: blog_domain.BlogPost
    ) ->blog_domain.BlogPostSummary:
    ...
    return blog_domain.BlogPostSummary(blog_post.id, blog_post.author_id,
        blog_post.title, summary, blog_post.url_fragment, blog_post.tags,
        blog_post.thumbnail_filename, blog_post.last_updated, blog_post.
        published_on)


def apply_change_dict(blog_post_id: str, change_dict: BlogPostChangeDict
    ) ->blog_domain.BlogPost:
    ...
    if 'title' in change_dict:
        ...
        url_fragment = generate_url_fragment(change_dict['title'].strip(),
            blog_post_id)
...
def update_blog_post(blog_post_id: str, change_dict: BlogPostChangeDict
    ) ->None:
...
def does_blog_post_with_title_exist(title: str, blog_post_id: str) ->bool:
    ...
    blog_post_models: Sequence[blog_models.BlogPostModel
        ] = blog_models.BlogPostModel.get_all().filter(blog_models.
        BlogPostModel.title == title).fetch()
...
def create_new_blog_post(author_id: str) ->blog_domain.BlogPost:
    ...
    new_blog_post_model = blog_models.BlogPostModel.create(blog_post_id,
        author_id)
...
def get_published_blog_post_summaries(offset: int=0, size: Optional[int]=None
...
def get_total_number_of_published_blog_post_summaries() ->int:
    ...
    """
    return blog_models.BlogPostRightsModel.query(blog_models.
        BlogPostRightsModel.blog_post_is_published == True).count()


def get_total_number_of_published_blog_post_summaries_by_author(author_id: str
    ) ->int:
...
def update_blog_models_author_and_published_on_date(blog_post_id: str,
    author_id: str, date: str) ->None:
...
def index_blog_post_summaries_given_ids(blog_post_ids: List[str]) ->None:
    ...
    if len(blog_post_summaries) > 0:
        search_services.index_blog_post_summaries([blog_post_summary for
            blog_post_summary in blog_post_summaries if blog_post_summary
             is not None])


def get_blog_post_ids_matching_query(query_string: str, tags: List[str],
    size: int, offset: Optional[int]=None) ->Tuple[List[str], Optional[int]]:
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        blog_post_ids, search_offset = (search_services.
            search_blog_post_summaries(query_string, tags,
            remaining_to_fetch, offset=search_offset))
        invalid_blog_post_ids = []
        for ind, model in enumerate(blog_models.BlogPostSummaryModel.
            get_multi(blog_post_ids)):
            if model is not None:
                valid_blog_post_ids.append(blog_post_ids[ind])
            else:
                invalid_blog_post_ids.append(blog_post_ids[ind])
        if (len(valid_blog_post_ids) == feconf.
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE or 
            search_offset is None):
            break
        if len(invalid_blog_post_ids) > 0:
            logging.error('Search index contains stale blog post ids: %s' %
                ', '.join(invalid_blog_post_ids))
    if (len(valid_blog_post_ids) < feconf.
        MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE and 
...
def create_blog_author_details_model(user_id: str) ->None:
    ...
    if user_settings.username:
        blog_models.BlogAuthorDetailsModel.create(user_id, user_settings.
            username, user_settings.user_bio)
...
def get_blog_author_details(user_id: str) ->blog_domain.BlogAuthorDetails:
    ...
    return blog_domain.BlogAuthorDetails(author_model.id, author_model.
        author_id, author_model.displayed_author_name, author_model.
        author_bio, author_model.last_updated)


def update_blog_author_details(user_id: str, displayed_author_name: str,
    author_bio: str) ->None:
