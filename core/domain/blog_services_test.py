...
class BlogServicesUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.change_dict: blog_services.BlogPostChangeDict = {
            'thumbnail_filename': 'test.svg', 'content': '<p>hi<p>', 'tags':
            ['one', 'two']}
        self.change_dict_one: blog_services.BlogPostChangeDict = {'title':
            'Sample Title', 'thumbnail_filename': 'thummbnail.svg',
            'content': '<p>Hello</p>', 'tags': []}
        self.change_dict_two: blog_services.BlogPostChangeDict = {'title':
            'Sample title two', 'thumbnail_filename': 'thummbnail.svg',
            'content': '<p>Hello</p>', 'tags': ['one', 'two']}
    ...
    def test_get_blog_post_summary_models_list_by_user_id(self) ->None:
        blog_posts = (blog_services.
            get_blog_post_summary_models_list_by_user_id(self.user_id_a, True))
        self.assertEqual(blog_posts, [])
        blog_posts = (blog_services.
            get_blog_post_summary_models_list_by_user_id(self.user_id_a, False)
            )
    ...
    def test_compute_summary_of_blog_post(self) ->None:
        expected_blog_post_summary = blog_domain.BlogPostSummary(self.
            blog_post_a_id, self.user_id_a, '', '', '', [], self.
            blog_post_a.thumbnail_filename, self.blog_post_a.last_updated,
            self.blog_post_a.published_on)
        blog_post_summary = blog_services.compute_summary_of_blog_post(self
            .blog_post_a)
    ...
    def test_get_published_blog_post_summaries(self) ->None:
        self.assertEqual(len(blog_services.
            get_published_blog_post_summaries()), 0)
        blog_services.update_blog_post(self.blog_post_a_id, self.
            change_dict_two)
        blog_services.publish_blog_post(self.blog_post_a_id)
        number_of_published_blogs = (blog_services.
            get_published_blog_post_summaries(0, 2))
        self.assertEqual(len(number_of_published_blogs), 1)
        number_of_published_blogs = (blog_services.
            get_published_blog_post_summaries(1, 1))
        self.assertEqual(len(number_of_published_blogs), 0)

    def test_get_total_number_of_published_blog_post_summaries_by_author(self
        ) ->None:
        self.assertEqual(blog_services.
        ...
        change_dict: blog_services.BlogPostChangeDict = {'title':
            'Sample title B', 'thumbnail_filename': 'test.svg', 'content':
            '<p>hi<p>', 'tags': ['one', 'two']}
    ...
    def test_get_published_blog_post_summaries_by_user_id(self) ->None:
        self.assertEqual(len(blog_services.
            get_published_blog_post_summaries_by_user_id(self.user_id_a, 20,
            0)), 0)
        blog_services.update_blog_post(self.blog_post_a_id, self.
            change_dict_two)
        blog_services.publish_blog_post(self.blog_post_a_id)
        no_of_published_blog_post = (blog_services.
            get_published_blog_post_summaries_by_user_id(self.user_id_a, 20, 0)
            )
        self.assertEqual(len(no_of_published_blog_post), 1)

    def test_get_blog_post_summary_from_model(self) ->None:
        blog_post_summary_model = blog_models.BlogPostSummaryModel.get(self
            .blog_post_a_id)
        blog_post_summary = blog_services.get_blog_post_summary_from_model(
            blog_post_summary_model)
        expected_blog_post_summary = blog_domain.BlogPostSummary(self.
            blog_post_a_id, self.user_id_a, '', '', '', [],
            blog_post_summary_model.thumbnail_filename,
            blog_post_summary_model.last_updated, blog_post_summary_model.
            published_on, blog_post_summary_model.deleted)
        self.assertEqual(blog_post_summary.to_dict(),
            expected_blog_post_summary.to_dict())

    def test_blog_post_summary_by_id(self) ->None:
        blog_post_summary = blog_services.get_blog_post_summary_by_id(self.
            blog_post_a_id)
        expected_blog_post_summary = blog_domain.BlogPostSummary(self.
            blog_post_a_id, self.user_id_a, '', '', '', [],
            blog_post_summary.thumbnail_filename, blog_post_summary.
            last_updated, blog_post_summary.published_on, blog_post_summary
            .deleted)
        self.assertEqual(blog_post_summary.to_dict(),
            expected_blog_post_summary.to_dict())

    def test_publish_blog_post(self) ->None:
        blog_post_rights = blog_services.get_blog_post_rights(self.
            blog_post_a_id)
    ...
    def test_cannot_publish_invalid_blog_post(self) ->None:
        ...
        with self.assertRaisesRegex(Exception,
            ...
            blog_services.publish_blog_post(self.blog_post_a_id)
        change_dict_three: blog_services.BlogPostChangeDict = {'title':
            'Sample', 'thumbnail_filename': 'thummbnail.svg', 'content': '',
            'tags': ['one', 'two']}
        ...
        with self.assertRaisesRegex(Exception,
            'The given blog post does not exist'):
            blog_services.publish_blog_post(self.blog_post_a_id)
    ...
    def test_filter_blog_post_ids(self) ->None:
        blog_services.update_blog_post(self.blog_post_a_id, self.
            change_dict_two)
        blog_services.publish_blog_post(self.blog_post_a_id)
        filtered_model_ids = blog_services.filter_blog_post_ids(self.
            user_id_a, True)
    ...
    def test_update_blog_post(self) ->None:
        ...
        blog_services.update_blog_post(self.blog_post_a_id, self.
            change_dict_one)
        updated_blog_post = blog_services.get_blog_post_by_id(self.
            blog_post_a_id)
        ...
        with self.assertRaisesRegex(Exception, 
            ...
            blog_services.update_blog_post(self.blog_post_b_id, self.
                change_dict_one)

    def test_get_blog_post_by_url_fragment(self) ->None:
        blog_services.update_blog_post(self.blog_post_a_id, self.
            change_dict_one)
        expected_blog_post = blog_services.get_blog_post_by_id(self.
            blog_post_a_id)
        lower_id = '-' + self.blog_post_a_id.lower()
    ...
    def test_does_blog_post_with_url_fragment_exist(self) ->None:
        blog_services.update_blog_post(self.blog_post_a_id, self.
            change_dict_one)
        lower_id = '-' + self.blog_post_a_id.lower()
        self.assertTrue(blog_services.
            does_blog_post_with_url_fragment_exist('sample-title' + lower_id))
        self.assertFalse(blog_services.
            does_blog_post_with_url_fragment_exist('title'))
    ...
    def test_check_can_edit_blog_post(self) ->None:
        ...
        self.assertFalse(blog_services.check_can_edit_blog_post(user_info_b,
            blog_post_rights))
        self.assertTrue(blog_services.check_can_edit_blog_post(user_info_a,
            blog_post_rights))
        self.assertFalse(blog_services.check_can_edit_blog_post(user_info_a,
            None))
        user_info_b.actions.append('EDIT_ANY_BLOG_POST')
        self.assertTrue(blog_services.check_can_edit_blog_post(user_info_b,
            blog_post_rights))
    ...
    def test_deassign_user_from_all_blog_posts(self) ->None:
        ...
        updated_blog_post_rights = blog_services.get_blog_post_rights(self.
            blog_post_a_id)
    ...
    def test_save_blog_post_rights(self) ->None:
        blog_post_rights = blog_domain.BlogPostRights(self.blog_post_a_id,
            [self.user_id_a, self.user_id_b], False)
        blog_services.save_blog_post_rights(blog_post_rights)
        fetched_blog_post_rights = blog_services.get_blog_post_rights(self.
            blog_post_a_id)
    ...
    def test_delete_blog_post(self) ->None:
        blog_services.delete_blog_post(self.blog_post_a_id)
        self.assertIsNone(blog_services.get_blog_post_rights(self.
            blog_post_a_id, strict=False))
        self.assertIsNone(blog_services.get_blog_post_by_id(self.
            blog_post_a_id, strict=False))
        self.assertIsNone(blog_services.get_blog_post_summary_by_id(self.
            blog_post_a_id, strict=False))
    ...
    def test_get_blog_post_summary_by_title(self) ->None:
        ...
        expected_blog_post_summary = blog_domain.BlogPostSummary(self.
            blog_post_a_id, self.user_id_a, 'Hello Bloggers', '', '', [],
            blog_post_summary.thumbnail_filename, blog_post_summary.
            last_updated, blog_post_summary.published_on)
        self.assertEqual(blog_post_summary.to_dict(),
            expected_blog_post_summary.to_dict())
    ...
    def test_update_blog_models_author_and_published_on_date_successfully(self
        ...
        model.update_timestamps()
        model.put()
        blog_services.update_blog_models_author_and_published_on_date(self.
            blog_post_a_id, self.user_id_b, '05/09/2000')
        blog_model = blog_models.BlogPostSummaryModel.get_by_id(self.
            blog_post_a_id)
        self.assertEqual(blog_model.author_id, self.user_id_b)
        self.assertEqual(blog_model.published_on, datetime.datetime(2000, 5,
            9, 0, 0))
        blog_summary_model = blog_models.BlogPostSummaryModel.get_by_id(self
            .blog_post_a_id)
        self.assertEqual(blog_summary_model.author_id, self.user_id_b)
        self.assertEqual(blog_summary_model.published_on, datetime.datetime
            (2000, 5, 9, 0, 0))
        blog_rights_model = blog_models.BlogPostRightsModel.get_by_id(self.
            blog_post_a_id)
    ...
    def test_update_blog_models_author_and_publish_date_with_invalid_date(self
        ...
        model.put()
        with self.assertRaisesRegex(Exception,
            "time data '123/09/2000, 00:00:00:00' does not match format '%m/%d/%Y, %H:%M:%S:%f'"
            ):
            blog_services.update_blog_models_author_and_published_on_date(self
                .blog_post_a_id, self.user_id_b, '123/09/2000')
        with self.assertRaisesRegex(Exception,
            "time data '01/38/2000, 00:00:00:00' does not match format '%m/%d/%Y, %H:%M:%S:%f'"
            ):
            blog_services.update_blog_models_author_and_published_on_date(self
                .blog_post_a_id, self.user_id_b, '01/38/2000')
        with self.assertRaisesRegex(Exception,
            "time data '01/22/31126, 00:00:00:00' does not match format '%m/%d/%Y, %H:%M:%S:%f'"
            ):
            blog_services.update_blog_models_author_and_published_on_date(self
                .blog_post_a_id, self.user_id_b, '01/22/31126')
    ...
    def test_index_blog_post_summaries_given_ids(self) ->None:
        ...
        all_blog_post_titles = ['title 0', 'title 1', 'title 2', 'title 3',
            'title 4']
        expected_blog_post_titles = all_blog_post_titles[:-1]
        all_blog_post_thumbnails = ['thumb0.svg', 'thumb1.svg',
            'thumb2.svg', 'thumb3.svg', 'thumb4.svg']
        all_blog_post_tags = ['tag0', 'tag1', 'tag2', 'tag3', 'tag4']
        expected_blog_post_tags = all_blog_post_tags[:-1]

        def mock_add_documents_to_index(docs: List[Dict[str, str]], index: int
            ) ->List[str]:
        ...
        for i in range(5):
            change_dict: blog_services.BlogPostChangeDict = {'title':
                all_blog_post_titles[i], 'thumbnail_filename':
                all_blog_post_thumbnails[i], 'content': 
                '<p>Hello Blog Post +</p>%s' % i, 'tags': [
                all_blog_post_tags[i]]}
    ...
    def test_updated_blog_post_is_added_correctly_to_index(self) ->None:
        ...
        old_blog_post_change_dict: blog_services.BlogPostChangeDict = {'title':
            old_blog_post_title, 'thumbnail_filename': 'thumbnail.svg',
            'content': '<p>Hello Blog Post</p>', 'tags': old_blog_post_tag}
        new_blog_post_title = 'title 1'
        new_blog_post_tags = ['tag1', 'tag2']
        new_blog_post_change_dict: blog_services.BlogPostChangeDict = {'title':
            new_blog_post_title, 'thumbnail_filename': 'changed_thumb.svg',
            'content': '<p>Hello Blog Post</p>', 'tags': new_blog_post_tags}
        actual_docs = []
        ...
        with add_docs_swap:
            ...
            if old_blog_post_summary.published_on:
                rank = math.floor(utils.get_time_in_millisecs(
                    old_blog_post_summary.published_on))
            else:
                rank = 0
            initial_blog_post_doc = {'id': blog_post.id, 'rank': rank,
                'tags': old_blog_post_tag, 'title': old_blog_post_title}
            ...
            if new_blog_post_summary.published_on:
                ...
                rank = 0
            updated_blog_post_doc = {'id': blog_post.id, 'rank': rank,
                'tags': new_blog_post_tags, 'title': new_blog_post_title}
...
class BlogAuthorDetailsTests(test_utils.GenericTestBase):
    ...
    def test_get_blog_author_details_model_raises_exception(self) ->None:
        ...
        def _mock_get_author_details_by_author(unused_user_id: str) ->None:
            return None
        get_author_details_swap = self.swap(blog_models.
            BlogAuthorDetailsModel, 'get_by_author',
    ...
    def test_update_blog_author_details(self) ->None:
        ...
        pre_update_author_details = blog_services.get_blog_author_details(self
            .user_id)
        assert pre_update_author_details is not None
        self.assertNotEqual(pre_update_author_details.displayed_author_name,
            new_author_name)
        self.assertNotEqual(pre_update_author_details.author_bio,
            new_author_bio)
        blog_services.update_blog_author_details(self.user_id,
            new_author_name, new_author_bio)
        updated_author_details = blog_services.get_blog_author_details(self
            .user_id)
        assert updated_author_details is not None
        self.assertEqual(updated_author_details.displayed_author_name,
            new_author_name)
...
class BlogPostSummaryQueriesUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.all_blog_post_ids = (self.ids_of_blog_posts_by_user_A + self.
            ids_of_blog_posts_by_user_B)
        change_dict_1: blog_services.BlogPostChangeDict = {'title':
            'Welcome to Oppia', 'thumbnail_filename': 'thumbnail.svg',
            'content': 'Hello Blog Authors', 'tags': ['Math', 'Science']}
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_A[0],
            change_dict_1)
        change_dict_2: blog_services.BlogPostChangeDict = {'title':
            'Welcome', 'thumbnail_filename': 'thumbnail.svg', 'content':
            'Hello Blog Authors', 'tags': ['Math', 'Social']}
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_A[1],
            change_dict_2)
        change_dict_3: blog_services.BlogPostChangeDict = {'title':
            'Intro to Mathematics in Oppia', 'thumbnail_filename':
            'thumbnail.svg', 'content': 'Hello Blog Authors', 'tags': ['Math']}
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_A[2],
            change_dict_3)
        change_dict_4: blog_services.BlogPostChangeDict = {'title':
            'New Lessons in Mathematics', 'thumbnail_filename':
            'thumbnail.svg', 'content': 'Hello Blog', 'tags': ['Math', 'Oppia']
            }
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_B[0],
            change_dict_4)
        change_dict_5: blog_services.BlogPostChangeDict = {'title':
            'Basic English Lessons', 'thumbnail_filename': 'thumbnail.svg',
            'content': 'Authors in Oppia', 'tags': ['English', 'Oppia',
            'Social']}
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_B[1],
            change_dict_5)
        change_dict_6: blog_services.BlogPostChangeDict = {'title': 'Basic',
            'thumbnail_filename': 'thumbnail.svg', 'content':
            'Basic Subject Lessons', 'tags': ['English', 'Science', 'Social']}
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_B[2],
            change_dict_6)
        change_dict_7: blog_services.BlogPostChangeDict = {'title':
            'Basic Learning', 'thumbnail_filename': 'thumbnail.svg',
            'content': 'Basic Subject Lessons', 'tags': ['English', 'Math',
            'Science', 'Social']}
        blog_services.update_blog_post(self.ids_of_blog_posts_by_user_B[3],
            change_dict_7)
        for blog_id in self.all_blog_post_ids[:6]:
            blog_services.publish_blog_post(blog_id)
        blog_services.index_blog_post_summaries_given_ids([self.
            all_blog_post_ids[6]])

    def test_get_blog_post_summaries_with_no_query(self) ->None:
        blog_post_ids, search_offset = (blog_services.
            get_blog_post_ids_matching_query('', [], feconf.
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE))
        self.assertEqual(sorted(blog_post_ids), sorted(self.
            all_blog_post_ids[:6]))
    ...
    def test_get_blog_post_summaries_with_deleted_blog_post(self) ->None:
        ...
        blog_post_ids = blog_services.get_blog_post_ids_matching_query('',
            [], feconf.MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE)[0]
        self.assertEqual(sorted(blog_post_ids), sorted(self.
            all_blog_post_ids[3:6]))
        for blog_id in self.all_blog_post_ids[3:6]:
            blog_services.delete_blog_post(blog_id)
        self.assertEqual(blog_services.get_blog_post_ids_matching_query('',
    ...
    def test_search_blog_post_summaries(self) ->None:
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            ...
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE)
        self.assertEqual(sorted(blog_post_ids), sorted([self.
            ids_of_blog_posts_by_user_A[0], self.
            ids_of_blog_posts_by_user_A[2]]))
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            'Basic', [], feconf.
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE)
        self.assertEqual(sorted(blog_post_ids), sorted([self.
            ids_of_blog_posts_by_user_B[1], self.
            ids_of_blog_posts_by_user_B[2]]))
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query('',
            ['Math', 'Social'], feconf.
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE)
        self.assertEqual(sorted(blog_post_ids), sorted([self.
            ids_of_blog_posts_by_user_A[1]]))
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            'Lessons', [], feconf.
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE)
        self.assertEqual(sorted(blog_post_ids), sorted([self.
            ids_of_blog_posts_by_user_B[0], self.
            ids_of_blog_posts_by_user_B[1]]))
        blog_post_ids, _ = blog_services.get_blog_post_ids_matching_query(
            'Lessons', ['Social'], feconf.
            MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE)
        self.assertEqual(sorted(blog_post_ids), sorted([self.
            ids_of_blog_posts_by_user_B[1]]))
    ...
    def test_blog_post_summaries_pagination_in_filled_search_results(self
        ...
        with self.swap(feconf,
            'MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE', 2):
            found_blog_post_ids = []
            blog_post_ids, search_offset = (blog_services.
                get_blog_post_ids_matching_query('', [], feconf.
                MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE))
            ...
            self.assertIsNone(search_offset)
            found_blog_post_ids += blog_post_ids
            self.assertEqual(sorted(found_blog_post_ids), sorted(self.
                all_blog_post_ids[:6]))

    def test_get_blog_post_ids_matching_query_with_stale_blog_post_ids(self
        ) ->None:
        ...
        search_results_page_size_swap = self.swap(feconf,
            'MAX_NUM_CARDS_TO_DISPLAY_ON_BLOG_SEARCH_RESULTS_PAGE', 5)
        ...
        self.assertEqual(observed_log_messages, [
            'Search index contains stale blog post ids: %s, %s' % (self.
            all_blog_post_ids[0], self.all_blog_post_ids[1]),
