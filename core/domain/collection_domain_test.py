...
class CollectionChangeTests(test_utils.GenericTestBase):
    ...
    def test_collection_change_object_with_missing_attribute_in_cmd(self
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            collection_domain.CollectionChange({'cmd':
                'edit_collection_node_property', 'property_name':
                'category', 'old_value': 'old_value'})

    def test_collection_change_object_with_extra_attribute_in_cmd(self) ->None:
        with self.assertRaisesRegex(utils.ValidationError,
            'The following extra attributes are present: invalid'):
            collection_domain.CollectionChange({'cmd':
                'edit_collection_node_property', 'exploration_id':
                'exploration_id', 'property_name': 'category', 'old_value':
                'old_value', 'new_value': 'new_value', 'invalid': 'invalid'})
    ...
    def test_collection_change_object_with_invalid_collection_property(self
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            collection_domain.CollectionChange({'cmd':
                'edit_collection_property', 'property_name': 'invalid',
                'old_value': 'old_value', 'new_value': 'new_value'})

    def test_collection_change_object_with_create_new(self) ->None:
        col_change_object = collection_domain.CollectionChange({'cmd':
            'create_new', 'category': 'category', 'title': 'title'})
    ...
    def test_collection_change_object_with_add_collection_node(self) ->None:
        col_change_object = collection_domain.CollectionChange({'cmd':
            'add_collection_node', 'exploration_id': 'exploration_id'})
    ...
    def test_collection_change_object_with_edit_collection_node_property(self
        ...
        col_change_object = collection_domain.CollectionChange({'cmd':
            'edit_collection_node_property', 'exploration_id':
            'exploration_id', 'property_name': 'title', 'new_value':
    ...
    def test_collection_change_object_with_migrate_schema_to_latest_version(
        ...
        self.assertEqual(col_change_object.cmd,
    ...
    def test_collection_change_object_with_add_collection_skill(self) ->None:
        col_change_object = collection_domain.CollectionChange({'cmd':
            'add_collection_skill', 'name': 'name'})
    ...
    def test_collection_change_object_with_delete_collection_skill(self
        ...
        col_change_object = collection_domain.CollectionChange({'cmd':
            'delete_collection_skill', 'skill_id': 'skill_id'})
    ...
    def test_collection_change_object_with_add_question_id_to_skill(self
        ...
        col_change_object = collection_domain.CollectionChange({'cmd':
            'add_question_id_to_skill', 'skill_id': 'skill_id',
    ...
    def test_collection_change_object_with_remove_question_id_from_skill(self
        ...
        col_change_object = collection_domain.CollectionChange({'cmd':
            'remove_question_id_from_skill', 'skill_id': 'skill_id',
            'question_id': 'question_id'})
    ...
    def test_to_dict(self) ->None:
        col_change_dict = {'cmd': 'remove_question_id_from_skill',
            'skill_id': 'skill_id', 'question_id': 'question_id'}
...
class CollectionDomainUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.save_new_valid_collection(self.COLLECTION_ID,
            'user@example.com', title='Title', category='Category',
            objective='Objective', exploration_id=self.EXPLORATION_ID)
        self.collection = collection_services.get_collection_by_id(self.
            COLLECTION_ID)
    ...
    def test_schema_version_validation(self) ->None:
        ...
        self._assert_validation_error('Expected schema version to be %s' %
            feconf.CURRENT_COLLECTION_SCHEMA_VERSION)

    def test_nodes_validation(self) ->None:
        self.collection.nodes = {}
        self._assert_validation_error('Expected nodes to be a list')
        self.collection.nodes = [collection_domain.CollectionNode.from_dict
            ({'exploration_id': '0'}), collection_domain.CollectionNode.
            from_dict({'exploration_id': '0'})]
        self._assert_validation_error(
    ...
    def test_collection_export_import(self) ->None:
        ...
        collection = collection_domain.Collection.create_default_collection('0'
            , title='title', category='category', objective='objective')
    ...
    def test_update_collection_contents_from_model(self) ->None:
        (versioned_collection_contents: collection_domain.
            VersionedCollectionDict) = {'schema_version': 1,
            'collection_contents': {}}
        ...
        self.assertEqual(versioned_collection_contents[
            'collection_contents'], {})
        collection_domain.Collection.update_collection_contents_from_model(
    ...
    def test_update_collection_contents_from_model_with_schema_version_5(self
        ...
        (versioned_collection_contents: collection_domain.
            VersionedCollectionDict) = ({'schema_version': 5,
            'collection_contents': {'nodes': [{'prerequisite_skill_ids': [
            '11', '22'], 'acquired_skill_ids': ['33', '44'], 'other_field':
            'value1'}, {'prerequisite_skill_ids': ['11', '22'],
            'acquired_skill_ids': ['33', '44'], 'other_field': 'value2'}]}})
        collection_domain.Collection.update_collection_contents_from_model(
            versioned_collection_contents, 5)
        self.assertEqual(versioned_collection_contents['schema_version'], 6)
        self.assertEqual(versioned_collection_contents[
            'collection_contents']['nodes'], [{'other_field': 'value1'}, {
            'other_field': 'value2'}])

    def test_update_collection_contents_from_model_with_invalid_schema_version(
        self) ->None:
        (versioned_collection_contents: collection_domain.
            VersionedCollectionDict) = ({'schema_version': feconf.
            CURRENT_COLLECTION_SCHEMA_VERSION, 'collection_contents': {}})
        with self.assertRaisesRegex(Exception, 
            'Collection is version .+ but current collection schema version is %d'
             % feconf.CURRENT_COLLECTION_SCHEMA_VERSION):
            collection_domain.Collection.update_collection_contents_from_model(
                versioned_collection_contents, feconf.
    ...
    def test_serialize_and_deserialize_returns_unchanged_collection(self
        ...
        """
        self.assertEqual(self.collection.to_dict(), collection_domain.
            Collection.deserialize(self.collection.serialize()).to_dict())
...
class ExplorationGraphUnitTests(test_utils.GenericTestBase):
    ...
    def test_next_explorations(self) ->None:
        ...
        self.assertEqual(collection.first_exploration_id, collection.
            get_next_exploration_id([]))
    ...
    def test_next_explorations_in_sequence(self) ->None:
        ...
        self.assertEqual(collection.get_next_exploration_id_in_sequence(
            exploration_id), None)
        collection.add_node('exp_id_1')
        collection.add_node('exp_id_2')
        self.assertEqual(collection.get_next_exploration_id_in_sequence(
    ...
    def test_nodes_are_in_playble_order(self) ->None:
        ...
        observed_exploration_ids = [node.exploration_id for node in
            sorted_nodes]
    ...
    def test_next_explorations_with_invalid_exploration_ids(self) ->None:
        ...
        self.assertEqual(collection.get_next_exploration_id(['fake_exp_id']
            ), 'exp_id_1')
...
class YamlCreationUnitTests(test_utils.GenericTestBase):
    ...
    def test_yaml_import_and_export(self) ->None:
        ...
        self.save_new_valid_exploration(self.EXPLORATION_ID,
            'user@example.com', end_state_name='End')
        collection = collection_domain.Collection.create_default_collection(
        ...
        collection2 = collection_domain.Collection.from_yaml('collection2',
            yaml_content)
    ...
    def test_from_yaml_with_no_schema_version_specified_raises_error(self
        ...
        collection = collection_domain.Collection(self.COLLECTION_ID,
            'title', 'category', 'objective', 'en', [], None, [], 0)
        yaml_content = collection.to_yaml()
        with self.assertRaisesRegex(Exception,
    ...
    def test_from_yaml_with_invalid_schema_version_raises_error(self) ->None:
        collection = collection_domain.Collection(self.COLLECTION_ID,
            'title', 'category', 'objective', 'en', [], 0, [], 0)
        yaml_content = collection.to_yaml()
        with self.assertRaisesRegex(Exception,
...
class SchemaMigrationMethodsUnitTests(test_utils.GenericTestBase):
    ...
    def test_correct_collection_contents_schema_conversion_methods_exist(self
        ...
        exist.
        """
        current_collection_schema_version = (feconf.
            CURRENT_COLLECTION_SCHEMA_VERSION)
        for version_num in range(1, current_collection_schema_version):
            self.assertTrue(hasattr(collection_domain.Collection, 
                '_convert_collection_contents_v%s_dict_to_v%s_dict' % (
                version_num, version_num + 1)))
        self.assertFalse(hasattr(collection_domain.Collection, 
            '_convert_collection_contents_v%s_dict_to_v%s_dict' % (
            current_collection_schema_version, 
    ...
    def test_correct_collection_schema_conversion_methods_exist(self) ->None:
        ...
        for version_num in range(1, current_collection_schema_version):
            self.assertTrue(hasattr(collection_domain.Collection, 
                '_convert_v%s_dict_to_v%s_dict' % (version_num, version_num +
                1)))
        self.assertFalse(hasattr(collection_domain.Collection, 
...
"""
    ...
    def test_load_from_v1(self) ->None:
        ...
        self.save_new_valid_exploration('Exp1', 'user@example.com',
            end_state_name='End')
        collection = collection_domain.Collection.from_yaml('cid', self.
            YAML_CONTENT_V1)
...
class CollectionSummaryTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.collection_summary_dict = {'category': 'category', 'status':
            constants.ACTIVITY_STATUS_PRIVATE, 'community_owned': True,
            'viewer_ids': ['viewer_id'], 'version': 1, 'editor_ids': [
            'editor_id'], 'title': 'title', 'collection_model_created_on':
            current_time, 'tags': [], 'collection_model_last_updated':
            current_time, 'contributor_ids': ['contributor_id'],
            'language_code': 'en', 'objective': 'objective',
            'contributors_summary': {}, 'id': 'col_id', 'owner_ids': [
            'owner_id']}
        self.collection_summary = collection_domain.CollectionSummary('col_id',
            'title', 'category', 'objective', 'en', [], constants.
            ACTIVITY_STATUS_PRIVATE, True, ['owner_id'], ['editor_id'], [
            'viewer_id'], ['contributor_id'], {}, 1, 1, current_time,
            current_time)

    def test_collection_summary_gets_created(self) ->None:
        self.assertEqual(self.collection_summary.to_dict(), self.
    ...
    def test_is_private(self) ->None:
        ...
        self.collection_summary = collection_domain.CollectionSummary('col_id',
            'title', 'category', 'objective', 'en', [], constants.
            ACTIVITY_STATUS_PUBLIC, True, ['owner_id'], ['editor_id'], [
            'viewer_id'], ['contributor_id'], {}, 1, 1, datetime.datetime.
    ...
    def test_is_editable_by(self) ->None:
        ...
        self.collection_summary = collection_domain.CollectionSummary('col_id',
            'title', 'category', 'objective', 'en', [], constants.
            ACTIVITY_STATUS_PUBLIC, False, ['owner_id'], ['editor_id'], [
            'viewer_id'], ['contributor_id'], {}, 1, 1, datetime.datetime.
            utcnow(), datetime.datetime.utcnow())
        self.assertFalse(self.collection_summary.is_editable_by('other_id'))

    def test_is_solely_owned_by_user_one_owner(self) ->None:
        self.assertTrue(self.collection_summary.is_solely_owned_by_user(
            'owner_id'))
        self.assertFalse(self.collection_summary.is_solely_owned_by_user(
            'other_id'))
        self.collection_summary = collection_domain.CollectionSummary('col_id',
            'title', 'category', 'objective', 'en', [], constants.
            ACTIVITY_STATUS_PUBLIC, True, ['other_id'], ['editor_id'], [
            'viewer_id'], ['contributor_id'], {}, 1, 1, datetime.datetime.
            utcnow(), datetime.datetime.utcnow())
        self.assertFalse(self.collection_summary.is_solely_owned_by_user(
            'owner_id'))
    ...
    def test_is_solely_owned_by_user_multiple_owners(self) ->None:
        ...
        self.collection_summary = collection_domain.CollectionSummary('col_id',
            'title', 'category', 'objective', 'en', [], constants.
            ACTIVITY_STATUS_PUBLIC, True, ['owner_id', 'other_id'], [
    ...
    def test_does_user_have_any_role(self) ->None:
        self.assertTrue(self.collection_summary.does_user_have_any_role(
            'owner_id'))
        self.assertTrue(self.collection_summary.does_user_have_any_role(
            'viewer_id'))
        self.assertFalse(self.collection_summary.does_user_have_any_role(
            'other_id'))
    ...
    def test_add_new_contribution_for_user_adds_user_to_contributors(self
        ...
        self.assertEqual(self.collection_summary.contributors_summary[
            'user_id'], 1)
        self.assertIn('user_id', self.collection_summary.contributor_ids)

    def test_add_new_contribution_for_user_increases_score_in_contributors(self
        ) ->None:
        self.collection_summary.add_contribution_by_user('user_id')
        self.collection_summary.add_contribution_by_user('user_id')
        self.assertIn('user_id', self.collection_summary.contributors_summary)
    ...
    def test_add_new_contribution_for_user_does_not_add_system_user(self
        ...
        self.collection_summary.add_contribution_by_user(feconf.
            SYSTEM_COMMITTER_ID)
        self.assertNotIn(feconf.SYSTEM_COMMITTER_ID, self.
