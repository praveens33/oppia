...
collection_models, user_models = models.Registry.import_models([models.
    Names.COLLECTION, models.Names.USER])
...
class CollectionQueriesUnitTests(CollectionServicesUnitTests):
    ...
    def test_get_collection_titles_and_categories(self) ->None:
        self.assertEqual(collection_services.
            get_collection_titles_and_categories([]), {})
        self.save_new_default_collection('A', self.owner_id, title='TitleA')
        self.assertEqual(collection_services.
            get_collection_titles_and_categories(['A']), {'A': {'category':
            'A category', 'title': 'TitleA'}})
        self.save_new_default_collection('B', self.owner_id, title='TitleB')
        self.assertEqual(collection_services.
            get_collection_titles_and_categories(['A']), {'A': {'category':
            'A category', 'title': 'TitleA'}})
        self.assertEqual(collection_services.
            get_collection_titles_and_categories(['A', 'B']), {'A': {
            'category': 'A category', 'title': 'TitleA'}, 'B': {'category':
            'A category', 'title': 'TitleB'}})
        self.assertEqual(collection_services.
            get_collection_titles_and_categories(['A', 'C']), {'A': {
            'category': 'A category', 'title': 'TitleA'}})

    def test_get_collection_from_model(self) ->None:
        rights_manager.create_new_collection_rights('collection_id', self.
            owner_id)
        collection_model = collection_models.CollectionModel(id=
            'collection_id', category='category', title='title', objective=
            'objective', collection_contents={'nodes': {}})
        collection_model.commit(self.owner_id, 'collection model created',
        ...
        self.assertEqual(collection.language_code, constants.
            DEFAULT_LANGUAGE_CODE)
        self.assertEqual(collection.version, 1)
        self.assertEqual(collection.schema_version, feconf.
            CURRENT_COLLECTION_SCHEMA_VERSION)

    def test_get_collection_from_model_with_schema_version_2_copies_nodes(self
        ) ->None:
        collection_model = MockCollectionModel(id='collection_id', category
            ='category', title='title', schema_version=2, objective=
            'objective', version=1, nodes=[{'exploration_id': 'exp_id1',
            'acquired_skills': ['11'], 'prerequisite_skills': ['22']}, {
            'exploration_id': 'exp_id2', 'acquired_skills': ['33'],
        ...
        self.assertEqual(collection.language_code, constants.
            DEFAULT_LANGUAGE_CODE)
        self.assertEqual(collection.version, 1)
        self.assertEqual(collection.nodes[0].to_dict(), {'exploration_id':
            'exp_id1'})
        self.assertEqual(collection.nodes[1].to_dict(), {'exploration_id':
            'exp_id2'})
        self.assertEqual(collection.schema_version, feconf.
            CURRENT_COLLECTION_SCHEMA_VERSION)
    ...
    def test_get_collection_from_model_with_invalid_schema_version_raises_error(
        ...
        rights_manager.create_new_collection_rights('collection_id', self.
            owner_id)
        collection_model = collection_models.CollectionModel(id=
            'collection_id', category='category', title='title',
            schema_version=0, objective='objective', collection_contents={
            'nodes': {}})
        collection_model.commit(self.owner_id, 'collection model created',
            [{'cmd': 'create_new', 'title': 'title', 'category': 'category'}])
    ...
    def test_get_different_collections_by_version(self) ->None:
        ...
        collection_services.update_collection(self.owner_id,
            'collection_id', [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'objective',
            'new_value': 'Some new objective'}, {'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'title',
            'new_value': 'Some new title'}, {'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'category',
            'new_value': 'Some new category'}], 'Changed properties')
    ...
    def test_save_collection_without_change_list_raises_error(self) ->None:
        collection = self.save_new_valid_collection('collection_id', self.
            owner_id)
        apply_change_list_swap = self.swap(collection_services,
            'apply_change_list', lambda _, __: collection)
        with apply_change_list_swap, self.assertRaisesRegex(Exception,
            'Unexpected error: received an invalid change list when trying to save collection'
            ):
            collection_services.update_collection(self.owner_id,
                'collection_id', None, 'commit message')
    ...
    def test_save_collection_with_mismatch_of_versions_raises_error(self
        ...
        with self.assertRaisesRegex(Exception,
            'Unexpected error: trying to update version 0 of collection from version 1. Please reload the page and try again.'
            ):
            collection_services.update_collection(self.owner_id,
                'collection_id', [{'cmd': collection_domain.
                CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'objective',
                'new_value': 'Some new objective'}], 'changed objective')

    def test_get_multiple_collections_from_model_by_id(self) ->None:
        rights_manager.create_new_collection_rights('collection_id_1', self
            .owner_id)
        collection_model = collection_models.CollectionModel(id=
            'collection_id_1', category='category 1', title='title 1',
            objective='objective 1', collection_contents={'nodes': {}})
        collection_model.commit(self.owner_id, 'collection model created',
            [{'cmd': 'create_new', 'title': 'title 1', 'category':
            'category 1'}])
        rights_manager.create_new_collection_rights('collection_id_2', self
            .owner_id)
        collection_model = collection_models.CollectionModel(id=
            'collection_id_2', category='category 2', title='title 2',
            objective='objective 2', collection_contents={'nodes': {}})
        collection_model.commit(self.owner_id, 'collection model created',
            [{'cmd': 'create_new', 'title': 'title 2', 'category':
            'category 2'}])
        ...
        self.assertEqual(collections['collection_id_1'].objective,
            'objective 1')
        ...
        self.assertEqual(collections['collection_id_2'].objective,
            'objective 2')
    ...
    def test_get_explorations_completed_in_collections(self) ->None:
        collection = self.save_new_valid_collection('collection_id', self.
            owner_id, exploration_id='exp_id')
        self.save_new_valid_exploration('exp_id_1', self.owner_id)
        collection.add_node('exp_id_1')
        completed_exp_ids = (collection_services.
            get_explorations_completed_in_collections(self.owner_id, [
            'collection_id']))
    ...
    def test_update_collection_by_swapping_collection_nodes(self) ->None:
        collection = self.save_new_valid_collection('collection_id', self.
            owner_id, exploration_id='exp_id_1')
        self.save_new_valid_exploration('exp_id_2', self.owner_id)
        collection_services.update_collection(self.owner_id,
            'collection_id', [{'cmd': collection_domain.
            CMD_ADD_COLLECTION_NODE, 'exploration_id': 'exp_id_2'}],
        ...
        collection_services.update_collection(self.owner_id,
            'collection_id', [{'cmd': collection_domain.
            CMD_SWAP_COLLECTION_NODES, 'first_index': 0, 'second_index': 1}
    ...
    def test_update_collection_with_invalid_cmd_raises_error(self) ->None:
        ...
        self.save_new_valid_collection('collection_id', self.owner_id)
        with self.assertRaisesRegex(Exception,
            'Command invalid command is not allowed'), logging_swap:
            collection_services.update_collection(self.owner_id,
                'collection_id', [{'cmd': 'invalid command'}], 'Commit message'
                )
        self.assertEqual(len(observed_log_messages), 1)
        self.assertEqual(observed_log_messages[0],
            "ValidationError Command invalid command is not allowed collection_id [{'cmd': 'invalid command'}]"
            )
...
class CollectionProgressUnitTests(CollectionServicesUnitTests):
    ...
    def _get_progress_model(self, user_id: str, collection_id: str) ->Optional[
    ...
    def _record_completion(self, user_id: str, collection_id: str,
    ...
    def setUp(self) ->None:
        super().setUp()
        self.save_new_valid_collection(self.COL_ID_0, self.owner_id,
        ...
        for exp_id in [self.EXP_ID_1, self.EXP_ID_2]:
            ...
            collection_services.update_collection(self.owner_id, self.
                COL_ID_0, [{'cmd': collection_domain.
                CMD_ADD_COLLECTION_NODE, 'exploration_id': exp_id}],
                'Added new exploration')
    ...
    def test_get_completed_exploration_ids(self) ->None:
        ...
        self.assertEqual(collection_services.get_completed_exploration_ids(
            self.owner_id, 'Fake'), [])
        self.assertIsNone(self._get_progress_model(self.owner_id, self.
            COL_ID_0))
        self.assertEqual(collection_services.get_completed_exploration_ids(
            self.owner_id, self.COL_ID_0), [])
        self._record_completion(self.owner_id, self.COL_ID_0, self.EXP_ID_0)
        self.assertIsNotNone(self._get_progress_model(self.owner_id, self.
            COL_ID_0))
        self.assertEqual(collection_services.get_completed_exploration_ids(
        ...
        self.assertEqual(collection_services.get_completed_exploration_ids(
            self.owner_id, self.COL_ID_0), [self.EXP_ID_0, self.EXP_ID_2,
            self.EXP_ID_1])

    def test_get_next_exploration_id_to_complete_by_user(self) ->None:
        self.assertEqual(collection_services.
            get_next_exploration_id_to_complete_by_user('Fake', self.
            COL_ID_0), self.EXP_ID_0)
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id Fake not found'):
            collection_services.get_next_exploration_id_to_complete_by_user(
                self.owner_id, 'Fake')
        self.assertEqual(collection_services.get_collection_by_id(self.
            COL_ID_0).first_exploration_id, self.EXP_ID_0)
        self.assertEqual(collection_services.
            get_next_exploration_id_to_complete_by_user(self.owner_id, self
            .COL_ID_0), self.EXP_ID_0)
        self._record_completion(self.owner_id, self.COL_ID_0, self.EXP_ID_0)
        self.assertEqual(collection_services.
            get_next_exploration_id_to_complete_by_user(self.owner_id, self
            .COL_ID_0), self.EXP_ID_1)
        self._record_completion(self.owner_id, self.COL_ID_0, self.EXP_ID_1)
        self._record_completion(self.owner_id, self.COL_ID_0, self.EXP_ID_2)
        self.assertEqual(collection_services.
            get_next_exploration_id_to_complete_by_user(self.owner_id, self
    ...
    def test_record_played_exploration_in_collection_context(self) ->None:
        completion_model = self._get_progress_model(self.owner_id, self.
            COL_ID_0)
        ...
        self.assertEqual(completion_model.completed_explorations, [self.
            EXP_ID_0])
        collection_services.record_played_exploration_in_collection_context(
            self.owner_id, self.COL_ID_0, self.EXP_ID_0)
        completion_model = self._get_progress_model(self.owner_id, self.
        ...
        assert completion_model is not None
        self.assertEqual(completion_model.completed_explorations, [self.
            EXP_ID_0, self.EXP_ID_2, self.EXP_ID_1])
...
class CollectionSummaryQueriesUnitTests(CollectionServicesUnitTests):
    ...
    def setUp(self) ->None:
        super().setUp()
        self.save_new_default_collection(self.COL_ID_0, self.owner_id,
            title='Bridges in England', category='Architecture')
        self.save_new_default_collection(self.COL_ID_1, self.owner_id,
            title='Introduce Oppia', category='Welcome')
        self.save_new_default_collection(self.COL_ID_2, self.owner_id,
            title='Introduce Interactions in Oppia', category='Welcome')
        self.save_new_default_collection(self.COL_ID_3, self.owner_id,
            title='Welcome', category='Welcome')
        self.save_new_default_collection(self.COL_ID_4, self.owner_id,
            title='Learning basic verbs in Spanish', category='Languages')
        self.save_new_default_collection(self.COL_ID_5, self.owner_id,
        ...
        collection_services.index_collections_given_ids([self.COL_ID_0,
            self.COL_ID_1, self.COL_ID_2, self.COL_ID_3, self.COL_ID_4])

    def _create_search_query(self, terms: List[str], categories: List[str]
        ) ->str:
        """Returns the search query derived from terms and categories."""
        query = ' '.join(terms)
        if categories:
            query += '%s)' % 'category=( OR '.join([('"%s"' % category) for
    ...
    def test_publish_collection_raise_exception_for_invalid_collection_id(self
        ) ->None:
        system_user = user_services.get_system_user()
        with self.assertRaisesRegex(Exception,
            'No collection summary model exists for the given id: Invalid_collection_id'
            ):
            with self.swap_to_always_return(rights_manager,
    ...
    def test_get_collection_summaries_with_no_query(self) ->None:
        col_ids, search_cursor = (collection_services.
            get_collection_ids_matching_query('', [], []))
        self.assertEqual(sorted(col_ids), [self.COL_ID_0, self.COL_ID_1,
            self.COL_ID_2, self.COL_ID_3, self.COL_ID_4])
    ...
    def test_get_collection_summaries_with_deleted_collections(self) ->None:
        ...
        self.assertEqual(collection_services.
            get_collection_ids_matching_query('', [], []), ([], None))

    def test_get_collection_summaries_with_deleted_collections_multi(self
        ) ->None:
        collection_services.delete_collections(self.owner_id, [self.
            COL_ID_0, self.COL_ID_2, self.COL_ID_4])
        col_ids = collection_services.get_collection_ids_matching_query('',
            [], [])[0]
        self.assertEqual(sorted(col_ids), [self.COL_ID_1, self.COL_ID_3])
        collection_services.delete_collections(self.owner_id, [self.
            COL_ID_1, self.COL_ID_3])
        self.assertEqual(collection_services.
            get_collection_ids_matching_query('', [], []), ([], None))
    ...
    def test_search_collection_summaries(self) ->None:
        ...
        col_ids = collection_services.get_collection_ids_matching_query('Oppia'
            , [], [])[0]
        ...
        col_ids = collection_services.get_collection_ids_matching_query(
            'England', [], [])[0]
        self.assertEqual(col_ids, [self.COL_ID_0])
        col_ids = collection_services.get_collection_ids_matching_query('in',
            [], [])[0]
        self.assertEqual(sorted(col_ids), [self.COL_ID_0, self.COL_ID_2,
            self.COL_ID_4])
        col_ids = collection_services.get_collection_ids_matching_query('in',
            ['Architecture', 'Welcome'], [])[0]
        self.assertEqual(sorted(col_ids), [self.COL_ID_0, self.COL_ID_2])
    ...
    def test_collection_summaries_pagination_in_filled_search_results(self
        ...
        with self.swap(feconf, 'SEARCH_RESULTS_PAGE_SIZE', 2):
            ...
            found_col_ids += col_ids
            self.assertEqual(sorted(found_col_ids), [self.COL_ID_0, self.
                COL_ID_1, self.COL_ID_2, self.COL_ID_3, self.COL_ID_4])
...
class CollectionCreateAndDeleteUnitTests(CollectionServicesUnitTests):
    ...
    def test_retrieval_of_collection(self) ->None:
        ...
        collection = self.save_new_default_collection(self.COLLECTION_0_ID,
            self.owner_id)
        retrieved_collection = collection_services.get_collection_by_id(self
            .COLLECTION_0_ID)
        self.assertEqual(collection.id, retrieved_collection.id)
        self.assertEqual(collection.title, retrieved_collection.title)
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id fake_collection not found'
            ):
...
doesnt_exist"""
    ...
    def test_soft_deletion_of_collection(self) ->None:
        ...
        self.assertEqual(count_at_least_editable_collection_summaries(self.
            owner_id), 1)
        collection_services.delete_collection(self.owner_id, self.
            COLLECTION_0_ID)
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id A_collection_0_id not found'
            ):
        ...
        self.assertIsNone(collection_models.CollectionSummaryModel.
            get_by_id(self.COLLECTION_0_ID))
        self.assertIsNotNone(collection_models.
            CollectionCommitLogEntryModel.get_by_id('collection-%s-%s' % (
            self.COLLECTION_0_ID, 1)))
        collection_snapshot_id = (collection_models.CollectionModel.
            get_snapshot_id(self.COLLECTION_0_ID, 1))
        self.assertIsNotNone(collection_models.
            CollectionSnapshotMetadataModel.get_by_id(collection_snapshot_id))
        self.assertIsNotNone(collection_models.
            CollectionSnapshotContentModel.get_by_id(collection_snapshot_id))
        collection_rights_snapshot_id = (collection_models.
            CollectionRightsModel.get_snapshot_id(self.COLLECTION_0_ID, 1))
        self.assertIsNotNone(collection_models.
            CollectionRightsSnapshotMetadataModel.get_by_id(
            collection_rights_snapshot_id))
        self.assertIsNotNone(collection_models.
            CollectionRightsSnapshotContentModel.get_by_id(
            collection_rights_snapshot_id))
    ...
    def test_soft_deletion_of_multiple_collections(self) ->None:
        ...
        self.assertEqual(count_at_least_editable_collection_summaries(self.
            owner_id), 2)
        collection_services.delete_collections(self.owner_id, [self.
            COLLECTION_0_ID, self.COLLECTION_1_ID])
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id A_collection_0_id not found'
            ):
            collection_services.get_collection_by_id(self.COLLECTION_0_ID)
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id A_collection_1_id not found'
            ):
        ...
        collection_0_snapshot_id = (collection_models.CollectionModel.
            get_snapshot_id(self.COLLECTION_0_ID, 1))
        collection_1_snapshot_id = (collection_models.CollectionModel.
            get_snapshot_id(self.COLLECTION_1_ID, 1))
        self.assertIsNotNone(collection_models.
            CollectionSnapshotMetadataModel.get_by_id(collection_0_snapshot_id)
            )
        self.assertIsNotNone(collection_models.
            CollectionSnapshotContentModel.get_by_id(collection_0_snapshot_id))
        self.assertIsNotNone(collection_models.
            CollectionSnapshotMetadataModel.get_by_id(collection_1_snapshot_id)
            )
        self.assertIsNotNone(collection_models.
            CollectionSnapshotContentModel.get_by_id(collection_1_snapshot_id))
        collection_0_rights_snapshot_id = (collection_models.
            CollectionRightsModel.get_snapshot_id(self.COLLECTION_0_ID, 1))
        collection_1_rights_snapshot_id = (collection_models.
            CollectionRightsModel.get_snapshot_id(self.COLLECTION_1_ID, 1))
        self.assertIsNotNone(collection_models.
            CollectionRightsSnapshotMetadataModel.get_by_id(
    ...
    def test_hard_deletion_of_collection(self) ->None:
        ...
        self.assertEqual(count_at_least_editable_collection_summaries(self.
            owner_id), 1)
        collection_services.delete_collection(self.owner_id, self.
            COLLECTION_0_ID, force_deletion=True)
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id A_collection_0_id not found'
            ):
            collection_services.get_collection_by_id(self.COLLECTION_0_ID)
    ...
    def test_hard_deletion_of_multiple_collections(self) ->None:
        ...
        self.assertEqual(count_at_least_editable_collection_summaries(self.
            owner_id), 2)
        collection_services.delete_collections(self.owner_id, [self.
            COLLECTION_0_ID, self.COLLECTION_1_ID], force_deletion=True)
        with self.assertRaisesRegex(Exception,
            'Entity for class CollectionModel with id A_collection_0_id not found'
            ):
            collection_services.get_collection_by_id(self.COLLECTION_0_ID)
    ...
    def test_summaries_of_hard_deleted_collections(self) ->None:
        ...
        self.assertEqual(count_at_least_editable_collection_summaries(self.
            owner_id), 0)
        self.assertNotIn(self.COLLECTION_0_ID, [collection.id for
            collection in collection_models.CollectionSummaryModel.get_all(
    ...
    def test_collection_is_removed_from_index_when_deleted(self) ->None:
        ...
        def mock_delete_docs(doc_ids: List[str], index: str) ->None:
            self.assertEqual(index, collection_services.
    ...
    def test_collections_are_removed_from_index_when_deleted(self) ->None:
        ...
        def mock_delete_docs(doc_ids: List[str], index: str) ->None:
            self.assertEqual(index, collection_services.
                SEARCH_INDEX_COLLECTIONS)
            self.assertEqual(doc_ids, [self.COLLECTION_0_ID, self.
                COLLECTION_1_ID])
        delete_docs_swap = self.swap(gae_search_services,
            'delete_documents_from_index', mock_delete_docs)
        with delete_docs_swap:
    ...
    def test_create_new_collection(self) ->None:
        collection_domain.Collection.create_default_collection(self.
            COLLECTION_0_ID)

    def test_save_and_retrieve_collection(self) ->None:
        collection = self.save_new_valid_collection(self.COLLECTION_0_ID,
            self.owner_id)
        collection_services._save_collection(self.owner_id, collection, '',
    ...
    def test_save_and_retrieve_collection_summary(self) ->None:
        ...
        collection_services._save_collection(self.owner_id, collection, '',
            _get_collection_change_list('title', ''))
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': 'edit_collection_property',
            'property_name': 'title', 'new_value': 'A new title'}, {'cmd':
            'edit_collection_property', 'property_name': 'category',
            'new_value': 'A new category'}], 'Change title and category')
        retrieved_collection_summary = (collection_services.
            get_collection_summary_by_id(self.COLLECTION_0_ID))
        ...
        self.assertEqual(retrieved_collection_summary.category,
            'A new category')

    def test_update_collection_by_migration_bot(self) ->None:
        exp_id = 'exp_id'
        self.save_new_valid_collection(self.COLLECTION_0_ID, self.owner_id,
        ...
        collection_services.update_collection(feconf.MIGRATION_BOT_USER_ID,
            self.COLLECTION_0_ID, [{'cmd': 'edit_collection_property',
            'property_name': 'title', 'new_value': 'New title'}],
            'Did migration.')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
    ...
    def test_update_collection_schema(self) ->None:
        ...
        rights_manager.publish_collection(self.owner, self.COLLECTION_0_ID)
        collection_services.update_collection(feconf.MIGRATION_BOT_USER_ID,
            self.COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_MIGRATE_SCHEMA_TO_LATEST_VERSION, 'from_version': 2,
            'to_version': 3}], 'Did migration.')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
        self.assertEqual(collection.version, 2)
...
class LoadingAndDeletionOfCollectionDemosTests(CollectionServicesUnitTests):
    ...
    def test_loading_and_validation_and_deletion_of_demo_collections(self
        ...
        self.assertEqual(collection_models.CollectionModel.
            get_collection_count(), 0)
        self.assertGreaterEqual(len(feconf.DEMO_COLLECTIONS), 1, msg=
            'There must be at least one demo collection.')
        ...
        self.assertEqual(collection_models.CollectionModel.
            get_collection_count(), len(feconf.DEMO_COLLECTIONS))
        for collection_id in feconf.DEMO_COLLECTIONS:
...
class UpdateCollectionNodeTests(CollectionServicesUnitTests):
    ...
    def setUp(self) ->None:
        super().setUp()
        self.save_new_valid_collection(self.COLLECTION_0_ID, self.owner_id,
            title=self.COLLECTION_TITLE, category=self.COLLECTION_CATEGORY,
            objective=self.COLLECTION_OBJECTIVE, exploration_id=self.
            EXPLORATION_ID)
    ...
    def test_add_node(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_ADD_COLLECTION_NODE, 'exploration_id': new_exp_id}],
            'Added new exploration')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
        self.assertEqual(collection.exploration_ids, [self.EXPLORATION_ID,
    ...
    def test_add_node_with_non_existent_exploration(self) ->None:
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            collection_services.update_collection(self.owner_id, self.
                COLLECTION_0_ID, [{'cmd': collection_domain.
                CMD_ADD_COLLECTION_NODE, 'exploration_id':
                non_existent_exp_id}], 'Added non-existent exploration')
    ...
    def test_add_node_with_private_exploration_in_public_collection(self
        ...
        rights_manager.publish_collection(self.owner, self.COLLECTION_0_ID)
        self.assertTrue(rights_manager.is_collection_public(self.
            COLLECTION_0_ID))
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            collection_services.update_collection(self.owner_id, self.
                COLLECTION_0_ID, [{'cmd': collection_domain.
                CMD_ADD_COLLECTION_NODE, 'exploration_id': private_exp_id}],
    ...
    def test_add_node_with_public_exploration_in_private_collection(self
        ...
        self.assertTrue(rights_manager.is_collection_private(self.
            COLLECTION_0_ID))
        self.assertTrue(rights_manager.is_exploration_public(public_exp_id))
        self.assertTrue(rights_manager.is_exploration_private(private_exp_id))
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_ADD_COLLECTION_NODE, 'exploration_id': public_exp_id}, {
    ...
    def test_delete_node(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_DELETE_COLLECTION_NODE, 'exploration_id': self.
            EXPLORATION_ID}], 'Deleted exploration')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
    ...
    def test_update_collection_category(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'category',
            'new_value': 'Some new category'}], 'Changed the category')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
        self.assertEqual(collection.category, 'Some new category')
    ...
    def test_update_collection_objective(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'objective',
            'new_value': 'Some new objective'}], 'Changed the objective')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
        self.assertEqual(collection.objective, 'Some new objective')
    ...
    def test_update_collection_language_code(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'language_code',
            'new_value': 'fi'}], 'Changed the language to Finnish')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
    ...
    def test_update_collection_tags(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'tags',
            'new_value': ['test']}], 'Add a new tag')
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            collection_services.update_collection(self.owner_id, self.
                COLLECTION_0_ID, [{'cmd': collection_domain.
                CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'tags',
                'new_value': ['duplicate', 'duplicate']}], 'Add a new tag')


def _get_collection_change_list(property_name: str, new_value: str) ->List[Dict
    [str, str]]:
    """Generates a change list for a single collection property change."""
    return [{'cmd': collection_domain.CMD_EDIT_COLLECTION_PROPERTY,
        'property_name': property_name, 'new_value': new_value}]


def _get_added_exploration_change_list(exploration_id: str) ->List[Dict[str,
    str]]:
    """Generates a change list for adding an exploration to a collection."""
    return [{'cmd': collection_domain.CMD_ADD_COLLECTION_NODE,
        'exploration_id': exploration_id}]


def _get_deleted_exploration_change_list(exploration_id: str) ->List[Dict[
    str, str]]:
    """Generates a change list for deleting an exploration from a collection."""
    return [{'cmd': collection_domain.CMD_DELETE_COLLECTION_NODE,
        'exploration_id': exploration_id}]
...
class CommitMessageHandlingTests(CollectionServicesUnitTests):
    ...
    def test_record_commit_message(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, _get_collection_change_list(collection_domain.
            COLLECTION_PROPERTY_TITLE, 'New Title'), 'A message')
        self.assertEqual(collection_services.
            get_collection_snapshots_metadata(self.COLLECTION_0_ID)[1][
    ...
    def test_demand_commit_message(self) ->None:
        ...
        with self.assertRaisesRegex(ValueError,
            ...
            collection_services.update_collection(self.owner_id, self.
                COLLECTION_0_ID, _get_collection_change_list(
    ...
    def test_unpublished_collections_can_accept_commit_message(self) ->None:
        ...
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, _get_collection_change_list(collection_domain.
            COLLECTION_PROPERTY_TITLE, 'New Title'), '')
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, _get_collection_change_list(collection_domain.
            COLLECTION_PROPERTY_TITLE, 'New Title'), None)
...
class CollectionSnapshotUnitTests(CollectionServicesUnitTests):
    ...
    def test_get_collection_snapshots_metadata(self) ->None:
        ...
        exp_id = 'exp_id0'
        v1_collection = self.save_new_valid_collection(self.COLLECTION_0_ID,
            self.owner_id, exploration_id=exp_id)
        snapshots_metadata = (collection_services.
            get_collection_snapshots_metadata(self.COLLECTION_0_ID))
        self.assertEqual(len(snapshots_metadata), 1)
        self.assertDictContainsSubset({'commit_cmds': [{'cmd': 'create_new',
            'title': 'A title', 'category': 'A category'}], 'committer_id':
            self.owner_id, 'commit_message':
            "New collection created with title 'A title'.", 'commit_type':
        ...
        self.assertDictContainsSubset({'commit_cmds': [{'cmd': 'create_new',
            ...
            'create', 'version_number': 1}, snapshots_metadata[0])
        self.assertIn('created_on_ms', snapshots_metadata[0])
        change_list = [{'cmd': 'edit_collection_property', 'property_name':
            'title', 'new_value': 'First title'}]
        collection_services.update_collection(self.owner_id, self.
            COLLECTION_0_ID, change_list, 'Changed title.')
        snapshots_metadata = (collection_services.
            get_collection_snapshots_metadata(self.COLLECTION_0_ID))
        ...
        self.assertDictContainsSubset({'commit_cmds': [{'cmd': 'create_new',
            ...
            "New collection created with title 'A title'.", 'commit_type':
            'create', 'version_number': 1}, snapshots_metadata[0])
        self.assertDictContainsSubset({'commit_cmds': change_list,
        ...
        with self.assertRaisesRegex(Exception, 'version 1, which is too old'):
            collection_services._save_collection(second_committer_id,
                v1_collection, '', _get_collection_change_list('title', ''))
        new_change_list = [{'cmd': 'edit_collection_property',
            'property_name': 'title', 'new_value': 'New title'}]
        collection_services.update_collection(second_committer_id, self.
            COLLECTION_0_ID, new_change_list, 'Second commit.')
        snapshots_metadata = (collection_services.
            get_collection_snapshots_metadata(self.COLLECTION_0_ID))
        ...
        self.assertDictContainsSubset({'commit_cmds': change_list,
            ...
            'Changed title.', 'commit_type': 'edit', 'version_number': 2},
            snapshots_metadata[1])
        self.assertDictContainsSubset({'commit_cmds': new_change_list,
    ...
    def test_versioning_with_add_and_delete_nodes(self) ->None:
        ...
        collection_services._save_collection(self.owner_id, collection,
            'Changed title.', _get_collection_change_list('title',
            'First title'))
        ...
        collection = collection_services.get_collection_by_id(self.
            COLLECTION_0_ID)
        collection.add_node(self.save_new_valid_exploration(
            'new_exploration_id', self.owner_id).id)
        collection_services._save_collection('committer_id_2', collection,
            'Added new exploration', _get_added_exploration_change_list(
            'new_exploration_id'))
        commit_dict_3 = {'committer_id': 'committer_id_2', 'commit_message':
            'Added new exploration', 'version_number': 3}
        snapshots_metadata = (collection_services.
            get_collection_snapshots_metadata(self.COLLECTION_0_ID))
        self.assertEqual(len(snapshots_metadata), 3)
        ...
        collection_services._save_collection('committer_id_3', collection,
            'Deleted exploration', _get_deleted_exploration_change_list(
            'new_exploration_id'))
        commit_dict_4 = {'committer_id': 'committer_id_3', 'commit_message':
            'Deleted exploration', 'version_number': 4}
        snapshots_metadata = (collection_services.
            get_collection_snapshots_metadata(self.COLLECTION_0_ID))
...
class CollectionSearchTests(CollectionServicesUnitTests):
    ...
    def test_index_collections_given_ids(self) ->None:
        ...
        all_collection_titles = ['title 0', 'title 1', 'title 2', 'title 3',
            'title 4']
        ...
        def mock_add_documents_to_index(docs: List[Dict[str, str]], index: str
            ...
            self.assertEqual(set(categories), set(
        ...
        for ind in range(5):
            self.save_new_valid_collection(all_collection_ids[ind], self.
                owner_id, title=all_collection_titles[ind], category=
                all_collection_categories[ind])
        for ind in range(4):
            rights_manager.publish_collection(self.owner,
...
class CollectionSummaryTests(CollectionServicesUnitTests):
    ...
    def test_is_editable_by(self) ->None:
        ...
        self.assertTrue(collection_summary.is_editable_by(user_id=self.
            owner_id))
        self.assertFalse(collection_summary.is_editable_by(user_id=self.
            editor_id))
        self.assertFalse(collection_summary.is_editable_by(user_id=self.
            viewer_id))
        rights_manager.assign_role_for_collection(self.owner, self.
            COLLECTION_0_ID, self.viewer_id, rights_domain.ROLE_VIEWER)
        rights_manager.assign_role_for_collection(self.owner, self.
            COLLECTION_0_ID, self.editor_id, rights_domain.ROLE_EDITOR)
        collection_summary = collection_services.get_collection_summary_by_id(
            self.COLLECTION_0_ID)
        assert collection_summary is not None
        self.assertTrue(collection_summary.is_editable_by(user_id=self.
    ...
    def test_contributor_ids(self) ->None:
        ...
        changelist_cmds = [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'title',
            'new_value': 'Collection Bob title'}]
        collection_services.update_collection(self.bob_id, self.
            COLLECTION_0_ID, changelist_cmds, 'Changed title to Bob title.')
        rights_manager.assign_role_for_collection(albert, self.
            COLLECTION_0_ID, self.viewer_id, rights_domain.ROLE_VIEWER)
        rights_manager.assign_role_for_collection(albert, self.
            COLLECTION_0_ID, self.editor_id, rights_domain.ROLE_EDITOR)
        collection_summary = collection_services.get_collection_summary_by_id(
            self.COLLECTION_0_ID)
        assert collection_summary is not None
        self.assertItemsEqual(collection_summary.contributor_ids, [self.
            albert_id, self.bob_id])

    def _check_contributors_summary(self, collection_id: str, expected:
        Dict[str, int]) ->None:
        """Checks the contributors summary with the expected summary."""
        contributors_summary = (collection_services.
            get_collection_summary_by_id(collection_id))
    ...
    def test_contributor_summary(self) ->None:
        self.save_new_valid_collection(self.COLLECTION_0_ID, self.albert_id)
        self._check_contributors_summary(self.COLLECTION_0_ID, {self.
            albert_id: 1})
        changelist_cmds = [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'title',
            'new_value': 'Collection Bob title'}]
        collection_services.update_collection(self.bob_id, self.
            COLLECTION_0_ID, changelist_cmds, 'Changed title.')
        self._check_contributors_summary(self.COLLECTION_0_ID, {self.
            albert_id: 1, self.bob_id: 1})
        collection_services.update_collection(self.bob_id, self.
            COLLECTION_0_ID, changelist_cmds, 'Changed title.')
        self._check_contributors_summary(self.COLLECTION_0_ID, {self.
            albert_id: 1, self.bob_id: 2})
        collection_services.update_collection(self.albert_id, self.
            COLLECTION_0_ID, changelist_cmds, 'Changed title.')
        self._check_contributors_summary(self.COLLECTION_0_ID, {self.
            albert_id: 2, self.bob_id: 2})
    ...
    def test_create_collection_summary_with_contributor_to_remove(self) ->None:
        ...
        collection_services.update_collection(self.bob_id, self.
            COLLECTION_0_ID, [{'cmd': collection_domain.
            CMD_EDIT_COLLECTION_PROPERTY, 'property_name': 'title',
            'new_value': 'Collection Bob title'}], 'Changed title.')
    ...
    def test_raises_error_when_collection_provided_with_no_last_updated_data(
        ...
        with self.swap_to_always_return(collection_services,
    ...
    def test_raises_error_when_collection_provided_with_no_created_on_data(self
        ) ->None:
        self.save_new_valid_collection('test_id', self.albert_id)
        collection = collection_services.get_collection_by_id('test_id')
...
class GetCollectionAndCollectionRightsTests(CollectionServicesUnitTests):
    ...
    def test_get_collection_and_collection_rights_object(self) ->None:
        ...
        collection, collection_rights = (collection_services.
            get_collection_and_collection_rights_by_id(collection_id))
        assert collection_rights is not None
        assert collection is not None
        self.assertEqual(collection.id, collection_id)
        self.assertEqual(collection_rights.id, collection_id)
        collection, collection_rights = (collection_services.
            get_collection_and_collection_rights_by_id('fake_id'))
        self.assertIsNone(collection)
