...
from typing import Callable, DefaultDict, Dict, Final, List, Optional, Sequence, Set, Type, Union
MYPY = False
if MYPY:
...
EMAIL_FOOTER = (
    'You can change your email preferences via the <a href="http://localhost:8181/preferences">Preferences</a> page.'
    )
...
class EmailToAdminTest(test_utils.EmailTestBase):
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Name'), (platform_parameter_list.ParamName.
    ...
    def test_email_to_admin_is_sent_correctly(self) ->None:
        ...
        system_name_ctx = self.swap(feconf, 'SYSTEM_EMAIL_NAME',
            dummy_system_name)
        system_email_ctx = self.swap(feconf, 'SYSTEM_EMAIL_ADDRESS',
            dummy_system_address)
        admin_email_ctx = self.swap(feconf, 'ADMIN_EMAIL_ADDRESS',
            dummy_admin_address)
        with system_name_ctx, system_email_ctx, admin_email_ctx:
            ...
            self.assertEqual(messages[0].sender,
                'DUMMY_SYSTEM_NAME <dummy@system.com>')
...
class EmailRightsTest(test_utils.GenericTestBase):
    ...
    def test_sender_id_validation(self) ->None:
        sender_ids_to_test = [feconf.SYSTEM_COMMITTER_ID, self.admin_id,
            self.moderator_id, self.editor_id]
        expected_validation_results = {feconf.EMAIL_INTENT_SIGNUP: (True, 
            False, False, False), feconf.EMAIL_INTENT_DAILY_BATCH: (True, 
            False, False, False), feconf.EMAIL_INTENT_MARKETING: (True, 
            False, False, False), feconf.EMAIL_INTENT_UNPUBLISH_EXPLORATION:
            (True, False, True, False), feconf.
            EMAIL_INTENT_DELETE_EXPLORATION: (True, False, True, False)}
            ...
            for ind, sender_id in enumerate(sender_ids_to_test):
                if results[ind]:
                    ...
                    with self.assertRaisesRegex(Exception, 'Invalid sender_id'
                        ...
                        email_manager.require_sender_id_is_valid(intent,
                            sender_id)
        with self.assertRaisesRegex(Exception, 'Invalid email intent string'):
            email_manager.require_sender_id_is_valid('', feconf.
                SYSTEM_COMMITTER_ID)
        with self.assertRaisesRegex(Exception, 'Invalid email intent string'):
            email_manager.require_sender_id_is_valid('', self.admin_id)
        with self.assertRaisesRegex(Exception, 'Invalid email intent string'):
            email_manager.require_sender_id_is_valid('invalid_intent',
                feconf.SYSTEM_COMMITTER_ID)
        with self.assertRaisesRegex(Exception, 'Invalid email intent string'):
            email_manager.require_sender_id_is_valid('invalid_intent', self
                .admin_id)
...
class ExplorationMembershipEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.exploration = self.save_new_default_exploration('A', self.
            editor_id, title=self.EXPLORATION_TITLE)
        self.expected_email_subject = ('%s - invitation to collaborate' %
            self.EXPLORATION_TITLE)
        self.can_send_editor_role_email_ctx = self.swap(feconf,
            'CAN_SEND_TRANSACTIONAL_EMAILS', True)
        self.can_not_send_editor_role_email_ctx = self.swap(feconf,
            'CAN_SEND_TRANSACTIONAL_EMAILS', False)
    ...
    def test_role_email_is_sent_when_editor_assigns_role(self) ->None:
        with self.can_send_editor_role_email_ctx:
            ...
            self.put_json('%s/%s' % (feconf.EXPLORATION_RIGHTS_PREFIX, self
                .exploration.id), {'version': self.exploration.version,
                'new_member_username': self.NEW_USER_USERNAME,
                'new_member_role': rights_domain.ROLE_EDITOR}, csrf_token=
                csrf_token)
    ...
    def test_email_is_not_sent_if_recipient_has_declined_such_emails(self
        ...
        user_services.update_email_preferences(self.new_user_id, True, 
            False, False, False)
        with self.can_send_editor_role_email_ctx:
            email_manager.send_role_notification_email(self.editor_id, self
                .new_user_id, rights_domain.ROLE_OWNER, self.exploration.id,
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True)])
    def test_that_email_not_sent_if_can_send_transactional_emails_is_false(self
        ) ->None:
        with self.can_not_send_editor_role_email_ctx:
            email_manager.send_role_notification_email(self.editor_id, self
                .new_user_id, rights_domain.ROLE_EDITOR, self.exploration.
                id, self.exploration.title)
            messages = self._get_sent_email_messages(self.NEW_USER_EMAIL)
            self.assertEqual(len(messages), 0)
    ...
    def test_role_emails_sent_are_correct(self) ->None:
        with self.can_send_editor_role_email_ctx:
            email_manager.send_role_notification_email(self.editor_id, self
                .new_user_id, rights_domain.ROLE_VIEWER, self.exploration.
                id, self.exploration.title)
            messages = self._get_sent_email_messages(self.NEW_USER_EMAIL)
            self.assertEqual(len(messages), 1)
            ...
            self.assertEqual(sent_email_model.recipient_email, self.
                NEW_USER_EMAIL)
            self.assertEqual(sent_email_model.sender_id, feconf.
                SYSTEM_COMMITTER_ID)
            self.assertEqual(sent_email_model.sender_email, '%s <%s>' % (
                self.EDITOR_USERNAME, feconf.NOREPLY_EMAIL_ADDRESS))
            self.assertEqual(sent_email_model.intent, feconf.
                EMAIL_INTENT_EDITOR_ROLE_NOTIFICATION)
            self.assertEqual(sent_email_model.subject, self.
    ...
    def test_correct_undefined_role_raises_an_exception(self) ->None:
        with self.can_send_editor_role_email_ctx:
            with self.assertRaisesRegex(Exception, 'Invalid role'):
                email_manager.send_role_notification_email(self.editor_id,
                    self.new_user_id, rights_domain.ROLE_NONE, self.
                    exploration.id, self.exploration.title)
...
class SignupEmailTests(test_utils.EmailTestBase):
    ...
    EMAIL_BODY: Final = (
        'Here is some HTML text.<br>With a <b>bold</b> bit and an <i>italic</i> bit.<br>'
        )
    NEW_FOOTER: Final = (
        'Unsubscribe from emails at your <a href="https://www.site.com/prefs">Preferences page</a>.'
        )
...
Unsubscribe from emails at your Preferences page."""
        self.expected_html_email_content = (
            'Hi editor,<br><br>Here is some HTML text.<br>With a <b>bold</b> bit and an <i>italic</i> bit.<br><br><br>Unsubscribe from emails at your <a href="https://www.site.com/prefs">Preferences page</a>.'
            )

    def _set_signup_email_content_platform_parameter(self,
        new_email_subject_content: str, new_email_body_content: str) ->None:
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            ...
            value, self.admin_id, 'Updating email subject.', [
            platform_parameter_domain.PlatformParameterRule.from_dict({
            'filters': [{'type': 'platform_type', 'conditions': [['=',
            'Web']]}], 'value_when_matched': new_email_subject_content})],
            platform_parameter_registry.Registry.get_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_SUBJECT_CONTENT.
            value).default_value)
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT.
            value, self.admin_id, 'Updating email body.', [
            platform_parameter_domain.PlatformParameterRule.from_dict({
            'filters': [{'type': 'platform_type', 'conditions': [['=',
            'Web']]}], 'value_when_matched': new_email_body_content})],
            platform_parameter_registry.Registry.get_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT.
            value).default_value)
    ...
    def _reset_signup_email_content_platform_parameters(self) ->None:
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_SUBJECT_CONTENT.
            value, self.admin_id, 'Resetting email subject.', [],
            platform_parameter_registry.Registry.get_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_SUBJECT_CONTENT.
            value).default_value)
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT.
            value, self.admin_id, 'Resetting email body.', [],
            platform_parameter_registry.Registry.get_platform_parameter(
            platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT.
            value).default_value)
    ...
    def _reset_the_email_platform_params_value(self) ->None:
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_SENDER_NAME.value, self
            .admin_id, 'Reset the sender name to default', [],
            platform_parameter_registry.Registry.get_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_SENDER_NAME.value).
            default_value)
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_FOOTER.value, self.
            admin_id, 'Reset the email footer to default', [],
            platform_parameter_registry.Registry.get_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_FOOTER.value).default_value
            )

    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_config_does_not_permit_it(self) ->None:
        self._set_signup_email_content_platform_parameter(self.
            new_email_subject_content, self.new_email_body_content)
        self.login(self.EDITOR_EMAIL)
        self.get_html_response('%s?return_url=/' % feconf.SIGNUP_URL)
        csrf_token = self.get_new_csrf_token()
        self.post_json(feconf.SIGNUP_DATA_URL, {'agreed_to_terms': True,
            'username': self.EDITOR_USERNAME, 'default_dashboard':
            constants.DASHBOARD_TYPE_LEARNER, 'can_receive_email_updates':
            feconf.DEFAULT_EMAIL_UPDATES_PREFERENCE}, csrf_token=csrf_token)
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Name'), (platform_parameter_list.ParamName.
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        SIGNUP_EMAIL_SUBJECT_CONTENT, PLACEHOLDER_SUBJECT), (
        platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT, '')])
    def test_email_not_sent_if_content_parameter_is_not_modified(self) ->None:
        ...
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with log_new_error_ctx:
                ...
                self.get_html_response('%s?return_url=/' % feconf.SIGNUP_URL)
                csrf_token = self.get_new_csrf_token()
                self.post_json(feconf.SIGNUP_DATA_URL, {'agreed_to_terms': 
                    True, 'username': self.EDITOR_USERNAME,
                    'default_dashboard': constants.DASHBOARD_TYPE_LEARNER,
                    'can_receive_email_updates': feconf.
                    DEFAULT_EMAIL_UPDATES_PREFERENCE}, csrf_token=csrf_token)
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Name'), (platform_parameter_list.ParamName.
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        SIGNUP_EMAIL_SUBJECT_CONTENT, 'Signup subject'), (
        platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT,
    ...
    def test_email_not_sent_if_content_config_is_partially_modified(self
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            ...
            platform_parameter_domain.PlatformParameterRule.from_dict({
            'filters': [{'type': 'platform_type', 'conditions': [['=',
            'Web']]}], 'value_when_matched': self.new_email_subject_content
            })], platform_parameter_registry.Registry.
            get_platform_parameter(platform_parameter_list.ParamName.
            SIGNUP_EMAIL_SUBJECT_CONTENT.value).default_value)
        log_new_error_counter = test_utils.CallCounter(logging.error)
        log_new_error_ctx = self.swap(logging, 'error', log_new_error_counter)
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with log_new_error_ctx:
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Email Sender'), (platform_parameter_list.
        ParamName.EMAIL_FOOTER, NEW_FOOTER), (platform_parameter_list.
        ParamName.SIGNUP_EMAIL_SUBJECT_CONTENT, 'Welcome!'), (
        platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT,
        EMAIL_BODY)])
    def test_contents_of_signup_email_are_correct(self) ->None:
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_SENDER_NAME.value, self
            .admin_id, 'Update sender name', [platform_parameter_domain.
            PlatformParameterRule.from_dict({'filters': [{'type':
            'platform_type', 'conditions': [['=', 'Web']]}],
        ...
        platform_parameter_registry.Registry.update_platform_parameter(
            platform_parameter_list.ParamName.EMAIL_FOOTER.value, self.
            admin_id, 'Update email footer', [platform_parameter_domain.
            PlatformParameterRule.from_dict({'filters': [{'type':
            'platform_type', 'conditions': [['=', 'Web']]}],
        ...
        self.assertEqual(messages[0].sender, 'Email Sender <%s>' % feconf.
    ...
    def test_email_only_sent_if_signup_was_successful(self) ->None:
        ...
        csrf_token = self.get_new_csrf_token()
        self.post_json(feconf.SIGNUP_DATA_URL, {'agreed_to_terms': True,
            'username': 'BadUsername!!!', 'default_dashboard': constants.
            DASHBOARD_TYPE_LEARNER, 'can_receive_email_updates': feconf.
            DEFAULT_EMAIL_UPDATES_PREFERENCE}, csrf_token=csrf_token,
    ...
    def test_record_of_sent_email_is_written_to_datastore(self) ->None:
        ...
        self.assertEqual(len(all_models), 1)
        sent_email_model = all_models[0]
        self.assertEqual(sent_email_model.recipient_id, self.
            get_user_id_from_email(self.EDITOR_EMAIL))
        ...
        self.assertEqual(sent_email_model.sender_email, 'Email Sender <%s>' %
        ...
        self.assertEqual(sent_email_model.html_body, self.
            expected_html_email_content)
        self._reset_the_email_platform_params_value()
        self._reset_signup_email_content_platform_parameters()
...
class DuplicateEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.new_email_html_body = 'Hi %s,<br><br>%s<br><br>%s' % (self.
            NEW_USER_USERNAME,
            'THIS IS A <b>PLACEHOLDER</b> AND SHOULD BE REPLACED.', self.
            new_footer)

        def _generate_hash_for_tests(unused_cls: Type[test_utils.TestBase],
            unused_recipient_id: str, unused_email_subject: str,
            unused_email_body: str) ->str:
            """Returns the generated hash for tests."""
            return 'Email Hash'
        self.generate_hash_ctx = self.swap(email_models.SentEmailModel,
            '_generate_hash', types.MethodType(_generate_hash_for_tests,
            email_models.SentEmailModel))

    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Name'), (platform_parameter_list.ParamName.
        EMAIL_FOOTER, NEW_FOOTER), (platform_parameter_list.ParamName.
        SIGNUP_EMAIL_SUBJECT_CONTENT, NEW_EMAIL_SUBJECT), (
        platform_parameter_list.ParamName.SIGNUP_EMAIL_BODY_CONTENT,
        PLACEHOLDER_HTML_BODY)])
    def test_send_email_does_not_resend_if_same_hash_exists(self) ->None:
        duplicate_email_ctx = self.swap(feconf,
            'DUPLICATE_EMAIL_INTERVAL_MINS', 1000)
        log_new_error_counter = test_utils.CallCounter(logging.error)
        log_new_error_ctx = self.swap(logging, 'error', log_new_error_counter)
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with duplicate_email_ctx, log_new_error_ctx:
                ...
                cleaned_plaintext_body = html_cleaner.strip_html_tags(
                    raw_plaintext_body)
                email_models.SentEmailModel.create(self.new_user_id, self.
                    NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID, feconf.
                    SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP, self.
                    new_email_subject, cleaned_plaintext_body, datetime.
                    datetime.utcnow())
                all_models = email_models.SentEmailModel.get_all().fetch()
                self.assertEqual(len(all_models), 1)
                email_manager.send_post_signup_email(self.new_user_id,
                    test_for_duplicate_email=True)
                self.assertEqual(log_new_error_counter.times_called, 1)
    ...
    def test_send_email_does_not_resend_within_duplicate_interval(self) ->None:
        ...
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with duplicate_email_ctx, log_new_error_ctx:
                ...
                self.assertEqual(len(all_models), 0)
                email_manager._send_email(self.new_user_id, feconf.
                    SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_SIGNUP,
                    'Email Subject', 'Email Body', feconf.SYSTEM_EMAIL_ADDRESS)
                messages = self._get_sent_email_messages(self.NEW_USER_EMAIL)
                self.assertEqual(1, len(messages))
                all_models = email_models.SentEmailModel.get_all().fetch()
                self.assertEqual(len(all_models), 1)
    ...
    def test_sending_email_with_different_recipient_but_same_hash(self) ->None:
        ...
        with duplicate_email_ctx, self.generate_hash_ctx:
            ...
            self.assertEqual(len(all_models), 0)
            email_models.SentEmailModel.create('recipient_id', self.
                NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID, feconf.
                SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP, self.
                new_email_subject, self.new_email_html_body, datetime.
                datetime.utcnow())
            all_models = email_models.SentEmailModel.get_all().fetch()
            self.assertEqual(len(all_models), 1)
            email_manager.send_post_signup_email(self.new_user_id,
            ...
            self.assertNotEqual(sent_email_model1.recipient_id,
                sent_email_model2.recipient_id)
            self.assertEqual(sent_email_model1.subject, sent_email_model2.
                subject)
            self.assertEqual(sent_email_model1.html_body, sent_email_model2
                .html_body)
    ...
    def test_sending_email_with_different_subject_but_same_hash(self) ->None:
        ...
        with duplicate_email_ctx, self.generate_hash_ctx:
            ...
            email_models.SentEmailModel.create(self.new_user_id, self.
                NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID, feconf.
                SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP, '%s%s' %
            ...
            self.assertEqual(sent_email_model1.email_hash,
                sent_email_model2.email_hash)
            self.assertEqual(sent_email_model1.recipient_id,
                sent_email_model2.recipient_id)
            self.assertNotEqual(sent_email_model1.subject,
                sent_email_model2.subject)
            self.assertEqual(sent_email_model1.html_body, sent_email_model2
                .html_body)
    ...
    def test_sending_email_with_different_body_but_same_hash(self) ->None:
        ...
        with duplicate_email_ctx, self.generate_hash_ctx:
            ...
            email_models.SentEmailModel.create(self.new_user_id, self.
                NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID, feconf.
                SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP, self.
                new_email_subject, '%s%s' % (self.new_email_html_body, 1),
                datetime.datetime.utcnow())
            all_models = email_models.SentEmailModel.get_all().fetch()
            self.assertEqual(len(all_models), 1)
            email_manager.send_post_signup_email(self.new_user_id,
            ...
            self.assertEqual(sent_email_model1.subject, sent_email_model2.
                subject)
            self.assertNotEqual(sent_email_model1.html_body,
                sent_email_model2.html_body)
    ...
    def test_duplicate_emails_are_sent_after_some_time_has_elapsed(self
        ...
        with duplicate_email_ctx:
            ...
            self.assertEqual(len(all_models), 0)
            email_sent_time = get_current_datetime_utc() - datetime.timedelta(
                minutes=4)
            email_models.SentEmailModel.create(self.new_user_id, self.
                NEW_USER_EMAIL, feconf.SYSTEM_COMMITTER_ID, feconf.
                SYSTEM_EMAIL_ADDRESS, feconf.EMAIL_INTENT_SIGNUP, self.
                new_email_subject, self.new_email_html_body, email_sent_time)
            all_models = email_models.SentEmailModel.get_all().fetch()
            self.assertEqual(len(all_models), 1)
            email_sent_time = get_current_datetime_utc() - datetime.timedelta(
                minutes=2)
...
class FeedbackMessageBatchEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.editor_id = self.get_user_id_from_email(self.EDITOR_EMAIL)
        self.exploration = self.save_new_default_exploration('A', self.
            editor_id, title='Title')
        self.expected_email_subject = (
            "You've received 3 new messages on your explorations")
        self.can_send_feedback_email_ctx = self.swap(feconf,
            'CAN_SEND_TRANSACTIONAL_EMAILS', True)
        self.can_not_send_feedback_email_ctx = self.swap(feconf,
            'CAN_SEND_TRANSACTIONAL_EMAILS', False)
    ...
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        feedback_messages: Dict[str, email_manager.FeedbackMessagesDict] = {
            self.exploration.id: {'title': self.exploration.title,
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True)])
    def test_email_not_sent_if_can_send_transactional_emails_is_false(self
        ) ->None:
        feedback_messages: Dict[str, email_manager.FeedbackMessagesDict] = {
            self.exploration.id: {'title': self.exploration.title,
            'messages': ['Message 1.1', 'Message 1.2', 'Message 1.3']}}
...
You can change your email preferences via the Preferences page."""
        ...
        with self.can_send_feedback_email_ctx:
            ...
            self.assertEqual(sent_email_model.sender_id, feconf.
                SYSTEM_COMMITTER_ID)
            self.assertEqual(sent_email_model.sender_email, 
                'Site Admin <%s>' % feconf.NOREPLY_EMAIL_ADDRESS)
            self.assertEqual(sent_email_model.intent, feconf.
                EMAIL_INTENT_FEEDBACK_MESSAGE_NOTIFICATION)
            self.assertEqual(sent_email_model.subject, self.
                expected_email_subject)
...
class SuggestionEmailTests(test_utils.EmailTestBase):
    ...
    def test_email_not_sent_if_can_send_emails_is_false(self) ->None:
        email_manager.send_suggestion_email(self.exploration.title, self.
            exploration.id, self.new_user_id, self.recipient_list)
        messages = self._get_sent_email_messages(self.EDITOR_EMAIL)
        self.assertEqual(len(messages), 0)
    ...
    def test_email_not_sent_if_can_send_transactional_emails_is_false(self
        ...
        with self.can_not_send_feedback_email_ctx:
            email_manager.send_suggestion_email(self.exploration.title,
                self.exploration.id, self.new_user_id, self.recipient_list)
        messages = self._get_sent_email_messages(self.EDITOR_EMAIL)
        self.assertEqual(len(messages), 0)
...
class SubscriptionEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.exploration = self.save_new_default_exploration('A', self.
            editor_id, title='Title')
        subscription_services.subscribe_to_creator(self.new_user_id, self.
            editor_id)
        self.can_send_subscription_email_ctx = self.swap(feconf,
            'CAN_SEND_TRANSACTIONAL_EMAILS', True)
        self.can_not_send_subscription_email_ctx = self.swap(feconf,
            'CAN_SEND_TRANSACTIONAL_EMAILS', False)

    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_emails_to_subscribers(self.editor_id, self.
            exploration.id, self.exploration.title)
        messages = self._get_sent_email_messages(self.NEW_USER_EMAIL)
        self.assertEqual(len(messages), 0)
    ...
    def test_that_subscription_emails_are_correct(self) ->None:
        ...
        expected_email_html_body = (
            'Hi newuser,<br><br>editor has published a new exploration! You can play it here: <a href="https://www.oppia.org/explore/A">Title</a><br><br>Thanks, and happy learning!<br><br>Best wishes,<br>- The Oppia Team<br><br>You can change your email preferences via the <a href="http://localhost:8181/preferences">Preferences</a> page.'
            )
        expected_email_text_body = """Hi newuser,
...
You can change your email preferences via the Preferences page."""
        with self.can_send_subscription_email_ctx:
            ...
            self.assertEqual(sent_email_model.sender_email, 
                'Site Admin <%s>' % feconf.NOREPLY_EMAIL_ADDRESS)
            self.assertEqual(sent_email_model.intent, feconf.
                EMAIL_INTENT_SUBSCRIPTION_NOTIFICATION)
...
class FeedbackMessageInstantEmailTests(test_utils.EmailTestBase):
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_instant_feedback_message_email(self.new_user_id,
            self.editor_id, 'editor message',
    ...
    def test_email_not_sent_if_can_send_transactional_emails_is_false(self
        ...
        with self.can_not_send_feedback_email_ctx:
            email_manager.send_instant_feedback_message_email(self.
                new_user_id, self.editor_id, 'editor message',
                'New Oppia message in "a subject"', self.exploration.title,
                self.exploration.id, 'a subject')
        messages = self._get_sent_email_messages(self.NEW_USER_EMAIL)
        self.assertEqual(len(messages), 0)
...
class FlagExplorationEmailTest(test_utils.EmailTestBase):
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_flag_exploration_email(self.exploration.title,
            self.exploration.id, self.new_user_id, self.report_text)
...
You can change your email preferences via the Preferences page."""
        ...
        sent_email_model = next(m for m in all_models if m.recipient_id ==
            self.moderator_id)
        self.assertEqual(sent_email_model.subject, expected_email_subject)
        ...
        self.assertEqual(sent_email_model.sender_email, 'Site Admin <%s>' %
            feconf.NOREPLY_EMAIL_ADDRESS)
        self.assertEqual(sent_email_model.intent, feconf.
            EMAIL_INTENT_REPORT_BAD_CONTENT)
        sent_email_model = next(m for m in all_models if m.recipient_id ==
            self.moderator2_id)
        self.assertEqual(sent_email_model.subject, expected_email_subject)
        self.assertEqual(sent_email_model.recipient_email, self.
...
class OnboardingReviewerInstantEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        user_services.update_email_preferences(self.reviewer_id, True, 
            False, False, False)

    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_mail_to_onboard_new_reviewers(self.reviewer_id,
            'Algebra')
...
class NotifyReviewerInstantEmailTests(test_utils.EmailTestBase):
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_mail_to_notify_users_to_review(self.reviewer_id,
            'Algebra')
        messages = self._get_sent_email_messages(self.REVIEWER_EMAIL)
        self.assertEqual(len(messages), 0)
...
class NotifyContributionAchievementEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        user_services.update_email_preferences(self.user_id, True, False, 
            False, False)
    ...
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'question', 'edit',
            None, 'Initial Contributor'))
    ...
    def test_that_email_not_sent_if_user_can_not_receive_emails(self) ->None:
        user_services.update_email_preferences(self.user_id, False, False, 
            False, False)
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'question', 'edit',
            None, 'Initial Contributor'))
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin')])
    def test_that_question_submitter_acceptance_ranking_email_is_sent(self
        ) ->None:
        expected_email_subject = 'Oppia Question Submitter Rank Achievement!'
        expected_email_html_body = (
            'Hi user,<br><br>This is to let you know that you have successfully achieved the Initial Contributor rank for submitting practice questions. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="http://localhost:8181/contributor-dashboard">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team'
            )
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'question',
            'acceptance', None, 'Initial Contributor'))
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info)
        messages = self._get_sent_email_messages(self.USER_EMAIL)
        self.assertEqual(len(messages), 1)
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin')])
    def test_that_translation_reviewer_review_ranking_email_is_sent(self
        ) ->None:
        expected_email_subject = 'Oppia Translation Reviewer Rank Achievement!'
        expected_email_html_body = (
            'Hi user,<br><br>This is to let you know that you have successfully achieved the Initial Contributor rank for reviewing translations in हिन्दी (Hindi). Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="http://localhost:8181/contributor-dashboard">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team'
            )
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'translation',
            'review', 'hi', 'Initial Contributor'))
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info)
        messages = self._get_sent_email_messages(self.USER_EMAIL)
        self.assertEqual(len(messages), 1)
    ...
    def test_that_question_reviewer_review_ranking_email_is_sent(self) ->None:
        ...
        expected_email_html_body = (
            'Hi user,<br><br>This is to let you know that you have successfully achieved the Initial Contributor rank for reviewing  practice questions. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="http://localhost:8181/contributor-dashboard">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team'
            )
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'question',
            'review', None, 'Initial Contributor'))
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info)
        messages = self._get_sent_email_messages(self.USER_EMAIL)
        self.assertEqual(len(messages), 1)
    ...
    def test_that_translation_reviewer_edit_ranking_email_is_sent(self) ->None:
        expected_email_subject = 'Oppia Translation Reviewer Rank Achievement!'
        expected_email_html_body = (
            'Hi user,<br><br>This is to let you know that you have successfully achieved the Initial Contributor rank for correcting translations in हिन्दी (Hindi). Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="http://localhost:8181/contributor-dashboard">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team'
            )
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'translation',
            'edit', 'hi', 'Initial Contributor'))
        email_manager.send_mail_to_notify_contributor_ranking_achievement(
            contributor_ranking_email_info)
        messages = self._get_sent_email_messages(self.USER_EMAIL)
        self.assertEqual(len(messages), 1)
    ...
    def test_that_question_reviewer_edit_ranking_email_is_sent(self) ->None:
        expected_email_subject = 'Oppia Question Reviewer Rank Achievement!'
        expected_email_html_body = (
            'Hi user,<br><br>This is to let you know that you have successfully achieved the Initial Contributor rank for correcting practice questions. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="http://localhost:8181/contributor-dashboard">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team'
            )
        contributor_ranking_email_info = (suggestion_registry.
            ContributorMilestoneEmailInfo(self.user_id, 'question', 'edit',
            None, 'Initial Contributor'))
...
class NotifyContributionDashboardReviewersEmailTests(test_utils.EmailTestBase):
    ...
    def _create_translation_suggestion_in_lang_with_html_and_datetime(self,
        language_code: str, translation_html: str, submission_datetime:
        datetime.datetime) ->suggestion_registry.BaseSuggestion:
        ...
        add_translation_change_dict = {'cmd': exp_domain.
            CMD_ADD_WRITTEN_TRANSLATION, 'state_name': feconf.
            DEFAULT_INIT_STATE_NAME, 'content_id': 'content_0',
            'language_code': language_code, 'content_html': feconf.
            DEFAULT_STATE_CONTENT_STR, 'translation_html': translation_html,
            'data_format': 'html'}
        translation_suggestion = suggestion_services.create_suggestion(feconf
            .SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.
            ENTITY_TYPE_EXPLORATION, self.target_id, feconf.
            CURRENT_STATE_SCHEMA_VERSION, self.author_id,
    ...
    def _create_question_suggestion_with_question_html_and_datetime(self,
        question_html: str, submission_datetime: datetime.datetime
        ...
        with self.swap(feconf, 'DEFAULT_STATE_CONTENT_STR', question_html):
            ...
            add_question_change_dict: Dict[str, Union[str, float,
                question_domain.QuestionDict]] = {'cmd': question_domain.
                CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
                'id': 'test_id', 'version': 12, 'question_state_data': self
                ._create_valid_question_data('default_state',
                content_id_generator).to_dict(), 'language_code': constants
                .DEFAULT_LANGUAGE_CODE,
                'question_state_data_schema_version': feconf.
                CURRENT_STATE_SCHEMA_VERSION, 'linked_skill_ids': [
                'skill_1'], 'inapplicable_skill_misconception_ids': [
                'skillid12345-1'], 'next_content_id_index':
                content_id_generator.next_content_id_index}, 'skill_id':
                self.skill_id, 'skill_difficulty': 0.3}
        question_suggestion = suggestion_services.create_suggestion(feconf.
            SUGGESTION_TYPE_ADD_QUESTION, feconf.ENTITY_TYPE_SKILL, self.
            skill_id, feconf.CURRENT_STATE_SCHEMA_VERSION, self.author_id,
            add_question_change_dict, 'test description')
        question_suggestion.last_updated = submission_datetime
        return question_suggestion

    def _create_reviewable_suggestion_email_infos_from_suggestions(self,
        suggestions: List[suggestion_registry.BaseSuggestion]) ->List[
        suggestion_registry.ReviewableSuggestionEmailInfo]:
        """Creates a list of ReviewableSuggestionEmailInfo objects from
        the given suggestions.
        """
        return [suggestion_services.
            create_reviewable_suggestion_email_info_from_suggestion(
            suggestion) for suggestion in suggestions]

    def _assert_email_data_stored_in_sent_email_model_is_correct(self,
        expected_email_html_body: str, reviewer_id: Optional[str],
        ...
        sent_email_models: Sequence[email_models.SentEmailModel
            ] = email_models.SentEmailModel.get_all().filter(email_models.
            SentEmailModel.recipient_id == reviewer_id).fetch()
        self.assertEqual(len(sent_email_models), 1)
        sent_email_model = sent_email_models[0]
        self.assertEqual(sent_email_model.subject, email_manager.
    ...
    def setUp(self) ->None:
        ...
        user_services.update_email_preferences(self.reviewer_1_id, True, 
            False, False, False)
        ...
        user_services.update_email_preferences(self.reviewer_2_id, True, 
            False, False, False)
        self.log_new_error_counter = test_utils.CallCounter(logging.error)
        self.log_new_error_ctx = self.swap(logging, 'error', self.
            log_new_error_counter)
        self.logged_info: List[str] = []
        self.log_new_info_ctx = self.swap(logging, 'info', self.
            _mock_logging_info)
        self.save_new_valid_exploration(self.target_id, self.author_id)
        self.save_new_skill(self.skill_id, self.author_id)
        question_suggestion = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>What is the meaning of life?</p>', self.
            mocked_review_submission_datetime))
        self.reviewable_suggestion_email_info = (suggestion_services.
            create_reviewable_suggestion_email_info_from_suggestion(
            question_suggestion))
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True), (platform_parameter_list.ParamName.
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, False)])
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, True)])
    def test_email_not_sent_if_reviewer_email_does_not_exist(self) ->None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with self.log_new_error_ctx:
                email_manager.send_mail_to_notify_contributor_dashboard_reviewers(
                    ['reviewer_id_with_no_email'], [[self.
                    reviewable_suggestion_email_info]])
            messages = self._get_all_sent_email_messages()
            self.assertEqual(len(messages), 0)
            self.assertEqual(self.log_new_error_counter.times_called, 1)
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, True)])
    def test_email_not_sent_if_no_suggestions_to_notify_the_reviewer_about(self
        ) ->None:
        ...
        messages = self._get_all_sent_email_messages()
        self.assertEqual(len(messages), 0)
        self.assertEqual(self.logged_info[0], 
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, True)])
    def test_email_sent_to_reviewer_with_question_waiting_a_day_for_review(self
        ) ->None:
        question_suggestion = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>What is the meaning of life?</p>', self.
            mocked_review_submission_datetime))
        reviewable_suggestion_email_info = (suggestion_services.
            create_reviewable_suggestion_email_info_from_suggestion(
            question_suggestion))
        review_wait_time = 1
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(days=review_wait_time))
        expected_email_html_body = (
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, True)])
    def test_email_sent_to_reviewer_with_question_waiting_days_for_review(self
        ) ->None:
        question_suggestion = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>What is the meaning of life?</p>', self.
    ...
    def test_email_sent_to_reviewer_with_question_waiting_an_hour_for_review(
        ...
        review_wait_time = 1
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(hours=review_wait_time))
        expected_email_html_body = (
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, True)])
    def test_email_sent_to_reviewer_with_question_waiting_hours_for_review(self
        ) ->None:
        question_suggestion = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>What is the meaning of life?</p>', self.
    ...
    def test_email_sent_to_reviewer_with_question_waiting_a_minute_for_review(
        ...
        review_wait_time = 1
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(minutes=review_wait_time))
        expected_email_html_body = (
    ...
    def test_email_sent_to_reviewer_with_question_waiting_seconds_for_review(
        ...
        review_wait_time = 5
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(seconds=review_wait_time))
        expected_email_html_body = (
            'Hi reviewer1,<br><br>There are new review opportunities that we think you might be interested in on the <a href="%s%s">Contributor Dashboard</a>. Here are some examples of contributions that have been waiting the longest for review:<br><br><ul><li>The following question suggestion was submitted for review 1 minute ago:<br>What is the meaning of life?</li><br></ul><br>Please take some time to review any of the above contributions (if they still need a review) or any other contributions on the dashboard. We appreciate your help!<br><br>Thanks again, and happy reviewing!<br>- The Oppia Contributor Dashboard Team<br><br>%s'
             % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL,
            platform_parameter_registry.Registry.get_platform_parameter(
    ...
    def test_email_sent_to_reviewer_with_multi_questions_waiting_for_a_review(
        self) ->None:
        mocked_datetime_for_utcnow = (self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1))
        question_suggestion_1 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 1</p>', self.mocked_review_submission_datetime +
            datetime.timedelta(hours=1)))
        question_suggestion_2 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 2</p>', self.mocked_review_submission_datetime +
            datetime.timedelta(days=1)))
        reviewable_suggestion_email_infos = (self.
            _create_reviewable_suggestion_email_infos_from_suggestions([
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, True)])
    def test_email_sent_to_multi_reviewers_with_multi_question_suggestions(self
        ) ->None:
        mocked_datetime_for_utcnow = (self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1, minutes=1))
        question_suggestion_1 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 1 for reviewer 1</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(hours=1,
            minutes=1)))
        question_suggestion_2 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 2 for reviewer 1</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            minutes=1)))
        question_suggestion_3 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 1 for reviewer 2</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1)))
        question_suggestion_4 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 2 for reviewer 2</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1)))
        reviewer_1_suggestion_email_infos = (self.
            _create_reviewable_suggestion_email_infos_from_suggestions([
            question_suggestion_1, question_suggestion_2]))
        reviewer_2_suggestion_email_infos = (self.
            _create_reviewable_suggestion_email_infos_from_suggestions([
        ...
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body_reviewer_1, self.reviewer_1_id, self.
            REVIEWER_1_EMAIL)
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body_reviewer_2, self.reviewer_2_id, self.
            REVIEWER_2_EMAIL)
    ...
    def test_email_sent_to_reviewer_with_translation_waiting_a_day_for_review(
        self) ->None:
        translation_suggestion = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'hi', '<p>Sample translation</p>', self.
            mocked_review_submission_datetime))
        reviewable_suggestion_email_info = (suggestion_services.
            create_reviewable_suggestion_email_info_from_suggestion(
            translation_suggestion))
        review_wait_time = 1
        reviewable_suggestion_email_info.submission_datetime = (self.
            mocked_review_submission_datetime)
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(days=review_wait_time))
        expected_email_html_body = (
    ...
    def test_email_sent_to_reviewer_with_multi_translation_waiting_for_review(
        ...
        mocked_datetime_for_utcnow = (self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1))
        translation_suggestion_1 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'en', '<p>Translation 1</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(hours=1)))
        translation_suggestion_2 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'fr', '<p>Translation 2</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1)))
        reviewable_suggestion_email_infos = (self.
            _create_reviewable_suggestion_email_infos_from_suggestions([
    ...
    def test_email_sent_to_multi_reviewers_with_multi_translations_suggestions(
        ...
        translation_suggestion_1 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'en', '<p>Translation 1 for reviewer 1</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(hours=1,
            minutes=1)))
        translation_suggestion_2 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'fr', '<p>Translation 2 for reviewer 1</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            minutes=1)))
        translation_suggestion_3 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'hi', '<p>Translation 1 for reviewer 2</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1)))
        translation_suggestion_4 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'fr', '<p>Translation 2 for reviewer 2</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(days=1,
            hours=1)))
        reviewer_1_suggestion_email_infos = (self.
            _create_reviewable_suggestion_email_infos_from_suggestions([
...
class NotifyAdminsSuggestionsWaitingTooLongForReviewEmailTests(test_utils.
    EmailTestBase):
    ...
    def _create_translation_suggestion_in_lang_with_html_and_datetime(self,
        ...
        with self.mock_datetime_utcnow(submission_datetime):
            translation_suggestion = suggestion_services.create_suggestion(
                feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.
                ENTITY_TYPE_EXPLORATION, self.target_id, feconf.
                CURRENT_STATE_SCHEMA_VERSION, self.author_id,
                add_translation_change_dict, 'test description')
        return translation_suggestion
    ...
    def _create_question_suggestion_with_question_html_and_datetime(self,
        ...
        with self.mock_datetime_utcnow(submission_datetime):
            question_suggestion = suggestion_services.create_suggestion(feconf
                .SUGGESTION_TYPE_ADD_QUESTION, feconf.ENTITY_TYPE_SKILL,
                self.skill_id, feconf.CURRENT_STATE_SCHEMA_VERSION, self.
                author_id, add_question_change_dict, 'test description')
        return question_suggestion
    ...
    def _assert_email_data_stored_in_sent_email_model_is_correct(self,
        expected_email_html_body: str, admin_id: Optional[str], admin_email:
        str) ->None:
        """Asserts that the created sent email model from the sent email
        contains the right information.
        """
        sent_email_models: Sequence[email_models.SentEmailModel
            ...
            SentEmailModel.recipient_id == admin_id).fetch()
        self.assertEqual(len(sent_email_models), 1)
        sent_email_model = sent_email_models[0]
        self.assertEqual(sent_email_model.subject, email_manager.
    ...
    def setUp(self) ->None:
        ...
        self.signup(self.AUTHOR_EMAIL, self.AUTHOR_USERNAME)
        self.author_id = self.get_user_id_from_email(self.AUTHOR_EMAIL)
        self.signup(self.CURRICULUM_ADMIN_1_EMAIL, self.
            CURRICULUM_ADMIN_1_USERNAME)
        self.admin_1_id = self.get_user_id_from_email(self.
            CURRICULUM_ADMIN_1_EMAIL)
        self.signup(self.CURRICULUM_ADMIN_2_EMAIL, self.
            CURRICULUM_ADMIN_2_USERNAME)
        self.admin_2_id = self.get_user_id_from_email(self.
            CURRICULUM_ADMIN_2_EMAIL)
        self.log_new_error_counter = test_utils.CallCounter(logging.error)
        self.log_new_error_ctx = self.swap(logging, 'error', self.
            log_new_error_counter)
        self.logged_info: List[str] = []
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        True)])
    def test_email_not_sent_if_no_suggestions_to_notify_the_admin_about(self
        ) ->None:
        with self.log_new_info_ctx:
            with self.swap(suggestion_models,
                'SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS', 0):
    ...
    def test_email_sent_to_admin_if_question_has_waited_too_long_for_a_review(
        ...
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(days=review_wait_time))
        expected_email_html_body = (
            'Hi user1,<br><br>There are suggestions on the <a href="%s%s">Contributor Dashboard</a> that have been waiting for more than 0 days for review. Please take a look at the suggestions mentioned below and help them get reviewed by going to the <a href="%s%s">admin roles page</a> and either:<br><br><ul><li>Add more reviewers to the suggestion types that have suggestions waiting too long for a review</li><br><li>Find the existing reviewers and email reviewers directly about the suggestions waiting for a review</li><br></ul><br>Here are the suggestions that have been waiting too long for a review:<br><br><ul><li>The following question suggestion was submitted for review 5 days ago:<br>What is the meaning of life?</li><br></ul><br>Thanks so much - we appreciate your help!<br>Best Wishes!<br><br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL,
        ...
        self.assertEqual(messages[0].html, expected_email_html_body)
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body, self.admin_1_id, self.
            CURRICULUM_ADMIN_1_EMAIL)
    ...
    def test_email_sent_to_admin_if_multiple_questions_have_waited_for_review(
        self) ->None:
        mocked_datetime_for_utcnow = (self.
            mocked_review_submission_datetime + datetime.timedelta(days=2,
            hours=1))
        question_suggestion_1 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 1</p>', self.mocked_review_submission_datetime))
        ...
        reviewable_suggestion_email_infos = (self.
            ...
            question_suggestion_1, question_suggestion_2]))
        expected_email_html_body = (
            'Hi user1,<br><br>There are suggestions on the <a href="%s%s">Contributor Dashboard</a> that have been waiting for more than 0 days for review. Please take a look at the suggestions mentioned below and help them get reviewed by going to the <a href="%s%s">admin roles page</a> and either:<br><br><ul><li>Add more reviewers to the suggestion types that have suggestions waiting too long for a review</li><br><li>Find the existing reviewers and email reviewers directly about the suggestions waiting for a review</li><br></ul><br>Here are the suggestions that have been waiting too long for a review:<br><br><ul><li>The following question suggestion was submitted for review 2 days ago:<br>Question 1</li><br><li>The following question suggestion was submitted for review 2 days ago:<br>Question 2</li><br></ul><br>Thanks so much - we appreciate your help!<br>Best Wishes!<br><br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL,
            feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_ADMIN_URL))
        with self.log_new_error_ctx:
            with self.swap(suggestion_models,
    ...
    def test_email_sent_to_admin_if_translation_has_waited_too_long_for_review(
        ...
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(days=review_wait_time))
        expected_email_html_body = (
            'Hi user1,<br><br>There are suggestions on the <a href="%s%s">Contributor Dashboard</a> that have been waiting for more than 0 days for review. Please take a look at the suggestions mentioned below and help them get reviewed by going to the <a href="%s%s">admin roles page</a> and either:<br><br><ul><li>Add more reviewers to the suggestion types that have suggestions waiting too long for a review</li><br><li>Find the existing reviewers and email reviewers directly about the suggestions waiting for a review</li><br></ul><br>Here are the suggestions that have been waiting too long for a review:<br><br><ul><li>The following हिन्दी (Hindi) translation suggestion was submitted for review 5 days ago:<br>Sample translation</li><br></ul><br>Thanks so much - we appreciate your help!<br>Best Wishes!<br><br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL,
            feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_ADMIN_URL))
        with self.log_new_error_ctx:
            with self.swap(suggestion_models,
    ...
    def test_email_sent_to_admin_if_multi_translations_have_waited_for_review(
        ...
        reviewable_suggestion_email_infos = (self.
            ...
            translation_suggestion_1, translation_suggestion_2]))
        expected_email_html_body = (
            'Hi user1,<br><br>There are suggestions on the <a href="%s%s">Contributor Dashboard</a> that have been waiting for more than 0 days for review. Please take a look at the suggestions mentioned below and help them get reviewed by going to the <a href="%s%s">admin roles page</a> and either:<br><br><ul><li>Add more reviewers to the suggestion types that have suggestions waiting too long for a review</li><br><li>Find the existing reviewers and email reviewers directly about the suggestions waiting for a review</li><br></ul><br>Here are the suggestions that have been waiting too long for a review:<br><br><ul><li>The following English translation suggestion was submitted for review 2 days ago:<br>Translation 1</li><br><li>The following français (French) translation suggestion was submitted for review 2 days ago:<br>Translation 2</li><br></ul><br>Thanks so much - we appreciate your help!<br>Best Wishes!<br><br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL,
            feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_ADMIN_URL))
        with self.log_new_error_ctx:
            with self.swap(suggestion_models,
    ...
    def test_email_sent_to_admin_if_multi_suggestion_types_waiting_for_review(
        ...
        suggestion_2 = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'fr', '<p>Translation 2</p>', self.
            mocked_review_submission_datetime + datetime.timedelta(minutes=5)))
        suggestion_3 = (self.
            _create_question_suggestion_with_question_html_and_datetime(
            '<p>Question 1</p>', self.mocked_review_submission_datetime +
            datetime.timedelta(hours=1)))
        ...
        sent_email_models: Sequence[email_models.SentEmailModel
            ...
            SentEmailModel.recipient_id == self.admin_1_id).fetch()
    ...
    def test_email_sent_to_multiple_admins(self) ->None:
        ...
        expected_email_html_body_admin_2 = (
            'Hi user2,<br><br>There are suggestions on the <a href="%s%s">Contributor Dashboard</a> that have been waiting for more than 0 days for review. Please take a look at the suggestions mentioned below and help them get reviewed by going to the <a href="%s%s">admin roles page</a> and either:<br><br><ul><li>Add more reviewers to the suggestion types that have suggestions waiting too long for a review</li><br><li>Find the existing reviewers and email reviewers directly about the suggestions waiting for a review</li><br></ul><br>Here are the suggestions that have been waiting too long for a review:<br><br><ul><li>The following question suggestion was submitted for review 5 days ago:<br>What is the meaning of life?</li><br></ul><br>Thanks so much - we appreciate your help!<br>Best Wishes!<br><br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL,
            feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_ADMIN_URL))
        with self.log_new_error_ctx:
            with self.swap(suggestion_models,
        ...
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body_admin_1, self.admin_1_id, self.
            CURRICULUM_ADMIN_1_EMAIL)
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body_admin_2, self.admin_2_id, self.
            CURRICULUM_ADMIN_2_EMAIL)
...
class NotifyReviewersNewSuggestionsTests(test_utils.EmailTestBase):
    ...
    mocked_review_submission_datetime = (mocked_review_submission_datetime.
        replace(tzinfo=None))
    AUTHOR_USERNAME: Final = 'author'
    AUTHOR_EMAIL: Final = 'author@example.com'
    REVIEWER_1_USERNAME: Final = 'reviewer1'
    ...
    def setUp(self) ->None:
        ...
        translation_suggestion = (self.
            _create_translation_suggestion_in_lang_with_html_and_datetime(
            'en', '<p>What is the meaning of life?</p>', self.
            mocked_review_submission_datetime))
        self.reviewable_suggestion_email_info = (suggestion_services.
            create_reviewable_suggestion_email_info_from_suggestion(
            translation_suggestion))
    ...
    def test_email_sent_to_reviewer_with_translation_waiting_days_for_review(
        ...
        mocked_datetime_for_utcnow = (reviewable_suggestion_email_info.
            submission_datetime + datetime.timedelta(days=review_wait_time))
        expected_email_html_body = (
            'Hi reviewer1,<br><br>There are new <a href="%s%s">opportunities</a> to review translations that we think you might be interested in on the Contributor Dashboard page. Here are some examples of contributions that are waiting for review:<br><br>The following suggestions are available for review: <br><br><ul><li>The following English translation suggestion was submitted for review 2 days ago:<br>What is the meaning of life?</li><br></ul><br>Please take some time to review any of the above contributions (if they still need a review) or any other contributions on the dashboard. We appreciate your help!<br><br>Thanks again, and happy reviewing!<br><br>The Oppia Contributor Dashboard Team'
            )
        with self.log_new_error_ctx:
            with self.mock_datetime_utcnow(mocked_datetime_for_utcnow):
                reviewer_ids_by_language: DefaultDict[str, List[str]
...
class NotifyAdminsContributorDashboardReviewersNeededTests(test_utils.
    EmailTestBase):
    ...
    def _create_translation_suggestion_with_language_code(self,
        language_code: str) ->suggestion_registry.BaseSuggestion:
        """Creates a translation suggestion in the given language_code."""
        add_translation_change_dict = {'cmd': exp_domain.
            CMD_ADD_WRITTEN_TRANSLATION, 'state_name': feconf.
            DEFAULT_INIT_STATE_NAME, 'content_id': 'content_0',
            'language_code': language_code, 'content_html': feconf.
            DEFAULT_STATE_CONTENT_STR, 'translation_html':
            '<p>This is the translated content.</p>', 'data_format': 'html'}
        return suggestion_services.create_suggestion(feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT, feconf.
            ENTITY_TYPE_EXPLORATION, self.target_id, feconf.
            CURRENT_STATE_SCHEMA_VERSION, self.author_id,
            add_translation_change_dict, 'test description')
    ...
    def _create_question_suggestion(self) ->suggestion_registry.BaseSuggestion:
        ...
        add_question_change_dict: Dict[str, Union[str, float,
            question_domain.QuestionDict]] = {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {'id':
            'test_id', 'version': 12, 'question_state_data': self.
            _create_valid_question_data('default_state',
            content_id_generator).to_dict(), 'language_code': constants.
            DEFAULT_LANGUAGE_CODE, 'question_state_data_schema_version':
            feconf.CURRENT_STATE_SCHEMA_VERSION, 'linked_skill_ids': [
            'skill_1'], 'inapplicable_skill_misconception_ids': [
            'skillid12345-1'], 'next_content_id_index':
            content_id_generator.next_content_id_index}, 'skill_id': self.
            skill_id, 'skill_difficulty': 0.3}
        return suggestion_services.create_suggestion(feconf.
            SUGGESTION_TYPE_ADD_QUESTION, feconf.ENTITY_TYPE_SKILL, self.
            skill_id, feconf.CURRENT_STATE_SCHEMA_VERSION, self.author_id,
            add_question_change_dict, 'test description')
    ...
    def setUp(self) ->None:
        ...
        self.log_new_info_ctx = self.swap(logging, 'info', self.
            _mock_logging_info)
        self.suggestion_types_needing_reviewers: Dict[str, Set[str]] = {feconf
            .SUGGESTION_TYPE_ADD_QUESTION: set()}
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, False)])
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True)])
    def test_email_not_sent_if_no_admins_to_notify(self) ->None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            with self.log_new_error_ctx:
                email_manager.send_mail_to_notify_admins_that_reviewers_are_needed(
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True)])
    def test_email_not_sent_if_no_suggestion_types_that_need_reviewers(self
        ) ->None:
        with self.log_new_info_ctx:
            email_manager.send_mail_to_notify_admins_that_reviewers_are_needed(
                [self.admin_1_id], [], [], {})
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True)])
    def test_email_sent_to_admin_if_question_suggestions_need_reviewers(self
        ) ->None:
        self._create_question_suggestion()
        suggestion_types_needing_reviewers = (suggestion_services.
            get_suggestion_types_that_need_reviewers())
        self.assertDictEqual(suggestion_types_needing_reviewers, {feconf.
            SUGGESTION_TYPE_ADD_QUESTION: set()})
        expected_email_html_body = (
            'Hi user1,<br><br>In the <a href="%s%s#/roles">admin roles page,</a> please add reviewers to the Contributor Dashboard community by entering their username(s) and allow reviewing for the suggestion types that need more reviewers bolded below.<br><br>There have been <b>question suggestions</b> created on the <a href="%s%s">Contributor Dashboard page</a> where there are not enough reviewers.<br><br>Thanks so much - we appreciate your help!<br><br>Best Wishes!<br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.ADMIN_URL, feconf.
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True)])
    def test_email_sent_to_admins_if_question_suggestions_need_reviewers(self
        ) ->None:
        self._create_question_suggestion()
        suggestion_types_needing_reviewers = (suggestion_services.
            get_suggestion_types_that_need_reviewers())
        ...
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body_for_admin_1, self.admin_1_id, self.
            CURRICULUM_ADMIN_1_EMAIL)
        self._assert_email_data_stored_in_sent_email_model_is_correct(
            expected_email_html_body_for_admin_2, self.admin_2_id, self.
            CURRICULUM_ADMIN_2_EMAIL)
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True)])
    def test_admin_email_sent_if_translations_need_reviewers_for_one_lang(self
        ) ->None:
        self._create_translation_suggestion_with_language_code('hi')
        suggestion_types_needing_reviewers = (suggestion_services.
            get_suggestion_types_that_need_reviewers())
        self.assertDictEqual(suggestion_types_needing_reviewers, {feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT: {'hi'}})
        expected_email_html_body = (
            'Hi user1,<br><br>In the <a href="%s%s#/roles">admin roles page,</a> please add reviewers to the Contributor Dashboard community by entering their username(s) and allow reviewing for the suggestion types that need more reviewers bolded below.<br><br>There have been <b>हिन्दी (Hindi) translation suggestions</b> created on the <a href="%s%s">Contributor Dashboard page</a> where there are not enough reviewers.<br><br>Thanks so much - we appreciate your help!<br><br>Best Wishes!<br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.ADMIN_URL, feconf.
            OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL))
        email_manager.send_mail_to_notify_admins_that_reviewers_are_needed([
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        ...
        EMAIL_SENDER_NAME, 'Site Admin'), (platform_parameter_list.
        ParamName.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, True)])
    def test_admin_emails_sent_if_translations_need_reviewers_for_one_lang(self
        ) ->None:
        self._create_translation_suggestion_with_language_code('hi')
        suggestion_types_needing_reviewers = (suggestion_services.
            get_suggestion_types_that_need_reviewers())
    ...
    def test_email_sent_to_admins_if_mutli_suggestion_types_needing_reviewers(
        ...
        suggestion_types_needing_reviewers = (suggestion_services.
            get_suggestion_types_that_need_reviewers())
        self.assertDictEqual(suggestion_types_needing_reviewers, {feconf.
            SUGGESTION_TYPE_TRANSLATE_CONTENT: {'fr', 'hi'}, feconf.
            SUGGESTION_TYPE_ADD_QUESTION: set()})
        expected_email_html_body_for_admin_1 = (
            'Hi user1,<br><br>In the <a href="%s%s#/roles">admin roles page,</a> please add reviewers to the Contributor Dashboard community by entering their username(s) and allow reviewing for the suggestion types that need more reviewers bolded below.<br><br>There have been <b>translation suggestions</b> created on the <a href="%s%s">Contributor Dashboard page</a> in languages where there are not enough reviewers. The languages that need more reviewers are:<br><ul><li><b>français (French)</b></li><br><li><b>हिन्दी (Hindi)</b></li><br></ul><br>Thanks so much - we appreciate your help!<br><br>Best Wishes!<br>- The Oppia Contributor Dashboard Team'
             % (feconf.OPPIA_SITE_URL, feconf.ADMIN_URL, feconf.
        ...
        self.assertEqual(sent_email_model.recipient_email, self.
            CURRICULUM_ADMIN_1_EMAIL)
        self.assertEqual(sent_email_model.html_body,
            expected_email_html_body_for_admin_1)
        self.assertEqual(sent_email_model.sender_id, feconf.SYSTEM_COMMITTER_ID
            )
        self.assertEqual(sent_email_model.sender_email, 'Site Admin <%s>' %
...
class QueryStatusNotificationEmailTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.recipient_a_id = self.get_user_id_from_email(self.
            RECIPIENT_A_EMAIL)
        self.recipient_b_id = self.get_user_id_from_email(self.
            RECIPIENT_B_EMAIL)
...
You can change your email preferences via the Preferences page."""
    ...
    def test_that_correct_failure_email_is_sent(self) ->None:
        ...
        expected_email_html_body = (
            'Hi submit,<br>Your query with id qid has failed due to error during execution. Please check the query parameters and submit query again.<br><br>Thanks!<br><br>Best wishes,<br>The Oppia Team<br><br>You can change your email preferences via the <a href="http://localhost:8181/preferences">Preferences</a> page.'
            )
        expected_email_text_body = """Hi submit,
...
key2: val2
"""
        email_manager.send_query_failure_email(self.submitter_id, query_id,
            query_params)
        messages = self._get_sent_email_messages(self.SUBMITTER_EMAIL)
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0].html, expected_email_html_body)
        ...
        self.assertEqual(sent_email_model.intent, feconf.
            EMAIL_INTENT_QUERY_STATUS_NOTIFICATION)
        admin_messages = self._get_sent_email_messages(feconf.
            ADMIN_EMAIL_ADDRESS)
    ...
    def test_send_user_query_email(self) ->None:
        ...
        email_manager.send_user_query_email(self.sender_id, self.
            recipient_ids, email_subject, email_body, email_intent)
        ...
        self.assertEqual(sent_email_model.sender_email, '%s <%s>' % (self.
            SENDER_USERNAME, self.SENDER_EMAIL))
...
class AccountDeletionEmailUnitTest(test_utils.EmailTestBase):
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_account_deleted_email(self.applicant_id, self.
            APPLICANT_EMAIL)
    ...
    def test_account_deletion_failed_email_is_sent_correctly(self) ->None:
        ...
        with admin_email_ctx:
            ...
            email_manager.send_account_deletion_failed_email(self.
                applicant_id, self.APPLICANT_EMAIL)
            messages = self._get_sent_email_messages(feconf.ADMIN_EMAIL_ADDRESS
                )
            self.assertEqual(len(messages), 1)
            self.assertEqual(messages[0].to, ['admin@system.com'])
            self.assertEqual(messages[0].subject,
                'WIPEOUT: Account deletion failed')
            self.assertIn(
                "The Wipeout process failed for the user with ID '%s' and email '%s'."
                 % (self.applicant_id, self.APPLICANT_EMAIL), messages[0].html)
    ...
    def test_that_correct_account_deleted_email_is_sent(self) ->None:
        ...
        expected_email_html_body = (
            'Hi applicant@example.com,<br><br>Your account was successfully deleted.<br><br>- The Oppia Team'
            )
        email_manager.send_account_deleted_email(self.applicant_id, self.
            APPLICANT_EMAIL)
        messages = self._get_sent_email_messages(self.APPLICANT_EMAIL)
...
class BulkEmailsTests(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.fake_sender_id = self.get_user_id_from_email(self.
            FAKE_SENDER_EMAIL)
        self.signup(self.RECIPIENT_A_EMAIL, self.RECIPIENT_A_USERNAME)
        self.signup(self.RECIPIENT_B_EMAIL, self.RECIPIENT_B_USERNAME)
        self.recipient_a_id = self.get_user_id_from_email(self.
    ...
    def test_that_correct_email_is_sent(self) ->None:
        ...
        email_manager.send_user_query_email(self.sender_id, self.
            recipient_ids, email_subject, email_html_body, feconf.
            BULK_EMAIL_INTENT_CREATE_EXPLORATION)
        messages_a = self._get_sent_email_messages(self.RECIPIENT_A_EMAIL)
        self.assertEqual(len(messages_a), 1)
    ...
    def test_that_exception_is_raised_for_unauthorised_sender(self) ->None:
        with self.assertRaisesRegex(Exception, 'Invalid sender_id for email'):
            email_manager.send_user_query_email(self.fake_sender_id, self.
                recipient_ids, 'email_subject', 'email_html_body', feconf.
    ...
    def test_that_test_email_is_sent_for_bulk_emails(self) ->None:
        ...
        email_manager.send_test_email_for_bulk_emails(self.sender_id,
...
class EmailPreferencesTests(test_utils.EmailTestBase):
    ...
    def test_can_users_receive_thread_email(self) ->None:
        ...
        user_services.set_email_preferences_for_exploration(user_ids[0],
            exp_id, mute_feedback_notifications=True)
        self.assertListEqual(email_manager.can_users_receive_thread_email(
            user_ids, exp_id, True), [True, True])
        self.assertListEqual(email_manager.can_users_receive_thread_email(
            user_ids, exp_id, False), [False, True])
        user_services.set_email_preferences_for_exploration(user_ids[1],
            exp_id, mute_suggestion_notifications=True)
        self.assertListEqual(email_manager.can_users_receive_thread_email(
            user_ids, exp_id, True), [True, False])
        self.assertListEqual(email_manager.can_users_receive_thread_email(
            user_ids, exp_id, False), [False, True])
        for user_id in user_ids:
            user_services.update_email_preferences(user_id, True, True, 
                False, True)
        self.assertListEqual(email_manager.can_users_receive_thread_email(
        ...
        user_services.set_email_preferences_for_exploration(user_ids[0],
            exp_id, mute_feedback_notifications=False)
        user_services.set_email_preferences_for_exploration(user_ids[1],
            exp_id, mute_suggestion_notifications=False)
...
class ModeratorActionEmailsTests(test_utils.EmailTestBase):
    ...
    def test_correct_moderator_action_email_sent(self) ->None:
        ...
        email_html_body = 'Dummy email body.<br>'
        email_manager.send_moderator_action_email(self.moderator_id, self.
            recipient_id, email_intent, exploration_title, email_html_body)
...
class CDUserEmailTest(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.translation_reviewer_id = self.get_user_id_from_email(self.
            TRANSLATION_REVIEWER_EMAIL)
        user_services.update_email_preferences(self.translation_reviewer_id,
            True, False, False, False)
        self.question_reviewer_id = self.get_user_id_from_email(self.
            QUESTION_REVIEWER_EMAIL)
        user_services.update_email_preferences(self.question_reviewer_id, 
            True, False, False, False)
        self.question_submitter_id = self.get_user_id_from_email(self.
            QUESTION_SUBMITTER_EMAIL)
        user_services.update_email_preferences(self.question_submitter_id, 
            True, False, False, False)
    ...
    def test_assign_translation_reviewer_email_for_can_send_emails_is_false(
        self) ->None:
        email_manager.send_email_to_new_cd_user(self.
            translation_reviewer_id, constants.
            CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION, language_code='hi')
        messages = self._get_sent_email_messages(self.
            TRANSLATION_REVIEWER_EMAIL)
        self.assertEqual(len(messages), 0)
    ...
    def test_without_language_code_email_not_sent_to_removed_translation_reviewer(
        ...
        with self.assertRaisesRegex(Exception,
            'The language_code cannot be None'):
            email_manager.send_email_to_removed_cd_user(self.
                translation_reviewer_id, constants.
                CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION)

    def test_assign_translation_reviewer_email_for_invalid_category(self
        ) ->None:
        with self.assertRaisesRegex(Exception, 'Invalid category'):
            email_manager.send_email_to_new_cd_user(self.
                translation_reviewer_id, 'invalid_category')

    def test_schema_of_new_cd_user_email_data_constant(self) ->None:
        self.assertEqual(sorted(email_manager.NEW_CD_USER_EMAIL_DATA.keys()
            ), [constants.CD_USER_RIGHTS_CATEGORY_REVIEW_QUESTION,
            constants.CD_USER_RIGHTS_CATEGORY_SUBMIT_QUESTION, constants.
            CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION])
        for category_details in email_manager.NEW_CD_USER_EMAIL_DATA.values():
            self.assertEqual(len(category_details), 5)
            self.assertTrue('description' in category_details or 
                'description_template' in category_details)
            self.assertTrue('category' in category_details)
            self.assertTrue('rights_message' in category_details or 
                'rights_message_template' in category_details)
            self.assertTrue('to_review' in category_details or 'to_submit' in
                category_details)
    ...
    def test_send_assigned_translation_reviewer_email(self) ->None:
        ...
        expected_email_html_body = (
            'Hi translator,<br><br>This is to let you know that the Oppia team has added you as a reviewer for हिन्दी (hindi) language translations. This allows you to review translation suggestions made by contributors in the हिन्दी (hindi) language.<br><br>You can check the translation suggestions waiting for review in the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
            )
        email_manager.send_email_to_new_cd_user(self.
            translation_reviewer_id, constants.
            CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION, language_code='hi')
    ...
    def test_send_assigned_question_reviewer_email(self) ->None:
        ...
        expected_email_html_body = (
            'Hi question,<br><br>This is to let you know that the Oppia team has added you as a reviewer for questions. This allows you to review question suggestions made by contributors.<br><br>You can check the question suggestions waiting for review in the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
            )
        email_manager.send_email_to_new_cd_user(self.question_reviewer_id,
            constants.CD_USER_RIGHTS_CATEGORY_REVIEW_QUESTION,
            language_code='hi')
    ...
    def test_send_assigned_question_submitter_email(self) ->None:
        ...
        expected_email_html_body = (
            'Hi questionSuggestor,<br><br>This is to let you know that the Oppia team has added you as a contributor to submit question suggestions for use in lessons.<br><br>You can now start to submit questions in the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
            )
        email_manager.send_email_to_new_cd_user(self.question_submitter_id,
    ...
    def test_remove_translation_reviewer_email_for_invalid_category(self
        ) ->None:
        with self.assertRaisesRegex(Exception, 'Invalid category'):
            email_manager.send_email_to_removed_cd_user(self.
                translation_reviewer_id, 'invalid_category')
    ...
    def test_schema_of_removed_reviewer_email_data_constant(self) ->None:
        self.assertEqual(sorted(email_manager.REMOVED_CD_USER_EMAIL_DATA.
            keys()), [constants.CD_USER_RIGHTS_CATEGORY_REVIEW_QUESTION,
            constants.CD_USER_RIGHTS_CATEGORY_SUBMIT_QUESTION, constants.
            CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION])
        for category_details in email_manager.REMOVED_CD_USER_EMAIL_DATA.values(
            ):
            self.assertEqual(len(category_details), 3)
            self.assertTrue('role_description' in category_details or 
    ...
    def test_send_removed_translation_reviewer_email(self) ->None:
        ...
        expected_email_html_body = (
            'Hi translator,<br><br>The Oppia team has removed you from the translation reviewer role in the हिन्दी (hindi) language. You won\'t be able to review translation suggestions made by contributors in the हिन्दी (hindi) language any more, but you can still contribute translations through the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
            )
        email_manager.send_email_to_removed_cd_user(self.
            translation_reviewer_id, constants.
            CD_USER_RIGHTS_CATEGORY_REVIEW_TRANSLATION, language_code='hi')
    ...
    def test_send_removed_question_reviewer_email(self) ->None:
        ...
        expected_email_html_body = (
            'Hi question,<br><br>The Oppia team has removed you from the question reviewer role. You won\'t be able to review question suggestions made by contributors any more, but you can still contribute questions through the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
            )
        email_manager.send_email_to_removed_cd_user(self.
            question_reviewer_id, constants.
            CD_USER_RIGHTS_CATEGORY_REVIEW_QUESTION, language_code='hi')
        messages = self._get_sent_email_messages(self.QUESTION_REVIEWER_EMAIL)
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0].html, expected_email_html_body)
        all_models: Sequence[email_models.SentEmailModel
    ...
    def test_send_removed_question_submitter_email(self) ->None:
        ...
        expected_email_html_body = (
            'Hi questionSuggestor,<br><br>The Oppia team has removed you from the question submitter role. You won\'t be able to submit question suggestions any more, but you can still contribute questions through the <a href="https://www.oppia.org/contributor-dashboard">Contributor Dashboard</a>.<br><br>Thanks, and happy contributing!<br><br>Best wishes,<br>The Oppia Community'
            )
        email_manager.send_email_to_removed_cd_user(self.
            question_submitter_id, constants.
            CD_USER_RIGHTS_CATEGORY_SUBMIT_QUESTION, language_code='hi')
        messages = self._get_sent_email_messages(self.QUESTION_SUBMITTER_EMAIL)
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0].html, expected_email_html_body)
        all_models: Sequence[email_models.SentEmailModel
...
class NotMergeableChangesEmailUnitTest(test_utils.EmailTestBase):
    ...
    def setUp(self) ->None:
        super().setUp()
        self.admin_email_ctx = self.swap(feconf, 'ADMIN_EMAIL_ADDRESS',
...
class MailchimpSecretTest(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.swap_webhook_secrets_return_secret = self.swap_with_checks(
            ...
            expected_args=[('MAILCHIMP_WEBHOOK_SECRET',), (
    ...
    def test_cloud_secrets_return_none_logs_exception(self) ->None:
        with self.swap_webhook_secrets_return_none:
            with self.capture_logging(min_level=logging.WARNING) as logs:
                self.assertFalse(email_manager.verify_mailchimp_secret(
                    'secret'))
                self.assertEqual([
...
class CurriculumAdminsChapterNotificationsReminderMailTests(test_utils.
    EmailTestBase):
    CURRICULUM_ADMIN_1_USERNAME: Final = 'user1'
    CURRICULUM_ADMIN_1_EMAIL: Final = 'user1@community.org'
    CURRICULUM_ADMIN_2_USERNAME: Final = 'user2'
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, False)])
    def test_email_not_sent_if_server_can_send_emails_is_false(self) ->None:
        email_manager.send_reminder_mail_to_notify_curriculum_admins([self.
            CURRICULUM_ADMIN_1_EMAIL, self.CURRICULUM_ADMIN_2_EMAIL], [])
        messages = self._get_sent_email_messages(self.CURRICULUM_ADMIN_1_EMAIL)
        self.assertEqual(len(messages), 0)
        messages = self._get_sent_email_messages(self.CURRICULUM_ADMIN_2_EMAIL)
        self.assertEqual(len(messages), 0)
    ...
    def test_email_not_sent_if_no_admins_to_notify(self) ->None:
        with self.capture_logging(min_level=logging.ERROR) as logs:
            ...
            self.assertEqual(len(messages), 0)
            self.assertEqual(logs[0],
                'There were no curriculum admins to notify.')
    ...
    def test_email_not_sent_if_no_overdue_or_upcoming_chapters(self) ->None:
        story_publication_timeliness = story_domain.StoryPublicationTimeliness(
            'story_1', 'Story', 'Topic', [], [])
        email_manager.send_reminder_mail_to_notify_curriculum_admins([self.
            admin_1_id], [story_publication_timeliness])
        messages = self._get_all_sent_email_messages()
        self.assertEqual(len(messages), 0)
    ...
    @test_utils.set_platform_parameters([(platform_parameter_list.ParamName
        .SERVER_CAN_SEND_EMAILS, True), (platform_parameter_list.ParamName.
        EMAIL_FOOTER, EMAIL_FOOTER), (platform_parameter_list.ParamName.
        EMAIL_SENDER_NAME, 'Oppia')])
    def test_email_sent_if_chapters_are_overdue(self) ->None:
        story_publication_timeliness = story_domain.StoryPublicationTimeliness(
            'story_1', 'Story', 'Topic', ['Chapter 1', 'Chapter 2'], [])
        expected_email_html_body = (
            'Dear Curriculum Admin, <br><br>The following stories have unpublished chapters which are behind schedule. Please publish them or adjust the planned publication date.<br><br><ol><li>Story (Topic) - <a href="%s%s/story_1">Link</a><ul><li>Chapter 1</li><li>Chapter 2</li></ul></li></ol>Regards,<br> Oppia Foundation'
        ...
        self.assertEqual(sent_email_model.sender_email, '. <%s>' % feconf.
            NOREPLY_EMAIL_ADDRESS)
        self.assertEqual(sent_email_model.intent, feconf.
    ...
    def test_email_sent_if_chapters_are_upcoming(self) ->None:
        story_publication_timeliness = story_domain.StoryPublicationTimeliness(
            'story_1', 'Story', 'Topic', [], ['Chapter 1', 'Chapter 2'])
        expected_email_html_body = (
            'Dear Curriculum Admin, <br><br>The following stories have unpublished chapters which are due for publication in the next %s days. Please ensure they are published on or before the planned date or adjust the planned publication date.<br><br><ol><li>Story (Topic) - <a href="%s%s/story_1">Link</a><ul><li>Chapter 1</li><li>Chapter 2</li></ul></li></ol>Regards,<br> Oppia Foundation'
