...
class FeatureFlagSpecTests(test_utils.GenericTestBase):
    ...
    def test_create_from_dict_returns_correct_instance(self) ->None:
        feature_flag_spec = feature_flag_domain.FeatureFlagSpec.from_dict({
            'description': 'for test', 'feature_stage': feature_flag_domain
            .FeatureStages.DEV.value})
        self.assertIsInstance(feature_flag_spec, feature_flag_domain.
            FeatureFlagSpec)
        self.assertEqual(feature_flag_spec.description, 'for test')
        self.assertEqual(feature_flag_spec.feature_stage,
    ...
    def test_from_dict_raises_error_when_invalid_feature_stage(self) ->None:
        with self.assertRaisesRegex(Exception,
            ...
            feature_flag_domain.FeatureFlagSpec.from_dict({'description':
                'for test', 'feature_stage': 'invalid'})

    def test_to_dict_returns_correct_dict(self) ->None:
        feature_flag_spec_dict: feature_flag_domain.FeatureFlagSpecDict = {
            'description': 'for test', 'feature_stage': feature_flag_domain
            .FeatureStages.DEV.value}
        feature_flag_spec = feature_flag_domain.FeatureFlagSpec('for test',
            feature_flag_domain.FeatureStages.DEV)
        self.assertDictEqual(feature_flag_spec.to_dict(),
...
class FeatureFlagConfigTests(test_utils.GenericTestBase):
    ...
    def test_create_from_dict_returns_correct_instance(self) ->None:
        ...
        feature_flag_config = feature_flag_domain.FeatureFlagConfig.from_dict({
            'force_enable_for_all_users': False, 'rollout_percentage': 0,
            'user_group_ids': [], 'last_updated': utils.
            convert_naive_datetime_to_string(current_time)})
        self.assertIsInstance(feature_flag_config, feature_flag_domain.
            FeatureFlagConfig)
    ...
    def test_to_dict_returns_correct_dict(self) ->None:
        ...
        feature_flag_config_dict: feature_flag_domain.FeatureFlagConfigDict = {
            ...
            'user_group_ids': [], 'last_updated': utils.
            convert_naive_datetime_to_string(current_time)}
        feature_flag_config = feature_flag_domain.FeatureFlagConfig(False, 
            0, [], current_time)
        self.assertDictEqual(feature_flag_config.to_dict(),
    ...
    def test_validate_feature_flag_config_passes_without_exception(self
        ) ->None:
        feature_flag_config = feature_flag_domain.FeatureFlagConfig(False, 
            0, [], datetime.datetime.utcnow())
    ...
    def test_validate_dev_feature_for_test_env_raises_exception(self) ->None:
        ...
        with self.swap(constants, 'DEV_MODE', False):
            with self.swap(feconf, 'ENV_IS_OPPIA_ORG_PRODUCTION_SERVER', False
                ...
                with self.assertRaisesRegex(utils.ValidationError,
                    'Feature flag in dev stage cannot be updated in test environment.'
                    ):
                    feature_flag_config.validate(feature_flag_domain.
                        ServerMode.DEV)
    ...
    def test_validate_dev_feature_for_prod_env_raises_exception(self) ->None:
        ...
        with self.swap(constants, 'DEV_MODE', False):
            with self.swap(feconf, 'ENV_IS_OPPIA_ORG_PRODUCTION_SERVER', True):
                with self.assertRaisesRegex(utils.ValidationError,
                    'Feature flag in dev stage cannot be updated in prod environment.'
                    ):
                    feature_flag_config.validate(feature_flag_domain.
                        ServerMode.DEV)
    ...
    def test_validate_test_feature_for_prod_env_raises_exception(self) ->None:
        ...
        with self.swap(constants, 'DEV_MODE', False):
            with self.swap(feconf, 'ENV_IS_OPPIA_ORG_PRODUCTION_SERVER', True):
                with self.assertRaisesRegex(utils.ValidationError,
                    'Feature flag in test stage cannot be updated in prod environment.'
                    ):
                    feature_flag_config.validate(feature_flag_domain.
                        ServerMode.TEST)
...
class FeatureFlagTests(test_utils.GenericTestBase):
    ...
    def test_create_from_dict_returns_correct_instance(self) ->None:
        current_time = get_current_datetime_utc()
        feature_flag = feature_flag_domain.FeatureFlag.from_dict({'name':
            'feature_a', 'description': 'for test', 'feature_stage':
        ...
        self.assertEqual(feature_flag.feature_flag_spec.feature_stage,
            feature_flag_domain.FeatureStages.DEV)
        self.assertFalse(feature_flag.feature_flag_config.
        ...
        self.assertEqual(feature_flag.feature_flag_config.last_updated,
            current_time)
    ...
    def test_to_dict_returns_correct_dict(self) ->None:
        ...
        feature_flag_spec = feature_flag_domain.FeatureFlagSpec('for test',
            feature_flag_domain.FeatureStages.DEV)
        feature_flag_dict: feature_flag_domain.FeatureFlagDict = {'name':
            'feature_a', 'description': 'for test', 'feature_stage':
            feature_flag_domain.FeatureStages.DEV.value,
            'force_enable_for_all_users': False, 'rollout_percentage': 0,
            'user_group_ids': [], 'last_updated': utils.
            convert_naive_datetime_to_string(current_time)}
        feature_flag = feature_flag_domain.FeatureFlag('feature_a',
            feature_flag_spec, feature_flag_config)
        feature_flag.validate()
        self.assertDictEqual(feature_flag.to_dict(), feature_flag_dict)

    def test_validate_feature_flag_with_invalid_name_raises_exception(self
        ) ->None:
        feature_flag_config = feature_flag_domain.FeatureFlagConfig(False, 
            0, [], datetime.datetime.utcnow())
        feature_flag_spec = feature_flag_domain.FeatureFlagSpec('for test',
            feature_flag_domain.FeatureStages.DEV)
        feature_flag = feature_flag_domain.FeatureFlag('Invalid~Name',
            feature_flag_spec, feature_flag_config)
        with self.assertRaisesRegex(utils.ValidationError, 
    ...
    def test_validate_feature_flag_with_perc_more_than_100_raises_exception(
        ...
        feature_flag = feature_flag_domain.FeatureFlag('Feature',
            feature_flag_spec, feature_flag_config)
        with self.assertRaisesRegex(utils.ValidationError,
            'Feature flag rollout-percentage should be between 0 and 100 inclusive.'
            ):
