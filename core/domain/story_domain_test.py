...
class StoryChangeTests(test_utils.GenericTestBase):
    ...
    def test_story_change_object_with_missing_attribute_in_cmd(self) ->None:
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            story_domain.StoryChange({'cmd': 'update_story_property',
                'property_name': 'title'})

    def test_story_change_object_with_extra_attribute_in_cmd(self) ->None:
        with self.assertRaisesRegex(utils.ValidationError,
            'The following extra attributes are present: invalid'):
            story_domain.StoryChange({'cmd': 'add_story_node', 'node_id':
                'node_id', 'invalid': 'invalid'})
    ...
    def test_story_change_object_with_invalid_story_property(self) ->None:
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            story_domain.StoryChange({'cmd': 'update_story_property',
                'property_name': 'invalid', 'old_value': 'old_value',
                'new_value': 'new_value'})
    ...
    def test_story_change_object_with_invalid_story_node_property(self) ->None:
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            story_domain.StoryChange({'cmd': 'update_story_node_property',
                'node_id': 'node_id', 'property_name': 'invalid',
    ...
    def test_story_change_object_with_invalid_story_contents_property(self
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            story_domain.StoryChange({'cmd':
                'update_story_contents_property', 'property_name':
                'invalid', 'old_value': 'old_value', 'new_value': 'new_value'})

    def test_story_change_object_with_add_story_node(self) ->None:
        story_change_object = story_domain.StoryChange({'cmd':
            'add_story_node', 'node_id': 'node_id', 'title': 'title'})
    ...
    def test_story_change_object_with_delete_story_node(self) ->None:
        story_change_object = story_domain.StoryChange({'cmd':
            'delete_story_node', 'node_id': 'node_id'})
    ...
    def test_story_change_object_with_update_story_node_property(self) ->None:
        story_change_object = story_domain.StoryChange({'cmd':
            'update_story_node_property', 'node_id': 'node_id',
            'property_name': 'title', 'new_value': 'new_value', 'old_value':
            'old_value'})
    ...
    def test_story_change_object_with_update_story_property(self) ->None:
        story_change_object = story_domain.StoryChange({'cmd':
            'update_story_property', 'property_name': 'title', 'new_value':
    ...
    def test_story_change_object_with_update_story_contents_property(self
        ...
        story_change_object = story_domain.StoryChange({'cmd':
            'update_story_contents_property', 'property_name':
            'initial_node_id', 'new_value': 'new_value', 'old_value':
            'old_value'})
        self.assertEqual(story_change_object.cmd,
            'update_story_contents_property')
    ...
    def test_story_change_object_with_update_story_node_outline_status(self
        ...
        story_change_object = story_domain.StoryChange({'cmd':
            'update_story_node_outline_status', 'node_id': 'node_id',
            'old_value': 'old_value', 'new_value': 'new_value'})
        self.assertEqual(story_change_object.cmd,
            'update_story_node_outline_status')
        self.assertEqual(story_change_object.node_id, 'node_id')
        self.assertEqual(story_change_object.old_value, 'old_value')
        self.assertEqual(story_change_object.new_value, 'new_value')

    def test_story_change_object_with_create_new(self) ->None:
        story_change_object = story_domain.StoryChange({'cmd': 'create_new',
            'title': 'title'})
        self.assertEqual(story_change_object.cmd, 'create_new')
        self.assertEqual(story_change_object.title, 'title')

    def test_story_change_object_with_migrate_schema_to_latest_version(self
        ) ->None:
        story_change_object = story_domain.StoryChange({'cmd':
            'migrate_schema_to_latest_version', 'from_version':
            'from_version', 'to_version': 'to_version'})
        self.assertEqual(story_change_object.cmd,
            'migrate_schema_to_latest_version')
...
class StoryDomainUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.story = self.save_new_story(self.story_id, self.USER_ID, self.
            TOPIC_ID, url_fragment='story-frag')
        ...
        self.story.update_node_destination_node_ids(self.NODE_ID_1, [self.
            NODE_ID_2])
    ...
    def _assert_valid_story_id(self, expected_error_substring: str,
        story_id: str) ->None:
    ...
    def test_serialize_and_deserialize_returns_unchanged_story(self) ->None:
        ...
        story = story_domain.Story.create_default_story(self.STORY_ID,
            'Title', 'Description', topic_id, 'title')
        self.assertEqual(story.to_dict(), story_domain.Story.deserialize(
            story.serialize()).to_dict())
    ...
    def _assert_valid_thumbnail_filename_for_story(self,
        expected_error_substring: str, thumbnail_filename: str) ->None:
    ...
    def _assert_valid_thumbnail_filename_for_story_node(self,
        expected_error_substring: str, thumbnail_filename: str) ->None:
    ...
    def test_story_node_update_thumbnail_filename(self) ->None:
        ...
        fs.commit('%s/new_image.svg' % constants.ASSET_TYPE_THUMBNAIL,
            raw_image, mimetype='image/svg+xml')
        self.story.update_node_thumbnail_filename(self.NODE_ID_1,
            'new_image.svg')
        node_index = self.story.story_contents.get_node_index(self.NODE_ID_1)
        self.assertEqual(self.story.story_contents.nodes[node_index].
            thumbnail_filename, 'new_image.svg')
        self.assertEqual(self.story.story_contents.nodes[node_index].
            thumbnail_size_in_bytes, len(raw_image))
        with self.assertRaisesRegex(Exception,
            'The node with id invalid_id is not part of this story.'):
            self.story.update_node_thumbnail_filename('invalid_id',
                'invalid_thumbnail.svg')
    ...
    def test_to_human_readable_dict(self) ->None:
        ...
        expected_dict: story_domain.HumanReadableStorySummaryDict = {'id':
            self.story_id, 'title': 'Title', 'description': 'Description',
            'node_titles': [], 'thumbnail_bg_color': None,
            'thumbnail_filename': None, 'url_fragment': 'story-frag'}
    ...
    def test_defaults(self) ->None:
        ...
        topic_id = utils.generate_random_string(12)
        story = story_domain.Story.create_default_story(self.STORY_ID,
            'Title', 'Description', topic_id, 'story-frag-default')
        expected_story_dict: story_domain.StoryDict = {'id': self.STORY_ID,
            'title': 'Title', 'thumbnail_filename': None,
            'thumbnail_bg_color': None, 'thumbnail_size_in_bytes': None,
            'description': 'Description', 'notes': feconf.
            DEFAULT_STORY_NOTES, 'story_contents': {'nodes': [],
            'initial_node_id': None, 'next_node_id': self.NODE_ID_1},
            'story_contents_schema_version': feconf.
            CURRENT_STORY_CONTENTS_SCHEMA_VERSION, 'language_code':
            constants.DEFAULT_LANGUAGE_CODE, 'corresponding_topic_id':
            topic_id, 'version': 0, 'url_fragment': 'story-frag-default',
            'meta_tag_content': ''}
    ...
    def test_get_acquired_skill_ids_for_node_ids(self) ->None:
        ...
        self.assertEqual(self.story.get_acquired_skill_ids_for_node_ids([
            self.NODE_ID_1, self.NODE_ID_2]), ['skill_1', 'skill_2'])
    ...
    def test_get_acquired_skill_ids_for_node_ids_multi_skills(self) ->None:
        ...
        self.assertEqual(self.story.get_acquired_skill_ids_for_node_ids([
            self.NODE_ID_1, self.NODE_ID_2]), ['skill_1', 'skill_2', 'skill_3']
            )
    ...
    def test_get_acquired_skill_ids_with_empty_node_ids_ids_is_empty_list(self
        ) ->None:
        self.story.story_contents.nodes[0].acquired_skill_ids = ['skill_1',
            'skill_2']
        self.story.story_contents.nodes[1].acquired_skill_ids = ['skill_1']
    ...
    def test_get_prerequisite_skill_ids(self) ->None:
        ...
        self.assertEqual(self.story.get_prerequisite_skill_ids_for_exp_id(
            'exp_id'), ['skill_1'])
        self.assertIsNone(self.story.get_prerequisite_skill_ids_for_exp_id(
            'exp_id_2'))
    ...
    def test_update_thumbnail_filename(self) ->None:
        ...
        with utils.open_file(os.path.join(feconf.TESTS_DATA_DIR,
            ...
            raw_image = f.read()
        fs = fs_services.GcsFileSystem(feconf.ENTITY_TYPE_STORY, self.story.id)
        fs.commit('%s/img.svg' % constants.ASSET_TYPE_THUMBNAIL, raw_image,
    ...
    def test_schema_version_validation(self) ->None:
        ...
        self._assert_validation_error(
            ...
            (feconf.CURRENT_STORY_CONTENTS_SCHEMA_VERSION, self.story.
    ...
    def test_add_node_validation(self) ->None:
        with self.assertRaisesRegex(Exception,
            'The node id node_4 does not match the expected next node id for the story'
            ):
    ...
    def test_get_number_from_node_id(self) ->None:
        self.assertEqual(story_domain.StoryNode.get_number_from_node_id(
            'node_10'), 10)
    ...
    def test_node_outline_finalized_validation(self) ->None:
        ...
        self.assertEqual(self.story.story_contents.nodes[0].outline,
            'new outline')
    ...
    def test_node_description_validation(self) ->None:
        ...
        self.story.story_contents.nodes[0].description = (
            'Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Dum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu,'
            )
        self._assert_validation_error(
            'Chapter description should be less than 152 chars')
        self.story.update_node_description('node_1', 'new description')
        self.assertEqual(self.story.story_contents.nodes[0].description,
            'new description')
    ...
    def test_story_node_update_planned_publication_date(self) ->None:
        ...
        self.story.update_node_planned_publication_date('node_1',
            current_time_msecs)
        self.assertEqual(self.story.story_contents.nodes[0].
    ...
    def test_story_node_update_last_modified(self) ->None:
        ...
        self.assertEqual(self.story.story_contents.nodes[0].
            last_modified_msecs, current_time_msecs)
    ...
    def test_story_node_update_first_publication_date(self) ->None:
        ...
        current_time = get_current_datetime_utc()
        current_time_msecs = utils.get_time_in_millisecs(current_time)
        self.story.update_node_first_publication_date('node_1',
            current_time_msecs)
        self.assertEqual(self.story.story_contents.nodes[0].
            first_publication_date_msecs, current_time_msecs)
    ...
    def test_nodes_validation(self) ->None:
        ...
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            ({'id': 'node_1', 'thumbnail_filename': None,
            'thumbnail_bg_color': None, 'thumbnail_size_in_bytes': None,
            'title': 'Title 1', 'description': 'Description 1',
            'destination_node_ids': [self.NODE_ID_2],
            'prerequisite_skill_ids': [], 'acquired_skill_ids': [],
            'outline': 'Outline', 'outline_is_finalized': False,
            'exploration_id': 'exploration_id', 'status': None,
            'planned_publication_date_msecs': None, 'last_modified_msecs':
            None, 'first_publication_date_msecs': None,
            'unpublishing_reason': 'BAD_CONTENT'})]
        self._assert_validation_error('Expected all destination nodes to exist'
            )
        self.story.story_contents.nodes.append(story_domain.StoryNode.
            create_default_story_node(self.NODE_ID_2, 'Title 2'))
        self.story.story_contents.nodes[0].acquired_skill_ids = ['skill_id',
            'skill_id', 'skill_id_1']
        self._assert_validation_error(
        ...
        self.assertEqual(self.story.story_contents.nodes[0].
            acquired_skill_ids, ['3'])
        self.story.update_node_acquired_skill_ids('node_1', ['3', '4'])
        self.assertEqual(self.story.story_contents.nodes[0].
            acquired_skill_ids, ['3', '4'])
        self.story.story_contents.nodes[0].prerequisite_skill_ids = ['skill_id'
            , 'skill_id', 'skill_id_1']
        self._assert_validation_error(
        ...
        self.assertFalse(self.story.story_contents.nodes[0].
    ...
    def test_node_is_upcoming(self) ->None:
        ...
        self.story.story_contents.nodes[0
            ].planned_publication_date_msecs = int(datetime.datetime(2023, 
            1, 1).timestamp() * 1000)
        self.story.story_contents.nodes[1
            ].status = constants.STORY_NODE_STATUS_READY_TO_PUBLISH
        self.story.story_contents.nodes[1
            ].planned_publication_date_msecs = int(datetime.datetime(2022, 
            12, 29).timestamp() * 1000)
        ...
        with self.swap(utils, 'get_current_time_in_millisecs',
            ...
            self.assertEqual(self.story.story_contents.nodes[0].
                is_node_upcoming(), True)
            self.assertEqual(self.story.story_contents.nodes[1].
                is_node_upcoming(), False)
    ...
    def test_node_is_behind_schedule(self) ->None:
        ...
        with self.swap(utils, 'get_current_time_in_millisecs',
            _mock_get_current_time_in_millisecs):
            self.assertEqual(self.story.story_contents.nodes[0].
                is_node_behind_schedule(), False)
            self.assertEqual(self.story.story_contents.nodes[1].
                is_node_behind_schedule(), True)
    ...
    def test_get_ordered_nodes_when_nodes_exist(self) ->None:
        ...
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            'thumbnail_filename': 'image1.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 1',
            'description': 'Description 1', 'destination_node_ids': [
            'node_3'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': None, 'status': 'Draft',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        node_2: story_domain.StoryNodeDict = {'id': 'node_2',
            'thumbnail_filename': 'image2.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 2',
            'description': 'Description 2', 'destination_node_ids': [
            ...
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        node_3: story_domain.StoryNodeDict = {'id': 'node_3',
            'thumbnail_filename': 'image3.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 3',
            'description': 'Description 3', 'destination_node_ids': [],
            'acquired_skill_ids': [], 'prerequisite_skill_ids': [],
            'outline': '', 'outline_is_finalized': False, 'exploration_id':
            None, 'status': 'Draft', 'planned_publication_date_msecs': 
            100.0, 'last_modified_msecs': 100.0,
            'first_publication_date_msecs': None, 'unpublishing_reason': None}
        self.story.story_contents.initial_node_id = 'node_2'
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            (node_1), story_domain.StoryNode.from_dict(node_2),
            story_domain.StoryNode.from_dict(node_3)]
        expected_list = [story_domain.StoryNode.from_dict(node_2),
            story_domain.StoryNode.from_dict(node_1), story_domain.
            StoryNode.from_dict(node_3)]
    ...
    def test_update_node_exploration_id_changes_node_exploration_id(self
        ) ->None:
        self.story.story_contents.next_node_id = 'node_3'
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 1',
            'description': 'Description 1', 'destination_node_ids': [
            'node_3'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': 'exp_1', 'status': 'Published',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': 100.0,
            'unpublishing_reason': None}
        node_2: story_domain.StoryNodeDict = {'id': 'node_2',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            ...
            'node_1'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': 'exp_2', 'status': 'Published',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': 100.0,
            'unpublishing_reason': None}
        self.story.story_contents.initial_node_id = 'node_2'
        ...
        self.assertEqual(self.story.story_contents.get_all_linked_exp_ids(),
            ['exp_3', 'exp_2'])
    ...
    def test_update_node_exploration_id_raises_error_when_introducing_duplicate_exploration_id(
        self) ->None:
        self.story.story_contents.next_node_id = 'node_3'
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
    ...
    def test_get_all_linked_exp_ids_ignores_nodes_without_linked_exp_id(self
        ) ->None:
        self.story.story_contents.next_node_id = 'node_3'
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
            ...
            'node_3'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': None, 'status': 'Published',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': 100.0,
            'unpublishing_reason': None}
        node_2: story_domain.StoryNodeDict = {'id': 'node_2',
    ...
    def test_get_linked_exp_ids_of_published_nodes(self) ->None:
        ...
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            ...
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 1',
            'description': 'Description 1', 'destination_node_ids': [
            'node_2'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': 'a', 'outline_is_finalized': False,
            'exploration_id': 'exp_1', 'status': 'Published',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        ...
        node_3: story_domain.StoryNodeDict = {'id': 'node_3',
            ...
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 1',
            'description': 'Description 1', 'destination_node_ids': [
            'node_4'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': 'a', 'outline_is_finalized': False,
            'exploration_id': 'exp_3', 'status': 'Ready to Publish',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            (node_1), story_domain.StoryNode.from_dict(node_2),
            story_domain.StoryNode.from_dict(node_3)]
        exp_ids = (self.story.story_contents.
            get_linked_exp_ids_of_published_nodes())
        self.assertEqual(exp_ids, ['exp_1', 'exp_2'])

    def test_get_published_node_count_counts_all_nodes_when_all_nodes_published(
        self) ->None:
        self.story.story_contents.next_node_id = 'node_3'
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
        ...
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            (node_1), story_domain.StoryNode.from_dict(node_2)]
        published_node_count = (self.story.story_contents.
            get_published_node_count())
    ...
    def test_update_story_contents_from_model_with_all_versions(self) ->None:
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            ...
            'node_3'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': 'a', 'outline_is_finalized': False,
            'exploration_id': 'exp_1', 'status': 'Draft',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        story_contents_dict_1: story_domain.StoryContentsDict = {'nodes': [
            node_1], 'initial_node_id': 'node_1', 'next_node_id': 'node_4'}
        version_dict: story_domain.VersionedStoryContentsDict = {
            'schema_version': 0, 'story_contents': story_contents_dict_1}
        self.assertEqual(version_dict['schema_version'], 0)
        story_domain.Story.update_story_contents_from_model(version_dict, 1,
            'node_1')
        self.assertEqual(version_dict['schema_version'], 2)
        self.assertIsNone(version_dict['story_contents']['nodes'][0][
            'thumbnail_filename'])
        self.assertIsNone(version_dict['story_contents']['nodes'][0][
            'thumbnail_bg_color'])
        self.assertEqual(version_dict['story_contents']['nodes'][0][
            'description'], 'Description 1')
        story_domain.Story.update_story_contents_from_model(version_dict, 2,
            'node_1')
        self.assertEqual(version_dict['schema_version'], 3)
        self.assertEqual(version_dict['story_contents']['nodes'][0][
            'description'], '')
        story_domain.Story.update_story_contents_from_model(version_dict, 3,
            'node_1')
        self.assertEqual(version_dict['schema_version'], 4)
        self.assertEqual(version_dict['story_contents']['nodes'][0][
            'thumbnail_size_in_bytes'], 21131)
        story_domain.Story.update_story_contents_from_model(version_dict, 4,
            'node_1')
    ...
    def test_get_node_with_corresponding_exp_id_with_valid_exp_id(self) ->None:
        ...
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            ...
            'node_3'], 'acquired_skill_ids': [], 'prerequisite_skill_ids':
            [], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': 'exp_1', 'status': None,
            'planned_publication_date_msecs': None, 'last_modified_msecs':
            None, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        self.story.story_contents.initial_node_id = 'node_1'
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            (node_1)]
        node_with_exp_1 = (self.story.story_contents.
            get_node_with_corresponding_exp_id('exp_1'))
    ...
    def test_all_nodes_visited(self) ->None:
        ...
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            ...
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 1',
            'description': 'Description 1', 'destination_node_ids': [
            'node_2'], 'acquired_skill_ids': ['skill_2'],
            'prerequisite_skill_ids': ['skill_1'], 'outline': '',
            'outline_is_finalized': False, 'exploration_id': 'exp_1',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_2: story_domain.StoryNodeDict = {'id': 'node_2',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 2',
            'description': 'Description 2', 'destination_node_ids': [],
            'acquired_skill_ids': ['skill_3'], 'prerequisite_skill_ids': [
            'skill_2'], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': 'exp_2', 'status': 'Draft',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        node_3: story_domain.StoryNodeDict = {'id': 'node_3',
            ...
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 3',
            'description': 'Description 3', 'destination_node_ids': [],
            'acquired_skill_ids': ['skill_4'], 'prerequisite_skill_ids': [
            'skill_3'], 'outline': '', 'outline_is_finalized': False,
            'exploration_id': 'exp_3', 'status': 'Draft',
            'planned_publication_date_msecs': 100.0, 'last_modified_msecs':
            100.0, 'first_publication_date_msecs': None,
            'unpublishing_reason': None}
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            (node_1), story_domain.StoryNode.from_dict(node_2),
            story_domain.StoryNode.from_dict(node_3)]
        node_1 = {'id': 'node_1', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131, 'title':
            'Title 1', 'description': 'Description 1',
            'destination_node_ids': ['node_2'], 'acquired_skill_ids': [
            'skill_2'], 'prerequisite_skill_ids': ['skill_1'], 'outline':
            '', 'outline_is_finalized': False, 'exploration_id': 'exp_1',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_2 = {'id': 'node_2', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131, 'title':
            'Title 2', 'description': 'Description 2',
            'destination_node_ids': [], 'acquired_skill_ids': ['skill_3'],
            'prerequisite_skill_ids': ['skill_2'], 'outline': '',
            'outline_is_finalized': False, 'exploration_id': 'exp_2',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_3 = {'id': 'node_2', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131, 'title':
            'Title 2', 'description': 'Description 2',
            'destination_node_ids': [], 'acquired_skill_ids': ['skill_4'],
            'prerequisite_skill_ids': ['skill_3'], 'outline': '',
            'outline_is_finalized': False, 'exploration_id': 'exp_3',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
        ...
        node_1 = {'id': 'node_1', 'title': 'Title 1', 'description':
            'Description 1', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131,
            'destination_node_ids': ['node_2'], 'acquired_skill_ids': [
            'skill_2'], 'prerequisite_skill_ids': ['skill_1'], 'outline':
            '', 'outline_is_finalized': False, 'exploration_id': 'exp_1',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
        ...
        node_2 = {'id': 'node_2', 'title': 'Title 2', 'description':
            'Description 2', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131,
            'destination_node_ids': ['node_3'], 'acquired_skill_ids': [
            'skill_3'], 'prerequisite_skill_ids': ['skill_2'], 'outline':
            '', 'outline_is_finalized': False, 'exploration_id': 'exp_2',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_3 = {'id': 'node_3', 'title': 'Title 2', 'description':
            'Description 3', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131,
            'destination_node_ids': [], 'acquired_skill_ids': ['skill_4'],
            'prerequisite_skill_ids': ['skill_3'], 'outline': '',
        ...
        node_1 = {'id': 'node_1', 'thumbnail_filename': 'image.svg',
            ...
            'Title 1', 'description': 'Description 1',
            'destination_node_ids': ['node_2'], 'acquired_skill_ids': [
            'skill_2'], 'prerequisite_skill_ids': ['skill_1', 'skill_0'],
            'outline': '', 'outline_is_finalized': False, 'exploration_id':
            'exp_1', 'status': 'Draft', 'planned_publication_date_msecs': 
            100.0, 'last_modified_msecs': 100.0,
            'first_publication_date_msecs': None, 'unpublishing_reason': None}
        node_2 = {'id': 'node_2', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            ...
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_3 = {'id': 'node_3', 'thumbnail_filename': 'image.svg',
            'thumbnail_bg_color': constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], 'thumbnail_size_in_bytes': 21131, 'title':
            'Title 3', 'description': 'Description 3',
            'destination_node_ids': [], 'acquired_skill_ids': [],
            'prerequisite_skill_ids': ['skill_4'], 'outline': '',
            'outline_is_finalized': False, 'exploration_id': 'exp_3',
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_4: story_domain.StoryNodeDict = {'id': 'node_4',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
            constants.ALLOWED_THUMBNAIL_BG_COLORS['chapter'][0],
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 4',
            'description': 'Description 4', 'destination_node_ids': [],
            'acquired_skill_ids': [], 'prerequisite_skill_ids': ['skill_2'],
            'outline': '', 'outline_is_finalized': False, 'exploration_id':
            'exp_4', 'status': 'Draft', 'planned_publication_date_msecs': 
            100.0, 'last_modified_msecs': 100.0,
            'first_publication_date_msecs': None, 'unpublishing_reason': None}
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
            (node_1), story_domain.StoryNode.from_dict(node_2),
            story_domain.StoryNode.from_dict(node_3), story_domain.
            StoryNode.from_dict(node_4)]
        self.story.validate()
    ...
    def test_rearrange_node_in_story_fail_with_invalid_to_index_value(self
        ...
        with self.assertRaisesRegex(Exception,
            'Expected to_index value to be a number, received a'):
    ...
    def test_rearrange_canonical_story_fail_with_out_of_bound_indexes(self
        ...
        node_1: story_domain.StoryNodeDict = {'id': 'node_1',
            ...
            'description': 'Description 1', 'destination_node_ids': [
            'node_2'], 'acquired_skill_ids': ['skill_2'],
            'prerequisite_skill_ids': ['skill_1', 'skill_0'], 'outline': '',
            'outline_is_finalized': False, 'exploration_id': None, 'status':
            'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        node_2: story_domain.StoryNodeDict = {'id': 'node_2',
            'thumbnail_filename': 'image.svg', 'thumbnail_bg_color':
            ...
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 2',
            'description': 'Description 2', 'destination_node_ids': [
            'node_4', 'node_3'], 'acquired_skill_ids': ['skill_3',
            'skill_4'], 'prerequisite_skill_ids': ['skill_2'], 'outline':
            '', 'outline_is_finalized': False, 'exploration_id': None,
            'status': 'Draft', 'planned_publication_date_msecs': 100.0,
            'last_modified_msecs': 100.0, 'first_publication_date_msecs':
            None, 'unpublishing_reason': None}
        self.story.story_contents.nodes = [story_domain.StoryNode.from_dict
    ...
    def test_rearrange_node_in_story(self) ->None:
        ...
        node_3: story_domain.StoryNodeDict = {'id': 'node_3',
            ...
            'thumbnail_size_in_bytes': 21131, 'title': 'Title 3',
            'description': 'Description 3', 'destination_node_ids': [],
            'acquired_skill_ids': [], 'prerequisite_skill_ids': ['skill_4'],
            'outline': '', 'outline_is_finalized': False, 'exploration_id':
            None, 'status': 'Draft', 'planned_publication_date_msecs': 
            100.0, 'last_modified_msecs': 100.0,
            'first_publication_date_msecs': None, 'unpublishing_reason': None}
    ...
    def test_story_contents_export_import(self) ->None:
        ...
        """
        story_node = story_domain.StoryNode(self.NODE_ID_1, 'Title',
            'Description', None, constants.ALLOWED_THUMBNAIL_BG_COLORS[
            'chapter'][0], None, [self.NODE_ID_2], [self.SKILL_ID_1], [self
            .SKILL_ID_2], 'Outline', False, self.EXP_ID, 'Draft', None,
            None, None, None)
        story_contents = story_domain.StoryContents([story_node], self.
            NODE_ID_1, '2')
        story_contents_dict = story_contents.to_dict()
        story_contents_from_dict = story_domain.StoryContents.from_dict(
            story_contents_dict)
        self.assertEqual(story_contents_from_dict.to_dict(),
            story_contents_dict)
    ...
    def test_validate_node_id(self) ->None:
        self.story.story_contents.nodes[0].destination_node_ids = [self.
            NODE_ID_1]
        self._assert_validation_error(
    ...
    def test_get_node_index_with_invalid_node_id(self) ->None:
        self.assertIsNone(self.story.story_contents.get_node_index(
    ...
    def test_story_summary_creation(self) ->None:
        curr_time = get_current_datetime_utc()
        story_summary = story_domain.StorySummary('story_id', 'title',
            'description', 'en', 1, ['Title 1'], '#F8BF74', 'image.svg',
            'story-frag-two', curr_time, curr_time)
        expected_dict = {'id': 'story_id', 'title': 'title', 'description':
            'description', 'language_code': 'en', 'version': 1,
            'node_titles': ['Title 1'], 'thumbnail_bg_color': '#F8BF74',
            'thumbnail_filename': 'image.svg', 'url_fragment':
            'story-frag-two', 'story_model_created_on': utils.
            get_time_in_millisecs(curr_time), 'story_model_last_updated':
            utils.get_time_in_millisecs(curr_time)}
...
class StorySummaryTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.story_summary_dict = {'story_model_created_on':
            time_in_millisecs, 'version': 1, 'story_model_last_updated':
            time_in_millisecs, 'description': 'description', 'title':
            'title', 'node_titles': ['Title 1', 'Title 2'],
            'thumbnail_bg_color': '#F8BF74', 'thumbnail_filename':
            'image.svg', 'language_code': 'en', 'id': 'story_id',
            'url_fragment': 'story-summary-frag'}
        self.story_summary = story_domain.StorySummary('story_id', 'title',
            'description', 'en', 1, ['Title 1', 'Title 2'], '#F8BF74',
            'image.svg', 'story-summary-frag', current_time, current_time)
    ...
    def test_validation_fails_with_lengthy_url_fragment(self) ->None:
        ...
        with self.assertRaisesRegex(utils.ValidationError, 
            'Story Url Fragment field should not exceed %d characters, received %s.'
             % (constants.MAX_CHARS_IN_STORY_URL_FRAGMENT, self.
    ...
    def test_validation_fails_with_invalid_chars_in_url_fragment(self) ->None:
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            'Story Url Fragment field contains invalid characters. Only lowercase words separated by hyphens are allowed. Received Abc Def!.'
            ):
            self.story_summary.validate()
...
class StoryPublicationTimelinessTest(test_utils.GenericTestBase):
    ...
    def test_story_publication_timeliness_gets_created(self) ->None:
        ...
        self.assertEqual(story_publication_timeliness.overdue_chapters, [
            'Chapter 1'])
        self.assertEqual(story_publication_timeliness.upcoming_chapters, [
            'Chapter 2'])
