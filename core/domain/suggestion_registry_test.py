...
class SuggestionEditStateContentUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.suggestion_dict: SuggestionEditStateContentDict = {'suggestion_id'
            : 'exploration.exp1.thread1', 'suggestion_type': feconf.
            SUGGESTION_TYPE_EDIT_STATE_CONTENT, 'target_type': feconf.
            ENTITY_TYPE_EXPLORATION, 'target_id': 'exp1',
            'target_version_at_submission': 1, 'status': suggestion_models.
            STATUS_ACCEPTED, 'author_name': 'author', 'final_reviewer_id':
            self.reviewer_id, 'change_cmd': {'cmd': exp_domain.
            CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain.
            STATE_PROPERTY_CONTENT, 'state_name': 'state_1', 'new_value':
            'new suggestion content', 'old_value': None}, 'score_category':
            'content.Algebra', 'language_code': None, 'last_updated': utils
            .get_time_in_millisecs(self.fake_date), 'created_on': utils.
            get_time_in_millisecs(self.fake_date), 'edited_by_reviewer': False}
    ...
    def test_create_suggestion_edit_state_content(self) ->None:
        ...
        observed_suggestion = suggestion_registry.SuggestionEditStateContent(
            ...
            expected_suggestion_dict['target_id'], expected_suggestion_dict
            ['target_version_at_submission'], expected_suggestion_dict[
            'status'], self.author_id, self.reviewer_id,
            expected_suggestion_dict['change_cmd'],
            expected_suggestion_dict['score_category'],
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        self.assertDictEqual(observed_suggestion.to_dict(),
    ...
    def test_validate_change_property_name(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionEditStateContent(
            ...
            fake_date, self.fake_date)
        suggestion.validate()
        suggestion.change_cmd.property_name = 'invalid_property'
        with self.assertRaisesRegex(utils.ValidationError,
            'Expected property_name to be content'):
            suggestion.validate()

    def test_validate_language_code_fails_when_language_codes_do_not_match(self
        ) ->None:
        expected_suggestion_dict = self.suggestion_dict
        suggestion = suggestion_registry.SuggestionEditStateContent(
            expected_suggestion_dict['suggestion_id'],
    ...
    def test_pre_update_validate_change_cmd(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionEditStateContent(
            ...
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        change_cmd = {'cmd': exp_domain.CMD_ADD_STATE, 'property_name':
            exp_domain.STATE_PROPERTY_CONTENT, 'state_name': suggestion.
            change_cmd.state_name, 'new_value': 'new suggestion content',
            'old_value': None}
        with self.assertRaisesRegex(utils.ValidationError,
            'The following extra attributes are present: new_value, old_value, property_name'
            ):
            suggestion.pre_update_validate(exp_domain.
                EditExpStatePropertyContentCmd(change_cmd))
    ...
    def test_pre_update_validate_change_property_name(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionEditStateContent(
            ...
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        change_cmd = {'cmd': exp_domain.CMD_EDIT_STATE_PROPERTY,
            'property_name': exp_domain.STATE_PROPERTY_PARAM_CHANGES,
            'state_name': suggestion.change_cmd.state_name, 'new_value':
            'new suggestion content', 'old_value': None}
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_update_validate_change_state_name(self) ->None:
        ...
        change_cmd = {'cmd': exp_domain.CMD_EDIT_STATE_PROPERTY,
            ...
            'state_name': 'invalid_state', 'new_value':
            'new suggestion content', 'old_value': None}
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_update_validate_change_new_value(self) ->None:
        ...
        new_content = state_domain.SubtitledHtml('content',
            '<p>new suggestion html</p>').to_dict()
        suggestion.change_cmd.new_value = new_content
        change_cmd: Dict[str, Union[Optional[str], state_domain.
            SubtitledHtmlDict]] = {'cmd': exp_domain.
            CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain.
            STATE_PROPERTY_CONTENT, 'state_name': suggestion.change_cmd.
            state_name, 'new_value': new_content, 'old_value': None}
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_update_validate_non_equal_change_cmd(self) ->None:
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            suggestion.pre_update_validate(exp_domain.
                EditExpStatePropertyContentCmd({'cmd': exp_domain.
                CMD_EDIT_EXPLORATION_PROPERTY, 'property_name': 'title',
                'new_value': 'Exploration 1 Albert title'}))

    def test_get_all_html_content_strings(self) ->None:
        change_dict: Dict[str, Union[Optional[str], Dict[str, str]]] = {'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_CONTENT, 'state_name': 'state_1', 'new_value':
            {'content_id': 'content', 'html': 'new suggestion content'},
            'old_value': None}
        suggestion = suggestion_registry.SuggestionEditStateContent(self.
            suggestion_dict['suggestion_id'], self.suggestion_dict[
            'target_id'], self.suggestion_dict[
            'target_version_at_submission'], self.suggestion_dict['status'],
            self.author_id, self.reviewer_id, change_dict, self.
            suggestion_dict['score_category'], self.suggestion_dict[
            'language_code'], False, self.fake_date, self.fake_date)
    ...
    def test_convert_html_in_suggestion_change(self) ->None:
        ...
        change_cmd: Dict[str, Union[str, Dict[str, str]]] = {'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_CONTENT, 'state_name': 'Introduction',
            'new_value': {'content_id': 'content', 'html':
            '<p>suggestion</p>'}, 'old_value': {'content_id': 'content',
            'html': html_content}}
        suggestion = suggestion_registry.SuggestionEditStateContent(self.
            suggestion_dict['suggestion_id'], self.suggestion_dict[
            'target_id'], self.suggestion_dict[
            'target_version_at_submission'], self.suggestion_dict['status'],
            self.author_id, self.reviewer_id, change_cmd, self.
            suggestion_dict['score_category'], self.suggestion_dict[
            'language_code'], False, self.fake_date, self.fake_date)
        suggestion.convert_html_in_suggestion_change(html_validation_service
            .add_math_content_to_math_rte_components)
        assert isinstance(suggestion.change_cmd.old_value, dict)
        self.assertEqual(suggestion.change_cmd.old_value['html'],
            expected_html_content)

    def test_get_target_entity_html_strings_returns_expected_strings(self
        ) ->None:
        change_dict: Dict[str, Union[str, Dict[str, str]]] = {'cmd':
            exp_domain.CMD_EDIT_STATE_PROPERTY, 'property_name': exp_domain
            .STATE_PROPERTY_CONTENT, 'state_name': 'state_1', 'new_value':
            {'content_id': 'content', 'html': 'new suggestion content'},
...
class SuggestionTranslateContentUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.suggestion_dict: suggestion_registry.BaseSuggestionDict = {
            'suggestion_id': 'exploration.exp1.thread1', 'suggestion_type':
            feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, 'target_type': feconf
            .ENTITY_TYPE_EXPLORATION, 'target_id': 'exp1',
            'target_version_at_submission': 1, 'status': suggestion_models.
            STATUS_ACCEPTED, 'author_name': 'author', 'final_reviewer_id':
            self.reviewer_id, 'change_cmd': {'cmd': exp_domain.
            CMD_ADD_WRITTEN_TRANSLATION, 'state_name': 'Introduction',
            'content_id': 'content', 'language_code': 'hi', 'content_html':
            '<p>This is a content.</p>', 'translation_html':
            '<p>This is translated html.</p>', 'data_format': 'html'},
            'score_category': 'translation.Algebra', 'language_code': 'hi',
            'last_updated': utils.get_time_in_millisecs(self.fake_date),
            'created_on': utils.get_time_in_millisecs(self.fake_date),
            'edited_by_reviewer': False}
        opportunity_models.ExplorationOpportunitySummaryModel(id='exp1',
            topic_id='Topic1', topic_name='New Topic', story_id='Story1',
            story_title='New Story', chapter_title='New chapter',
            content_count=10, translation_counts={},
            incomplete_translation_language_codes=[language['id'] for
            language in constants.SUPPORTED_AUDIO_LANGUAGES],
            language_codes_needing_voice_artists=['en']).put()
    ...
    def test_pre_update_validate_fails_for_invalid_change_cmd(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(
            ...
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        change = {'cmd': exp_domain.CMD_DELETE_STATE, 'state_name':
            'Introduction'}
        with self.assertRaisesRegex(utils.ValidationError, 
            'The new change_cmd cmd must be equal to %s' % exp_domain.
    ...
    def test_pre_update_validate_change_state_name(self) ->None:
        ...
        change = {'cmd': exp_domain.CMD_ADD_WRITTEN_TRANSLATION,
            'state_name': 'State 1', 'content_id': 'content',
            'language_code': 'hi', 'content_html':
            '<p>This is a content.</p>', 'translation_html':
            '<p>This is the updated translated html.</p>', 'data_format':
            'html'}
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_update_validate_change_language_code(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(
            ...
            fake_date, self.fake_date)
        change = {'cmd': exp_domain.CMD_ADD_WRITTEN_TRANSLATION,
            'state_name': 'Introduction', 'content_id': 'content',
            'language_code': 'en', 'content_html':
            '<p>This is a content.</p>', 'translation_html':
            '<p>This is the updated translated html.</p>', 'data_format':
            'html'}
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_update_validate_change_content_html(self) ->None:
        ...
        change = {'cmd': exp_domain.CMD_ADD_WRITTEN_TRANSLATION,
            'state_name': 'Introduction', 'content_id': 'content',
            'language_code': 'en', 'content_html':
            '<p>This is the changed content.</p>', 'translation_html':
            '<p>This is the updated translated html.</p>', 'data_format':
            'html'}
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_accept_validate_state_name(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(
            ...
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        exp_services.update_exploration(self.author_id, 'exp1', [exp_domain
            .ExplorationChange({'cmd': exp_domain.CMD_EDIT_STATE_PROPERTY,
            'property_name': exp_domain.STATE_PROPERTY_CONTENT, 'new_value':
            {'content_id': 'content', 'html': '<p>This is a content.</p>'},
            'state_name': 'Introduction'})], 'Added state')
        suggestion.change_cmd.state_name = 'Introduction'
        suggestion.pre_accept_validate()
        suggestion.change_cmd.state_name = 'invalid_state_name'
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_accept_suggestion_adds_translation_in_exploration(self) ->None:
        ...
        translations = (translation_fetchers.
            get_all_entity_translations_for_entity(feconf.
            TranslatableEntityType.EXPLORATION, exp.id, exp.version))
        self.assertEqual(len(translations), 0)
        suggestion = suggestion_registry.SuggestionTranslateContent(self.
            suggestion_dict['suggestion_id'], self.suggestion_dict[
            'target_id'], self.suggestion_dict[
            'target_version_at_submission'], self.suggestion_dict['status'],
            self.author_id, self.reviewer_id, self.suggestion_dict[
            'change_cmd'], self.suggestion_dict['score_category'], self.
            suggestion_dict['language_code'], False, self.fake_date, self.
            fake_date)
        suggestion.accept(
    ...
    def test_accept_suggestion_with_set_of_string_adds_translation(self
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(self.
            ...
            'target_id'], self.suggestion_dict[
            'target_version_at_submission'], self.suggestion_dict['status'],
            self.author_id, self.reviewer_id, {'cmd': exp_domain.
            CMD_ADD_WRITTEN_TRANSLATION, 'state_name': 'Introduction',
            'content_id': 'content', 'language_code': 'hi', 'content_html':
            ['text1', 'text2'], 'translation_html': ['translated text1',
            'translated text2'], 'data_format': 'set_of_normalized_string'},
            self.suggestion_dict['score_category'], self.suggestion_dict[
            'language_code'], False, self.fake_date, self.fake_date)
        suggestion.accept(
            'Accepted suggestion by translator: Add translation change.')
        translations = (translation_fetchers.
    ...
    def test_accept_suggestion_with_psedonymous_author_adds_translation(self
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(
            ...
            expected_suggestion_dict['target_id'], expected_suggestion_dict
            ['target_version_at_submission'], expected_suggestion_dict[
            'status'], self.PSEUDONYMOUS_ID, self.reviewer_id,
            expected_suggestion_dict['change_cmd'],
            expected_suggestion_dict['score_category'],
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
    ...
    def test_get_all_html_content_strings(self) ->None:
        suggestion = suggestion_registry.SuggestionTranslateContent(self.
            ...
            fake_date)
        actual_outcome_list = suggestion.get_all_html_content_strings()
        expected_outcome_list = ['<p>This is translated html.</p>',
    ...
    def test_get_all_html_content_strings_for_content_lists(self) ->None:
        suggestion = suggestion_registry.SuggestionTranslateContent(self.
            ...
            'language_code'], False, self.fake_date, self.fake_date)
        actual_outcome_list = suggestion.get_all_html_content_strings()
        expected_outcome_list = ['translated text1', 'translated text2',
            'text1', 'text2']
        self.assertEqual(expected_outcome_list, actual_outcome_list)
    ...
    def test_get_target_entity_html_strings_returns_expected_strings(self
        ...
        suggestion = suggestion_registry.SuggestionTranslateContent(self.
            ...
            fake_date)
        actual_outcome_list = suggestion.get_target_entity_html_strings()
        expected_outcome_list = [self.suggestion_dict['change_cmd'][
            'content_html']]
        self.assertEqual(expected_outcome_list, actual_outcome_list)
    ...
    def test_convert_html_in_suggestion_change(self) ->None:
        ...
        change_dict = {'cmd': exp_domain.CMD_ADD_WRITTEN_TRANSLATION,
            'state_name': 'Introduction', 'content_id': 'content',
            'language_code': 'hi', 'content_html': html_content,
        ...
        suggestion.convert_html_in_suggestion_change(html_validation_service
            .add_math_content_to_math_rte_components)
        self.assertEqual(suggestion.change_cmd.content_html,
            expected_html_content)


TestChangeDictType = Dict[str, Union[str, float, Dict[str, Union[
    state_domain.StateDict, int, str, List[str]]]]]
...
class SuggestionAddQuestionTest(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.reviewer_id = self.get_user_id_from_email(self.REVIEWER_EMAIL)
        self.suggestion_dict: suggestion_registry.BaseSuggestionDict = {
            'suggestion_id': 'skill1.thread1', 'suggestion_type': feconf.
            SUGGESTION_TYPE_ADD_QUESTION, 'target_type': feconf.
            ENTITY_TYPE_SKILL, 'target_id': 'skill1',
            'target_version_at_submission': 1, 'status': suggestion_models.
            STATUS_ACCEPTED, 'author_name': 'author', 'final_reviewer_id':
            self.reviewer_id, 'change_cmd': {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
            'question_state_data': self._create_valid_question_data(
            'default_state', content_id_generator).to_dict(),
            'language_code': 'en', 'question_state_data_schema_version':
            ...
            'skill_1', 'skill_difficulty': 0.3}, 'score_category':
            'question.topic_1', 'language_code': 'en', 'last_updated':
            utils.get_time_in_millisecs(self.fake_date), 'created_on':
            utils.get_time_in_millisecs(self.fake_date),
            'edited_by_reviewer': False}
    ...
    def test_validate_change_type(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            fake_date, self.fake_date)
        suggestion.validate()
        suggestion.change_cmd = 'invalid_change'
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_validate_change_question_dict(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            fake_date, self.fake_date)
        suggestion.validate()
        suggestion.change_cmd.question_dict = None
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_validate_change_question_state_data_schema_version(self) ->None:
        ...
        question_dict: question_domain.QuestionDict = (suggestion.
    ...
    def test_validate_change_skill_difficulty_none(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            fake_date, self.fake_date)
        suggestion.validate()
        suggestion.change_cmd.skill_difficulty = None
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_accept_validate_change_skill_id(self) ->None:
        ...
        suggestion.pre_accept_validate()
        suggestion.change_cmd.skill_id = None
        with self.assertRaisesRegex(utils.ValidationError,
    ...
    def test_pre_update_validate_change_cmd(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        change = {'cmd': question_domain.CMD_UPDATE_QUESTION_PROPERTY,
            'property_name': question_domain.
            QUESTION_PROPERTY_LANGUAGE_CODE, 'new_value': 'bn', 'old_value':
            'en'}
        with self.assertRaisesRegex(utils.ValidationError,
            'The new change_cmd cmd must be equal to create_new_fully_specified_question'
            ):
            suggestion.pre_update_validate(question_domain.QuestionChange(
                change))
    ...
    def test_pre_update_validate_change_skill_id(self) ->None:
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            fake_date, self.fake_date)
        content_id_generator = translation_domain.ContentIdGenerator()
        change: ChangeType = {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
            'question_state_data': self._create_valid_question_data(
            'default_state', content_id_generator).to_dict(),
            'language_code': 'en', 'question_state_data_schema_version':
            feconf.CURRENT_STATE_SCHEMA_VERSION, 'next_content_id_index':
            content_id_generator.next_content_id_index}, 'skill_id': 'skill_2'}
        with self.assertRaisesRegex(utils.ValidationError,
            'The new change_cmd skill_id must be equal to skill_1'):
            suggestion.pre_update_validate(question_domain.
                CreateNewFullySpecifiedQuestionCmd(change))
    ...
    def test_pre_update_validate_complains_if_nothing_changed(self) ->None:
        ...
        change: ChangeType = {'cmd': question_domain.
            ...
            feconf.CURRENT_STATE_SCHEMA_VERSION, 'next_content_id_index':
            content_id_generator.next_content_id_index}, 'skill_id':
            'skill_1', 'skill_difficulty': 0.3}
        suggestion = suggestion_registry.SuggestionAddQuestion(
            'exploration.exp1.thread1', 'exp1', 1, suggestion_models.
            STATUS_ACCEPTED, self.author_id, self.reviewer_id, change,
            'question.topic_1', 'en', False, self.fake_date, self.fake_date)
        content_id_generator = translation_domain.ContentIdGenerator()
        new_change: ChangeType = {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
            'question_state_data': self._create_valid_question_data(
            'default_state', content_id_generator).to_dict(),
            'language_code': 'en', 'question_state_data_schema_version':
    ...
    def test_pre_update_validate_accepts_a_change_in_skill_difficulty_only(self
        ) ->None:
        content_id_generator = translation_domain.ContentIdGenerator()
        change: ChangeType = {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
        ...
        new_change: ChangeType = {'cmd': question_domain.
            ...
            feconf.CURRENT_STATE_SCHEMA_VERSION, 'next_content_id_index':
            content_id_generator.next_content_id_index}, 'skill_id':
            'skill_1', 'skill_difficulty': 0.6}
        self.assertEqual(suggestion.pre_update_validate(question_domain.
            CreateNewFullySpecifiedQuestionSuggestionCmd(new_change)), None)
    ...
    def test_pre_update_validate_accepts_a_change_in_state_data_only(self
        ...
        new_change: ChangeType = {'cmd': question_domain.
            ...
            'question_state_data': self._create_valid_question_data(
            'default_state', content_id_generator).to_dict(),
            'language_code': 'hi', 'question_state_data_schema_version':
            feconf.CURRENT_STATE_SCHEMA_VERSION, 'next_content_id_index':
            content_id_generator.next_content_id_index}, 'skill_id':
            'skill_1', 'skill_difficulty': 0.3}
        self.assertEqual(suggestion.pre_update_validate(question_domain.
    ...
    def test_validate_language_code_fails_when_language_codes_do_not_match(self
        ...
        suggestion = suggestion_registry.SuggestionAddQuestion(
            ...
            expected_suggestion_dict['language_code'], False, self.
            fake_date, self.fake_date)
        assert isinstance(expected_suggestion_dict['change_cmd'][
            'question_dict'], dict)
        expected_question_dict = cast(question_domain.QuestionDict,
    ...
    def test_get_all_html_content_strings(self) ->None:
        suggestion = suggestion_registry.SuggestionAddQuestion(self.
            suggestion_dict['suggestion_id'], self.suggestion_dict[
            'target_id'], self.suggestion_dict[
            'target_version_at_submission'], self.suggestion_dict['status'],
            self.author_id, self.reviewer_id, self.suggestion_dict[
            ...
            fake_date)
        actual_outcome_list = suggestion.get_all_html_content_strings()
        expected_outcome_list = ['', '', '<p>This is a hint.</p>',
    ...
    def test_convert_html_in_suggestion_change(self) ->None:
        ...
        answer_group = {'outcome': {'dest': None, 'dest_if_really_stuck':
            None, 'feedback': {'content_id': 'feedback_1', 'html': ''},
            'labelled_as_correct': True, 'param_changes': [],
            'refresher_exploration_id': None,
            'missing_prerequisite_skill_id': None}, 'rule_specs': [{
            'inputs': {'x': 0}, 'rule_type': 'Equals'}], 'training_data': [
            ], 'tagged_skill_misconception_id': None}
        question_state_dict = {'content': {'content_id': 'content_1',
            'html': html_content}, 'recorded_voiceovers': {
            'voiceovers_mapping': {'content_1': {}, 'feedback_1': {},
            'feedback_2': {}, 'hint_1': {}, 'solution': {}}}, 'interaction':
            {'answer_groups': [answer_group],
            'confirmed_unclassified_answers': [], 'customization_args': {
            'choices': {'value': [{'html': 'option 1', 'content_id':
            'ca_choices_0'}]}, 'showChoicesInShuffledOrder': {'value': True
            }}, 'default_outcome': {'dest': None, 'dest_if_really_stuck':
            None, 'feedback': {'content_id': 'feedback_2', 'html':
            'Correct Answer'}, 'param_changes': [],
            'refresher_exploration_id': None, 'labelled_as_correct': True,
            'missing_prerequisite_skill_id': None}, 'hints': [{
            'hint_content': {'content_id': 'hint_1', 'html': 'Hint 1'}}],
            'solution': {'answer_is_exclusive': False, 'correct_answer': 0,
            'explanation': {'content_id': 'solution', 'html':
            '<p>This is a solution.</p>'}}, 'id': 'MultipleChoiceInput'},
            'param_changes': [], 'solicit_answer_details': False,
        ...
        suggestion_dict: suggestion_registry.BaseSuggestionDict = {
            ...
            self.reviewer_id, 'change_cmd': {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
            'question_state_data': question_state_dict, 'language_code':
            'en', 'question_state_data_schema_version': feconf.
            CURRENT_STATE_SCHEMA_VERSION, 'linked_skill_ids': ['skill_1'],
            'inapplicable_skill_misconception_ids': ['skillid12345-1']},
            'skill_id': 'skill_1', 'skill_difficulty': 0.3},
            'score_category': 'question.skill1', 'language_code': 'en',
            'last_updated': utils.get_time_in_millisecs(self.fake_date),
            'created_on': utils.get_time_in_millisecs(self.fake_date),
            'edited_by_reviewer': False}
        suggestion = suggestion_registry.SuggestionAddQuestion(suggestion_dict
            ['suggestion_id'], suggestion_dict['target_id'],
        ...
        question_dict: question_domain.QuestionDict = (suggestion.
            change_cmd.question_dict)
        self.assertEqual(question_dict['question_state_data']['content'][
            'html'], expected_html_content)

    def test_accept_suggestion_with_images(self) ->None:
        html_content = (
            '<p>Value</p><oppia-noninteractive-math math_content-with-value="{&amp;quot;raw_latex&amp;quot;: &amp;quot;+,-,-,+&amp;quot;, &amp;quot;svg_filename&amp;quot;: &amp;quot;img.svg&amp;quot;}"></oppia-noninteractive-math>'
            )
        content_id_generator = translation_domain.ContentIdGenerator()
        question_state_dict = self._create_valid_question_data('default_state',
        ...
        with utils.open_file(os.path.join(feconf.TESTS_DATA_DIR,
            'test_svg.svg'), 'rb', encoding=None) as f:
            raw_image = f.read()
        image_context = feconf.IMAGE_CONTEXT_QUESTION_SUGGESTIONS
        fs_services.save_original_and_compressed_versions_of_image('img.svg',
            image_context, 'skill1', raw_image, 'image', False)
        ...
        suggestion_dict: suggestion_registry.BaseSuggestionDict = {
            ...
            'next_content_id_index': content_id_generator.
            next_content_id_index}, 'skill_id': 'skill1',
            'skill_difficulty': 0.3}, 'score_category': 'question.skill1',
            'language_code': 'en', 'last_updated': utils.
            get_time_in_millisecs(self.fake_date), 'created_on': utils.
            get_time_in_millisecs(self.fake_date), 'edited_by_reviewer': False}
        suggestion = suggestion_registry.SuggestionAddQuestion(suggestion_dict
            ['suggestion_id'], suggestion_dict['target_id'],
    ...
    def test_accept_suggestion_with_image_region_interactions(self) ->None:
        ...
        fs_services.save_original_and_compressed_versions_of_image('image.png',
            'question_suggestions', 'skill1', original_image_content,
            'image', True)
        image_and_region_ca_dict: domain.ImageAndRegionDict = {'imagePath':
            'image.png', 'labeledRegions': [{'label': 'Region1', 'region':
            {'regionType': 'Rectangle', 'area': [[0.2644628099173554, 
            0.21807065217391305], [0.9201101928374655, 0.8847373188405797]]}}]}
        question_state_dict: state_domain.StateDict = {'content': {'html':
            '<p>Text</p>', 'content_id': 'content_0'},
            'classifier_model_id': None, 'linked_skill_id': None,
            'interaction': {'answer_groups': [{'rule_specs': [{'rule_type':
            'IsInRegion', 'inputs': {'x': 'Region1'}}], 'outcome': {'dest':
            None, 'dest_if_really_stuck': None, 'feedback': {'html':
            '<p>assas</p>', 'content_id': 'feedback_2'},
            'labelled_as_correct': True, 'param_changes': [],
            'refresher_exploration_id': None,
            ...
            'highlightRegionsOnHover': {'value': False}}, 'default_outcome':
            {'dest': None, 'dest_if_really_stuck': None, 'feedback': {
            'html': '<p>wer</p>', 'content_id': 'default_outcome_1'},
            'labelled_as_correct': False, 'param_changes': [],
            'refresher_exploration_id': None,
            'missing_prerequisite_skill_id': None}, 'hints': [{
            'hint_content': {'html': '<p>assaas</p>', 'content_id':
            'hint_3'}}], 'id': 'ImageClickInput', 'solution': None},
            'param_changes': [], 'recorded_voiceovers': {
            'voiceovers_mapping': {'content_0': {}, 'default_outcome_1': {},
            'feedback_2': {}, 'hint_3': {}}}, 'solicit_answer_details': 
            False, 'card_is_checkpoint': False,
            'inapplicable_skill_misconception_ids': []}
        suggestion_dict: suggestion_registry.BaseSuggestionDict = {
            'suggestion_id': 'skill1.thread1', 'suggestion_type': feconf.
            SUGGESTION_TYPE_ADD_QUESTION, 'target_type': feconf.
            ...
            'question_state_data': question_state_dict, 'language_code':
            'en', 'question_state_data_schema_version': feconf.
            CURRENT_STATE_SCHEMA_VERSION, 'linked_skill_ids': ['skill1'],
            'next_content_id_index': 4,
            'inapplicable_skill_misconception_ids': []}, 'skill_id':
            'skill1', 'skill_difficulty': 0.3}, 'score_category':
            'question.skill1', 'language_code': 'en', 'last_updated': utils
            .get_time_in_millisecs(self.fake_date), 'created_on': utils.
            get_time_in_millisecs(self.fake_date), 'edited_by_reviewer': False}
        self.save_new_skill('skill1', self.author_id, description='description'
            )
        suggestion = suggestion_registry.SuggestionAddQuestion(suggestion_dict
            ...
            self.fake_date)
        suggestion.accept('commit_message')
        question = question_services.get_questions_by_skill_ids(1, [
            'skill1'], False)[0]
        destination_fs = fs_services.GcsFileSystem(feconf.
            ENTITY_TYPE_QUESTION, question.id)
    ...
    def test_contructor_updates_state_shema_in_change_cmd(self) ->None:
        score_category = '%s%sskill_id' % (suggestion_models.
            SCORE_TYPE_QUESTION, suggestion_models.SCORE_CATEGORY_DELIMITER)
        change: TestChangeDictType = {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
            'question_state_data': self.VERSION_27_STATE_DICT,
            'question_state_data_schema_version': 27, 'language_code': 'en',
            'linked_skill_ids': ['skill_id'],
            'inapplicable_skill_misconception_ids': []}, 'skill_id':
            'skill_id', 'skill_difficulty': 0.3}
        assert isinstance(change['question_dict'], dict)
        self.assertEqual(change['question_dict'][
            'question_state_data_schema_version'], 27)
        suggestion = suggestion_registry.SuggestionAddQuestion('suggestionId',
            'target_id', 1, suggestion_models.STATUS_IN_REVIEW, self.
            author_id, 'test_reviewer', change, score_category, 'en', False,
            self.fake_date, self.fake_date)
        assert isinstance(suggestion.change_cmd.question_dict, dict)
        self.assertEqual(suggestion.change_cmd.question_dict[
    ...
    def test_contructor_raise_exception_for_invalid_state_shema_version(self
        ...
        change: TestChangeDictType = {'cmd': question_domain.
            CMD_CREATE_NEW_FULLY_SPECIFIED_QUESTION, 'question_dict': {
            'question_state_data': self.VERSION_27_STATE_DICT,
            'question_state_data_schema_version': 23, 'language_code': 'en',
            'linked_skill_ids': ['skill_id'],
            'inapplicable_skill_misconception_ids': []}, 'skill_id':
            'skill_id', 'skill_difficulty': 0.3}
        assert isinstance(change['question_dict'], dict)
        ...
        with self.assertRaisesRegex(utils.ValidationError,
            ...
            suggestion_registry.SuggestionAddQuestion('suggestionId',
                'target_id', 1, suggestion_models.STATUS_IN_REVIEW, self.
                author_id, 'test_reviewer', change, score_category, 'en', 
...
class CommunityContributionStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_initial_object_with_valid_arguments_has_correct_properties(self
        ...
        community_contribution_stats = (suggestion_registry.
            CommunityContributionStats(self.
            translation_reviewer_counts_by_lang_code, self.
            translation_suggestion_counts_by_lang_code, self.
            question_reviewer_count, self.question_suggestion_count))
    ...
    def test_set_translation_reviewer_count_for_lang_code_updates_empty_dict(
        ...
        self.assertDictEqual(community_contribution_stats.
            translation_reviewer_counts_by_lang_code, {self.
            sample_language_code: 2})
    ...
    def test_set_translation_reviewer_count_for_lang_code_adds_new_lang_key(
        ...
        community_contribution_stats.set_translation_reviewer_count_for_language_code(
            'hi', 2)
        self.assertDictEqual(community_contribution_stats.
            translation_reviewer_counts_by_lang_code, {'en': 1, 'hi': 2})
    ...
    def test_set_translation_suggestion_count_for_lang_code_adds_new_lang_key(
        ...
        self._assert_community_contribution_stats_is_in_default_state()
        (community_contribution_stats.
            translation_suggestion_counts_by_lang_code) = {'en': 1}
        community_contribution_stats.set_translation_suggestion_count_for_language_code(
            'hi', 2)
        self.assertDictEqual(community_contribution_stats.
            translation_suggestion_counts_by_lang_code, {'en': 1, 'hi': 2})

    def test_get_translation_language_codes_that_need_reviewers_for_one_lang(
        self) ->None:
        stats = suggestion_services.get_community_contribution_stats()
        stats.set_translation_suggestion_count_for_language_code(self.
            sample_language_code, 1)
        language_codes_that_need_reviewers = (stats.
            get_translation_language_codes_that_need_reviewers())
        self.assertEqual(language_codes_that_need_reviewers, {self.
    ...
    def test_translation_reviewers_are_needed_if_suggestions_but_no_reviewers(
        ...
        self.assertTrue(stats.
            are_translation_reviewers_needed_for_lang_code(self.
            sample_language_code))
    ...
    def test_translation_reviewers_are_needed_if_num_suggestions_past_max(self
        ...
        stats.set_translation_suggestion_count_for_language_code(self.
            sample_language_code, 2)
        stats.set_translation_reviewer_count_for_language_code(self.
            sample_language_code, 1)
        reviewers_are_needed = (stats.
            are_translation_reviewers_needed_for_lang_code(self.
            sample_language_code))
    ...
    def test_question_reviewers_are_needed_if_suggestions_zero_reviewers(self
        ) ->None:
        stats = suggestion_services.get_community_contribution_stats()
    ...
    def test_question_reviewers_not_needed_if_no_reviewers_no_sugestions(self
        ) ->None:
        stats = suggestion_services.get_community_contribution_stats()
        self._assert_community_contribution_stats_is_in_default_state()
    ...
    def test_validate_question_reviewer_count_fails_for_negative_count(self
        ...
        community_contribution_stats = (suggestion_services.
            get_community_contribution_stats())
        community_contribution_stats.question_reviewer_count = (self.
            negative_count)
        with self.assertRaisesRegex(utils.ValidationError, 
    ...
    def test_validate_question_suggestion_count_fails_for_negative_count(self
        ...
        community_contribution_stats = (suggestion_services.
            get_community_contribution_stats())
        community_contribution_stats.question_suggestion_count = (self.
            negative_count)
        with self.assertRaisesRegex(utils.ValidationError, 
    ...
    def test_validate_question_reviewer_count_fails_for_non_integer_count(self
        ) ->None:
        community_contribution_stats = (suggestion_services.
            get_community_contribution_stats())
        community_contribution_stats.question_reviewer_count = (self.
...
class ReviewableSuggestionEmailInfoUnitTests(test_utils.GenericTestBase):
    ...
    submission_datetime: datetime.datetime = get_current_datetime_utc()

    def test_initial_object_with_valid_arguments_has_correct_properties(self
        ) ->None:
        reviewable_suggestion_email_info = (suggestion_registry.
            ReviewableSuggestionEmailInfo(self.suggestion_type, self.
            language_code, self.suggestion_content, self.submission_datetime))
        self.assertEqual(reviewable_suggestion_email_info.suggestion_type,
            self.suggestion_type)
        self.assertEqual(reviewable_suggestion_email_info.language_code,
            self.language_code)
        self.assertEqual(reviewable_suggestion_email_info.
...
class TranslationReviewStatsUnitTests(test_utils.GenericTestBase):
    ...
    FIRST_CONTRIBUTION_DATE: Final = from_milliseconds_utc(1616173836 * 1000.0
        ).date
    LAST_CONTRIBUTION_DATE: Final = from_milliseconds_utc(1616173836 * 1000.0
        ).date

    def test_create_translation_review_stats(self) ->None:
        expected_stats_dict = {'language_code': self.LANGUAGE_CODE,
            'contributor_user_id': self.CONTRIBUTOR_USER_ID, 'topic_id':
            self.TOPIC_ID, 'reviewed_translations_count': self.
        ...
        actual_stats = suggestion_registry.TranslationReviewStats(self.
            LANGUAGE_CODE, self.CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.
            REVIEWED_TRANSLATIONS_COUNT, self.
            REVIEWED_TRANSLATION_WORD_COUNT, self.
            ACCEPTED_TRANSLATIONS_COUNT, self.
            ACCEPTED_TRANSLATION_WORD_COUNT, self.
            ACCEPTED_TRANSLATIONS_WITH_REVIEWER_EDITS_COUNT, self.
            FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE)
...
class QuestionContributionStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_question_contribution_stats(self) ->None:
        expected_stats_dict = {'contributor_user_id': self.
            ...
            'first_contribution_date': self.FIRST_CONTRIBUTION_DATE,
            'last_contribution_date': self.LAST_CONTRIBUTION_DATE}
        actual_stats = suggestion_registry.QuestionContributionStats(self.
            CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.
            SUBMITTED_QUESTION_COUNT, self.ACCEPTED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_WITHOUT_REVIEWER_EDITS_COUNT, self.
            FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE)
        self.assertDictEqual(actual_stats.to_dict(), expected_stats_dict)
...
class QuestionReviewStatsUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_question_review_stats(self) ->None:
        expected_stats_dict = {'contributor_user_id': self.
            ...
            'first_contribution_date': self.FIRST_CONTRIBUTION_DATE,
            'last_contribution_date': self.LAST_CONTRIBUTION_DATE}
        actual_stats = suggestion_registry.QuestionReviewStats(self.
            CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.
            REVIEWED_QUESTIONS_COUNT, self.ACCEPTED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_WITH_REVIEWER_EDITS_COUNT, self.
            FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE)
        self.assertDictEqual(actual_stats.to_dict(), expected_stats_dict)
...
class ContributorMilestoneEmailInfoUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_contribution_milestone_email_info(self) ->None:
        actual_info = suggestion_registry.ContributorMilestoneEmailInfo(self
            .CONTRIBUTOR_USER_ID, self.CONTRIBUTION_TYPE, self.
            CONTRIBUTION_SUBTYPE, self.LANGUAGE_CODE, self.RANK_NAME)
        self.assertEqual(actual_info.contributor_user_id, self.
            CONTRIBUTOR_USER_ID)
        self.assertEqual(actual_info.contribution_type, self.CONTRIBUTION_TYPE)
        self.assertEqual(actual_info.contribution_subtype, self.
...
class ContributorStatsSummaryUnitTests(test_utils.GenericTestBase):
    ...
    def test_create_contribution_stats_summary(self) ->None:
        expected_translation_contribution_stats = {'language_code': self.
            LANGUAGE_CODE, 'contributor_user_id': self.CONTRIBUTOR_USER_ID,
            'topic_id': self.TOPIC_ID, 'submitted_translations_count': self
            ...
            CONTRIBUTION_DATES}
        expected_translation_review_stats = {'language_code': self.
            LANGUAGE_CODE, 'contributor_user_id': self.CONTRIBUTOR_USER_ID,
            ...
            'first_contribution_date': self.FIRST_CONTRIBUTION_DATE,
            'last_contribution_date': self.LAST_CONTRIBUTION_DATE}
        expected_question_contribution_stats = {'contributor_user_id': self
            .CONTRIBUTOR_USER_ID, 'topic_id': self.TOPIC_ID,
            'submitted_questions_count': self.SUBMITTED_QUESTION_COUNT,
            'accepted_questions_count': self.ACCEPTED_QUESTIONS_COUNT,
            'accepted_questions_without_reviewer_edits_count': self.
            ...
            'first_contribution_date': self.FIRST_CONTRIBUTION_DATE,
            'last_contribution_date': self.LAST_CONTRIBUTION_DATE}
        expected_question_review_stats = {'contributor_user_id': self.
            CONTRIBUTOR_USER_ID, 'topic_id': self.TOPIC_ID,
            'reviewed_questions_count': self.REVIEWED_QUESTIONS_COUNT,
            'accepted_questions_count': self.ACCEPTED_QUESTIONS_COUNT,
            'accepted_questions_with_reviewer_edits_count': self.
            ...
            'first_contribution_date': self.FIRST_CONTRIBUTION_DATE,
            'last_contribution_date': self.LAST_CONTRIBUTION_DATE}
        expected_contribution_summary = {'contributor_user_id': self.
            CONTRIBUTOR_USER_ID, 'translation_contribution_stats': [
        ...
        translation_contribution_stats = (suggestion_registry.
            TranslationContributionStats(self.LANGUAGE_CODE, self.
            CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.
            ...
            ACCEPTED_TRANSLATIONS_WITHOUT_REVIEWER_EDITS_COUNT, self.
            ACCEPTED_TRANSLATION_WORD_COUNT, self.
            REJECTED_TRANSLATIONS_COUNT, self.
            REJECTED_TRANSLATION_WORD_COUNT, self.CONTRIBUTION_DATES))
        translation_review_stats = suggestion_registry.TranslationReviewStats(
            ...
            self.REVIEWED_TRANSLATIONS_COUNT, self.
            REVIEWED_TRANSLATION_WORD_COUNT, self.
            ACCEPTED_TRANSLATIONS_COUNT, self.
            ACCEPTED_TRANSLATION_WORD_COUNT, self.
            ACCEPTED_TRANSLATIONS_WITH_REVIEWER_EDITS_COUNT, self.
            FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE)
        question_contribution_stats = (suggestion_registry.
            QuestionContributionStats(self.CONTRIBUTOR_USER_ID, self.
            TOPIC_ID, self.SUBMITTED_QUESTION_COUNT, self.
            ACCEPTED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_WITHOUT_REVIEWER_EDITS_COUNT, self.
            FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE))
        question_review_stats = suggestion_registry.QuestionReviewStats(self
            .CONTRIBUTOR_USER_ID, self.TOPIC_ID, self.
            REVIEWED_QUESTIONS_COUNT, self.ACCEPTED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_WITH_REVIEWER_EDITS_COUNT, self.
            FIRST_CONTRIBUTION_DATE, self.LAST_CONTRIBUTION_DATE)
        contribution_summary = suggestion_registry.ContributorStatsSummary(self
            .CONTRIBUTOR_USER_ID, [translation_contribution_stats], [
            question_contribution_stats], [translation_review_stats], [
            question_review_stats])
        self.assertDictEqual(contribution_summary.to_dict(),
            expected_contribution_summary)


class TranslationSubmitterTotalContributionStatsUnitTests(test_utils.
    GenericTestBase):
    ...
    def test_to_frontend_dict(self) ->None:
        ...
        self.save_new_topic(topic_id_1, self.user_id, name='topic1',
            abbreviated_name='name1', url_fragment='name-one', description=
            'Description', canonical_story_ids=[self.story_id_1, self.
            story_id_2], additional_story_ids=[self.story_id_3],
            uncategorized_skill_ids=[self.skill_id_1, self.skill_id_2],
            subtopics=[], next_subtopic_id=1)
        self.save_new_topic(topic_id_2, self.user_id, name='topic2',
            abbreviated_name='name2', url_fragment='name-two', description=
            'Description', canonical_story_ids=[self.story_id_1, self.
            story_id_2], additional_story_ids=[self.story_id_3],
            uncategorized_skill_ids=[self.skill_id_1, self.skill_id_2],
            subtopics=[], next_subtopic_id=1)
        expected_frontend_dict = {'language_code': self.
            SUGGESTION_LANGUAGE_CODE, 'contributor_name': username,
        ...
        actual_stats = (suggestion_registry.
            TranslationSubmitterTotalContributionStats(self.
            SUGGESTION_LANGUAGE_CODE, user_settings.user_id, [topic_id_1,
            topic_id_2], self.RECENT_REVIEW_OUTCOMES, self.
            RECENT_PERFORMANCE, self.OVERALL_ACCURACY, self.
            SUBMITTED_TRANSLATIONS_COUNT, self.
            SUBMITTED_TRANSLATION_WORD_COUNT, self.
            ACCEPTED_TRANSLATIONS_COUNT, self.
            ACCEPTED_TRANSLATIONS_WITHOUT_REVIEWER_EDITS_COUNT, self.
            ACCEPTED_TRANSLATION_WORD_COUNT, self.
            REJECTED_TRANSLATIONS_COUNT, self.
            REJECTED_TRANSLATION_WORD_COUNT, self.FIRST_CONTRIBUTION_DATE,
            self.LAST_CONTRIBUTION_DATE))
        self.assertDictEqual(actual_stats.to_frontend_dict(),
            expected_frontend_dict)


class TranslationReviewerTotalContributionStatsUnitTests(test_utils.
    GenericTestBase):
    ...
    def test_to_frontend_dict(self) ->None:
        ...
        self.save_new_topic(topic_id_2, self.user_id, name='topic2',
            ...
            uncategorized_skill_ids=[self.skill_id_1, self.skill_id_2],
            subtopics=[], next_subtopic_id=1)
        expected_stats_dict = {'language_code': self.
            SUGGESTION_LANGUAGE_CODE, 'contributor_name': username,
            ...
            (self.LAST_CONTRIBUTION_DATE)}
        actual_stats = (suggestion_registry.
            TranslationReviewerTotalContributionStats(self.
            SUGGESTION_LANGUAGE_CODE, user_settings.user_id, [topic_id_1,
            topic_id_2], self.REVIEWED_TRANSLATIONS_COUNT, self.
            ACCEPTED_TRANSLATIONS_COUNT, self.
            ACCEPTED_TRANSLATIONS_WITH_REVIEWER_EDITS_COUNT, self.
            ACCEPTED_TRANSLATION_WORD_COUNT, self.
            REJECTED_TRANSLATIONS_COUNT, self.FIRST_CONTRIBUTION_DATE, self
            .LAST_CONTRIBUTION_DATE))
        self.assertDictEqual(actual_stats.to_frontend_dict(),
            expected_stats_dict)


class QuestionSubmitterTotalContributionStatsUnitTests(test_utils.
    GenericTestBase):
    ...
    def test_to_frontend_dict(self) ->None:
        ...
        self.save_new_topic(topic_id_2, self.user_id, name='topic2',
            ...
            uncategorized_skill_ids=[self.skill_id_1, self.skill_id_2],
            subtopics=[], next_subtopic_id=1)
        expected_stats_dict = {'contributor_name': username, 'topic_names':
            self.TOPIC_IDS_WITH_QUESTION_SUBMISSIONS, 'recent_performance':
            self.RECENT_PERFORMANCE, 'overall_accuracy': self.
            OVERALL_ACCURACY, 'submitted_questions_count': self.
            SUBMITTED_QUESTIONS_COUNT, 'accepted_questions_count': self.
            ACCEPTED_QUESTIONS_COUNT,
            'accepted_questions_without_reviewer_edits_count': self.
            ACCEPTED_QUESTIONS_WITHOUT_REVIEWER_EDITS_COUNT,
            'rejected_questions_count': self.REJECTED_QUESTIONS_COUNT,
            'first_contribution_date': self.FIRST_CONTRIBUTION_DATE.
            strftime('%b %d, %Y'), 'last_contributed_in_days': utils.
            get_number_of_days_since_date(self.LAST_CONTRIBUTION_DATE)}
        actual_stats = (suggestion_registry.
            QuestionSubmitterTotalContributionStats(user_settings.user_id,
            [topic_id_1, topic_id_2], self.RECENT_REVIEW_OUTCOMES, self.
            RECENT_PERFORMANCE, self.OVERALL_ACCURACY, self.
            SUBMITTED_QUESTIONS_COUNT, self.ACCEPTED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_WITHOUT_REVIEWER_EDITS_COUNT, self.
            REJECTED_QUESTIONS_COUNT, self.FIRST_CONTRIBUTION_DATE, self.
            LAST_CONTRIBUTION_DATE))
        self.assertDictEqual(actual_stats.to_frontend_dict(),
            expected_stats_dict)


class QuestionReviewerTotalContributionStatsUnitTests(test_utils.
    GenericTestBase):
    ...
    def test_to_frontend_dict(self) ->None:
        ...
        expected_stats_dict = {'contributor_name': username, 'topic_names':
            ...
            get_number_of_days_since_date(self.LAST_CONTRIBUTION_DATE)}
        actual_stats = (suggestion_registry.
            QuestionReviewerTotalContributionStats(user_settings.user_id, [
            topic_id_1, topic_id_2], self.REVIEWED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_COUNT, self.
            ACCEPTED_QUESTIONS_WITH_REVIEWER_EDITS_COUNT, self.
            REJECTED_QUESTIONS_COUNT, self.FIRST_CONTRIBUTION_DATE, self.
            LAST_CONTRIBUTION_DATE))
