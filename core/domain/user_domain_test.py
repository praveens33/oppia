...
class MockModifiableUserData(user_domain.ModifiableUserData):
    ...
    def __init__(self, display_alias: str, pin: Optional[str],
        ...
        Optional[str]=None, fake_field: Optional[str]=None) ->None:
        super().__init__(display_alias, pin, preferred_language_codes,
            preferred_site_language_code, preferred_audio_language_code,
            preferred_translation_language_code, user_id=None)
        self.fake_field = fake_field
    CURRENT_SCHEMA_VERSION = 2

    @classmethod
    def from_dict(cls, modifiable_user_data_dict: MockModifiableUserDataDict
        ) ->MockModifiableUserData:
        return MockModifiableUserData(modifiable_user_data_dict[
            'display_alias'], modifiable_user_data_dict['pin'],
    ...
    def _convert_v1_dict_to_v2_dict(cls, user_data_dict:
        MockModifiableUserDataDict) ->MockModifiableUserDataDict:
    ...
    @classmethod
    def from_raw_dict(cls, raw_user_data_dict: MockModifiableUserDataDict
        ) ->MockModifiableUserData:
...
class UserSettingsTests(test_utils.GenericTestBase):
    ...
    def test_validate_invalid_length_pin_raises_error(self) ->None:
        ...
        error_msg = 'User PIN can only be of length %s or %s' % (feconf.
    ...
    def test_validation_none__email_raises_error(self) ->None:
        ...
        with self.assertRaisesRegex(utils.ValidationError, 
            'Expected email to be a string, received %s' % self.
            user_settings.email):
            self.user_settings.validate()

    def test_validation_wrong_email_raises_error(self) ->None:
        invalid_emails_list = ['testemail.com', '@testemail.com',
            'testemail.com@']
    ...
    def test_cannot_get_human_readable_user_ids_with_invalid_user_ids(self
        ...
        assert_raises_user_not_found = self.assertRaisesRegex(Exception,
            'User not found.')
    ...
    def test_get_human_readable_user_ids(self) ->None:
        user_models.UserSettingsModel(id='unregistered_user_id', email=
            'user@example.com', username='').put()
        user_ids = user_services.get_human_readable_user_ids([self.owner_id,
            feconf.SYSTEM_COMMITTER_ID, 'unregistered_user_id'])
    ...
    def test_get_human_readable_user_ids_with_nonexistent_id_non_strict_passes(
        ...
        self.assertEqual(human_readable_user_ids, [user_services.
...
class UserGroupDomainTests(test_utils.GenericTestBase):
    ...
    def test_to_dict_returns_correct_user_group_dict(self) ->None:
        self.assertEqual(self.user_group.to_dict(), {'user_group_id':
            'USER_GROUP_ID', 'name': 'USERGROUPNAME', 'member_usernames': [
            'user1', 'user2', 'user3']})

    def test_user_group_from_dict(self) ->None:
        user_group_dict: user_domain.UserGroupDict = {'user_group_id':
            'USER_GROUP_ID', 'name': 'USERGROUPNAME', 'member_usernames': [
            'user1', 'user2', 'user3']}
...
class UserContributionsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.signup(self.OWNER_EMAIL, self.OWNER_USERNAME)
        self.owner_id = self.get_user_id_from_email(self.OWNER_EMAIL)
        self.user_contributions = user_services.get_user_contributions(self
            .owner_id, strict=True)
    ...
    def test_save_user_contributions(self) ->None:
        ...
        updated_contributions = user_services.get_user_contributions(self.
            owner_id, strict=True)
        self.assertEqual(updated_contributions.user_id, self.owner_id)
        self.assertEqual(updated_contributions.created_exploration_ids, [
            'e1', 'e3'])
        self.assertEqual(updated_contributions.edited_exploration_ids, [
            'e2', 'e4'])
    ...
    def test_cannot_update_dashboard_stats_log_with_invalid_schema_version(self
        ) ->None:
...
class UserGlobalPrefsTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        """Testing init method."""
        user_global_prefs = user_domain.UserGlobalPrefs(True, False, True, 
            False)
    ...
    def test_create_default_prefs(self) ->None:
        ...
        default_user_global_prefs = (user_domain.UserGlobalPrefs.
            create_default_prefs())
        self.assertEqual(default_user_global_prefs.
            can_receive_email_updates, feconf.DEFAULT_EMAIL_UPDATES_PREFERENCE)
        self.assertEqual(default_user_global_prefs.
            can_receive_editor_role_email, feconf.
            DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE)
        self.assertEqual(default_user_global_prefs.
            can_receive_feedback_message_email, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE)
        self.assertEqual(default_user_global_prefs.
            can_receive_subscription_email, feconf.
...
class UserExplorationPrefsTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        ...
        mute_feedback_notifications = (user_exp_prefs.
            mute_feedback_notifications)
        mute_suggestion_notifications = (user_exp_prefs.
    ...
    def test_create_default_prefs(self) ->None:
        """Testing create_default_prefs."""
        default_user_exp_prefs = (user_domain.UserExplorationPrefs.
            create_default_prefs())
        self.assertEqual(default_user_exp_prefs.mute_feedback_notifications,
            feconf.DEFAULT_FEEDBACK_NOTIFICATIONS_MUTED_PREFERENCE)
        self.assertEqual(default_user_exp_prefs.
    ...
    def test_to_dict(self) ->None:
        """Testing to_dict."""
        user_exp_prefs = user_domain.UserExplorationPrefs(False, True)
        default_user_global_prefs = (user_domain.UserExplorationPrefs.
            create_default_prefs())
        test_dict = user_exp_prefs.to_dict()
        default_dict = default_user_global_prefs.to_dict()
        self.assertEqual(test_dict, {'mute_feedback_notifications': False,
            'mute_suggestion_notifications': True})
        self.assertEqual(default_dict, {'mute_feedback_notifications':
            feconf.DEFAULT_FEEDBACK_NOTIFICATIONS_MUTED_PREFERENCE,
            'mute_suggestion_notifications': feconf.
            DEFAULT_SUGGESTION_NOTIFICATIONS_MUTED_PREFERENCE})
...
class ExpUserLastPlaythroughTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        ...
        exp_last_playthrough = user_domain.ExpUserLastPlaythrough('user_id0',
            'exp_id0', 0, current_time, 'state0')
...
class IncompleteActivitiesTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        """Testing init method."""
        incomplete_activities = user_domain.IncompleteActivities('user_id0',
            ['exp_id0'], ['collect_id0'], ['story_id0'], ['topic_id0'])
        ...
        self.assertListEqual(incomplete_activities.
            partially_learnt_topic_ids, ['topic_id0'])
    ...
    def test_add_partially_learnt_topic_id(self) ->None:
        ...
        self.assertListEqual(incomplete_activities.
            partially_learnt_topic_ids, ['topic_id0', 'topic_id1'])
...
class CompletedActivitiesTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        """Testing init method."""
        completed_activities = user_domain.CompletedActivities('user_id0',
            ['exp_id0'], ['collect_id0'], ['story_id0'], ['topic_id0'])
...
class LearnerPlaylistTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        """Testing init method."""
        learner_playlist = user_domain.LearnerPlaylist('user_id0', [
            'exp_id0'], ['collect_id0'])
    ...
    def test_insert_collection_id_at_given_position(self) ->None:
        ...
        learner_playlist = user_domain.LearnerPlaylist('user_id0', [
            'exp_id0'], ['collect_id0'])
        self.assertListEqual(learner_playlist.collection_ids, ['collect_id0'])
        learner_playlist.insert_collection_id_at_given_position('collect_id1',
            1)
        learner_playlist.insert_collection_id_at_given_position('collect_id2',
            1)
...
class UserContributionProficiencyTests(test_utils.GenericTestBase):
    ...
    def test_can_user_review_category(self) ->None:
        ...
        self.user_proficiency.increment_score(feconf.
            MINIMUM_SCORE_REQUIRED_TO_REVIEW)
...
class UserContributionRightsTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        ...
        self.assertEqual(self.user_contribution_rights.
            can_review_translation_for_language_codes, ['hi'])
        self.assertEqual(self.user_contribution_rights.
            can_review_voiceover_for_language_codes, [])
        self.assertEqual(self.user_contribution_rights.can_review_questions,
            True)
    ...
    def test_incorrect_language_code_for_translation_raise_error(self) ->None:
        (self.user_contribution_rights.
            can_review_translation_for_language_codes) = ['invalid_lang_code']
        with self.assertRaisesRegex(utils.ValidationError,
            'Invalid language_code: invalid_lang_code'):
            self.user_contribution_rights.validate()

    def test_can_review_voiceover_for_language_codes_with_duplicate_values(self
        ) ->None:
        (self.user_contribution_rights.can_review_voiceover_for_language_codes
            ) = ['hi']
        self.user_contribution_rights.validate()
    ...
    def test_can_review_translation_for_language_codes_with_duplicate_values(
        ...
        self.user_contribution_rights.validate()
        (self.user_contribution_rights.
            can_review_translation_for_language_codes) = ['hi', 'hi']
        with self.assertRaisesRegex(utils.ValidationError,
...
class ModifiableUserDataTests(test_utils.GenericTestBase):
    ...
    def test_initialization_with_none_user_id_is_successful(self) ->None:
        ...
        user_data_dict: user_domain.RawUserDataDict = {'schema_version': 1,
            'display_alias': 'display_alias', 'pin': '123',
        ...
        self.assertEqual(modifiable_user_data.preferred_language_codes, [
            'preferred_language_codes'])
        self.assertEqual(modifiable_user_data.preferred_site_language_code,
            'preferred_site_language_code')
        self.assertEqual(modifiable_user_data.preferred_audio_language_code,
            'preferred_audio_language_code')
        self.assertEqual(modifiable_user_data.
            preferred_translation_language_code,
            'preferred_translation_language_code')
    ...
    def test_initialization_with_valid_user_id_is_successful(self) ->None:
        ...
        user_data_dict: user_domain.RawUserDataDict = {'schema_version': 1,
            ...
            'preferred_audio_language_code',
            'preferred_translation_language_code':
            'preferred_translation_language_code', 'user_id': 'user_id'}
        modifiable_user_data = user_domain.ModifiableUserData.from_raw_dict(
            user_data_dict)
        self.assertEqual(modifiable_user_data.display_alias, 'display_alias')
        self.assertEqual(modifiable_user_data.pin, '123')
    ...
    def test_from_raw_dict_with_none_schema_version_raises_error(self) ->None:
        user_data_dict: user_domain.RawUserDataDict = {'schema_version':
            None, 'display_alias': 'display_alias', 'pin': '123',
            'preferred_language_codes': ['preferred_language_codes'],
            'preferred_site_language_code': 'preferred_site_language_code',
            'preferred_audio_language_code':
            'preferred_audio_language_code',
    ...
    def test_from_raw_dict_with_invalid_schema_version_raises_error(self
        ...
        user_data_dict: user_domain.RawUserDataDict = {'schema_version': 1,
            ...
            'preferred_translation_language_code':
            'preferred_translation_language_code', 'user_id': 'user_id'}
        current_version_plus_one = (user_domain.ModifiableUserData.
    ...
    def test_from_raw_dict_with_invalid_schema_version_type_raises_error(self
        ...
        version = '-1'
        user_data_dict['schema_version'] = version
        error_msg = ('Version has invalid type, expected int, received %s' %
            type(version))
        with self.assertRaisesRegex(Exception, error_msg):
            user_domain.ModifiableUserData.from_raw_dict(user_data_dict)
    ...
    def test_mock_modifiable_user_data_class_with_all_attributes_given(self
        ...
        user_data_dict: MockModifiableUserDataDict = {'schema_version': 2,
            'display_alias': 'name', 'pin': '123',
            'preferred_language_codes': ['en', 'es'],
            'preferred_site_language_code': 'es',
            'preferred_audio_language_code': 'en',
            'preferred_translation_language_code': 'en', 'user_id': None,
    ...
    def test_mock_migration_from_old_version_to_new_works_correctly(self
        ...
        user_data_dict: MockModifiableUserDataDict = {'schema_version': 1,
            'display_alias': 'name', 'pin': '123',
            'preferred_language_codes': ['en', 'es'],
            'preferred_site_language_code': 'es',
            'preferred_audio_language_code': 'en',
...
class ExplorationUserDataTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        ...
        expected_exploration_user_data_dict = {'rating': None, 'rated_on':
            None, 'draft_change_list': None,
            'draft_change_list_last_updated': None,
            'draft_change_list_exp_version': None, 'draft_change_list_id': 
            0, 'mute_suggestion_notifications': feconf.
            DEFAULT_SUGGESTION_NOTIFICATIONS_MUTED_PREFERENCE,
        ...
        self.assertEqual(exploration_user_data.to_dict(),
            expected_exploration_user_data_dict)

    def test_to_dict(self) ->None:
        exploration_user_data = user_domain.ExplorationUserData('user1',
            'exp1', 4, datetime.datetime(2022, 4, 1, 0, 0, 0, 0), None,
            None, None, 0, False, False, 1, 'checkpoint2', 2, 'checkpoint1')
        expected_exploration_user_data_dict = {'rating': 4, 'rated_on':
            datetime.datetime(2022, 4, 1, 0, 0, 0, 0), 'draft_change_list':
            None, 'draft_change_list_last_updated': None,
            'draft_change_list_exp_version': None, 'draft_change_list_id': 
            0, 'mute_suggestion_notifications': False,
...
class LearnerGroupUserDetailsTests(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        learner_group_user_details = user_domain.LearnerGroupUserDetails(
            'group_id_1', True)
        expected_learner_grp_user_details_dict = {'group_id': 'group_id_1',
            'progress_sharing_is_turned_on': True}
        self.assertEqual(learner_group_user_details.group_id, 'group_id_1')
        self.assertEqual(learner_group_user_details.
            progress_sharing_is_turned_on, True)
        self.assertEqual(learner_group_user_details.to_dict(),
...
class LearnerGroupsUserTest(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        learner_group_user_details = user_domain.LearnerGroupUserDetails(
            'group_id_1', False)
        learner_group_user = user_domain.LearnerGroupsUser('user1', [
            'group_id_2', 'group_id_3'], [learner_group_user_details], 1)
        expected_learner_group_user_dict = {'user_id': 'user1',
            'invited_to_learner_groups_ids': ['group_id_2', 'group_id_3'],
            'learner_groups_user_details': [{'group_id': 'group_id_1',
        ...
        self.assertEqual(learner_group_user.invited_to_learner_groups_ids,
            ['group_id_2', 'group_id_3'])
        self.assertEqual(learner_group_user.learner_groups_user_details, [
            learner_group_user_details])
        self.assertEqual(learner_group_user.
            learner_groups_user_details_schema_version, 1)
        self.assertEqual(learner_group_user.to_dict(),
    ...
    def test_validation(self) ->None:
        learner_group_user_details = user_domain.LearnerGroupUserDetails(
            'group_id_1', True)
        self._assert_validation_error(user_domain.LearnerGroupsUser('user1',
            ['group_id_1'], [learner_group_user_details], 1),
...
class TranslationCoordinatorStatsUnitTests(test_utils.GenericTestBase):
    ...
    expected_stats_dict = {'language_id': 'en', 'coordinator_ids': ['user1',
        'user2'], 'coordinators_count': 2}

    def test_to_dict(self) ->None:
        actual_stats = user_domain.TranslationCoordinatorStats('en', [
            'user1', 'user2'], 2)
...
class UserContributionRightsUnitTest(test_utils.GenericTestBase):
    ...
    def test_initialization(self) ->None:
        user_contribution_rights = user_domain.UserContributionRights('a',
            ['en', 'es'], ['fr'], True, False)
        ...
        self.assertEqual(user_contribution_rights.
            can_review_voiceover_for_language_codes, ['fr'])
    ...
    def test_can_submit_at_least_one_item(self) ->None:
        user_contribution_rights = user_domain.UserContributionRights('a',
            [], [], True, False)
        self.assertFalse(user_contribution_rights.
            can_submit_at_least_one_item())
