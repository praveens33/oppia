...
def _get_change_list(state_name: str, property_name: str, new_value: bool
...
class UserServicesUnitTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        user_data_dict: user_domain.RawUserDataDict = {'schema_version': 1,
            ...
            'preferred_translation_language_code': None, 'user_id': 'user_id'}
    ...
    def test_set_and_get_username(self) ->None:
        ...
        self.assertEqual(username, user_services.get_username(user_settings
            .user_id))
    ...
    def test_set_username_to_existing_username_raises_error(self) ->None:
        ...
        for i, auth_id in enumerate(auth_ids):
            user_ids.append(user_services.create_new_user(auth_id,
    ...
    def test_get_username_for_system_user(self) ->None:
        self.assertEqual(feconf.SYSTEM_COMMITTER_ID, user_services.
            get_username(feconf.SYSTEM_COMMITTER_ID))
        self.assertEqual(feconf.MIGRATION_BOT_USERNAME, user_services.
            get_username(feconf.MIGRATION_BOT_USER_ID))

    def test_get_username_for_pseudonymous_id(self) ->None:
        self.assertEqual('User_Aaaaaaaa', user_services.get_username(
            'pid_%s' % ('a' * 32)))
        self.assertEqual('User_Bbbbbbbb', user_services.get_username(
            'pid_%s' % ('b' * 32)))

    def test_get_usernames_for_pseudonymous_ids(self) ->None:
        self.assertEqual(['User_Aaaaaaaa', 'User_Bbbbbbbb'], user_services.
            get_usernames(['pid_%s' % ('a' * 32), 'pid_%s' % ('b' * 32)]))
    ...
    def test_get_usernames_system_admin(self) ->None:
        self.assertEqual([feconf.SYSTEM_COMMITTER_ID], user_services.
            get_usernames([feconf.SYSTEM_COMMITTER_ID]))
    ...
    def test_get_username_for_user_being_deleted(self) ->None:
        ...
        user_id = user_services.create_new_user(auth_id, 'user@example.com'
            ).user_id
        user_services.set_username(user_id, username)
        user_services.mark_user_for_deletion(user_id)
        self.assertEqual(user_services.get_username(user_id), user_services
            .USERNAME_FOR_USER_BEING_DELETED)

    def test_get_username_none(self) ->None:
        user_id = user_services.create_new_user('fakeUser', 'user@example.com'
            ).user_id
    ...
    def test_is_username_taken_when_deleted_user_had_same_username(self
        ...
        user_services.save_deleted_username(user_domain.UserSettings.
    ...
    def test_set_invalid_usernames(self) ->None:
        ...
        bad_usernames_with_expected_error_message = [(' bob ',
            'Usernames can only have alphanumeric characters.'), ('@',
            'Usernames can only have alphanumeric characters.'), ('',
            'Empty username supplied.'), ('a' * 100,
            'A username can have at most 30 characters.'), ('ADMIN',
            'This username is not available.'), ('admin',
            'This username is not available.'), ('AdMiN2020',
            'This username is not available.'), ('AbcOppiaMigrationBotXyz',
            'This username is not available.'), ('OppiaMigrATIONBOTXyz',
            'This username is not available.'), ('AbcOppiaSuggestionBotXyz',
            'This username is not available.'), ('AAAOPPIASuggestionBotBBB',
            'This username is not available.'), ('xyzOppia',
            'This username is not available.'), ('oppiaXyz',
            'This username is not available.'), ('abcOppiaXyz',
    ...
    def test_update_user_settings_for_invalid_display_alias_raises_error(self
        ...
        bad_display_aliases_with_expected_error = [('',
            'Expected display_alias to be a string, received .'), (0,
            'Expected display_alias to be a string, received 0.'), (None,
            'Expected display_alias to be a string, received None.')]
        ...
        for display_alias, error_msg in bad_display_aliases_with_expected_error:
            with self.assertRaisesRegex(utils.ValidationError, error_msg):
                ...
                user_services.update_multiple_users_data([self.
                    modifiable_new_user_data])
    ...
    def test_create_new_user_with_invalid_emails_raises_exception(self) ->None:
        bad_email_addresses_with_expected_error_message = [('@',
            ...
            'No user email specified.'), (None,
            'Expected email to be a string, received None'), (['a', '@',
            'b.com'], re.escape(
    ...
    def test_create_new_user_with_invalid_email_creates_no_user_models(self
        ...
        user_ids_in_user_settings = [model.id for model in user_models.
            UserSettingsModel.get_all()]
        user_ids_in_user_auth_details = [model.id for model in auth_models.
            UserAuthDetailsModel.get_all()]
        user_ids_in_user_contributions = [model.id for model in user_models
            .UserContributionsModel.get_all()]
    ...
    def test_email_truncation(self) ->None:
        email_addresses = [('a@b.c', '..@b.c'), ('ab@c.d', 'a..@c.d'), (
            'abc@def.gh', 'a..@def.gh'), ('abcd@efg.h', 'a..@efg.h'), (
            'abcdefgh@efg.h', 'abcde..@efg.h')]
        for ind, (actual_email, expected_email) in enumerate(email_addresses):
            user_settings = user_services.create_new_user(str(ind),
                actual_email)
    ...
    def test_get_user_id_from_username(self) ->None:
        ...
        user_services.set_username(user_settings.user_id, username)
        self.assertEqual(user_services.get_username(user_settings.user_id),
            username)
        self.assertEqual(user_services.get_user_id_from_username(username),
            user_settings.user_id)
        self.assertEqual(user_services.get_user_id_from_username('USERNAME'
            ), user_settings.user_id)
        self.assertIsNone(user_services.get_user_id_from_username(
            'fakeUsername'))
        with self.assertRaisesRegex(Exception,
    ...
    def test_get_multi_user_ids_from_usernames(self) ->None:
        ...
        with self.assertRaisesRegex(Exception,
            ...
            user_services.get_multi_user_ids_from_usernames([
                ...
                'fakeUsername4', 'fakeUsername5', 'fakeUsername6',
                'fakeUsername7', username2, 'fakeUsername9'], strict=True)
    ...
    def test_get_user_settings_from_username_for_no_username_is_none(self
        ...
        self.assertIsNone(user_services.get_user_settings_from_username(
            'fakeUsername'))
    ...
    def test_get_user_settings_from_email_for_nonexistent_email_is_none(self
        ...
        self.assertIsNone(user_services.get_user_settings_from_email(
    ...
    def test_get_user_settings_by_auth_id_for_nonexistent_auth_id_is_none(self
        ...
        self.assertIsNone(user_services.get_user_settings_by_auth_id(
            'auth_id_x'))
    ...
    def test_get_users_setting_retrieves_settings_for_system_user(self) ->None:
        ...
        less_than_time = get_current_datetime_utc()
        ...
        greater_than_time = get_current_datetime_utc()
    ...
    def test_fetch_gravatar_success(self) ->None:
        ...
        expected_gravatar_filepath = os.path.join(self.
            get_static_asset_filepath(), 'assets', 'images', 'avatar',
            'gravatar_example.png')
        with utils.open_file(expected_gravatar_filepath, 'rb', encoding=None
            ) as f:
        ...
        self.assertEqual(gravatar, utils.convert_png_to_data_url(
    ...
    def test_default_identicon_data_url(self) ->None:
        identicon_filepath = os.path.join(self.get_static_asset_filepath(),
            'assets', 'images', 'avatar', 'user_blue_72px.png')
        identicon_data_url = utils.convert_png_to_data_url(identicon_filepath)
        self.assertEqual(identicon_data_url, user_services.
    ...
    def test_get_users_email_preferences(self) ->None:
        ...
        self.assertEqual(user_email_prefs.can_receive_email_updates, feconf
            .DEFAULT_EMAIL_UPDATES_PREFERENCE)
        self.assertEqual(user_email_prefs.can_receive_editor_role_email,
            feconf.DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE)
        self.assertEqual(user_email_prefs.
            can_receive_feedback_message_email, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE)
        self.assertEqual(user_email_prefs.can_receive_subscription_email,
            feconf.DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)

    def test_add_user_to_mailing_list(self) ->None:

        def _mock_add_or_update_user_status(unused_email: str, merge_fields:
            Dict[str, str], unused_tag: str, *, can_receive_email_updates: bool
            ) ->bool:
        ...
        fn_swap = self.swap(bulk_email_services,
            'add_or_update_user_status', _mock_add_or_update_user_status)
        with fn_swap:
            self.assertTrue(user_services.add_user_to_mailing_list(
    ...
    def test_set_and_get_user_email_preferences(self) ->None:
        ...
        with logging_swap:
            user_services.update_email_preferences(user_id, feconf.
                DEFAULT_EMAIL_UPDATES_PREFERENCE, feconf.
        ...
        def _mock_add_or_update_user_status(unused_email: str,
            unused_merge_fields: Dict[str, str], unused_tag: str, *,
            can_receive_email_updates: bool) ->bool:
            """Mocks bulk_email_services.add_or_update_user_status()."""
            return not can_receive_email_updates
        bulk_email_swap = self.swap(bulk_email_services,
            'add_or_update_user_status', _mock_add_or_update_user_status)
        with bulk_email_swap:
            bulk_email_signup_message_should_be_shown = (user_services.
                update_email_preferences(user_id, True, feconf.
                DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
                DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
                DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE))
        ...
        self.assertEqual(email_preferences.
            ...
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE)
        user_services.update_email_preferences(user_id, feconf.
            DEFAULT_EMAIL_UPDATES_PREFERENCE, False, False, False)
        email_preferences = user_services.get_email_preferences(user_id)
    ...
    def test_get_and_set_user_email_preferences_with_error(self) ->None:
        ...
        def _mock_add_or_update_user_status(_email: str,
            _can_receive_updates: bool) ->None:
            """Mocks bulk_email_services.add_or_update_user_status().
        ...
        with self.swap(bulk_email_services, 'add_or_update_user_status',
            ...
            try:
                user_services.update_email_preferences(user_id, False,
                    feconf.DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
                    DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
                    DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)
            except Exception:
                email_preferences = user_services.get_email_preferences(user_id
        ...
        user_services.update_email_preferences(user_id, True, feconf.
            DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
            DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)
        email_preferences = user_services.get_email_preferences(user_id)
    ...
    def test_set_and_get_user_email_preferences_for_exploration(self) ->None:
        ...
        email_preferences = (user_services.
            get_email_preferences_for_exploration(user_id, exploration_id))
        self.assertEqual(email_preferences.mute_feedback_notifications,
            feconf.DEFAULT_FEEDBACK_NOTIFICATIONS_MUTED_PREFERENCE)
        self.assertEqual(email_preferences.mute_suggestion_notifications,
            feconf.DEFAULT_SUGGESTION_NOTIFICATIONS_MUTED_PREFERENCE)
        user_services.set_email_preferences_for_exploration(user_id,
            exploration_id, mute_feedback_notifications=feconf.
        ...
        self.assertEqual(email_preferences.mute_suggestion_notifications,
            feconf.DEFAULT_SUGGESTION_NOTIFICATIONS_MUTED_PREFERENCE)
        user_services.set_email_preferences_for_exploration(user_id,
            exploration_id, mute_suggestion_notifications=True)
        email_preferences = (user_services.
            get_email_preferences_for_exploration(user_id, exploration_id))
        self.assertEqual(email_preferences.mute_feedback_notifications,
            feconf.DEFAULT_FEEDBACK_NOTIFICATIONS_MUTED_PREFERENCE)
        ...
        user_services.set_email_preferences_for_exploration(user_id,
            exploration_id, mute_feedback_notifications=True)
        email_preferences = (user_services.
            get_email_preferences_for_exploration(user_id, exploration_id))
    ...
    def test_get_users_email_preferences_for_exploration(self) ->None:
        ...
        user_services.set_email_preferences_for_exploration(user_ids[1],
            exploration_ids[1], mute_feedback_notifications=True,
        ...
        self.assertEqual(exp_prefs[0].mute_feedback_notifications, feconf.
            DEFAULT_FEEDBACK_NOTIFICATIONS_MUTED_PREFERENCE)
        self.assertEqual(exp_prefs[0].mute_suggestion_notifications, feconf
            .DEFAULT_SUGGESTION_NOTIFICATIONS_MUTED_PREFERENCE)
    ...
    def test_get_usernames_by_role(self) ->None:
        ...
        self.assertEqual(set(user_services.get_usernames_by_role(feconf.
            ROLE_ID_MODERATOR)), set(['name1', 'name2']))
        self.assertEqual(set(user_services.get_usernames_by_role(feconf.
            ROLE_ID_TOPIC_MANAGER)), set(['name3', 'name4']))
    ...
    def test_get_user_ids_by_role(self) ->None:
        ...
        user_services.add_user_role(user_ids[0], feconf.ROLE_ID_MODERATOR)
        user_services.add_user_role(user_ids[1], feconf.ROLE_ID_MODERATOR)
        user_services.add_user_role(user_ids[2], feconf.
            ROLE_ID_CURRICULUM_ADMIN)
        user_services.add_user_role(user_ids[3], feconf.
            ROLE_ID_CURRICULUM_ADMIN)
        self.assertEqual(set(user_services.get_user_ids_by_role(feconf.
            ROLE_ID_MODERATOR)), set([user_ids[0], user_ids[1]]))
        self.assertEqual(set(user_services.get_user_ids_by_role(feconf.
            ROLE_ID_CURRICULUM_ADMIN)), set([user_ids[2], user_ids[3]]))

    def test_get_system_user_returns_system_user_action_info(self) ->None:
        system_user_action = user_services.get_system_user()
        expected_actions = set(['MANAGE_TOPIC_RIGHTS',
            'EDIT_ANY_PUBLIC_ACTIVITY', 'DELETE_ANY_SKILL',
            ...
            'CHANGE_TOPIC_STATUS', 'SEND_MODERATOR_EMAILS',
            'FLAG_EXPLORATION', 'ACCESS_CREATOR_DASHBOARD',
            'EDIT_ANY_TOPIC', 'ACCEPT_ANY_SUGGESTION',
    ...
    def test_remove_user_role(self) ->None:
        user_id = user_services.create_new_user('someUser', 'user@example.com'
            ).user_id
        user_settings_model = user_models.UserSettingsModel.get_by_id(user_id)
    ...
    def test_add_user_role(self) ->None:
        ...
        user_id = user_services.create_new_user(auth_id, user_email).user_id
        user_services.set_username(user_id, username)
        self.assertEqual(user_services.get_user_roles_from_id(user_id), [
    ...
    def test_adding_other_roles_to_full_user_updates_roles(self) ->None:
        ...
        user_services.add_user_role(user_id, feconf.ROLE_ID_COLLECTION_EDITOR)
        user_settings_model = user_models.UserSettingsModel.get_by_id(user_id)
        self.assertEqual(user_settings_model.roles, [feconf.
            ROLE_ID_FULL_USER, feconf.ROLE_ID_COLLECTION_EDITOR])
        self.assertFalse(user_settings_model.banned)
        user_services.add_user_role(user_id, feconf.ROLE_ID_TOPIC_MANAGER)
        user_settings_model = user_models.UserSettingsModel.get_by_id(user_id)
        self.assertEqual(user_settings_model.roles, [feconf.
            ROLE_ID_FULL_USER, feconf.ROLE_ID_COLLECTION_EDITOR, feconf.
        ...
        self.assertEqual(user_settings_model.roles, [feconf.
            ROLE_ID_FULL_USER, feconf.ROLE_ID_COLLECTION_EDITOR, feconf.
            ROLE_ID_TOPIC_MANAGER, feconf.ROLE_ID_MODERATOR])
        self.assertFalse(user_settings_model.banned)
        user_services.add_user_role(user_id, feconf.ROLE_ID_CURRICULUM_ADMIN)
        user_settings_model = user_models.UserSettingsModel.get_by_id(user_id)
        self.assertEqual(user_settings_model.roles, [feconf.
            ROLE_ID_FULL_USER, feconf.ROLE_ID_COLLECTION_EDITOR, feconf.
            ROLE_ID_TOPIC_MANAGER, feconf.ROLE_ID_MODERATOR, feconf.
            ROLE_ID_CURRICULUM_ADMIN])
        self.assertFalse(user_settings_model.banned)
    ...
    def test_profile_user_settings_have_correct_roles(self) ->None:
        ...
        profile_user_data_dict: user_domain.RawUserDataDict = {'schema_version'
            : 1, 'display_alias': 'display_alias3', 'pin': '12345',
            'preferred_language_codes': [constants.DEFAULT_LANGUAGE_CODE],
            'preferred_site_language_code': None,
            'preferred_audio_language_code': None,
        ...
        profile_user_id = user_services.create_new_profiles(auth_id,
            user_email, [modifiable_user_data])[0].user_id
        profile_user_settings_model = user_models.UserSettingsModel.get_by_id(
            profile_user_id)
        self.assertEqual(profile_user_settings_model.roles, [feconf.
    ...
    def test_add_user_role_to_mobile_learner_raises_exception(self) ->None:
        ...
        user_services.create_new_profiles(auth_id, user_email, [self.
            modifiable_new_user_data])
        profile_user_id = (user_services.
            get_all_profiles_auth_details_by_parent_user_id(user_id)[0].user_id
            )
        self.assertEqual(user_services.get_user_roles_from_id(
            profile_user_id), [feconf.ROLE_ID_MOBILE_LEARNER])
        error_msg = 'The role of a Mobile Learner cannot be changed.'
        with self.assertRaisesRegex(Exception, error_msg):
            user_services.add_user_role(profile_user_id, feconf.
                ROLE_ID_FULL_USER)
    ...
    def test_add_duplicate_user_role_raises_exception(self) ->None:
        ...
        with self.assertRaisesRegex(Exception, expected_error_msg):
            user_services.add_user_role(user_id, feconf.
    ...
    def test_add_full_user_role_to_learner_raises_exception(self) ->None:
        ...
        self.assertEqual(user_services.get_user_roles_from_id(user_id), [
            feconf.ROLE_ID_FULL_USER])
        error_msg = ('Adding a %s role is not allowed.' % feconf.
            ROLE_ID_MOBILE_LEARNER)
        with self.assertRaisesRegex(Exception, error_msg):
    ...
    def test_is_user_blog_post_author_returns_true_for_authors(self) ->None:
        ...
        self.add_user_role(self.BLOG_EDITOR_USERNAME, feconf.
            ROLE_ID_BLOG_POST_EDITOR)
    ...
    def test_removing_role_from_mobile_learner_user_raises_exception(self
        ...
        error_msg = 'The role of a Mobile Learner cannot be changed.'
        with self.assertRaisesRegex(Exception, error_msg):
            user_services.remove_user_role(profile_user_id, feconf.
                ROLE_ID_TOPIC_MANAGER)
    ...
    def test_create_new_user_creates_a_new_user_auth_details_entry(self
        ...
        self.assertEqual(auth_services.get_auth_id_from_user_id(user_id),
            new_auth_id)
    ...
    def test_get_auth_details_by_user_id_for_existing_user_works_fine(self
        ...
        self.assertEqual(user_auth_details.gae_id, user_auth_details_model.
            gae_id)
        self.assertEqual(user_auth_details.parent_user_id,
            user_auth_details_model.parent_user_id)

    def test_get_auth_details_by_user_id_non_existing_user_returns_none(self
        ) ->None:
        non_existent_user_id = 'id_x'
        self.assertIsNone(user_services.get_auth_details_by_user_id(
            non_existent_user_id))
    ...
    def test_create_new_profile_with_parent_user_pin_set_is_success(self
        ...
        self.modifiable_new_user_data.display_alias = display_alias_2
        self.modifiable_new_user_data.pin = profile_pin
        user_services.create_new_profiles(auth_id, email, [self.
            modifiable_new_user_data])
        user_auth_details_models = (user_services.
            get_all_profiles_auth_details_by_parent_user_id(user_id))
    ...
    def test_create_multiple_new_profiles_for_same_user_works_correctly(self
        ...
        self.modifiable_new_user_data.display_alias = display_alias_2
        self.modifiable_new_user_data.pin = profile_pin
        new_user_data_dict_2: user_domain.RawUserDataDict = {'schema_version':
            1, 'display_alias': display_alias_3, 'pin': None,
            'preferred_language_codes': [constants.DEFAULT_LANGUAGE_CODE],
            'preferred_site_language_code': None,
            'preferred_audio_language_code': None,
            'preferred_translation_language_code': None, 'user_id': None}
        modifiable_new_user_data_2 = (user_domain.ModifiableUserData.
            from_raw_dict(new_user_data_dict_2))
        user_settings_list = user_services.create_new_profiles(auth_id,
            email, [self.modifiable_new_user_data, modifiable_new_user_data_2])
        profile_1_id = user_settings_list[0].user_id
        profile_2_id = user_settings_list[1].user_id
        user_auth_details_models = [{'id': model.id, 'auth_id': model.
            gae_id, 'parent_user_id': model.parent_user_id} for model in
            auth_services.get_all_profiles_by_parent_user_id(user_id)]
        expected_user_auth_output = [{'id': profile_1_id, 'auth_id': None,
            'parent_user_id': user_id}, {'id': profile_2_id, 'auth_id':
            None, 'parent_user_id': user_id}]
        self.assertItemsEqual(user_auth_details_models,
            expected_user_auth_output)
        user_settings_models = []
        for model in user_models.UserSettingsModel.get_multi([profile_1_id,
            profile_2_id]):
            assert model is not None
            user_settings_models.append({'id': model.id, 'display_alias':
                model.display_alias, 'pin': model.pin, 'roles': model.roles})
        expected_user_settings_output = [{'id': profile_1_id,
            'display_alias': display_alias_2, 'pin': profile_pin, 'roles':
            [feconf.ROLE_ID_MOBILE_LEARNER]}, {'id': profile_2_id,
            'display_alias': display_alias_3, 'pin': None, 'roles': [feconf
            .ROLE_ID_MOBILE_LEARNER]}]
        self.assertItemsEqual(user_settings_models,
    ...
    def test_create_new_profile_with_nonexistent_user_raises_error(self
        ...
        with self.assertRaisesRegex(Exception, error_msg):
            self.modifiable_new_user_data.display_alias = display_alias
            self.modifiable_new_user_data.pin = profile_pin
            user_services.create_new_profiles(non_existent_auth_id,
                non_existent_email, [self.modifiable_new_user_data])
    ...
    def test_update_users_data_for_multiple_users_works_correctly(self) ->None:
        ...
        user_settings_list = user_services.create_new_profiles(auth_id,
            email, [self.modifiable_new_user_data, modifiable_new_user_data_2])
        profile_user_ids = [user_settings_list[0].user_id,
        ...
        user_services.update_multiple_users_data([self.
            modifiable_new_user_data, modifiable_new_user_data_2])
        user_auth_details_models = []
        for model in auth_models.UserAuthDetailsModel.get_multi(
            profile_user_ids):
            assert model is not None
            user_auth_details_models.append({'id': model.id, 'auth_id':
                model.gae_id, 'parent_user_id': model.parent_user_id})
        expected_auth_details_output = [{'id': profile_user_ids[0],
            'auth_id': None, 'parent_user_id': user_id}, {'id':
            profile_user_ids[1], 'auth_id': None, 'parent_user_id': user_id}]
        self.assertItemsEqual(expected_auth_details_output,
            user_auth_details_models)
        user_settings_models = []
        for model_setting_model in user_models.UserSettingsModel.get_multi(
            profile_user_ids):
            assert model_setting_model is not None
            user_settings_models.append({'id': model_setting_model.id,
                'display_alias': model_setting_model.display_alias, 'pin':
                model_setting_model.pin})
        expected_user_settings_output = [{'id': profile_user_ids[0],
            'display_alias': 'xyz', 'pin': profile_pin}, {'id':
            profile_user_ids[1], 'display_alias': display_alias_3, 'pin':
            '345'}]
        self.assertItemsEqual(expected_user_settings_output,
            user_settings_models)
    ...
    def _signup_test_users_and_create_user_groups(self) ->None:
        ...
        user_services.create_new_user_group('USERGROUP1', ['user1', 'user2',
            'user3'])
    ...
    def test_get_all_user_groups(self) ->None:
        ...
        if user_groups_data[0].name == 'USERGROUP1':
            self.assertEqual(user_groups_data[0].member_usernames, ['user1',
                'user2', 'user3'])
        else:
            self.assertEqual(user_groups_data[0].member_usernames, ['user1',
                'user2'])
    ...
    def test_update_existing_user_group_name(self) ->None:
        self._signup_test_users_and_create_user_groups()
        user_groups_data = user_services.get_all_user_groups()
        self.assertTrue(user_groups_data[0].name in ['USERGROUP1',
            'USERGROUP2'])
        user_group_id = user_groups_data[0].user_group_id
        user_services.update_user_group(user_groups_data[0].user_group_id,
            'USERGROUP3', ['user1', 'user2'])
        user_groups_data = user_services.get_all_user_groups()
    ...
    def test_update_existing_user_group_users(self) ->None:
        ...
        if user_groups_data[0].name == 'USERGROUP1':
            ...
            self.assertEqual(user_groups_data[1].member_usernames, ['user3',
                'user4'])
    ...
    def test_user_group_trying_to_update_not_exists_raise_error(self) ->None:
        ...
        with self.assertRaisesRegex(Exception,
            ...
            user_services.update_user_group('INVALID_USER_GROUP_ID',
                'INVALID_USER_GROUP', ['user1', 'user2', 'user3'])
        user_services.delete_user_group(user_groups_data[0].user_group_id)
        user_services.delete_user_group(user_groups_data[1].user_group_id)
    ...
    def test_user_group_containing_duplicate_users_raises_error(self) ->None:
        ...
        user_groups_data = user_services.get_all_user_groups()
        user_services.update_user_group(user_groups_data[0].user_group_id,
            'USERGROUP1', ['user1', 'user2'])
        with self.assertRaisesRegex(Exception,
            "Users list of user-group USERGROUP1 contains duplicates: \\[\\'user2\\']\\."
            ):
            user_services.update_user_group(user_groups_data[0].
                user_group_id, 'USERGROUP1', ['user1', 'user2', 'user2'])
    ...
    def test_user_group_containing_invalid_user_raises_exception(self) ->None:
        ...
        user_services.update_user_group(user_groups_data[0].user_group_id,
            'USERGROUP1', ['user1', 'user2'])
        with self.assertRaisesRegex(Exception,
            "Following users of user-group USERGROUP1 does not exist: \\[\\'user5\\', \\'user6\\']\\."
            ):
            user_services.update_user_group(user_groups_data[0].
                user_group_id, 'USERGROUP1', ['user1', 'user2', 'user5',
                'user6'])
    ...
    def test_get_current_date_as_string(self) ->None:
        custom_datetimes = [datetime.date(2011, 1, 1), datetime.date(2012, 
            2, 28)]
        datetime_strings = [custom_datetime.strftime(feconf.
            DASHBOARD_STATS_DATETIME_STRING_FORMAT) for custom_datetime in
            custom_datetimes]
    ...
    def test_parse_date_from_string(self) ->None:
        self.assertEqual(user_services.parse_date_from_string('2016-06-30'),
            {'year': 2016, 'month': 6, 'day': 30})
        self.assertEqual(user_services.parse_date_from_string('2016-07-05'),
    ...
    def test_record_user_started_state_translation_tutorial(self) ->None:
        ...
        user_settings = user_services.get_user_settings(user_id)
        self.assertIsInstance(user_settings.
            last_started_state_translation_tutorial, datetime.datetime)
        self.assertTrue(user_settings.
            last_started_state_translation_tutorial is not None)

    def test_get_human_readable_user_ids(self) ->None:
        auth_ids = ['regular_user', 'user_being_deleted', 'no_username_user']
        user_emails = ['reuglar_user@example.com',
            'user_being_deleted@example.com', 'no_username_user@example.com']
        user_ids = []
        for i, auth_id in enumerate(auth_ids):
            user_ids.append(user_services.create_new_user(auth_id,
                user_emails[i]).user_id)
        ...
        user_settings_for_no_username = user_services.get_user_settings(
            user_ids[2])
        usernames = ['regularUsername', user_services.
            LABEL_FOR_USER_BEING_DELETED, 
            '[Awaiting user registration: %s]' %
            user_settings_for_no_username.truncated_email]
        self.assertEqual(usernames, user_services.
            get_human_readable_user_ids(user_ids))
    ...
    def test_record_user_started_state_editor_tutorial(self) ->None:
        ...
        self.assertEqual(user_settings.last_started_state_editor_tutorial,
            prev_started_state)
        user_services.record_user_started_state_editor_tutorial(user_id)
        user_settings = user_services.get_user_settings(user_id)
        self.assertGreaterEqual(user_settings.
            last_started_state_editor_tutorial, prev_started_state)
    ...
    def test_create_user_contributions(self) ->None:
        ...
        contributions = user_services.get_user_contributions(user_id,
            strict=True)
        self.assertEqual(['exp1', 'exp2', 'exp3'], contributions.
            created_exploration_ids)
        self.assertEqual(['exp2', 'exp3', 'exp4'], contributions.
    ...
    def test_update_user_contributions(self) ->None:
        ...
        self.assertEqual([], pre_add_contributions.created_exploration_ids)
        self.assertEqual([], pre_add_contributions.edited_exploration_ids)
        user_services.update_user_contributions(user_id, created_exp_ids,
            edited_exp_ids)
        contributions = user_services.get_user_contributions(user_id,
            strict=True)
        self.assertEqual(['exp1', 'exp2', 'exp3'], contributions.
    ...
    def test_update_user_contributions_for_invalid_user_raises_error(self
        ...
        with self.assertRaisesRegex(Exception, 
            ...
            'non_existent_user_id'):
            user_services.update_user_contributions('non_existent_user_id',
    ...
    def test_log_username_change(self) ->None:
        ...
        all_models_before_update = (audit_models.UsernameChangeAuditModel.
            get_all())
        self.assertEqual(all_models_before_update.count(), 0)
        user_services.log_username_change(committer_id, 'oldUsername',
            'newUsername')
        all_models_after_update = (audit_models.UsernameChangeAuditModel.
            get_all())
    ...
    def test_raises_error_if_none_destination_is_provided_for_checkpoint(self
        ) ->None:
        state = state_domain.State.create_default_state('state_1',
            'content_0', 'default_outcome_1')
        state_answer_group: List[state_domain.AnswerGroup] = [state_domain.
            AnswerGroup(state_domain.Outcome(None, None, state_domain.
            SubtitledHtml('feedback_2', '<p>state outcome html</p>'), False,
            [], None, None), [state_domain.RuleSpec('Equals', {'x': {
            'contentId': 'rule_input_3', 'normalizedStrSet': [
            'Test rule spec.']}})], [], None)]
        ...
        state_answer_group = [state_domain.AnswerGroup(state_domain.Outcome
            ('destination', None, state_domain.SubtitledHtml('feedback_4',
            '<p>state outcome html</p>'), False, [], None, None), [
            state_domain.RuleSpec('Equals', {'x': {'contentId':
            'rule_input_5', 'normalizedStrSet': ['Test rule spec.']}})], [],
            None)]
        state.update_interaction_answer_groups(state_answer_group)
...
"""
    ...
    def test_user_checkpoint_progress_is_updated_correctly(self) ->None:
        ...
        user_services.clear_learner_checkpoint_progress(self.viewer_id,
            self.EXP_ID)
        exp_user_data = exp_fetchers.get_exploration_user_data(self.
            viewer_id, self.EXP_ID)
        assert exp_user_data is not None
    ...
    def test_sync_logged_in_learner_checkpoint_progress_with_current_exp_version(
        self) ->None:
        self.login(self.VIEWER_EMAIL)
        exp_user_data = (user_services.
            sync_logged_in_learner_checkpoint_progress_with_current_exp_version
...
class UpdateContributionMsecTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.signup(self.OWNER_EMAIL, self.OWNER_USERNAME)
        self.owner_id = self.get_user_id_from_email(self.OWNER_EMAIL)
        user_services.add_user_role(self.owner_id, feconf.
    ...
    def test_contribution_msec_updates_on_published_explorations(self) ->None:
        exploration = self.save_new_valid_exploration(self.EXP_ID, self.
            admin_id, end_state_name='End')
        init_state_name = exploration.init_state_name
        exp_services.publish_exploration_and_update_user_profiles(self.
            admin, self.EXP_ID)
        ...
        exp_services.update_exploration(self.editor_id, self.EXP_ID, [
            exp_domain.ExplorationChange({'cmd': 'edit_state_property',
            'state_name': init_state_name, 'property_name': 'widget_id',
            'new_value': 'MultipleChoiceInput'}), exp_domain.
            ExplorationChange({'cmd': 'edit_state_property', 'state_name':
            init_state_name, 'property_name': 'widget_customization_args',
            'new_value': {'choices': {'value': [{'content_id':
            'ca_choices_0', 'html': '<p>Choice 1</p>'}]},
            'showChoicesInShuffledOrder': {'value': True}}})], 'commit')
    ...
    def test_contribution_msec_does_not_update_until_exp_is_published(self
        ...
        exp_services.update_exploration(self.editor_id, self.EXP_ID, [
            exp_domain.ExplorationChange({'cmd': 'rename_state',
    ...
    def test_contribution_msec_updates_on_published_collections(self) ->None:
        self.save_new_valid_collection(self.COL_ID, self.admin_id, title=
            self.COLLECTION_TITLE, category=self.COLLECTION_CATEGORY,
            objective=self.COLLECTION_OBJECTIVE, exploration_id=self.EXP_ID)
        collection_services.publish_collection_and_update_user_profiles(self
            .admin, self.COL_ID)
        exp_services.publish_exploration_and_update_user_profiles(self.
            admin, self.EXP_ID)
        self.assertIsNotNone(user_services.get_user_settings(self.admin_id)
        ...
        collection_services.update_collection(self.editor_id, self.COL_ID,
            [{'cmd': 'edit_collection_property', 'property_name': 'title',
            'new_value': 'Some new title'}], 'Changed the title')
        self.assertIsNotNone(user_services.get_user_settings(self.editor_id
            ).first_contribution_msec)
    ...
    def test_contribution_msec_does_not_update_until_collection_is_published(
        ...
        self.assertIsNone(user_services.get_user_settings(self.admin_id).
            first_contribution_msec)
        collection_services.update_collection(self.admin_id, self.COL_ID, [
            {'cmd': 'edit_collection_property', 'property_name': 'title',
        ...
        rights_manager.assign_role_for_collection(self.admin, self.COL_ID,
            self.editor_id, 'editor')
        collection_services.update_collection(self.editor_id, self.COL_ID,
            [{'cmd': 'edit_collection_property', 'property_name':
            'category', 'new_value': 'Some new category'}], '')
        self.assertIsNone(user_services.get_user_settings(self.editor_id).
            first_contribution_msec)
        collection_services.publish_collection_and_update_user_profiles(self
            .admin, self.COL_ID)
    ...
    def test_contribution_msec_does_not_change_if_collection_unpublished(self
        ...
        self.save_new_valid_collection(self.COL_ID, self.owner_id, title=
            self.COLLECTION_TITLE, category=self.COLLECTION_CATEGORY,
            objective=self.COLLECTION_OBJECTIVE, exploration_id=self.EXP_ID)
        collection_services.publish_collection_and_update_user_profiles(self
            .owner, self.COL_ID)
...
class UserDashboardStatsTests(test_utils.GenericTestBase):
    ...
    def test_get_user_dashboard_stats(self) ->None:
        ...
        init_state_name = exploration.init_state_name
        event_services.StartExplorationEventHandler.record(self.EXP_ID, 1,
            init_state_name, self.USER_SESSION_ID, {}, feconf.PLAY_TYPE_NORMAL)
        event_services.StatsEventsHandler.record(self.EXP_ID, 1, {
            'num_starts': 1, 'num_actual_starts': 0, 'num_completions': 0,
            'state_stats_mapping': {}})
        self.assertEqual(user_services.get_dashboard_stats(self.owner_id),
    ...
    def test_get_weekly_dashboard_stats_when_stats_model_is_none(self) ->None:
        ...
        event_services.StartExplorationEventHandler.record(self.EXP_ID, 1,
            init_state_name, self.USER_SESSION_ID, {}, feconf.PLAY_TYPE_NORMAL)
        self.assertEqual(user_services.get_weekly_dashboard_stats(self.
            owner_id), [])
        self.assertEqual(user_services.get_last_week_dashboard_stats(self.
            owner_id), None)
        with self.swap(user_services, 'get_current_date_as_string', self.
            mock_get_current_date_as_string):
            user_services.update_dashboard_stats_log(self.owner_id)
        self.assertEqual(user_services.get_weekly_dashboard_stats(self.
            owner_id), [{self.CURRENT_DATE_AS_STRING: {'total_plays': 1,
            'num_ratings': 0, 'average_ratings': None}}])
    ...
    def test_get_user_impact_score(self) ->None:
        ...
        with self.swap(user_models.UserStatsModel, 'impact_score',
            ...
            impact_score_for_user_with_no_activity = (user_services.
                get_user_impact_score(self.owner_id))
            self.assertEqual(impact_score_for_user_with_no_activity, 0)
            exploration = self.save_new_valid_exploration(self.EXP_ID, self
                .owner_id, end_state_name='End')
            init_state_name = exploration.init_state_name
            event_services.StartExplorationEventHandler.record(self.EXP_ID,
                1, init_state_name, self.USER_SESSION_ID, {}, feconf.
                PLAY_TYPE_NORMAL)
            event_services.StatsEventsHandler.record(self.EXP_ID, 1, {
                'num_starts': 1, 'num_actual_starts': 0, 'num_completions':
                0, 'state_stats_mapping': {}})
            model = user_models.UserStatsModel.get_or_create(self.owner_id)
            self.assertEqual(model.impact_score, expected_impact_score)
            impact_score_for_user_with_some_learner_activity = (user_services
                .get_user_impact_score(self.owner_id))
            self.assertEqual(impact_score_for_user_with_some_learner_activity,
                expected_impact_score)

    def test_get_dashboard_stats_for_user_with_no_stats_model(self) ->None:
        fake_user_id = 'id_x'
        stats = user_services.get_dashboard_stats(fake_user_id)
        self.assertEqual(stats, {'total_plays': 0, 'num_ratings': 0,
...
class LastLoginIntegrationTests(test_utils.GenericTestBase):
    ...
    def test_legacy_user(self) ->None:
        ...
        """
        previous_last_logged_in_datetime = user_services.get_user_settings(self
            .viewer_id).last_logged_in
        ...
        self.assertLess(last_logged_in, user_services.get_user_settings(
    ...
    def test_last_logged_in_only_updated_if_enough_time_has_elapsed(self
        ...
        with self.mock_datetime_utcnow(mocked_datetime_utcnow):
            self.login(self.VIEWER_EMAIL)
            self.get_html_response(feconf.LIBRARY_INDEX_URL)
            self.assertEqual(user_services.get_user_settings(self.viewer_id
                ).last_logged_in, previous_last_logged_in_datetime)
            self.logout()
        ...
        with self.mock_datetime_utcnow(mocked_datetime_utcnow):
            self.login(self.VIEWER_EMAIL)
            self.get_html_response(feconf.LIBRARY_INDEX_URL)
            self.assertGreater(user_services.get_user_settings(self.
                viewer_id).last_logged_in, previous_last_logged_in_datetime)
            self.logout()
...
class LastExplorationCreatedIntegrationTests(test_utils.GenericTestBase):
    ...
    def test_legacy_user(self) ->None:
        ...
        self.save_new_valid_exploration(self.EXP_ID_A, self.owner_id,
            end_state_name='End')
        owner_settings = user_services.get_user_settings(self.owner_id)
    ...
    def test_last_exp_edit_time_gets_updated(self) ->None:
        ...
        with self.mock_datetime_utcnow(user_settings.
            last_created_an_exploration - datetime.timedelta(hours=13)):
            user_services.record_user_created_an_exploration(self.owner_id)
        owner_settings = user_services.get_user_settings(self.owner_id)
        previous_last_created_an_exploration = (owner_settings.
            last_created_an_exploration)
        self.assertIsNotNone(previous_last_created_an_exploration)
        self.save_new_valid_exploration(self.EXP_ID_B, self.owner_id,
            end_state_name='End')
        owner_settings = user_services.get_user_settings(self.owner_id)
...
class CommunityContributionStatsUnitTests(test_utils.GenericTestBase):
    ...
    def _assert_community_contribution_stats_is_in_default_state(self) ->None:
        ...
        state.
        """
        community_contribution_stats = (suggestion_services.
            get_community_contribution_stats())
        self.assertEqual(community_contribution_stats.
    ...
    def test_grant_reviewer_translation_reviewing_rights_increases_count(self
        ...
        user_services.allow_user_to_review_translation_in_language(self.
            reviewer_1_id, 'hi')
    ...
    def test_remove_all_reviewer_translation_reviewing_rights_decreases_count(
        ...
        self.assertDictEqual(stats.
            translation_suggestion_counts_by_lang_code, {})
        user_services.remove_translation_review_rights_in_language(self.
            reviewer_1_id, 'hi')
    ...
    def test_remove_reviewer_question_reviewing_rights_decreases_count(self
        ) ->None:
        user_services.allow_user_to_review_question(self.reviewer_1_id)
        stats = suggestion_services.get_community_contribution_stats()
        self.assertEqual(stats.question_reviewer_count, 1)
    ...
    def test_remove_question_contribution_reviewer_decreases_count(self
        ) ->None:
        user_services.allow_user_to_review_question(self.reviewer_1_id)
        stats = suggestion_services.get_community_contribution_stats()
        self.assertEqual(stats.question_reviewer_count, 1)
    ...
    def test_grant_reviewer_voiceover_reviewing_permissions_does_nothing(self
        ...
        user_services.allow_user_to_review_voiceover_in_language(self.
            reviewer_1_id, 'hi')
        self._assert_community_contribution_stats_is_in_default_state()
    ...
    def test_remove_reviewer_voiceover_reviewing_permissions_does_nothing(self
        ...
        user_services.allow_user_to_review_voiceover_in_language(self.
            reviewer_1_id, 'hi')
        self._assert_community_contribution_stats_is_in_default_state()
        user_services.remove_voiceover_review_rights_in_language(self.
            reviewer_1_id, 'hi')
        self._assert_community_contribution_stats_is_in_default_state()
...
class UserContributionReviewRightsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.voice_artist_id = self.get_user_id_from_email(self.
            VOICE_ARTIST_EMAIL)
        self.signup(self.QUESTION_REVIEWER_EMAIL, self.
            QUESTION_REVIEWER_USERNAME)
        self.question_reviewer_id = self.get_user_id_from_email(self.
            QUESTION_REVIEWER_EMAIL)
        self.signup(self.QUESTION_SUBMITTER_EMAIL, self.
            QUESTION_SUBMITTER_USERNAME)
    ...
    def test_translation_review_assignement_adds_language_in_sorted_order(self
        ...
        user_contribution_rights = user_services.get_user_contribution_rights(
            self.translator_id)
        self.assertEqual(user_contribution_rights.
        ...
        user_contribution_rights = user_services.get_user_contribution_rights(
            self.translator_id)
        self.assertEqual(user_contribution_rights.
            can_review_translation_for_language_codes, ['en', 'hi'])
    ...
    def test_voiceover_review_assignement_adds_language_in_sorted_order(self
        ...
        user_contribution_rights = user_services.get_user_contribution_rights(
            self.voice_artist_id)
        self.assertEqual(user_contribution_rights.
            can_review_voiceover_for_language_codes, ['en', 'hi'])

    def test_assign_user_review_question_suggestion(self) ->None:
        self.assertFalse(user_services.can_review_question_suggestions(self
            .voice_artist_id))
        user_services.allow_user_to_review_question(self.voice_artist_id)
        self.assertTrue(user_services.can_review_question_suggestions(self.
            voice_artist_id))

    def test_assign_user_submit_question_suggestion(self) ->None:
        self.assertFalse(user_services.can_submit_question_suggestions(self
            .voice_artist_id))
        user_services.allow_user_to_submit_question(self.voice_artist_id)
        self.assertTrue(user_services.can_submit_question_suggestions(self.
            voice_artist_id))

    def test_get_users_contribution_rights_with_multiple_reviewer_user_ids(self
        ) ->None:
        ...
        users_contribution_rights = (user_services.
            get_users_contribution_rights(expected_reviewer_ids))
    ...
    def test_get_users_contribution_rights_with_one_reviewer_user_id(self
        ) ->None:
        user_services.allow_user_to_review_translation_in_language(self.
            translator_id, 'hi')
        user_services.allow_user_to_review_translation_in_language(self.
    ...
    def test_get_reviewer_user_ids_to_notify_when_reviewers_want_notifications(
        ...
        user_services.update_email_preferences(self.question_reviewer_id, 
            True, feconf.DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
            DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)
        user_services.update_email_preferences(self.translator_id, True,
            feconf.DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
            DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)
    ...
    def test_get_reviewer_user_ids_to_notify_when_reviewers_do_not_want_emails(
        ...
        user_services.allow_user_to_review_translation_in_language(self.
            translator_id, 'hi')
        user_services.update_email_preferences(self.question_reviewer_id, 
            False, feconf.DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
            DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)
        user_services.update_email_preferences(self.translator_id, False,
            feconf.DEFAULT_EDITOR_ROLE_EMAIL_PREFERENCE, feconf.
            DEFAULT_FEEDBACK_MESSAGE_EMAIL_PREFERENCE, feconf.
            DEFAULT_SUBSCRIPTION_EMAIL_PREFERENCE)
        reviewer_ids_to_notify = user_services.get_reviewer_user_ids_to_notify(
    ...
    def test_removal_of_all_review_rights_deletes_model(self) ->None:
        ...
        right_model = user_models.UserContributionRightsModel.get_by_id(self
            .translator_id)
    ...
    def test_get_question_reviewer_usernames_with_lanaguge_code_raise_error(
        self) ->None:
        with self.assertRaisesRegex(Exception,
            'Expected language_code to be None'):
            user_services.get_contributor_usernames(constants.
    ...
    def test_get_contributor_usernames_with_invalid_category_raises(self
        ) ->None:
        with self.assertRaisesRegex(Exception,
            'Invalid category: invalid_category'):
            user_services.get_contributor_usernames('invalid_category',
                language_code='hi')

    def test_get_contributor_usernames_for_translation_returns_correctly(self
        ) ->None:
        usernames = user_services.get_contributor_usernames(constants.
    ...
    def test_remove_question_submit_rights(self) ->None:
        ...
        pre_user_contribution_rights = (user_services.
            get_user_contribution_rights(user_id))
...
class TranslationCoordinatorRightsTests(test_utils.GenericTestBase):
    ...
    def setUp(self) ->None:
        ...
        self.signup('translationcoordinator@example.com',
        ...
        self.set_curriculum_admins([self.CURRICULUM_ADMIN_USERNAME])
        self.set_translation_coordinators([user_services.get_username(self.
            user_id_translationcoordinator)], 'en')
        ...
        self.user_admin = user_services.get_user_actions_info(self.
            user_id_admin)
    ...
    def test_raises_error_if_guest_user_trying_to_deassign_roles_from_topic(
        ...
        with self.assertRaisesRegex(Exception,
            ...
            user_services.deassign_user_from_all_languages(guest_user,
                'user_id')
        with self.assertRaisesRegex(Exception,
    ...
    def test_reassigning_role_to_same_user(self) ->None:
        with self.assertRaisesRegex(Exception,
            ...
            user_services.assign_coordinator(self.user_admin, self.
                user_translationcoordinator, 'en')
    ...
    def test_assigning_role_to_a_user(self) ->None:
        ...
        self.assertFalse(user_services.check_user_is_coordinator(user_id_c,
            'en'))
        user_services.assign_coordinator(self.user_admin, user_c, 'en')
        self.assertTrue(user_services.check_user_is_coordinator(user_id_c,
            'en'))
    ...
    def test_guest_user_cannot_be_deassgined(self) ->None:
        with self.assertRaisesRegex(Exception,
            ...
            user_services.deassign_coordinator(self.user_admin, self.user_a,
                'no_model')
    ...
    def test_deassigning_for_non_existing_language_model(self) ->None:
        ...
        with self.assertRaisesRegex(Exception,
            'Cannot change the role of the Guest user.'):
            user_services.deassign_coordinator(self.user_admin, guest_user,
                'en')
    ...
    def test_get_translation_rights_from_model(self) ->None:
        ...
        self.assertEqual(model.coordinators_count, model_object.
            coordinators_count)
    ...
    def test_deassign_user_from_all_languages(self) ->None:
        ...
        user_services.deassign_user_from_all_languages(self.user_admin,
            user_id_c)
        self.assertEqual(0, len(user_services.
            get_translation_rights_with_user(user_id_c)))

    def test_check_user_is_coordinator_for_no_language_model(self) ->None:
        self.assertFalse(user_services.check_user_is_coordinator('user1',
