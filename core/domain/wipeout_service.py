...
(app_feedback_report_models, base_models, blog_models, collection_models,
    config_models, exp_models, feedback_models, improvements_models,
    question_models, skill_models, story_models, subtopic_models,
    suggestion_models, topic_models, user_models) = (models.Registry.
    import_models([models.Names.APP_FEEDBACK_REPORT, models.Names.
    BASE_MODEL, models.Names.BLOG, models.Names.COLLECTION, models.Names.
    CONFIG, models.Names.EXPLORATION, models.Names.FEEDBACK, models.Names.
    IMPROVEMENTS, models.Names.QUESTION, models.Names.SKILL, models.Names.
    STORY, models.Names.SUBTOPIC, models.Names.SUGGESTION, models.Names.
    TOPIC, models.Names.USER]))
datastore_services = models.Registry.import_datastore_services()
transaction_services = models.Registry.import_transaction_services()
bulk_email_services = models.Registry.import_bulk_email_services()
WIPEOUT_LOGS_PREFIX: Final = '[WIPEOUT]'
PERIOD_AFTER_WHICH_USERNAME_CANNOT_BE_REUSED: Final = datetime.timedelta(weeks
    =1)


def get_pending_deletion_request(user_id: str
    ...
    pending_deletion_request_model = (user_models.
        PendingDeletionRequestModel.get_by_id(user_id))
    return wipeout_domain.PendingDeletionRequest(pending_deletion_request_model
        .id, pending_deletion_request_model.username,
...
def save_pending_deletion_requests(pending_deletion_requests: List[
    wipeout_domain.PendingDeletionRequest]) ->None:
    ...
    pending_deletion_request_models = (user_models.
        PendingDeletionRequestModel.get_multi(user_ids, include_deleted=True))
    ...
    for deletion_request_model, deletion_request in zip(
        ...
        deletion_request_dict = {'username': deletion_request.username,
            'email': deletion_request.email,
            'normalized_long_term_username': deletion_request.
            normalized_long_term_username, 'deletion_complete':
            deletion_request.deletion_complete,
            'pseudonymizable_entity_mappings': deletion_request.
...
def pre_delete_user(user_id: str) ->None:
    ...
    linked_profile_user_ids = [user.user_id for user in user_services.
    ...
    for profile_user_settings in profile_users_settings_list:
        ...
        pending_deletion_requests.append(wipeout_domain.
            PendingDeletionRequest.create_default(profile_id,
            profile_user_settings.username, profile_user_settings.email))
    if feconf.ROLE_ID_MOBILE_LEARNER not in user_settings.roles:
        taskqueue_services.defer(taskqueue_services.
            FUNCTION_ID_REMOVE_USER_FROM_RIGHTS_MODELS, taskqueue_services.
            QUEUE_NAME_ONE_OFF_JOBS, user_id)
        user_services.update_email_preferences(user_id, False, False, False,
            False)
        bulk_email_services.permanently_delete_user_from_list(user_settings
            .email)
    user_services.mark_user_for_deletion(user_id)
    date_now = get_current_datetime_utc()
    date_before_which_username_should_be_saved = (date_now -
        PERIOD_AFTER_WHICH_USERNAME_CANNOT_BE_REUSED)
    if user_settings.created_on is None:
        raise Exception('No data available for when the user was created on.')
    normalized_long_term_username = (user_settings.normalized_username if 
        user_settings.created_on <
        date_before_which_username_should_be_saved else None)
    pending_deletion_requests.append(wipeout_domain.PendingDeletionRequest.
        create_default(user_id, user_settings.username, user_settings.email,
...
def delete_users_pending_to_be_deleted() ->None:
    ...
    for request_model in pending_deletion_request_models:
        pending_deletion_request = get_pending_deletion_request(request_model
            .id)
...
def run_user_deletion(pending_deletion_request: wipeout_domain.
    PendingDeletionRequest) ->str:
...
def run_user_deletion_completion(pending_deletion_request: wipeout_domain.
    PendingDeletionRequest) ->str:
    ...
    if not pending_deletion_request.deletion_complete:
        ...
        _delete_models_with_delete_at_end_policy(pending_deletion_request.
            user_id)
        ...
        if pending_deletion_request.normalized_long_term_username is not None:
            user_services.save_deleted_username(pending_deletion_request.
                normalized_long_term_username)
        server_can_send_emails = (platform_parameter_services.
            get_platform_parameter_value(platform_parameter_list.ParamName.
            SERVER_CAN_SEND_EMAILS.value))
        if server_can_send_emails:
            email_manager.send_account_deleted_email(pending_deletion_request
                .user_id, pending_deletion_request.email)
        ...
        if server_can_send_emails:
            email_manager.send_account_deletion_failed_email(
                pending_deletion_request.user_id, pending_deletion_request.
                email)
...
def _delete_models_with_delete_at_end_policy(user_id: str) ->None:
    ...
    """
    for model_class in models.Registry.get_storage_model_classes([models.
        Names.AUTH, models.Names.USER]):
...
def _delete_profile_picture(pending_deletion_request: wipeout_domain.
    ...
    if fs.isfile(filename_webp):
        ...
        logging.error(
            '%s Profile picture of username %s in .webp format does not exists.'
             % (WIPEOUT_LOGS_PREFIX, username))
...
def remove_user_from_user_groups(user_id: str) ->None:
    ...
    user_group_models: Sequence[user_models.UserGroupModel
        ] = user_models.UserGroupModel.query(user_models.UserGroupModel.
        user_ids == user_id).fetch()

    @transaction_services.run_in_transaction_wrapper
    def _remove_user_from_groups_transactional(group_models: List[
        user_models.UserGroupModel]) ->None:
    ...
    for i in range(0, len(user_group_models), feconf.
...
def delete_user(pending_deletion_request: wipeout_domain.PendingDeletionRequest
    ) ->None:
    ...
    if feconf.ROLE_ID_MOBILE_LEARNER not in user_roles:
        ...
        _pseudonymize_one_model_class(pending_deletion_request,
            improvements_models.ExplorationStatsTaskEntryModel,
            'resolver_id', models.Names.IMPROVEMENTS)
        _pseudonymize_one_model_class(pending_deletion_request,
            app_feedback_report_models.AppFeedbackReportModel,
            'scrubbed_by', models.Names.APP_FEEDBACK_REPORT)
        ...
        _pseudonymize_activity_models_without_associated_rights_models(
            ...
            question_models.QuestionSnapshotMetadataModel, question_models.
            QuestionCommitLogEntryModel, 'question_id')
        _pseudonymize_activity_models_without_associated_rights_models(
            pending_deletion_request, models.Names.SKILL, skill_models.
            SkillSnapshotMetadataModel, skill_models.
            SkillCommitLogEntryModel, 'skill_id')
        _pseudonymize_activity_models_without_associated_rights_models(
            pending_deletion_request, models.Names.STORY, story_models.
            StorySnapshotMetadataModel, story_models.
            StoryCommitLogEntryModel, 'story_id')
        _pseudonymize_activity_models_without_associated_rights_models(
            pending_deletion_request, models.Names.SUBTOPIC,
            subtopic_models.SubtopicPageSnapshotMetadataModel,
            subtopic_models.SubtopicPageCommitLogEntryModel, 'subtopic_page_id'
            )
        _pseudonymize_activity_models_with_associated_rights_models(
            pending_deletion_request, models.Names.EXPLORATION, exp_models.
            ExplorationSnapshotMetadataModel, exp_models.
            ExplorationRightsSnapshotMetadataModel, exp_models.
            ExplorationRightsSnapshotContentModel, exp_models.
            ExplorationCommitLogEntryModel, 'exploration_id', feconf.
            EXPLORATION_RIGHTS_CHANGE_ALLOWED_COMMANDS, ['owner_ids',
            'editor_ids', 'voice_artist_ids', 'viewer_ids'])
        _remove_user_id_from_contributors_in_summary_models(user_id,
        ...
        _pseudonymize_activity_models_with_associated_rights_models(
            ...
            'collection_id', feconf.
            COLLECTION_RIGHTS_CHANGE_ALLOWED_COMMANDS, ['owner_ids',
            'editor_ids', 'voice_artist_ids', 'viewer_ids'])
        _remove_user_id_from_contributors_in_summary_models(user_id,
            collection_models.CollectionSummaryModel)
        _pseudonymize_activity_models_with_associated_rights_models(
            pending_deletion_request, models.Names.TOPIC, topic_models.
            TopicSnapshotMetadataModel, topic_models.
            TopicRightsSnapshotMetadataModel, topic_models.
            TopicRightsSnapshotContentModel, topic_models.
            TopicCommitLogEntryModel, 'topic_id', feconf.
            TOPIC_RIGHTS_CHANGE_ALLOWED_COMMANDS, ['manager_ids'])
...
def _verify_profile_picture_is_deleted(username: str) ->bool:
    ...
    if fs.isfile(filename_png):
        logging.error(
            '%s Profile picture in .png format is not deleted for user having username %s.'
             % (WIPEOUT_LOGS_PREFIX, username))
        all_profile_images_deleted = False
    elif fs.isfile(filename_webp):
        logging.error(
            '%s Profile picture in .webp format is not deleted for user having username %s.'
             % (WIPEOUT_LOGS_PREFIX, username))
        all_profile_images_deleted = False
    return all_profile_images_deleted


def verify_user_deleted(user_id: str, include_delete_at_end_models: bool=False
    ) ->bool:
    ...
    if not auth_services.verify_external_auth_associations_are_deleted(user_id
        ...
        return False
    policies_not_to_verify = [base_models.DELETION_POLICY.KEEP, base_models
    ...
    for model_class in models.Registry.get_all_storage_model_classes():
        if (model_class.get_deletion_policy() not in policies_not_to_verify and
...
def remove_user_from_activities_with_associated_rights_models(user_id: str
    ) ->None:
    ...
    subscribed_exploration_summaries = (exp_fetchers.
        get_exploration_summaries_where_user_has_role(user_id))
    explorations_to_be_deleted_ids = [exp_summary.id for exp_summary in
        subscribed_exploration_summaries if exp_summary.is_private() and
        exp_summary.is_solely_owned_by_user(user_id)]
    exp_services.delete_explorations(user_id,
        explorations_to_be_deleted_ids, force_deletion=True)
    explorations_to_release_ownership_ids = [exp_summary.id for exp_summary in
        subscribed_exploration_summaries if not exp_summary.is_private() and
        exp_summary.is_solely_owned_by_user(user_id) and not exp_summary.
        community_owned]
    for exp_id in explorations_to_release_ownership_ids:
        rights_manager.release_ownership_of_exploration(user_services.
            get_system_user(), exp_id)
    explorations_to_remove_user_from_ids = [exp_summary.id for exp_summary in
        subscribed_exploration_summaries if not exp_summary.
        is_solely_owned_by_user(user_id) and exp_summary.
        does_user_have_any_role(user_id)]
    for exp_id in explorations_to_remove_user_from_ids:
        rights_manager.deassign_role_for_exploration(user_services.
            get_system_user(), exp_id, user_id)
    explorations_rights = (rights_manager.
        get_exploration_rights_where_user_is_owner(user_id))
    explorations_to_be_deleted_ids = [exploration_rights.id for
        exploration_rights in explorations_rights if exploration_rights.
        is_private() and exploration_rights.is_solely_owned_by_user(user_id)]
    exp_services.delete_explorations(user_id,
        explorations_to_be_deleted_ids, force_deletion=True)
    subscribed_collection_summaries = (collection_services.
        get_collection_summaries_where_user_has_role(user_id))
    collections_to_be_deleted_ids = [col_summary.id for col_summary in
        subscribed_collection_summaries if col_summary.is_private() and
        col_summary.is_solely_owned_by_user(user_id)]
    collection_services.delete_collections(user_id,
        collections_to_be_deleted_ids, force_deletion=True)
    collections_to_release_ownership_ids = [col_summary.id for col_summary in
        subscribed_collection_summaries if not col_summary.is_private() and
        col_summary.is_solely_owned_by_user(user_id) and not col_summary.
        community_owned]
    for col_id in collections_to_release_ownership_ids:
        rights_manager.release_ownership_of_collection(user_services.
            get_system_user(), col_id)
    collections_to_remove_user_from_ids = [col_summary.id for col_summary in
        subscribed_collection_summaries if not col_summary.
        is_solely_owned_by_user(user_id) and col_summary.
        does_user_have_any_role(user_id)]
    for col_id in collections_to_remove_user_from_ids:
        rights_manager.deassign_role_for_collection(user_services.
            get_system_user(), col_id, user_id)
    collection_rights = (rights_manager.
        get_collection_rights_where_user_is_owner(user_id))
    collections_to_be_deleted_ids = [collection_rights.id for
        collection_rights in collection_rights if collection_rights.
        is_private() and collection_rights.is_solely_owned_by_user(user_id)]
    collection_services.delete_collections(user_id,
        collections_to_be_deleted_ids, force_deletion=True)
    topic_services.deassign_user_from_all_topics(user_services.
        get_system_user(), user_id)


def _generate_entity_to_pseudonymized_ids_mapping(entity_ids: List[str]
    ) ->Dict[str, str]:
    ...
    Returns:
        ...
        user ID.
    """
    return {entity_id: user_models.PseudonymizedUserModel.get_new_id('') for
...
def _save_pseudonymizable_entity_mappings_to_same_pseudonym(
    ...
    entity_category: models.Names, entity_ids: List[str]) ->None:
    ...
    Args:
        ...
        entity_ids: list(str). List of entity IDs for which to generate new
            ...
            in some way by the user who is currently being deleted.
    """
    if (entity_category.value not in pending_deletion_request.
        pseudonymizable_entity_mappings):
        pseudonymized_id = user_models.PseudonymizedUserModel.get_new_id('')
        pending_deletion_request.pseudonymizable_entity_mappings[
            entity_category.value] = {entity_id: pseudonymized_id for
            entity_id in entity_ids}
        save_pending_deletion_requests([pending_deletion_request])
...
def _collect_and_save_entity_ids_from_snapshots_and_commits(
    pending_deletion_request: wipeout_domain.PendingDeletionRequest,
    activity_category: models.Names, snapshot_metadata_model_classes: List[
    Type[base_models.BaseSnapshotMetadataModel]], commit_log_model_class:
    Optional[Type[base_models.BaseCommitLogEntryModel]],
    commit_log_model_field_name: Optional[str]) ->Tuple[List[base_models.
    BaseSnapshotMetadataModel], List[base_models.BaseCommitLogEntryModel]]:
    ...
    for snapshot_model_class in snapshot_metadata_model_classes:
        snapshot_metadata_models.extend(snapshot_model_class.query(
            datastore_services.any_of(snapshot_model_class.committer_id ==
            user_id, snapshot_model_class.commit_cmds_user_ids == user_id, 
            snapshot_model_class.content_user_ids == user_id)).fetch())
    snapshot_metadata_ids = set(model.get_unversioned_instance_id() for
        model in snapshot_metadata_models)
    commit_log_ids = set()
    ...
    if commit_log_model_class is not None:
        commit_log_models = list(commit_log_model_class.query(
            commit_log_model_class.user_id == user_id).fetch())
        if commit_log_model_field_name is None:
            raise Exception(
                'Field name can only be None when commit log model class is not provided.'
                )
        commit_log_ids = set(getattr(model, commit_log_model_field_name) for
            model in commit_log_models)
        if snapshot_metadata_ids != commit_log_ids:
            logging.error(
                "%s The commit log model '%s' and snapshot models %s IDs differ. Snapshots without commit logs: %s, commit logs without snapshots: %s."
                 % (WIPEOUT_LOGS_PREFIX, commit_log_model_class.__name__, [
                snapshot_metadata_model_class.__name__ for
                snapshot_metadata_model_class in
                snapshot_metadata_model_classes], list(
                snapshot_metadata_ids - commit_log_ids), list(
                commit_log_ids - snapshot_metadata_ids)))
...
def _pseudonymize_config_models(pending_deletion_request: wipeout_domain.
    PendingDeletionRequest) ->None:
    ...
    """
    snapshot_model_classes = (config_models.
        PlatformParameterSnapshotMetadataModel,)
    snapshot_metadata_models, _ = (
        _collect_and_save_entity_ids_from_snapshots_and_commits(
        pending_deletion_request, models.Names.CONFIG, list(
        snapshot_model_classes), None, None))

    @transaction_services.run_in_transaction_wrapper
    def _pseudonymize_models_transactional(activity_related_models: List[
        base_models.BaseModel], pseudonymized_id: str) ->None:
        ...
        Args:
            ...
            pseudonymized_id: str. New pseudonymized user ID to be used for
                the models.
        """
        metadata_models = [model for model in activity_related_models if
            isinstance(model, snapshot_model_classes)]
    ...
    config_ids_to_pids = (pending_deletion_request.
        pseudonymizable_entity_mappings[models.Names.CONFIG.value])
    for config_id, pseudonymized_id in config_ids_to_pids.items():
        config_related_models = [model for model in
            snapshot_metadata_models if model.get_unversioned_instance_id() ==
            config_id]
        for i in range(0, len(config_related_models), feconf.
            MAX_NUMBER_OF_OPS_IN_TRANSACTION):
            _pseudonymize_models_transactional(config_related_models[i:i +
                feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION], pseudonymized_id)


def _pseudonymize_activity_models_without_associated_rights_models(
    pending_deletion_request: wipeout_domain.PendingDeletionRequest,
    activity_category: models.Names, snapshot_model_class: Type[base_models
    .BaseSnapshotMetadataModel], commit_log_model_class: Type[base_models.
    BaseCommitLogEntryModel], commit_log_model_field_name: str) ->None:
    ...
    snapshot_metadata_models, commit_log_models = (
        _collect_and_save_entity_ids_from_snapshots_and_commits(
        pending_deletion_request, activity_category, [snapshot_model_class],
        commit_log_model_class, commit_log_model_field_name))
    ...
    def _pseudonymize_models_transactional(activity_related_models: List[
        ...
        """
        metadata_models = [model for model in activity_related_models if
            isinstance(model, snapshot_model_class)]
        for metadata_model in metadata_models:
            metadata_model.committer_id = pseudonymized_id
            metadata_model.update_timestamps()
        commit_log_models = [model for model in activity_related_models if
            isinstance(model, commit_log_model_class)]
    ...
    activity_ids_to_pids = (pending_deletion_request.
        pseudonymizable_entity_mappings[activity_category.value])
    for activity_id, pseudonymized_id in activity_ids_to_pids.items():
        activity_related_models: List[base_models.BaseModel] = [model for
            model in snapshot_metadata_models if model.
            get_unversioned_instance_id() == activity_id]
        ...
        for i in range(0, len(activity_related_models), feconf.
            MAX_NUMBER_OF_OPS_IN_TRANSACTION):
            _pseudonymize_models_transactional(activity_related_models[i:i +
                feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION], pseudonymized_id)


def _pseudonymize_activity_models_with_associated_rights_models(
    pending_deletion_request: wipeout_domain.PendingDeletionRequest,
    activity_category: models.Names, snapshot_metadata_model_class: Type[
    base_models.BaseSnapshotMetadataModel],
    rights_snapshot_metadata_model_class: Type[base_models.
    BaseSnapshotMetadataModel], rights_snapshot_content_model_class: Type[
    base_models.BaseSnapshotContentModel], commit_log_model_class: Type[
    base_models.BaseCommitLogEntryModel], commit_log_model_field_name: str,
    allowed_commands: List[feconf.ValidCmdDict], rights_user_id_fields:
    List[str]) ->None:
    ...
    snapshot_metadata_models, commit_log_models = (
        _collect_and_save_entity_ids_from_snapshots_and_commits(
        pending_deletion_request, activity_category, [
        snapshot_metadata_model_class, rights_snapshot_metadata_model_class
        ], commit_log_model_class, commit_log_model_field_name))
    ...
    def _pseudonymize_models_transactional(activity_related_models: List[
        ...
        pseudonymized_username = user_services.get_pseudonymous_username(
            pseudonymized_id)
        snapshot_metadata_models = [model for model in
            activity_related_models if isinstance(model,
            snapshot_metadata_model_class)]
        ...
        rights_snapshot_metadata_models = [model for model in
            activity_related_models if isinstance(model,
            rights_snapshot_metadata_model_class)]
        for rights_snapshot_metadata_model in rights_snapshot_metadata_models:
            for commit_cmd in rights_snapshot_metadata_model.commit_cmds:
                user_id_attribute_names = next(cmd[
                    'user_id_attribute_names'] for cmd in allowed_commands if
                    cmd['name'] == commit_cmd['cmd'])
            ...
            assign_commit_message_match = re.match(rights_domain.
                ASSIGN_ROLE_COMMIT_MESSAGE_REGEX,
            ...
            if assign_commit_message_match:
                rights_snapshot_metadata_model.commit_message = (
                    ...
                    pseudonymized_username, assign_commit_message_match.
                    group(2), assign_commit_message_match.group(3)))
            deassign_commit_message_match = re.match(rights_domain.
                DEASSIGN_ROLE_COMMIT_MESSAGE_REGEX,
                rights_snapshot_metadata_model.commit_message)
            if deassign_commit_message_match:
                rights_snapshot_metadata_model.commit_message = (
                    rights_domain.DEASSIGN_ROLE_COMMIT_MESSAGE_TEMPLATE % (
                    pseudonymized_username, deassign_commit_message_match.
                    group(2)))
            rights_snapshot_metadata_model.content_user_ids = [(
                pseudonymized_id if model_user_id == user_id else
                model_user_id) for model_user_id in
                rights_snapshot_metadata_model.content_user_ids]
        ...
        rights_snapshot_content_models = [model for model in
            activity_related_models if isinstance(model,
            rights_snapshot_content_model_class)]
        for rights_snapshot_content_model in rights_snapshot_content_models:
            ...
            for field_name in rights_user_id_fields:
                model_dict[field_name] = [(pseudonymized_id if field_id ==
                    user_id else field_id) for field_id in model_dict[
                    field_name]]
    ...
    activity_ids_to_pids = (pending_deletion_request.
        pseudonymizable_entity_mappings[activity_category.value])
    for activity_id, pseudonymized_id in activity_ids_to_pids.items():
        activity_related_snapshot_metadata_models = [model for model in
            snapshot_metadata_models if model.get_unversioned_instance_id() ==
            activity_id]
        activity_related_rights_snapshots_ids = [model.id for model in
            activity_related_snapshot_metadata_models if isinstance(model,
            rights_snapshot_metadata_model_class)]
        ...
        activity_related_models: List[base_models.BaseModel] = [model for
            model in commit_log_models if getattr(model,
            commit_log_model_field_name) == activity_id]
...
def _remove_user_id_from_contributors_in_summary_models(user_id: str,
    summary_model_class: Union[Type[collection_models.
    CollectionSummaryModel], Type[exp_models.ExpSummaryModel]]) ->None:
    ...
    """
    related_summary_models: Sequence[Union[collection_models.
        CollectionSummaryModel, exp_models.ExpSummaryModel]
        ] = summary_model_class.query(summary_model_class.contributor_ids ==
        user_id).fetch()

    @transaction_services.run_in_transaction_wrapper
    def _remove_user_id_from_models_transactional(summary_models: List[
        base_models.BaseModel]) ->None:
        """Remove the user ID from contributor_ids and contributor_summary
        fields.
        ...
        for summary_model in related_summary_models:
            summary_model.contributor_ids = [contributor_id for
                contributor_id in summary_model.contributor_ids if 
    ...
    for i in range(0, len(related_summary_models), feconf.
        MAX_NUMBER_OF_OPS_IN_TRANSACTION):
        _remove_user_id_from_models_transactional(related_summary_models[i:
            i + feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION])


def _pseudonymize_one_model_class(pending_deletion_request: wipeout_domain.
    PendingDeletionRequest, model_class: Type[base_models.BaseModel],
    name_of_property_containing_user_ids: str, module_name: models.Names
    ) ->None:
    ...
    """
    user_id = pending_deletion_request.user_id
    models_to_pseudonymize: Sequence[base_models.BaseModel
        ] = model_class.query(getattr(model_class,
        name_of_property_containing_user_ids) == user_id).fetch()
    ...
    report_ids_to_pids = (pending_deletion_request.
        pseudonymizable_entity_mappings[module_name.value])

    @transaction_services.run_in_transaction_wrapper
    def _pseudonymize_models_transactional(models_to_pseudonymize: List[
        base_models.BaseModel]) ->None:
        """Pseudonymize user ID fields in the models.
        ...
        for model in models_to_pseudonymize:
            setattr(model, name_of_property_containing_user_ids,
    ...
    for i in range(0, len(models_to_pseudonymize), feconf.
        MAX_NUMBER_OF_OPS_IN_TRANSACTION):
        _pseudonymize_models_transactional(models_to_pseudonymize[i:i +
            feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION])


def _pseudonymize_feedback_models(pending_deletion_request: wipeout_domain.
    PendingDeletionRequest) ->None:
    ...
    feedback_thread_models: Sequence[feedback_models.GeneralFeedbackThreadModel
        ] = feedback_thread_model_class.query(datastore_services.any_of(
    ...
    feedback_message_models: Sequence[feedback_models.
        GeneralFeedbackMessageModel] = feedback_message_model_class.query(
    ...
    general_suggestion_models: Sequence[suggestion_models.
        GeneralSuggestionModel] = suggestion_model_class.query(
        datastore_services.any_of(suggestion_model_class.author_id ==
        user_id, suggestion_model_class.final_reviewer_id == user_id)).fetch()
    ...
    @transaction_services.run_in_transaction_wrapper
    def _pseudonymize_models_transactional(feedback_related_models: List[
        base_models.BaseModel], pseudonymized_id: str) ->None:
        """Pseudonymize user ID fields in the models.
        ...
        Args:
            ...
            pseudonymized_id: str. New pseudonymized user ID to be used for
                the models.
        """
        feedback_thread_models = [model for model in
            feedback_related_models if isinstance(model,
            feedback_thread_model_class)]
        for feedback_thread_model in feedback_thread_models:
            ...
            if (feedback_thread_model.last_nonempty_message_author_id ==
                user_id):
                feedback_thread_model.last_nonempty_message_author_id = (
                    pseudonymized_id)
            feedback_thread_model.update_timestamps()
        feedback_message_models = [model for model in
            feedback_related_models if isinstance(model,
        ...
        general_suggestion_models = [model for model in
            feedback_related_models if isinstance(model,
        ...
        datastore_services.put_multi(all_models)
    feedback_ids_to_pids = (pending_deletion_request.
    ...
    for feedback_id, pseudonymized_id in feedback_ids_to_pids.items():
        feedback_related_models: List[base_models.BaseModel] = [model for
            model in feedback_thread_models if model.id == feedback_id]
        ...
        for i in range(0, len(feedback_related_models), feconf.
            MAX_NUMBER_OF_OPS_IN_TRANSACTION):
            _pseudonymize_models_transactional(feedback_related_models[i:i +
                feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION], pseudonymized_id)


def _pseudonymize_blog_post_models(pending_deletion_request: wipeout_domain
    .PendingDeletionRequest) ->None:
    ...
    blog_post_models_list: Sequence[blog_models.BlogPostModel
        ] = blog_post_model_class.query(blog_post_model_class.author_id ==
        user_id).fetch()
    ...
    blog_post_summary_models: Sequence[blog_models.BlogPostSummaryModel
        ...
        blog_post_summary_model_class.author_id == user_id).fetch()
    ...
    _save_pseudonymizable_entity_mappings_to_different_pseudonyms(
        pending_deletion_request, models.Names.BLOG, list(
        blog_related_model_ids))
    blog_models.BlogPostRightsModel.deassign_user_from_all_blog_posts(user_id)

    @transaction_services.run_in_transaction_wrapper
    def _pseudonymize_models_transactional(blog_posts_related_models: List[
        base_models.BaseModel], pseudonymized_id: str) ->None:
        """Pseudonymize user ID fields in the models.
        ...
        blog_post_models_list: List[Union[blog_models.BlogPostModel,
            blog_models.BlogPostSummaryModel, blog_models.
            BlogAuthorDetailsModel]] = [model for model in
            blog_posts_related_models if isinstance(model,
            blog_post_model_class)]
        ...
        blog_post_summary_models_list: List[Union[blog_models.BlogPostModel,
            blog_models.BlogPostSummaryModel, blog_models.
            BlogAuthorDetailsModel]] = [model for model in
            blog_posts_related_models if isinstance(model,
            blog_post_summary_model_class)]
        ...
        all_models: List[Union[blog_models.BlogPostModel, blog_models.
            BlogPostSummaryModel, blog_models.BlogAuthorDetailsModel]
        ...
        for model in blog_posts_related_models:
            if isinstance(model, blog_author_details_model_class):
                if model.author_id == user_id:
                    ...
                    break
        datastore_services.put_multi(all_models)
    blog_post_ids_to_pids = (pending_deletion_request.
    ...
    for blog_related_ids, pseudonymized_id in blog_post_ids_to_pids.items():
        blog_posts_related_models = [model for model in itertools.chain(
            blog_post_models_list, blog_post_summary_models, [
            blog_author_details_model]) if model is not None and model.id ==
            blog_related_ids]
        transaction_slices = utils.grouper(blog_posts_related_models,
            feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION)
        for transaction_slice in transaction_slices:
            _pseudonymize_models_transactional([m for m in
                transaction_slice if m is not None], pseudonymized_id)
...
def _pseudonymize_version_history_models(pending_deletion_request:
    ...
    version_history_models: Sequence[exp_models.ExplorationVersionHistoryModel
        ] = version_history_model_class.query(user_id ==
        version_history_model_class.committer_ids).fetch()

    @transaction_services.run_in_transaction_wrapper
    def _pseudonymize_models_transactional(version_history_models: List[
        exp_models.ExplorationVersionHistoryModel], exp_ids_to_pids: Dict[
        str, str]) ->None:
        """Pseudonymize user ID fields in the models.
        ...
        for model in version_history_models:
            ...
            if model.metadata_last_edited_committer_id == user_id:
                model.metadata_last_edited_committer_id = exp_ids_to_pids[model
                    .exploration_id]
            for idx, committer_id in enumerate(model.committer_ids):
                if committer_id == user_id:
                    model.committer_ids[idx] = exp_ids_to_pids[model.
                        exploration_id]
    ...
    for i in range(0, len(version_history_models), feconf.
        MAX_NUMBER_OF_OPS_IN_TRANSACTION):
        _pseudonymize_models_transactional(version_history_models[i:i +
            feconf.MAX_NUMBER_OF_OPS_IN_TRANSACTION], exp_ids_to_pids)
