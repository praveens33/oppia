...
exp_models, opportunity_models, suggestion_models, translation_models = (models
    .Registry.import_models([models.Names.EXPLORATION, models.Names.
    OPPORTUNITY, models.Names.SUGGESTION, models.Names.TRANSLATION]))
...
class RejectTranslationSuggestionsForTranslatedContentsJob(base_jobs.JobBase):
    ...
    def run(self) ->beam.PCollection[job_run_result.JobRunResult]:
        ...
        """
        entity_translation_models = _get_entity_translation_models(self.
            pipeline)
        updated_suggestion_dicts = (entity_translation_models | 
            'Get translation suggestion dicts' >> beam.ParDo(
            ComputeSuggestionsInReviewForTranslatedContents(
            is_datastore_change=True)) | 'Flatten the list' >> beam.FlatMap
            (lambda x: x))
        suggestion_dicts = (updated_suggestion_dicts | 
            'Get updated suggestion dicts' >> beam.Map(lambda
            updated_suggestion_dict: updated_suggestion_dict[
            'suggestion_dict']))
        updated_suggestions = (updated_suggestion_dicts | 
            'Get updated suggestion models' >> beam.Map(lambda
            updated_suggestion_dict: updated_suggestion_dict[
            'updated_suggestion']))
        job_run_results = (suggestion_dicts | 
            'Report the suggestions to be rejected' >> beam.Map(lambda
            result: job_run_result.JobRunResult.as_stdout(
            f'Results are - {result}')))
        updated_suggestions_count_job_run_results = (updated_suggestions | 
            'Updated translation suggestion models count' >>
            job_result_transforms.CountObjectsToJobRunResult(
            'REJECTED SUGGESTIONS COUNT'))
        unused_put_results = (updated_suggestions | 
            'Put models into the datastore' >> ndb_io.PutModels())
        return (job_run_results, updated_suggestions_count_job_run_results
            ) | 'Combine results' >> beam.Flatten()


class AuditRejectTranslationSuggestionsForTranslatedContentsJob(base_jobs.
    JobBase):
    ...
    def run(self) ->beam.PCollection[job_run_result.JobRunResult]:
        ...
        entity_translation_models = _get_entity_translation_models(self.
            pipeline)
        suggestion_dicts = (entity_translation_models | 
            'Get suggestions to be rejected list' >> beam.ParDo(
            ComputeSuggestionsInReviewForTranslatedContents(
            is_datastore_change=False)) | 'Flatten the list' >> beam.
            FlatMap(lambda x: x))
        job_run_results = (suggestion_dicts | 
            'Report the suggestions to be rejected' >> beam.Map(lambda
            result: job_run_result.JobRunResult.as_stdout(
...
class DeleteTranslationsForInvalidContentIDsJob(base_jobs.JobBase):
    ...
    def run(self) ->beam.PCollection[job_run_result.JobRunResult]:
        ...
        entity_translation_models = _get_entity_translation_models(self.
            pipeline)
        deletion_result_dicts = (entity_translation_models | 
            'Get deletion results' >> beam.ParDo(
            ComputeTranslationsWithInvalidContentIds(is_datastore_change=
            True)) | 'Filter out None values' >> beam.Filter(lambda x: x is not
            None))
        deleted_translations_count_job_run_results = (deletion_result_dicts |
            'Deleted translations counts' >> beam.Map(lambda x: x[
            'deleted_translations_count']) | 
            'Total deleted translations count' >> beam.CombineGlobally(sum) |
            'Only create result for non-zero number of objects' >> beam.
            Filter(lambda x: x > 0) | 
            'Report total deleted translations count' >> beam.Map(lambda
            result: job_run_result.JobRunResult.as_stdout(
            f'DELETED TRANSLATIONS COUNT SUCCESS: {result}')))
        updated_entity_translation_models = (deletion_result_dicts | 
            'Updated entity translation models' >> beam.Map(lambda x: x[
        ...
        invalid_translation_dicts = (deletion_result_dicts | 
            'Get invalid translation dicts' >> beam.Map(lambda x: x[
            'invalid_translation_dicts']) | 'Flatten the list' >> beam.
            FlatMap(lambda x: x))
        job_run_results = (invalid_translation_dicts | 
            'Report translations to be deleted' >> beam.Map(lambda result:
            job_run_result.JobRunResult.as_stdout(f'Results are - {result}')))
        latest_version_updated_entity_translation_models = (
            updated_entity_translation_models | 'Add entity id as key' >>
            beam.WithKeys(lambda model: model.entity_id) | 
            'Group by entity id' >> beam.GroupByKey() | 
            'Filter model with latest entity version' >> beam.ParDo(
            GetLatestModel()) | 
            'Get list of latest entity translation model' >> beam.Values())
        updated_exp_opportunity_models = (
            latest_version_updated_entity_translation_models | 
            'Get updated exploration opportunity models' >> beam.ParDo(
            ComputeUpdatedExpOpportunityModel()))
        unused_put_results = (updated_entity_translation_models,
            updated_exp_opportunity_models) | 'Merge lists' >> beam.Flatten(
            ) | 'Put models into the datastore' >> ndb_io.PutModels()
        return (job_run_results, deleted_translations_count_job_run_results,
            updated_entity_translation_models_count_job_run_results
            ) | 'Combine results' >> beam.Flatten()
...
class AuditDeleteTranslationsForInvalidContentIDsJob(base_jobs.JobBase):
    ...
    def run(self) ->beam.PCollection[job_run_result.JobRunResult]:
        ...
        entity_translation_models = _get_entity_translation_models(self.
            pipeline)
        invalid_translation_dicts = (entity_translation_models | 
            'Get invalid translation dicts' >> beam.ParDo(
            ComputeTranslationsWithInvalidContentIds(is_datastore_change=
            False)) | 'Flatten the list' >> beam.FlatMap(lambda x: x))
        job_run_results = (invalid_translation_dicts | 
            'Report translations to be deleted' >> beam.Map(lambda result:
            job_run_result.JobRunResult.as_stdout(f'Results are - {result}')))
        ...
        invalid_entity_translation_models_count_job_run_results = (
            invalid_translation_dicts | 
            'Invalid entity translation model ids' >> beam.Map(lambda x: x[
            'entity_translation_model_id']) | 'Create pair' >> beam.Map(lambda
            x: (x, None)) | 'Group pairs' >> beam.GroupByKey() | 
        ...
        return (job_run_results, invalid_translations_count_job_run_results,
            invalid_entity_translation_models_count_job_run_results
            ) | 'Combine results' >> beam.Flatten()


class ComputeSuggestionsInReviewForTranslatedContents(beam.DoFn):
    ...
    def process(self, entity_translation_model: translation_models.
        EntityTranslationsModel) ->Union[Iterable[List[Dict[str, Union[str,
        int, suggestion_models.GeneralSuggestionModel]]]], Iterable[List[
        Dict[str, Union[suggestion_models.GeneralSuggestionModel, Dict[str,
        ...
        with datastore_services.get_ndb_context():
            ...
            suggestions: Sequence[suggestion_models.GeneralSuggestionModel
                ...
                feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT, suggestion_models
                .GeneralSuggestionModel.target_id ==
                entity_translation_model.entity_id, suggestion_models.
                GeneralSuggestionModel.target_version_at_submission ==
                entity_translation_model.entity_version, suggestion_models.
                GeneralSuggestionModel.language_code ==
                entity_translation_model.language_code, suggestion_models.
                GeneralSuggestionModel.status == suggestion_models.
                STATUS_IN_REVIEW).fetch()
            if self.is_datastore_change:
                updated_suggestion_dicts: List[Dict[str, Union[
                    suggestion_models.GeneralSuggestionModel, Dict[str,
                    Union[str, int, suggestion_models.
                    GeneralSuggestionModel]]]]] = []
                for suggestion in suggestions:
                    if suggestion.change_cmd['content_id'
                        ] in content_ids_not_needing_update:
                        suggestion.status = suggestion_models.STATUS_REJECTED
                        suggestion.final_reviewer_id = (feconf.
                            SUGGESTION_BOT_USER_ID)
                        suggestion.last_updated = get_current_datetime_utc()
                        updated_suggestion_dicts.append({
                            'updated_suggestion': suggestion,
                            'suggestion_dict': {'entity_id':
                            entity_translation_model.entity_id,
                            'entity_version': entity_translation_model.
                            entity_version, 'entity_translation_model_id':
                            entity_translation_model.id, 'content_id':
                            suggestion.change_cmd['content_id'],
                            'suggestion_id': suggestion.id}})
                yield updated_suggestion_dicts
            else:
                suggestion_dicts: List[Dict[str, Union[str, int,
                    suggestion_models.GeneralSuggestionModel]]] = []
                for suggestion in suggestions:
                    if suggestion.change_cmd['content_id'
                        ] in content_ids_not_needing_update:
                        suggestion_dicts.append({'entity_id':
                            entity_translation_model.entity_id,
                            'entity_version': entity_translation_model.
                            entity_version, 'entity_translation_model_id':
                            entity_translation_model.id, 'content_id':
...
class ComputeTranslationsWithInvalidContentIds(beam.DoFn):
    ...
    def process(self, entity_translation_model: translation_models.
        EntityTranslationsModel) ->Union[Iterable[Optional[Dict[str, Union[
        translation_models.EntityTranslationsModel, int, List[Dict[str,
        Union[str, int]]]]]]], Iterable[List[Dict[str, Union[str, int]]]]]:
        ...
        with datastore_services.get_ndb_context():
            exp_model = exp_models.ExplorationModel.get(
                entity_translation_model.entity_id, strict=True, version=
            ...
            translated_content_ids = list(entity_translation_model.
                translations.keys())
            ...
            if self.is_datastore_change:
                ...
                for content_id in translated_content_ids:
                    if content_id not in exp_content_ids:
                        invalid_translation_dicts.append({'entity_id':
                            entity_translation_model.entity_id,
                            'entity_version': entity_translation_model.
                            entity_version, 'entity_translation_model_id':
                            entity_translation_model.id, 'content_id':
                        ...
                        is_updated = True
                if is_updated:
                    result: Dict[str, Union[translation_models.
                        EntityTranslationsModel, int, List[Dict[str, Union[
                        str, int]]]]] = {'entity_translation_model':
                        entity_translation_model,
                        'deleted_translations_count':
                        deleted_translations_count,
...
class ComputeUpdatedExpOpportunityModel(beam.DoFn):
    """DoFn to compute updated exp opportunity model."""

    def process(self, entity_translation_model: translation_models.
        EntityTranslationsModel) ->Iterable[opportunity_models.
        ...
        """
        with datastore_services.get_ndb_context():
            exp_opportunity_model = (opportunity_models.
                ExplorationOpportunitySummaryModel.get(
                entity_translation_model.entity_id))
            new_translation_count = len(entity_translation_model.
                translations.keys())
            exp_opportunity_model.translation_counts[entity_translation_model
                .language_code] = new_translation_count
            yield exp_opportunity_model


class GetLatestModel(beam.DoFn):
    """DoFn to compute latest entity translation model."""

    def process(self, element: Tuple[str, List[translation_models.
        EntityTranslationsModel]]) ->Iterable[Tuple[str, translation_models
        ...
        with datastore_services.get_ndb_context():
            ...
            version_list = list(model.entity_version for model in
...
def _get_entity_translation_models(pipeline: beam.Pipeline) ->beam.PCollection[
    ...
    Returns:
        ...
        EntityTranslationsModel.
    """
    entity_translation_models = (pipeline | 
        'Get all entity translation models' >> ndb_io.GetModels(
        translation_models.EntityTranslationsModel.get_all(include_deleted=
        False)))
