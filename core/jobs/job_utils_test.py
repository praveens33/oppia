...
class GetModelClassTests(test_utils.TestBase):
    ...
    def test_get_from_existing_model(self) ->None:
        self.assertIs(job_utils.get_model_class('BaseModel'), base_models.
            BaseModel)
...
class GetModelKindTests(test_utils.TestBase):
    ...
    def test_get_from_datastore_model_class(self) ->None:
        self.assertEqual(job_utils.get_model_kind(base_models.BaseModel),
            'BaseModel')
...
class BeamEntityToAndFromModelTests(test_utils.TestBase):
    ...
    def test_get_model_from_beam_entity(self) ->None:
        beam_entity = beam_datastore_types.Entity(beam_datastore_types.Key(
            ('FooModel', 'abc'), project=feconf.OPPIA_PROJECT_ID, namespace
            =self.namespace))
        beam_entity.set_properties({'prop': '123'})
        self.assertEqual(FooModel(id='abc', project=feconf.OPPIA_PROJECT_ID,
            prop='123'), job_utils.get_ndb_model_from_beam_entity(beam_entity))

    def test_get_beam_key_from_ndb_key(self) ->None:
        beam_key = beam_datastore_types.Key(('FooModel', 'abc'), project=
            feconf.OPPIA_PROJECT_ID, namespace=self.namespace)
    ...
    def test_get_model_from_beam_entity_with_time(self) ->None:
        utcnow = get_current_datetime_utc()
        beam_entity = beam_datastore_types.Entity(beam_datastore_types.Key(
            ('CoreModel', 'abc'), project=feconf.OPPIA_PROJECT_ID,
            namespace=self.namespace))
        beam_entity.set_properties({'prop': 3.14, 'created_on': utcnow.
            replace(tzinfo=datetime.timezone.utc), 'last_updated': None,
            'deleted': False})
        self.assertEqual(CoreModel(id='abc', project=feconf.
            OPPIA_PROJECT_ID, prop=3.14, created_on=utcnow), job_utils.
            get_ndb_model_from_beam_entity(beam_entity))

    def test_from_and_then_to_model(self) ->None:
        model = FooModel(id='abc', project=feconf.OPPIA_PROJECT_ID, prop='123')
        self.assertEqual(model, job_utils.get_ndb_model_from_beam_entity(
            job_utils.get_beam_entity_from_ndb_model(model)))

    def test_from_and_then_to_beam_entity(self) ->None:
        beam_entity = beam_datastore_types.Entity(beam_datastore_types.Key(
            ('CoreModel', 'abc'), project=feconf.OPPIA_PROJECT_ID))
        beam_entity.set_properties({'prop': 123, 'created_on': datetime.
            datetime.utcnow(), 'last_updated': datetime.datetime.utcnow(),
            'deleted': False})
        self.assertEqual(beam_entity, job_utils.
            get_beam_entity_from_ndb_model(job_utils.
            get_ndb_model_from_beam_entity(beam_entity)))
...
class GetBeamQueryFromNdbQueryTests(test_utils.TestBase):
    ...
    def test_query_with_range_like_filter(self) ->None:
        ...
        beam_query = job_utils.get_beam_query_from_ndb_query(query)
        self.assertEqual(beam_query.filters, (('prop', '>=', 3), ('prop',
            '<', 6)))
