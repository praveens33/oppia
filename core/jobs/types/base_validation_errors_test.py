...
class AuditErrorsTestBase(core_test_utils.TestBase):
    ...
    NOW: datetime.datetime = get_current_datetime_utc()
...
class ErrorMessageTests(core_test_utils.TestBase):
    ...
    def test_error_message_with_wrong_input(self) ->None:
        error = base_validation_errors.BaseAuditError('testing string',
            'non-existing model', None)
...
class BaseAuditErrorTests(AuditErrorsTestBase):
    ...
    def test_message_raises_type_error_if_assigned_a_non_string_value(self
        ...
        class ErrorWithIntMessage(base_validation_errors.BaseAuditError):
            ...
            def __init__(self, model: Union[base_models.BaseModel, str]
                ) ->None:
    ...
    def test_equality_between_same_types_and_same_values(self) ->None:
        self.assertEqual(FooError(self.model), FooError(job_utils.
            clone_model(self.model)))

    def test_equality_between_same_types_and_different_values(self) ->None:
        self.assertNotEqual(FooError(self.model), FooError(job_utils.
            clone_model(self.model, id='987')))

    def test_hashable(self) ->None:
        set_of_errors = {FooError(self.model), FooError(job_utils.
            clone_model(self.model))}
    ...
    def test_pickling_sub_classes(self) ->None:
        ...
        pickled_foo_error, pickled_bar_error = pickle.dumps(foo_error
            ), pickle.dumps(bar_error)
        unpickled_foo_error, unpickled_bar_error = pickle.loads(
            pickled_foo_error), pickle.loads(pickled_bar_error)
...
class InconsistentTimestampsErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        model = base_models.BaseModel(id='123', created_on=self.NOW,
            last_updated=self.YEAR_AGO)
        error = base_validation_errors.InconsistentTimestampsError(model)
        self.assertEqual(error.stderr, 
            'InconsistentTimestampsError in BaseModel(id="123"): created_on=%r is later than last_updated=%r'
...
class InvalidCommitStatusErrorTests(AuditErrorsTestBase):
    ...
    def test_message_for_invalid_post_commit_status(self) ->None:
        model = base_models.BaseCommitLogEntryModel(id='123', created_on=
            self.YEAR_AGO, last_updated=self.NOW, commit_type=
            'invalid-type', user_id='', post_commit_status='invalid',
            post_commit_is_private=False, commit_cmds=[])
        error = base_validation_errors.InvalidCommitStatusError(model)
        self.assertEqual(error.stderr,
            'InvalidCommitStatusError in BaseCommitLogEntryModel(id="123"): post_commit_status is invalid'
            )

    def test_message_for_private_post_commit_status(self) ->None:
        model = base_models.BaseCommitLogEntryModel(id='123', created_on=
            self.YEAR_AGO, last_updated=self.NOW, commit_type=
            'invalid-type', user_id='', post_commit_status='private',
            post_commit_is_private=False, commit_cmds=[])
        error = base_validation_errors.InvalidPrivateCommitStatusError(model)
        self.assertEqual(error.stderr,
            'InvalidPrivateCommitStatusError in BaseCommitLogEntryModel(id="123"): post_commit_status=private but post_commit_is_private=False'
            )

    def test_message_for_public_post_commit_status(self) ->None:
        model = base_models.BaseCommitLogEntryModel(id='123', created_on=
            self.YEAR_AGO, last_updated=self.NOW, commit_type=
            'invalid-type', user_id='', post_commit_status='public',
            post_commit_is_private=True, commit_cmds=[])
        error = base_validation_errors.InvalidPrivateCommitStatusError(model)
        self.assertEqual(error.stderr,
            'InvalidPrivateCommitStatusError in BaseCommitLogEntryModel(id="123"): post_commit_status=public but post_commit_is_private=True'
            )

    def test_message_for_public_post_commit_status_raise_exception(self
        ) ->None:
        model = base_models.BaseCommitLogEntryModel(id='123', created_on=
            self.YEAR_AGO, last_updated=self.NOW, commit_type='create',
            user_id='', post_commit_status='public',
            post_commit_community_owned=False, commit_cmds=[])
        error = base_validation_errors.InvalidPublicCommitStatusError(model)
        self.assertEqual(error.stderr,
            'InvalidPublicCommitStatusError in BaseCommitLogEntryModel(id="123"): post_commit_status=public but post_commit_community_owned=False'
            )
...
class ModelMutatedDuringJobErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        ...
        self.assertEqual(error.stderr, 
            'ModelMutatedDuringJobError in BaseModel(id="123"): last_updated=%r is later than the audit job\'s start time'
             % model.last_updated)
...
class ModelIdRegexErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        model = base_models.BaseModel(id='?!"', created_on=self.YEAR_AGO,
            last_updated=self.NOW)
        error = base_validation_errors.ModelIdRegexError(model, '[abc]{3}')
        self.assertEqual(error.stderr,
            'ModelIdRegexError in BaseModel(id="?!\\""): id does not match the expected regex="[abc]{3}"'
            )
...
class ModelExpiredErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        model = base_models.BaseModel(id='123', deleted=True, created_on=
            self.YEAR_AGO, last_updated=self.YEAR_AGO)
        error = base_validation_errors.ModelExpiredError(model)
        self.assertEqual(error.stderr, 
            'ModelExpiredError in BaseModel(id="123"): deleted=True when older than %d days'
...
class ModelDomainObjectValidateErrorTests(AuditErrorsTestBase):
    ...
    def test_model_domain_object_validate_error(self) ->None:
        ...
        error = base_validation_errors.ModelDomainObjectValidateError(model,
            error_message)
        msg = (
            'ModelDomainObjectValidateError in BaseModel(id="123"): Entity fails domain validation with the error: %s'
             % error_message)
...
class InvalidCommitTypeErrorTests(AuditErrorsTestBase):
    ...
    def test_model_invalid_id_error(self) ->None:
        model = base_models.BaseCommitLogEntryModel(id='123', created_on=
            self.YEAR_AGO, last_updated=self.NOW, commit_type=
            'invalid-type', user_id='', post_commit_status='', commit_cmds=[])
        error = base_validation_errors.InvalidCommitTypeError(model)
        self.assertEqual(error.stderr,
            'InvalidCommitTypeError in BaseCommitLogEntryModel(id="123"): Commit type invalid-type is not allowed'
            )
...
class ModelRelationshipErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        error = base_validation_errors.ModelRelationshipError(model_property
            .ModelProperty(FooModel, FooModel.bar_id), '123', 'BarModel', '123'
            )
        self.assertEqual(error.stderr,
            'ModelRelationshipError in FooModel(id="123"): FooModel.bar_id="123" should correspond to the ID of an existing BarModel, but no such model exists'
            )
...
class CommitCmdsNoneErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        model = base_models.BaseCommitLogEntryModel(id='invalid',
            created_on=self.YEAR_AGO, last_updated=self.NOW, commit_type=
            'test', user_id='', post_commit_status='', commit_cmds=[{}])
        error = base_validation_errors.CommitCmdsNoneError(model)
        self.assertEqual(error.stderr,
            'CommitCmdsNoneError in BaseCommitLogEntryModel(id="invalid"): No commit command domain object defined for entity with commands: [{}]'
            )
...
class CommitCmdsValidateErrorTests(AuditErrorsTestBase):
    ...
    def test_message(self) ->None:
        ...
        error = base_validation_errors.CommitCmdsValidateError(model, {
            'cmd-invalid': 'invalid_test_command'}, error_message)
        self.assertEqual(error.stderr,
            'CommitCmdsValidateError in BaseCommitLogEntryModel(id="invalid"): Commit command domain validation for command: {\'cmd-invalid\': \'invalid_test_command\'} failed with error: Missing cmd key in change dict'
            )
