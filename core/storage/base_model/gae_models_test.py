...
class BaseModelUnitTests(test_utils.GenericTestBase):
    ...
    def test_error_cases_for_get_method(self) ->None:
        with self.assertRaisesRegex(base_models.BaseModel.
            EntityNotFoundError,
    ...
    def test_put(self) ->None:
        ...
        model.put()
        model_id = model.id
        self.assertIsNotNone(base_models.BaseModel.get_by_id(model_id).
            created_on)
        self.assertIsNotNone(base_models.BaseModel.get_by_id(model_id).
            last_updated)
        ...
        model.update_timestamps(update_last_updated_time=False)
        model.put()
        self.assertEqual(base_models.BaseModel.get_by_id(model_id).
            last_updated, last_updated)
        model.update_timestamps()
        model.put()
        self.assertNotEqual(base_models.BaseModel.get_by_id(model_id).
            last_updated, last_updated)
    ...
    def test_put_without_update_timestamps(self) ->None:
        ...
        with self.assertRaisesRegex(Exception, re.escape(
            'did not call update_timestamps()')):
            model.put()
    ...
    def test_put_multi(self) ->None:
        ...
        models_2_without_none = cast(List[base_models.BaseModel],
            base_models.BaseModel.get_multi(model_ids))
        base_models.BaseModel.update_timestamps_multi(models_2_without_none,
            update_last_updated_time=False)
        ...
        models_3_without_none = cast(List[base_models.BaseModel],
            base_models.BaseModel.get_multi(model_ids))
...
class BaseHumanMaintainedModelTests(test_utils.GenericTestBase):
    ...
    def test_put(self) ->None:
        with self.assertRaisesRegex(NotImplementedError,
            'Use put_for_human or put_for_bot instead'):
            self.model_instance.put()

    def test_put_for_human(self) ->None:
        previous_last_updated_by_human = (self.model_instance.
            last_updated_by_human)
        self.model_instance.update_timestamps()
        self.model_instance.put_for_human()
        self.assertNotEqual(previous_last_updated_by_human, self.
    ...
    def test_put_for_bot(self) ->None:
        ...
        self.assertEqual(previous_last_updated_by_human, self.
            model_instance.last_updated_by_human)
...
class TestCommitLogEntryModel(base_models.BaseCommitLogEntryModel):
    ...
    @classmethod
    def get_instance_id(cls, target_entity_id: str, version: Union[int, str]
        ) ->str:
...
class BaseCommitLogEntryModelTests(test_utils.GenericTestBase):
    ...
    def test_get_deletion_policy_is_locally_pseudonymize(self) ->None:
        self.assertEqual(base_models.BaseCommitLogEntryModel.
    ...
    def test_get_model_association_to_user_is_not_corresponding_to_user(self
        ...
        self.assertEqual(base_models.BaseCommitLogEntryModel.
            get_model_association_to_user(), base_models.
...
class BaseSnapshotMetadataModelTests(test_utils.GenericTestBase):
    ...
    def test_has_reference_to_user_id(self) ->None:
        model1 = base_models.BaseSnapshotMetadataModel(id='model_id-1',
            committer_id='committer_id', commit_type='create',
            commit_cmds_user_ids=['commit_cmds_user_1_id',
            'commit_cmds_user_2_id'], content_user_ids=['content_user_1_id',
            'content_user_2_id'])
        model1.update_timestamps()
        model1.put()
        self.assertTrue(base_models.BaseSnapshotMetadataModel.
        ...
        self.assertFalse(base_models.BaseSnapshotMetadataModel.
    ...
    def test_export_data_nontrivial(self) ->None:
        ...
        model1 = version_model.SNAPSHOT_METADATA_CLASS.create('model_id-1',
            'committer_id', 'create', None, [])
        model1.update_timestamps()
        model1.put()
        model2 = version_model.SNAPSHOT_METADATA_CLASS.create('model_id-2',
            'committer_id', 'create', 'Hi this is a commit.', [{'cmd':
            'some_command'}, {'cmd2': 'another_command'}])
        model2.update_timestamps()
        model2.put()
        user_data = version_model.SNAPSHOT_METADATA_CLASS.export_data(
            'committer_id')
        expected_data = {'model_id-1': {'commit_type': 'create',
            'commit_message': None}, 'model_id-2': {'commit_type': 'create',
    ...
    def test_export_when_commit_message_contains_user_id(self) ->None:
        ...
        user_data = version_model.SNAPSHOT_METADATA_CLASS.export_data(
            'committer_id')
        expected_data = {'model_id-1': {'commit_type': 'create',
            'commit_message': 'Test <user ID>'}, 'model_id-2': {
            'commit_type': 'create', 'commit_message': 'Hi this is a commit.'}}
        self.assertEqual(user_data, expected_data)
...
class CommitLogEntryModelTests(test_utils.GenericTestBase):
    ...
    def test_get_commit(self) ->None:
        model1 = TestCommitLogEntryModel.create(entity_id='id',
            committer_id='user', commit_cmds=[], commit_type='create',
            commit_message='New commit created.', version=1, status=
            constants.ACTIVITY_STATUS_PUBLIC, community_owned=False)
        model1.update_timestamps()
        model1.put()
        ...
        self.assertEqual(test_model.post_commit_status, constants.
    ...
    def test_get_all_commits(self) ->None:
        model1 = TestCommitLogEntryModel.create(entity_id='id',
            ...
            commit_message='New commit created.', version=1, status=
            constants.ACTIVITY_STATUS_PUBLIC, community_owned=False)
        model2 = TestCommitLogEntryModel.create(entity_id='id',
            committer_id='user', commit_cmds=[], commit_type='edit',
            commit_message='New commit created.', version=2, status=
            constants.ACTIVITY_STATUS_PUBLIC, community_owned=False)
        model1.update_timestamps()
        model1.put()
        model2.update_timestamps()
...
class VersionedModelTests(test_utils.GenericTestBase):
    ...
    def test_retrieval_of_multiple_version_models_for_fake_id(self) ->None:
        with self.assertRaisesRegex(ValueError,
            'The given entity_id fake_id is invalid'):
    ...
    def test_commit_with_model_instance_deleted_raises_error(self) ->None:
        ...
        with self.assertRaisesRegex(Exception,
            'This model instance has been deleted.'):
            model1.commit(feconf.SYSTEM_COMMITTER_ID, '', [])
    ...
    def test_trusted_commit_with_no_snapshot_metadata_raises_error(self
        ...
        with self.assertRaisesRegex(Exception,
            'No snapshot metadata class defined.'):
            model1.commit(feconf.SYSTEM_COMMITTER_ID, '', [])
        model1 = TestVersionedModel(id='model_id1')
        model1.SNAPSHOT_CONTENT_CLASS = None
        with self.assertRaisesRegex(Exception,
            'No snapshot content class defined.'):
            model1.commit(feconf.SYSTEM_COMMITTER_ID, '', [])
        model1 = TestVersionedModel(id='model_id1')
        with self.assertRaisesRegex(Exception,
            'Expected commit_cmds to be a list of dicts, received'):
            model1.commit(feconf.SYSTEM_COMMITTER_ID, '', {})
        model1 = TestVersionedModel(id='model_id1')
        with self.assertRaisesRegex(Exception,
            'Expected commit_cmds to be a list of dicts, received'):
    ...
    def test_force_deletion(self) ->None:
        ...
        model.delete(feconf.SYSTEM_COMMITTER_ID, 'commit_msg',
            force_deletion=True)
        self.assertIsNone(TestVersionedModel.get_by_id(model_id))
        for model_snapshot_id in model_snapshot_ids:
            self.assertIsNone(TestSnapshotContentModel.get_by_id(
                model_snapshot_id))
            self.assertIsNone(TestSnapshotMetadataModel.get_by_id(
                model_snapshot_id))
    ...
    def test_delete_multi(self) ->None:
        ...
        model_1_snapshot_ids = [model_1.get_snapshot_id(model_1.id,
            version_number) for version_number in model_1_version_numbers]
        ...
        model_2_snapshot_ids = [model_2.get_snapshot_id(model_2.id,
            version_number) for version_number in model_2_version_numbers]
    ...
    def test_commit_with_invalid_change_list_raises_error(self) ->None:
        ...
        with self.assertRaisesRegex(Exception, 'Invalid commit_cmd:'):
            model1.commit(feconf.SYSTEM_COMMITTER_ID, '', [{'invalid_cmd':
                'value'}])
    ...
    def test_get_version(self) ->None:
        ...
        test_version_model = TestVersionedModel.get_version('nonexistent_id1',
            4, strict=False)
        ...
        test_version_model = TestVersionedModel.get_version('model_id1', 4,
            strict=False)
        self.assertIsNone(test_version_model)
        with self.assertRaisesRegex(base_models.BaseModel.
            EntityNotFoundError,
    ...
    def test_get_multi_versions(self) ->None:
        ...
        model1.commit(feconf.SYSTEM_COMMITTER_ID, '', [])
        model1.commit(feconf.SYSTEM_COMMITTER_ID, '', [])
        models_by_version = TestVersionedModel.get_multi_versions('model_id1',
            [1, 2])
...
class BaseModelTests(test_utils.GenericTestBase):
    ...
    def test_create_raises_error_when_many_id_collisions_occur(self) ->None:
        get_by_id_swap = self.swap(TestBaseModel, 'get_by_id', types.
            MethodType(lambda _, __: True, TestBaseModel))
