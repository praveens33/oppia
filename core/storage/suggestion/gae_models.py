...
class GeneralSuggestionModel(base_models.BaseModel):
    ...
    def get_all_stale_suggestion_ids(cls) ->List[str]:
        ...
        """
        threshold_time = get_current_datetime_utc() - datetime.timedelta(0,
            0, 0, THRESHOLD_TIME_BEFORE_ACCEPT_IN_MSECS)
        suggestion_models: Sequence[GeneralSuggestionModel] = cls.get_all(
            ).filter(cls.status == STATUS_IN_REVIEW).filter(cls.
            last_updated < threshold_time).fetch()
        return [suggestion_model.id for suggestion_model in suggestion_models]
    ...
    def get_suggestions_waiting_too_long_for_review(cls) ->Sequence[
        ...
        threshold_time = get_current_datetime_utc() - datetime.timedelta(days
            =SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS)
        return cls.get_all().filter(datastore_services.all_of(cls.status ==
            STATUS_IN_REVIEW, cls.last_updated < threshold_time, cls.
            suggestion_type.IN(feconf.CONTRIBUTOR_DASHBOARD_SUGGESTION_TYPES))
